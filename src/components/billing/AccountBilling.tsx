...
import { formatPlanInterval, formatPlanPrice, formatSubscriptionStatus } from "@/lib/revenue-os/subscriptions-contract";
...
type Billing = {
  ...
  invoices: Array<{ id: string; status: string; amountDue: number; amountPaid?: number; currency: string; created: string | null; hostedInvoiceUrl: string | null }>;
...
export function AccountBilling({ tenantSlug, userEmail, userName, emailUpdates }: { tenantSlug: string; userEmail: string; userName: string; emailUpdates: boolean }) {
  ...
  async function act(action: "cancel" | "resume" | "change", subscriptionId: string) {
    if (action === "cancel" && !window.confirm("Cancel this subscription at the next renewal? You keep access until then.")) return;
    ...
    try {
      ...
      await load();
      const nextPlan = action === "change" ? data?.plans.find((plan) => plan.id === selectedPlan[subscriptionId]) : null;
      setNotice(action === "cancel" ? "Your subscription will cancel at the next renewal." : action === "resume" ? "Cancellation removed. Your subscription will continue to renew." : `Your change to ${nextPlan?.name || "the new plan"} is scheduled for the next renewal.`);
    } catch (cause) {
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="flex flex-wrap items-center justify-between gap-4 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-black/10"><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-black/50">Billing account</p><p className="mt-2 text-sm">{data.customer?.email || "No billing account yet"}</p><p className="mt-1 text-sm text-black/55">Payment details stay in Stripe’s secure portal.</p></div>{data.customer && <button type="button" onClick={() => void portal()} disabled={busy !== ""} className="min-h-11 rounded-xl border border-black/15 px-4 text-sm font-semibold hover:bg-black/5 disabled:opacity-50">{busy === "portal" ? "Opening secure portal…" : "Manage payment method"}</button>}</section>
      <section><h2 className="text-xl font-semibold tracking-tight">Subscriptions</h2><div className="mt-4 space-y-4">{data.subscriptions.length ? data.subscriptions.map((subscription) => <article key={subscription.id} className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-black/10"><div className="flex flex-wrap items-start justify-between gap-4"><div><h3 className="font-semibold">{subscription.plan?.name || "Subscription"}</h3><p className="mt-1 text-sm text-black/60">{formatSubscriptionStatus(subscription.status)} · {subscription.plan ? `${formatPlanPrice(subscription.plan.amount, subscription.plan.currency)} / ${formatPlanInterval(subscription.plan.interval)}` : "Plan details unavailable"}</p></div><span className={`rounded-full px-3 py-1 text-xs font-semibold ${subscription.cancel_at_period_end ? "bg-amber-100 text-amber-900" : "bg-emerald-50 text-emerald-800"}`}>{subscription.cancel_at_period_end ? "Cancels at renewal" : "Renews automatically"}</span></div><p className="mt-4 text-sm text-black/60">{subscription.cancel_at_period_end ? "Access through:" : "Renews on:"} {subscription.current_period_end ? new Date(subscription.current_period_end).toLocaleDateString() : "Not available"}</p>{subscription.pending_plan_id && <p className="mt-2 text-sm text-black/60">{data.plans.find((plan) => plan.id === subscription.pending_plan_id)?.name || "Plan change"} starts at the next renewal.</p>}<div className="mt-5 flex flex-wrap items-center gap-3">{subscription.cancel_at_period_end ? <button type="button" onClick={() => void act("resume", subscription.stripe_subscription_id)} disabled={busy !== ""} className="min-h-10 rounded-xl border border-black/15 px-4 text-sm font-semibold">Keep subscription</button> : <button type="button" onClick={() => void act("cancel", subscription.stripe_subscription_id)} disabled={busy !== ""} className="min-h-10 rounded-xl border border-red-200 px-4 text-sm font-semibold text-red-700">Cancel at renewal</button>}{!subscription.pending_plan_id && data.plans.length > 1 && <><select aria-label="New plan" className="min-h-10 rounded-xl border border-black/15 bg-white px-3 text-sm" value={selectedPlan[subscription.stripe_subscription_id] || ""} onChange={(event) => setSelectedPlan((current) => ({ ...current, [subscription.stripe_subscription_id]: event.target.value }))}><option value="">Change plan…</option>{data.plans.filter((plan) => plan.id !== subscription.plan?.id).map((plan) => <option key={plan.id} value={plan.id}>{plan.name} · {formatPlanPrice(plan.amount, plan.currency)}/{formatPlanInterval(plan.interval)}</option>)}</select>{selectedPlan[subscription.stripe_subscription_id] && <button type="button" onClick={() => void act("change", subscription.stripe_subscription_id)} disabled={busy !== ""} className="min-h-10 rounded-xl border border-black/15 px-4 text-sm font-semibold">Schedule change</button>}</>}</div></article>) : <p className="rounded-2xl bg-white p-6 text-sm text-black/60 shadow-sm ring-1 ring-black/10">No subscriptions yet. <a className="font-semibold underline" href={`/t/${tenantSlug}/subscribe`}>View plans</a>.</p>}</div></section>
      <section><h2 className="text-xl font-semibold tracking-tight">Invoices</h2>{data.providerError && <p role="status" className="mt-3 text-sm text-black/60">{data.providerError}</p>}<div className="mt-4 divide-y divide-black/10 rounded-2xl bg-white shadow-sm ring-1 ring-black/10">{data.invoices.length ? data.invoices.map((invoice) => <div key={invoice.id} className="flex flex-wrap items-center justify-between gap-3 p-4 text-sm"><span>{invoice.created ? new Date(invoice.created).toLocaleDateString() : "Invoice"}</span><span className="tabular-nums">{formatPlanPrice(invoice.status === "paid" ? invoice.amountPaid || invoice.amountDue || 0 : invoice.amountDue || 0, invoice.currency || "usd")}</span><span className="text-black/60">{invoice.status === "paid" ? "Paid" : invoice.status === "open" ? "Due" : invoice.status}</span>{invoice.hostedInvoiceUrl && <a className="font-semibold underline" href={invoice.hostedInvoiceUrl}>View invoice</a>}</div>) : <p className="p-6 text-sm text-black/60">No invoices yet.</p>}</div></section>
    </div>
  );
}
