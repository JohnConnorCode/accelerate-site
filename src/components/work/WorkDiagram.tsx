...
const specs: Record<WorkDiagramData["variant"], DiagramSpec> = {
  ...
  "healthcare-object-map": {
    label: "Healthcare real estate deal object connected to the people and information required to close",
    nodes: [
    ...
    footer: "The model organizes the transaction, rather than treating it as notes attached to a contact.",
  },
  ...
  "superdebate-operating-loop": {
    ...
    footer: "Each debate creates a public experience and a better operating foundation for the next one.",
  },
  ...
  "thrive-builder-loop": {
    ...
    footer: "Capital becomes useful when the right builder can find and understand the route to it.",
  },
...
export function WorkDiagram({ media, compact = false, inverted = false }: { media: WorkDiagramData; compact?: boolean; inverted?: boolean }) {
  ...
  return (
    ...
    >
      <div aria-hidden="true" className={`${inverted ? "opacity-[0.09]" : "opacity-[0.055]"} absolute inset-0 [background-image:linear-gradient(currentColor_1px,transparent_1px),linear-gradient(90deg,currentColor_1px,transparent_1px)] [background-size:2.5rem_2.5rem]`} />
      <div className="relative">
        {!compact ? (
          <div className={`${inverted ? "border-white/20" : "border-[var(--rule)]"} mb-9 flex items-baseline justify-between gap-5 border-b pb-4`}>
            <p className={`${inverted ? "text-white/65" : "text-[var(--mid)]"} font-mono text-[10px] uppercase tracking-[0.15em]`}>System model</p>
            <p className={`${inverted ? "text-white/65" : "text-[var(--mid)]"} text-right font-mono text-[9px] uppercase tracking-[0.12em]`}>Conceptual · not product UI</p>
          </div>
        ) : null}
