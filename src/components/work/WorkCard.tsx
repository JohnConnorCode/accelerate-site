    ...
    <WorkReveal className={`${workAccentClasses[project.accent]} h-full`} delay={typeof index === "number" ? (index % 2) * 0.08 : 0} role="card">
      <article className="group h-full border-t border-[var(--rule)] pt-5" data-work-card={project.slug} data-work-accent={project.accent}>
        <Link href={`/work/${project.slug}`} className="flex h-full min-h-11 flex-col focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--case-accent)] focus-visible:ring-offset-4 focus-visible:ring-offset-[var(--bg)]" aria-label={`View ${project.name} case study`}>
          <div data-work-stagger="1" className="origin-center transition-transform duration-300 ease-out group-hover:-translate-y-0.5 group-focus-visible:-translate-y-0.5 group-active:scale-[0.96] motion-reduce:transition-none">
          ...
          <div className="flex flex-1 flex-col py-6">
            ...
            <h3 data-work-stagger="3" className={`${featured ? "text-[clamp(1.8rem,3.25vw,3.35rem)]" : "text-[clamp(1.5rem,2.1vw,2.35rem)]"} max-w-[19ch] text-balance font-display font-medium leading-[0.98] tracking-[-0.04em] text-[var(--fg)]`}>{project.cardHeadline}</h3>
            <p data-work-stagger="4" className="mt-4 max-w-[58ch] text-pretty text-[0.97rem] leading-7 text-[var(--mid)]">{project.cardDescription}</p>
            <div data-work-stagger="5" className="mt-auto flex flex-wrap items-center gap-x-6 gap-y-3 pt-6 font-mono text-[10px] uppercase tracking-[0.12em]">
