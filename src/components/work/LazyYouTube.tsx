...
export function LazyYouTube({ media, priority = false }: { media: WorkVideo; priority?: boolean }) {
  ...
  return (
    <div className="relative aspect-video overflow-hidden bg-black" data-lazy-video={media.youtubeId}>
      {playing ? (
        <iframe
        ...
        >
          <Image src={media.poster} alt="" fill priority={priority} sizes="(min-width: 1024px) 80vw, 100vw" className="object-cover opacity-90 transition-[opacity,transform] duration-300 ease-out group-hover:scale-[1.015] group-hover:opacity-100 motion-reduce:transition-none" />
          ...
          <span className="absolute bottom-4 left-4 bg-black/75 px-3 py-2 font-mono text-[9px] uppercase tracking-[0.14em] text-white">Play motion study</span>
        </button>
      )}
    </div>
  );
