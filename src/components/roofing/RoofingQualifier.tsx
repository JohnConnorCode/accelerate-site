...
const selectClass = "min-h-12 w-full appearance-none rounded-xl border border-white/12 bg-white/[0.055] px-4 text-[15px] text-white outline-none transition-[border-color,box-shadow,background-color] duration-200 focus:border-[#d7ff5f]/70 focus:bg-white/[0.075] focus:shadow-[0_0_0_3px_rgba(215,255,95,0.09)]";
...
export function RoofingQualifier() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/qualify/resume?token=${encodeURIComponent(resume)}`)
      ...
      .then((data: { email: string; bookingMode?: "manual" | "calendly" }) => {
        ...
        setStatus("qualified");
        requestAnimationFrame(() => document.querySelector("#book")?.scrollIntoView({ behavior: "smooth", block: "start" }));
      })
      .catch(() => undefined);
  }, []);
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const data = await response.json() as { error?: string; qualified?: boolean; token?: string; email?: string; bookingMode?: "manual" | "calendly" };
      ...
      trackConversion("qualifier_completed", { funnel: "roofing", qualified: data.qualified ? 1 : 0 });
    ...
    <div id="book" className="scroll-mt-8">
      <AnimatePresence mode="wait" initial={false}>
          ...
          >
            ...
            <div className="grid gap-5 sm:grid-cols-2">
              ...
              <label>
                ...
                <select value={form.role} onChange={(event) => update("role", event.target.value)} required className={selectClass}>
              ...
              <label>
                ...
                <select value={form.revenueBand} onChange={(event) => update("revenueBand", event.target.value)} required className={selectClass}>
          ...
          >
            ...
            {bookingMode === "calendly" ? (
              ...
              <div className="rounded-[28px] bg-[#11120f] p-7 shadow-[0_0_0_1px_rgba(255,255,255,0.09),0_28px_90px_rgba(0,0,0,0.3)] sm:p-10">
                <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-[#d7ff5f]">Manual review is active</p>
                <h2 className="mt-5 text-balance font-display text-3xl font-medium tracking-[-0.035em] text-white sm:text-4xl">John will take it from here.</h2>
                <p className="mt-4 max-w-xl text-pretty leading-7 text-white/62">We sent a confirmation to {form.email}. John will review the company and email you personally within one business day with the best next step and times to talk.</p>
                <div className="mt-7 grid gap-3 sm:grid-cols-3">
                  {["Company reviewed", "Personal reply", "Written finding after the call"].map((item, index) => (
                    <div key={item} className="rounded-xl bg-white/[0.05] p-4 shadow-[inset_0_0_0_1px_rgba(255,255,255,0.07)]">
                      <span className="font-mono text-[9px] tabular-nums text-[#d7ff5f]">0{index + 1}</span>
        ...
        {status === "nurture" && (
          ...
          >
            <span className="grid size-10 place-items-center rounded-xl bg-white/8 text-[#d7ff5f]"><Check className="size-5" /></span>
            <h2 className="mt-6 text-balance font-display text-3xl font-medium tracking-[-0.035em] text-white">Start with the follow-up playbook.</h2>
            <p className="mt-4 max-w-xl text-pretty leading-7 text-white/62">The managed system is built for established operators with enough inquiry volume to support hands-on optimization. We emailed you a practical next step now, and we can revisit the audit as the business grows.</p>
            <Link href="/learn/automate-lead-follow-up" className="mt-7 inline-flex min-h-11 items-center gap-2 rounded-xl bg-white px-5 font-mono text-[11px] uppercase tracking-[0.12em] text-[#11120f] transition-[scale,background-color] duration-150 hover:bg-white/90 active:scale-[0.96]">Read the playbook <ArrowRight className="size-4" /></Link>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
