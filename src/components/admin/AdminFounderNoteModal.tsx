...
export function AdminFounderNoteModal() {
  ...
  useEffect(() => {
    const show = (event: Event) => {
      ...
      setPersonQuery("");
      setPeople([]);
      setSelectedPerson(detail.contactEmail ? { name: detail.contactLabel || detail.contactEmail, email: detail.contactEmail, type: "Canonical contact" } : null);
  ...
  const searchPeople = (value: string) => {
    ...
    if (value.trim().length < 3) { setPeople([]); setSearching(false); return; }
    setSearching(true);
    searchTimer.current = setTimeout(async () => {
      try {
        const result = await fetchJson<{ results: PersonResult[] }>(`/api/admin/search?q=${encodeURIComponent(value.trim())}`);
        setPeople(result.results.filter((person) => person.type === "Canonical contact").slice(0, 5));
      } catch {
        setPeople([]);
      } finally {
        setSearching(false);
  ...
  return (
    <AdminDialog open={open} onClose={close} title="Capture a note" labelledBy="founder-note-title" maxWidth="md">
      <AdminSurface padding="lg" className="admin-dialog-surface">
        <div className="flex items-start justify-between gap-4">
          <div className="flex min-w-0 gap-3">
            <span className="grid size-10 shrink-0 place-items-center rounded-xl bg-black/[0.045] text-[var(--admin-ink)] dark:bg-white/[0.065]"><NotebookPen className="size-[18px]" /></span>
            <div><p className="admin-eyebrow">Operating memory</p><h2 id="founder-note-title" className="admin-dialog-title">Capture what you know</h2></div>
          </div>
          <button type="button" onClick={close} className="admin-icon-button" aria-label="Close note"><X className="size-4" /></button>
        </div>
        ...
        <form onSubmit={submit} className="mt-5 space-y-4">
          ...
          <div>
            <button type="button" onClick={() => setShowAttachment((current) => !current)} className="inline-flex min-h-10 items-center gap-2 rounded-lg px-2 text-xs font-semibold text-[var(--admin-muted)] transition-[color,transform] duration-150 hover:text-[var(--admin-ink)] active:scale-[0.96]">
              <Link2 className="size-3.5" /> {selectedPerson ? `Attached to ${selectedPerson.name}` : "Attach to a person"}
            </button>
            {showAttachment && (
              <div className="relative mt-2 rounded-xl bg-black/[0.025] p-2 dark:bg-white/[0.03]">
                <div className="relative">
                  ...
                  <input value={personQuery} onChange={(event) => searchPeople(event.target.value)} placeholder="Search canonical contacts" className="min-h-11 w-full rounded-lg bg-[var(--admin-surface)] pl-9 pr-9 text-sm text-[var(--admin-ink)] outline-none shadow-[var(--admin-shadow-border)]" />
