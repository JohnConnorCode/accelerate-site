...
export function AdminAIPanel() {
  ...
  return <AnimatePresence initial={false}>{ai.open && <>
    ...
    <motion.div ref={panelRef} role="dialog" aria-label="AI command center" aria-modal="true" className="fixed inset-0 z-[71] flex min-h-0 flex-col bg-[var(--admin-canvas)] pb-[env(safe-area-inset-bottom)] pt-[env(safe-area-inset-top)] shadow-[-24px_0_70px_rgba(0,0,0,0.22)] sm:inset-y-0 sm:left-auto sm:w-[min(480px,100vw)] sm:py-0" initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ duration: 0.26, ease: [0.16, 1, 0.3, 1] }}>
      <header className="flex min-h-16 items-center justify-between gap-3 border-b border-[var(--admin-border)] px-4">
        <div className="flex min-w-0 items-center gap-3"><span className="grid size-9 place-items-center rounded-xl bg-[var(--admin-ink)] text-[var(--admin-surface)]"><Bot className="size-4" /></span><div className="min-w-0"><p className="text-sm font-semibold text-[var(--admin-ink)]">AI Command Center</p><p className="truncate text-[10px] text-[var(--admin-muted)]">Live records · staged actions · visible evidence</p></div></div>
