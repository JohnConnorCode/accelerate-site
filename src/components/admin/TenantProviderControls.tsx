...
import { Bot, CalendarDays, Check, Clipboard, Eye, EyeOff, KeyRound, Mail, PlugZap, RefreshCw, Unplug, type LucideIcon } from "lucide-react";
...
type Provider = "openrouter" | "resend" | "calendly";
interface ProviderConnection { id: string; provider: string; account_email: string | null; reply_to_email: string | null; status: string; credential_version: number; connected_at: string | null; credential_source: "tenant" | "platform" | null; key_metadata: { label: string | null; limit: number | null; limit_remaining: number | null; limit_reset: string | null; usage: number | null; is_free_tier: boolean | null; expires_at: string | null; verified_at: string | null } | null }

const providerName = (provider: Provider) => provider === "openrouter" ? "OpenRouter" : provider === "resend" ? "Resend" : "Calendly";
...
export function TenantProviderControls() {
  ...
  const configure = async (event: FormEvent, provider: Provider) => {
    ...
    setBusy(provider);
    try {
      const body = provider === "openrouter" ? { action: "configure_openrouter", apiKey: openRouterKey } : provider === "resend" ? { action: "configure_resend", apiKey: resendKey, fromEmail: resendFromEmail, replyToEmail: resendReplyToEmail, webhookSecret: resendWebhook || undefined } : { action: "configure_calendly", webhookSecret: calendlyWebhook };
      await fetchJson("/api/admin/tenant/providers", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
      if (provider === "openrouter") setOpenRouterKey("");
      else if (provider === "resend") { setResendKey(""); setResendFromEmail(""); setResendReplyToEmail(""); setResendWebhook(""); }
      else setCalendlyWebhook("");
      await query.refetch();
      toast.success(`${providerName(provider)} credentials verified and saved for this workspace`);
  ...
  const disconnect = async () => {
    ...
    try {
      ...
      toast.success(`${providerName(provider)} disconnected from this workspace`);
  ...
  return <section aria-labelledby="workspace-provider-heading" className="space-y-4">
    ...
    <AdminReadBody loading={query.isLoading} hasData={Boolean(query.data)} error={query.error?.message} onRetry={() => void query.refetch()} refreshing={query.isFetching} loadingFallback={<div className="grid gap-4 xl:grid-cols-2"><LoadingSkeleton rows={4} /><LoadingSkeleton rows={4} /></div>} label="Loading workspace credentials">
      {query.data && <div className="grid gap-4 xl:grid-cols-2">
        <ProviderCard icon={Bot} name="OpenRouter" detail="Workspace-owned AI usage and billing" connection={connection("openrouter")}>
          <div className="mb-5 rounded-[13px] bg-sky-500/[0.07] p-3.5 shadow-[var(--admin-shadow-border)]"><p className="text-pretty text-xs font-semibold text-[var(--admin-ink)]">Your key, your spend</p><p className="admin-copy mt-1 text-pretty text-[11px] leading-5">AI requests in this workspace use this OpenRouter account. The key is verified without generating tokens, then encrypted and never shown again.</p></div>
          {connection("openrouter")?.key_metadata && <KeyMetadata connection={connection("openrouter")!} />}
          <form onSubmit={(event) => void configure(event, "openrouter")} className="space-y-4"><SecretField label="OpenRouter API key" description="Create a dedicated key with a monthly limit in OpenRouter, then paste it here." value={openRouterKey} onChange={setOpenRouterKey} placeholder="sk-or-v1-…" /><a href="https://openrouter.ai/settings/keys" target="_blank" rel="noreferrer" className="inline-flex min-h-10 items-center text-xs font-semibold text-[var(--admin-ink)] underline decoration-[var(--admin-border)] underline-offset-4 transition-[opacity] hover:opacity-70">Create or manage OpenRouter keys</a><ProviderActions provider="openrouter" connected={connection("openrouter")?.status === "connected"} busy={busy === "openrouter"} disabled={Boolean(busy) || openRouterKey.trim().length < 24} onDisconnect={() => setDisconnecting("openrouter")} /></form>
        </ProviderCard>
...
function KeyMetadata({ connection }: { connection: ProviderConnection }) {
  const metadata = connection.key_metadata;
  if (!metadata) return null;
  const money = (value: number | null) => value === null ? "Not reported" : new Intl.NumberFormat(undefined, { style: "currency", currency: "USD", maximumFractionDigits: 2 }).format(value);
  return <div className="mb-5 grid grid-cols-2 gap-2 sm:grid-cols-4">{[
    ["Key", metadata.label || "Verified key"],
    ["Usage", money(metadata.usage)],
    ["Remaining", money(metadata.limit_remaining)],
    ["Limit cycle", metadata.limit_reset || "Not set"],
  ].map(([label, value]) => <div key={label} className="rounded-[11px] bg-[var(--admin-soft)] p-3"><p className="font-mono text-[8px] font-semibold uppercase tracking-[0.1em] text-[var(--admin-muted)]">{label}</p><p className="mt-1 truncate text-xs font-semibold tabular-nums text-[var(--admin-ink)]" title={value}>{value}</p></div>)}</div>;
}
...
function ProviderActions({ provider, connected, busy, disabled, onDisconnect }: { provider: Provider; connected: boolean; busy: boolean; disabled: boolean; onDisconnect: () => void }) {
  return <div className="flex flex-col gap-2 sm:flex-row"><button disabled={disabled} className="admin-action-control min-h-11 flex-1 px-3.5">{busy ? <RefreshCw className="size-3.5 animate-spin motion-reduce:animate-none" /> : <PlugZap className="size-3.5" />}{connected ? "Verify and rotate key" : `Connect ${providerName(provider)}`}</button>{connected && <button type="button" disabled={Boolean(busy)} onClick={onDisconnect} className="admin-secondary-control min-h-11 px-3.5"><Unplug className="size-3.5" />Disconnect</button>}</div>;
}
