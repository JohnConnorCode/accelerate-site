...
export function RevenueSetupGate({
  title = "Finish the Revenue OS migration",
  migration = "migrations/20260816-revenue-os.sql",
  detail = "Existing funnel records are imported idempotently.",
}: {
  title?: string;
  migration?: string;
  detail?: string;
  ...
  return (
    <AdminSurface tone="attention" padding="lg" className="text-center">
      ...
      <p className="admin-copy mx-auto mt-2 max-w-lg text-pretty text-sm leading-6">
        Apply <code className="font-mono text-xs">{migration}</code>, then refresh Setup Center.{" "}
        {detail}
      </p>
      <Link
        href="/admin/setup"
