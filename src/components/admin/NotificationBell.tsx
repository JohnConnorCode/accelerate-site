...
export function NotificationBell({ placement = "sidebar" }: { placement?: "sidebar" | "mobile" }) {
  ...
  const [desktopPosition, setDesktopPosition] = useState<{ left: number; top: number } | null>(
    null,
  );
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        aria-label={
          isOpen
            ? "Close command center alerts"
            : `Open command center alerts${signalCount ? `, ${signalCount} need attention` : ""}`
        }
          ...
          {isOpen && (
            <>
              ...
              >
                {placement === "mobile" && (
                  <span
                    className="mx-auto mt-2 block h-1 w-9 rounded-full bg-[var(--admin-ink)]/20"
                    aria-hidden="true"
                  />
                )}
                <div className="flex items-center justify-between border-b border-[var(--admin-border)] px-4 py-3">
                  <div>
                    ...
                    <h4 className="mt-0.5 text-sm font-semibold text-[var(--admin-ink)]">
                      Notifications
                    </h4>
                  </div>
                  <div className="flex items-center gap-1">
                    ...
                    <button
                      type="button"
                      onClick={() => closePanel()}
                      className="grid size-10 place-items-center rounded-lg text-[var(--admin-muted)] transition-[background-color,color,transform] duration-150 hover:bg-black/[0.04] hover:text-[var(--admin-ink)] active:scale-[0.96] dark:hover:bg-white/[0.05]"
                ...
                <div className="admin-notification-scroll overflow-y-auto overscroll-contain">
                  {priority?.status === "ready" && priority.summary.total > 0 && (
                    <Link
                      ...
                      className="group mx-3 mt-3 flex min-h-12 items-center gap-3 rounded-xl bg-[var(--admin-surface-subtle)] px-3 py-2.5 text-left shadow-[var(--admin-shadow-border)] transition-[box-shadow,transform] duration-150 hover:shadow-[var(--admin-shadow-border-hover)] active:scale-[0.98]"
                    >
                      <span className="grid size-8 shrink-0 place-items-center rounded-lg bg-[var(--admin-surface)] text-[var(--admin-ink)] shadow-[var(--admin-shadow-border)]">
                        <Target className="size-4" />
                      </span>
                      <span className="min-w-0 flex-1">
                        <span className="block text-xs font-semibold text-[var(--admin-ink)]">
                          {priority.summary.total} priority{" "}
                          {priority.summary.total === 1 ? "item" : "items"}
                        </span>
                        <span className="mt-0.5 block text-[10px] text-[var(--admin-muted)]">
                          {priority.summary.urgent ? `${priority.summary.urgent} urgent · ` : ""}
                          Open Today
                        </span>
                      </span>
                  ...
                  <div className="flex items-center justify-between px-4 pb-2 pt-3">
                    <p className="text-[10px] font-semibold uppercase tracking-[0.1em] text-[var(--admin-muted)]">
                      Notifications
                    </p>
                    {unreadCount > 0 && (
                            ...
                            <p
                              className={cn(
                                ...
                                notification.read
                                  ...
                                  : "text-[var(--admin-ink)] font-medium",
                              )}
                            >
                              {notification.title}
                            </p>
