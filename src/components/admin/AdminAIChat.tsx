...
function MessageActions({ message, onRetry }: { message: AdminAIMessage; onRetry?: () => void }) {
  ...
  return <div className="mt-2 flex flex-wrap items-center gap-1 border-t border-[var(--admin-border)] pt-2">
    ...
    <button type="button" onClick={() => window.dispatchEvent(new CustomEvent("admin:add-note", { detail: { initialNote: message.content, captureSource: "ai_answer" } }))} className="inline-flex min-h-10 items-center gap-1.5 rounded-xl px-2.5 text-xs font-semibold text-[var(--admin-muted)] shadow-[var(--admin-shadow-border)] transition-[color,transform] hover:text-[var(--admin-ink)] active:scale-[0.96]" aria-label="Save answer as founder note"><NotebookPen className="size-3.5" />Save note</button>
    <Link href={`/admin/ai?view=runs&run=${encodeURIComponent(message.runId)}`} className="inline-flex min-h-10 items-center gap-1.5 rounded-xl px-2.5 text-xs font-semibold text-[var(--admin-muted)] shadow-[var(--admin-shadow-border)] transition-[color,transform] hover:text-[var(--admin-ink)] active:scale-[0.96]" aria-label="Inspect this AI run"><ExternalLink className="size-3.5" />Inspect run</Link>
...
export function AdminAIChat({ mode = "page" }: { mode?: "page" | "panel" }) {
  ...
  const mobileConversationBar = mode === "page" ? <div className="flex items-center gap-2 border-b border-[var(--admin-border)] p-3 md:hidden">
    <label className="min-w-0 flex-1"><span className="sr-only">AI conversation</span><select value={ai.activeConversationId ?? ""} onChange={(event) => void ai.selectConversation(event.target.value || null)} className="min-h-11 w-full rounded-xl bg-[var(--admin-surface)] px-3 text-xs font-semibold text-[var(--admin-ink)] shadow-[var(--admin-shadow-border)]"><option value="">New conversation</option>{ai.conversations.map((conversation) => <option key={conversation.id} value={conversation.id}>{conversation.title}</option>)}</select></label>
    <button type="button" onClick={ai.startNew} className="admin-icon-button !size-11" aria-label="New AI conversation"><MessageSquarePlus className="size-4" /></button>
  </div> : null;
  ...
  const chat = <section className="flex min-h-0 flex-1 flex-col">
    ...
    <div className="flex items-center gap-2 border-b border-[var(--admin-border)] px-4 py-2.5 text-[11px] text-[var(--admin-muted)]"><Bot className="size-3.5" /><span>Live records</span><span aria-hidden="true">·</span><span>Visible evidence</span><span aria-hidden="true">·</span><span>Changes staged for approval</span></div>
    {mobileConversationBar}
    <div ref={scrollRef} className={cn("flex-1 overflow-y-auto", mode === "page" ? "min-h-0 px-3 py-4 sm:px-6" : "min-h-0 px-4 py-4")} role="log" aria-label="AI conversation">
      ...
      {!ai.loadingHistory && ai.messages.length === 0 && <div className="mx-auto flex min-h-full max-w-xl flex-col items-center justify-center py-8 text-center">
        ...
        <div className="mt-5 grid w-full gap-2 sm:grid-cols-3">{starters.map((starter) => <button key={starter.label} type="button" disabled={ai.schemaReady === false} onClick={() => void ai.send(starter.prompt)} className="min-h-12 rounded-xl px-3 py-2 text-left shadow-[var(--admin-shadow-border)] transition-[box-shadow,transform] duration-150 hover:shadow-[var(--admin-shadow-border-hover)] active:scale-[0.96] disabled:opacity-40"><span className="block text-[10px] font-semibold uppercase tracking-[0.09em] text-[var(--admin-muted)]">{starter.label}</span><span className="mt-0.5 block text-xs font-medium text-[var(--admin-ink)]">{starter.prompt}</span></button>)}</div>
      </div>}
      <div className="space-y-5">{ai.messages.map((message, index) => { const isLatestAssistant = message.role === "assistant" && !ai.messages.slice(index + 1).some((item) => item.role === "assistant"); return <article key={message.id} className={cn("text-sm leading-6", message.role === "user" ? "ml-auto max-w-[85%] rounded-2xl rounded-br-md bg-[var(--admin-ink)] px-4 py-3 text-[var(--admin-surface)] shadow-[var(--admin-shadow-border)]" : "max-w-3xl border-l-2 border-[var(--admin-border)] py-1 pl-4 text-[var(--admin-ink)]")}>
    ...
    <form onSubmit={submit} className="border-t border-[var(--admin-border)] p-3 sm:p-4">
      ...
      <div className="admin-composer">
        <textarea aria-label="Ask the business" value={ai.draft} onChange={(event) => ai.setDraft(event.target.value.slice(0, 8000))} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); event.currentTarget.form?.requestSubmit(); } }} rows={1} placeholder="Ask about priorities, pipeline, conversations, or next actions…" className="admin-composer-field max-h-32" />
  ...
  return <div className="grid h-[clamp(440px,calc(100dvh-19rem),720px)] min-h-0 overflow-hidden rounded-2xl bg-[var(--admin-surface)] shadow-[var(--admin-shadow)] md:grid-cols-[228px_minmax(0,1fr)]"><div className="hidden p-4 md:block">{conversationList}</div>{chat}</div>;
}
