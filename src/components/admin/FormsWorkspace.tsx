...
export function FormsWorkspace() {
  ...
  const changeStatus = async (form: FormDefinition, status: FormDefinition["status"]) => {
    setNotice(null);
    try {
      await postAction({ action: "status", form: { id: form.id, expectedUpdatedAt: form.updated_at, status } });
      refresh();
    } catch (error) {
  ...
  const review = async (id: string, decision: "accepted" | "rejected") => {
    ...
    try {
      ...
      submissionsQuery.refetch();
      setNotice(
        decision === "rejected" ? "Response rejected." : receipt.intakeStatus === "completed" ? "Response accepted into the pipeline." : `Response accepted. Intake ${receipt.intakeStatus === "needs_attention" ? "needs attention" : "is pending"} in the action queue (action ${receipt.actionId}).`,
      );
    } catch (error) {
