...
export function AdminTable<T>({
  ...
  return (
    <AdminSurface padding="none" className="overflow-hidden">
                ...
                >
                  {col.sortable ? (
                    <button
                      ...
                      className="inline-flex min-h-10 items-center gap-1.5 rounded-md px-1 text-left transition-[color,transform] duration-150 hover:text-[var(--admin-ink)] active:scale-[0.96]"
                    >
                      {col.label}
                      <ArrowUpDown
                        className={cn(
                    ...
                    {isExpanded && renderExpanded && (
                      ...
                      >
                        <td
                          ...
                          className="bg-[var(--admin-surface-subtle)] px-4 py-4"
                        >
                          {renderExpanded(item)}
                        </td>
                      </motion.tr>
      ...
      {data.length === 0 && <EmptyState message={emptyMessage} icon={emptyIcon} />}
    </AdminSurface>
  );
}
