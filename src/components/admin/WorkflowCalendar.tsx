                        ...
                        {dayItems.map((item, index) => (
                          <li key={`${key}:${index}`}>
                            <button
                              type="button"
                              onClick={() => onOpen(item)}
                              className="min-h-11 w-full overflow-hidden rounded-md bg-[var(--admin-accent-soft)] px-1.5 py-1 text-left text-xs leading-4 text-[var(--admin-ink)] hover:bg-[var(--admin-hover-tint)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--admin-accent)]"
                            >
                              <span className="block truncate font-medium">
                                {descriptor.title(item)}
                              </span>
                              {descriptor.summary?.(item) && (
                                <span className="hidden truncate text-[var(--admin-muted)] sm:block">
                                  {descriptor.summary(item)}
                                </span>
                              )}
                            </button>
