...
function PricingCard({ pkg }: { pkg: ServicePackage }) {
  ...
  return (
    <AnimateOnScroll
      ...
      className={cn("relative flex h-full flex-col", featured && "lg:-mt-4 lg:mb-4 lg:z-10")}
    >
      {featured && (
      ...
      <div
        ...
        className={cn(
          ...
          featured ? "border-[var(--fg)]" : "border-[color-mix(in_srgb,var(--fg)_14%,transparent)]",
        )}
      >
        <div className="mb-6">
            ...
            <li key={feature.name} className="flex items-start gap-3">
              ...
              <span
                className={cn(
                  ...
                  feature.included ? "text-white-secondary" : "text-white-muted",
                )}
              >
                {feature.name}
        ...
        <Link
          ...
          className={cn("btn w-full justify-center", featured ? "" : "")}
        >
          {pkg.ctaText}
      ...
      <table className="w-full min-w-[640px] border-collapse" role="table">
        <thead>
          <tr>
            <th
              scope="col"
              className="sticky top-[calc(var(--site-header-h)+var(--safe-top))] z-10 bg-[color-mix(in_srgb,var(--bg-base)_94%,transparent)] py-4 pl-4 pr-4 text-left font-mono text-[0.62rem] uppercase tracking-[0.2em] text-white-muted backdrop-blur-md sm:pl-0"
            >
              Feature
            </th>
            {packages.map((pkg) => (
          ...
          {featureNames.map((name, idx) => (
            <tr
              ...
              className={cn(
                ...
                idx % 2 === 0 && "bg-[color-mix(in_srgb,var(--fg)_3%,transparent)]",
              )}
            >
              ...
              {packages.map((pkg) => {
                ...
                return (
                  <td key={pkg.id} className="px-4 py-3 text-center">
                    {included ? (
                      <Check
                        className="mx-auto h-4 w-4 text-heading"
                        strokeWidth={2.5}
                        aria-label={`${name} included in ${pkg.name}`}
                      />
                    ) : (
    ...
    <>
            ...
            <div className="min-w-0">
              ...
              <HeroEntranceItem step={3}>
                <p className="mt-7 max-w-xl text-lg leading-relaxed text-white-secondary">
                  Clear scope, clear pricing, no long-term contracts. Every package is built and run
                  by our team, not software handed off for you to manage, and working from day one.
                </p>
              </HeroEntranceItem>
            </div>
              ...
              <div className="relative overflow-hidden border border-[color-mix(in_srgb,var(--fg)_14%,transparent)] p-7">
                ...
                <ul className="flex flex-col gap-3.5">
                  {[
                    ...
                    <li
                      key={g}
                      className="flex items-start gap-3 text-sm leading-relaxed text-white-secondary"
                    >
      ...
      <Section width="wide" divide className="bg-[var(--bg-section-deep)]">
        ...
        <p className="mb-6 max-w-2xl text-base leading-relaxed text-white-muted sm:mb-10">
          See exactly what&apos;s included in each package so you can choose with confidence.
        </p>
        ...
        <div className="grid items-center gap-12 lg:grid-cols-[1.2fr_1fr] lg:gap-16">
          ...
          <div className="flex flex-col gap-7">
            <p className="text-lg leading-relaxed text-white-secondary">
              Book a free strategy call. We&apos;ll learn your business and recommend the package
              that fits. No pitch, no obligation.
            </p>
