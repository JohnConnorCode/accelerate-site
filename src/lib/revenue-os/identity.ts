...
export async function findCanonicalContactByEmail(supabase: SupabaseClient, value: string | null | undefined): Promise<CanonicalEmailMatch | null> {
  const email = normalizeEmail(value);
  if (!email) return null;
  const [primary, alternate] = await Promise.all([
    supabase.from("contacts").select("id,full_name,primary_email").ilike("primary_email", exactIlike(email)).limit(3),
    supabase.from("contacts").select("id,full_name,primary_email").contains("alternate_emails", [email]).limit(3),
  ]);
...
export async function resolveOrCreateIdentity(supabase: SupabaseClient, input: ResolveIdentityInput) {
  ...
  if (input.sourceRecordType && input.sourceRecordId) {
    const { data, error } = await supabase.from("companies").select("id,name,domain")
      .eq("source_record_type", input.sourceRecordType).eq("source_record_id", input.sourceRecordId).limit(2);
    if (error) throw new Error(error.message);
    if ((data?.length ?? 0) > 1) throw new Error(`Ambiguous company source identity for ${input.sourceRecordType}:${input.sourceRecordId}`);
    company = data?.[0] ?? null;
  }
  if (!company && domain) {
    const { data, error } = await supabase.from("companies").select("id, name, domain").ilike("domain", exactIlike(domain)).limit(2);
    if (error) throw new Error(error.message);
  ...
  if (input.sourceRecordType && input.sourceRecordId) {
    const { data, error } = await supabase.from("contacts").select("id,full_name,primary_email")
      .eq("source_record_type", input.sourceRecordType).eq("source_record_id", input.sourceRecordId).limit(2);
    if (error) throw new Error(error.message);
    if ((data?.length ?? 0) > 1) throw new Error(`Ambiguous contact source identity for ${input.sourceRecordType}:${input.sourceRecordId}`);
    contact = data?.[0] ?? null;
  }
  if (!contact && email) {
...
export interface ContactImportIdentityMatch {
  ...
  reason: string;
  contact: { id: string; full_name: string; primary_email: string | null; phone: string | null; title: string | null; company_id: string | null; metadata: Record<string, unknown> } | null;
  company: { id: string; name: string; domain: string | null; website: string | null; industry: string | null } | null;
  domain: string | null;
...
export async function inspectContactImportIdentity(
  ...
  if (email) {
    const [primary, alternate] = await Promise.all([
      supabase.from("contacts").select("id,full_name,primary_email,phone,title,company_id,metadata").ilike("primary_email", exactIlike(email)).limit(3),
      supabase.from("contacts").select("id,full_name,primary_email,phone,title,company_id,metadata").contains("alternate_emails", [email]).limit(3),
    ]);
    if (primary.error) throw new Error(primary.error.message);
  ...
  if (domain) {
    const result = await supabase.from("companies").select("id,name,domain,website,industry").ilike("domain", exactIlike(domain)).limit(3);
  ...
  if (contacts.size > 1 || companies.length > 1) {
    return {
      status: "ambiguous",
      reason: contacts.size > 1 ? `More than one contact uses ${email}` : `More than one company uses ${domain}`,
      contact: null,
      company: null,
      domain,
      contactCandidates,
  ...
  return {
    ...
    reason: contact ? `Exact email match: ${email}` : company ? `New contact; exact company domain match: ${domain}` : "No deterministic identity match",
    contact,
    company,
    domain,
    contactCandidates,
...
export async function importApprovedContact(
  ...
  const expectedContact = input.expectedContactId
    ? current.contactCandidates.find((candidate) => candidate.id === input.expectedContactId) ?? null
    : null;
  const expectedCompany = input.expectedCompanyId
    ? current.companyCandidates.find((candidate) => candidate.id === input.expectedCompanyId) ?? null
    : null;

  if (current.status === "ambiguous") {
    if (hasAmbiguousContact && !expectedContact) throw new Error(`Identity needs review: ${current.reason}`);
    if (hasAmbiguousCompany && !expectedCompany) throw new Error(`Identity needs review: ${current.reason}`);
    if (expectedContact && hasAmbiguousContact) {
      const selected = await supabase.from("contacts").select("id,full_name,primary_email,phone,title,company_id,metadata")
        .eq("id", expectedContact.id).maybeSingle();
      if (selected.error) throw new Error(selected.error.message);
      if (!selected.data) throw new Error("Identity changed after review: the approved contact match is stale");
      contact = selected.data;
    }
    if (expectedCompany && hasAmbiguousCompany) {
      const selected = await supabase.from("companies").select("id,name,domain,website,industry").eq("id", expectedCompany.id).maybeSingle();
      if (selected.error) throw new Error(selected.error.message);
      if (!selected.data) throw new Error("Company identity changed after review; analyze the batch again");
  ...
  if (input.action === "create" && contact) throw new Error("Identity changed after review: this email now belongs to an existing contact");
  if (input.action === "update" && !contact) {
    throw new Error("Identity changed after review: the approved contact match is stale");
  }
  if (input.action === "update" && input.expectedContactId && contact?.id !== input.expectedContactId) {
    throw new Error("Identity changed after review: the approved contact match is stale");
  }
  ...
  if (!company && current.domain && input.data.companyName) {
    ...
    }).select("id,name,domain,website,industry").single();
    ...
    if (Object.keys(companyUpdates).length) {
      const updated = await supabase.from("companies").update(companyUpdates).eq("id", company.id).select("id,name,domain,website,industry").single();
  ...
  if (contact) {
    ...
    if (!contact.phone && input.data.phone) { updates.phone = input.data.phone; changedFields.push("phone"); }
    if (!contact.title && input.data.role) { updates.title = input.data.role; changedFields.push("role"); }
    if (!contact.company_id && company?.id) { updates.company_id = company.id; changedFields.push("company"); }
    const existingMetadata = contact.metadata && typeof contact.metadata === "object" ? contact.metadata : {};
    updates.metadata = {
      ...existingMetadata,
    ...
    const updated = await supabase.from("contacts").update(updates).eq("id", contact.id).select("id").single();
    if (updated.error) throw new Error(updated.error.message);
    ...
    const inserted = await supabase.from("contacts").insert({
      ...
      metadata: {
        ...
        ...(input.data.notes ? { import_notes: input.data.notes } : {}),
        ...(input.data.companyName && !company ? { unlinked_company_name: input.data.companyName } : {}),
      },
    }).select("id").single();
    if (inserted.error) throw new Error(inserted.error.message);
