...
export function summarizePipelineTotals(
  ...
  // Delegates the open/pipeline-value/weighted-value formula to pipeline-metrics.ts
  // (also used by today-snapshot.ts) so there is exactly one shared computation,
  // and adds wonRevenue on top for callers (Analytics, the Today overview route)
  // that need the full totals shape.
  const { openOpportunities, pipelineValue, weightedValue } = pipelineMetrics(
    opportunities,
    stages,
  );
  const wonRevenue = opportunities.reduce((sum, item) => sum + Number(item.won_value || 0), 0);
  return { openOpportunities, pipelineValue, weightedValue, wonRevenue };
}
