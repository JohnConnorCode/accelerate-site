...
export async function linkConversationRecord(
  ...
  return (await runOperatorAction(supabase, {
    actionType: "link_conversation_record",
    title: "Link conversation to business records",
    payload,
    actorEmail: input.actorEmail,
  })) as ConversationItem;
}
  ...
  for (const field of ["contact_id", "company_id", "opportunity_id"] as const)
    associationWrite =
      conversation[field] == null
        ? associationWrite.is(field, null)
        : associationWrite.eq(field, conversation[field]);
  associationWrite =
    conversation.metadata == null
      ? associationWrite.is("metadata", null)
      : associationWrite.eq("metadata", JSON.stringify(conversation.metadata));
  if (conversation.updated_at)
    associationWrite = associationWrite.eq("updated_at", conversation.updated_at);
  const { data: associated, error: patchError } = await associationWrite.select("id").maybeSingle();
  if (patchError) throw new Error(patchError.message);
  if (!associated)
    throw new Error("Conversation changed during association; retry against current human edits");
