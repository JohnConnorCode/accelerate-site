...
export function summarizeOperatorQueue(items: OperatorQueueItem[]) {
  const byKind = {
    ...
    proposal: 0,
    system: 0,
  } satisfies Record<OperatorQueueItem["kind"], number>;
...
export function validateOperatorQueue(items: OperatorQueueItem[]): OperatorQueueItem[] {
  for (const item of items) {
    if (
      !item.id ||
      !item.priorityReason?.trim() ||
      !item.sourceTimestamp?.trim() ||
      !item.recommendedNextAction?.trim() ||
      !item.href?.startsWith("/admin/")
    ) {
      throw new Error(
        `Priority item ${item.id || "without an id"} does not satisfy the operator queue contract`,
      );
    }
  }
  return items;
...
export function operatorTaskQueuePresentation(
  task: { source?: string | null; title: string; priority: string; dueDate: string | null },
  today: string,
) {
  ...
  if (isRecovery) {
    ...
    return {
      kind: "reply" as const,
      priorityReason: booking
        ? "Recovery booking needs immediate confirmation"
        : "Recovery reply needs a personal response",
  ...
  const priorityReason =
    task.dueDate && task.dueDate < today
      ...
      : task.priority === "high"
        ...
        : task.dueDate
          ? `Due ${task.dueDate}`
          : "Open operator commitment";
  return {
    kind: "task" as const,
    priorityReason,
    recommendedNextAction:
      "Open the linked record and complete the commitment, or snooze it with a reason.",
  };
}

export function operatorTaskRecordHref(task: {
...
export async function loadOperatorQueue(
  ...
  const [actions, tasks, conversations, proposals, meetings, campaignExceptions, health] =
    await Promise.all([
      ...
      supabase
        .from("conversations")
        .select("id,subject,unread_count,last_message_at,intent")
        .gt("unread_count", 0)
      ...
      supabase
        ...
        .select("id,title,start_at,updated_at,opportunity_id,status")
        .gte("start_at", now)
        .lte("start_at", inFortyEightHours)
        .neq("status", "cancelled")
        .limit(30),
      supabase
        .from("campaign_members")
        .select(
          "id,campaign_id,stop_reason,updated_at,campaigns!campaign_members_campaign_id_tenant_fkey(name)",
        )
        .eq("status", "stopped")
  ...
  if (options) {
    [actions, tasks, conversations, proposals, meetings, campaignExceptions].forEach(
      (result, index) => {
        if (result.error)
          options.onSourceError(
            ["Approvals", "Tasks", "Conversations", "Proposals", "Calendar", "Campaigns"][index]!,
          );
      },
    );
  }
  ...
  for (const task of tasks.data ?? []) {
    ...
    const presentation = operatorTaskQueuePresentation(
      { source: task.source, title: task.title, priority: task.priority, dueDate: task.due_date },
      now.slice(0, 10),
    );
    items.push({
      id: `task:${task.id}`,
      ...
      urgency:
        task.priority === "high"
          ? "high"
          : task.due_date && task.due_date < now.slice(0, 10)
            ? "critical"
            : "normal",
      dueAt: task.due_date,
      sourceTimestamp: task.created_at,
      priorityReason: presentation.priorityReason,
      recommendedNextAction: task.related_id
        ? presentation.recommendedNextAction
        : "Complete the commitment, or snooze it to a specific date.",
  ...
  for (const conversation of conversations.data ?? [])
    items.push({
      ...
      urgency:
        conversation.intent === "buying" || conversation.intent === "complaint" ? "high" : "normal",
      dueAt: conversation.last_message_at,
      sourceTimestamp: conversation.last_message_at || "unknown",
      priorityReason:
        conversation.intent === "buying"
          ? "Buying signal needs a reply"
          : conversation.intent === "complaint"
            ? "Complaint needs a reply"
            : "Unread customer reply",
      recommendedNextAction:
        "Open the conversation, read the latest message, and send or stage a grounded reply.",
    ...
    items.push({
      ...
      summary:
        proposal.status === "viewed"
          ? "Viewed and awaiting a response."
      ...
      priorityReason:
        proposal.expires_at && proposal.expires_at < inSevenDays
          ? "Proposal expires within seven days"
          : proposal.status === "viewed"
            ? "Client viewed; follow up deliberately"
            : "Proposal is awaiting a response",
      recommendedNextAction:
        proposal.status === "viewed"
          ? "Review the proposal activity and send a deliberate follow-up."
          : "Confirm delivery and schedule the next follow-up before expiry.",
  ...
  for (const meeting of meetings.data ?? [])
    items.push({
      ...
      urgency:
        meeting.start_at &&
        meeting.start_at <= new Date(nowDate.getTime() + 24 * 3_600_000).toISOString()
          ? "high"
          : "normal",
      ...
      priorityReason:
        meeting.start_at &&
        meeting.start_at <= new Date(nowDate.getTime() + 24 * 3_600_000).toISOString()
          ? "Meeting starts within 24 hours"
          : "Meeting starts within 48 hours",
      recommendedNextAction:
        "Review the linked opportunity and prepare objectives, context, and open questions.",
      href: meeting.opportunity_id
        ? `/admin/pipeline/${meeting.opportunity_id}`
        : "/admin/bookings",
  ...
  for (const member of campaignExceptions.data ?? []) {
    ...
    items.push({
      ...
      recommendedNextAction:
        "Open the campaign, inspect the delivery receipt, and decide whether to correct or suppress the recipient.",
  ...
  for (const concern of health.concerns)
    items.push({
      ...
      priorityReason:
        concern.kind === "webhook"
          ? "Inbound event failed to process"
          : concern.kind === "job"
            ? "Automated work did not complete cleanly"
            : concern.kind === "source"
              ? "Source synchronization is degraded"
              : "Integration health is degraded",
      recommendedNextAction:
        "Open Setup Center, inspect the failing receipt, and run the listed behavioral recovery check.",
      href: "/admin/setup",
