...
export async function loadOperatorQueue(supabase: SupabaseClient): Promise<OperatorQueueItem[]> {
  ...
  const [actions, tasks, conversations, proposals, meetings, campaignExceptions, health] = await Promise.all([
    ...
    supabase.from("tasks").select("id,title,description,priority,due_date,snoozed_until,related_type,related_id,opportunity_id,created_at").in("status", ["pending", "snoozed"]).lte("due_date", inSevenDays.slice(0, 10)).limit(50),
    ...
    supabase.from("campaign_members").select("id,campaign_id,stop_reason,updated_at,campaigns(name)").eq("status", "stopped").eq("stop_reason", "send_failed_requires_reconciliation").limit(30),
  ...
  for (const task of tasks.data ?? []) {
    ...
    recommendedNextAction: task.related_id ? "Open the linked record and complete the commitment, or snooze it with a reason." : "Complete the commitment, or snooze it to a specific date.",
