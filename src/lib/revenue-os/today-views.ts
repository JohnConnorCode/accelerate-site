...
function actor(db: SupabaseClient) {
  ...
  if (
    context?.kind !== "actor" ||
    context.tenant.id !== tenantIdForDatabase(db) ||
    context.database !== db
  )
...
export async function readTodayViews(db: SupabaseClient) {
  ...
  const { data, error } = await db
    .from("today_workspace_views")
    .select("owner_key,revision,document")
    .eq("tenant_id", context.tenant.id)
    .in("owner_key", ["workspace", context.user.id]);
  ...
  for (const row of data ?? []) {
    ...
    if (!parsed.success)
      throw new Error("A saved view needs recovery. Your stored preferences are preserved.");
    result[row.owner_key === "workspace" ? "workspace" : "personal"] = {
...
export async function saveTodayViews(db: SupabaseClient, raw: unknown) {
  ...
  if (input.scope === "workspace" && context.role !== "admin")
  ...
  if (error)
    throw new Error(
      error.code === "40001"
        ? "Today views changed in another session. Reload before saving."
        : "Your view could not be saved. Retry with the same changes.",
    );
...
export async function previewTodayViewChange(db: SupabaseClient, raw: unknown) {
  ...
  if (current[input.scope].revision !== input.revision)
    throw new Error("Today views changed. Read and preview again.");
  if (input.scope === "workspace" && !current.canManageWorkspace)
    throw new Error("Workspace administrator access required.");
  const facts = { tenantId: current.tenantId, userId: current.userId, change: input };
  return {
    ...facts,
    before: current[input.scope].document,
...
export async function proposeTodayViewChange(db: SupabaseClient, raw: unknown, actorEmail: string) {
  const input = z
    .object({ change: todaySaveSchema, digest: z.string().length(64) })
    .strict()
    .parse(raw);
  ...
  const stored = await db.from("today_view_proposals").insert({
    ...
    actor_id: preview.userId,
    digest: preview.digest,
    preview,
  });
  if (stored.error && stored.error.code !== "23505")
    throw new Error("Could not retain the exact Today preview.");
  return proposeAction(db, {
    actionType: "today_view_change",
    title: "Update Today workspace",
    ...
    dedupeKey: "today-view:" + preview.digest,
...
export async function readTodayViewProposal(db: SupabaseClient, digest: string) {
  ...
  const preview = z
    .object({
      tenantId: z.uuid(),
      userId: z.uuid(),
      change: todaySaveSchema,
      before: todayDocumentSchema,
      after: todayDocumentSchema,
    ...
    .parse(data.preview);
  if (
    context.tenant.id !== preview.tenantId ||
    context.user.id !== preview.userId ||
    digest !== hash({ tenantId: preview.tenantId, userId: preview.userId, change: preview.change })
  )
...
export async function executeTodayViewChange(db: SupabaseClient, raw: unknown) {
  const input = z
    .object({ tenantId: z.uuid(), userId: z.uuid(), digest: z.string().length(64) })
    .strict()
    .parse(raw);
  const context = actor(db);
