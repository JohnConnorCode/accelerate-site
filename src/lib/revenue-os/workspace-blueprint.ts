...
const FORBIDDEN_KEY_PATTERN = /(secret|passwd|password|api[_-]?key|access[_-]?token|refresh[_-]?token|private[_-]?key|client[_-]?secret|webhook[_-]?secret|connection[_-]?string)/i;
...
export function validateAgainstCapabilities(
  ...
  for (const workflow of blueprint.workflows) {
    for (const step of workflow.steps) {
      checkCapability(`workflow:${workflow.key}/step:${step.key}`, "workflow_step", step.capabilityKey);
    }
...
export function diffBlueprints(previous: WorkspaceBlueprint, next: WorkspaceBlueprint): BlueprintDiff {
  const added: string[] = [];
...
export function analyzeImpact(
  ...
  const affectedBoards = next.boards.filter((board) => touched.has(board.sourceType)).map((b) => b.key);
  const affectedViews = next.views.filter((view) => touched.has(view.sourceType)).map((v) => v.key);
  const affectedWorkflows = next.workflows.filter((workflow) => mentions(workflow)).map((w) => w.key);
  const affectedCoworkers = next.coworkers.filter((coworker) => mentions(coworker)).map((c) => c.key);
...
export async function saveBlueprintVersion(
  ...
  const { data: inserted, error: versionError } = await supabase
    ...
    .select("tenant_id,blueprint_id,version,parent_version,document,change_summary,created_by,source_agent_run_id,created_at")
    .single();
...
export function classifyBlueprintImpact(blueprint: WorkspaceBlueprint): BlueprintImpactItem[] {
  ...
  for (const item of blueprint.navigation) {
    items.push({ ref: `navigation:${item.label}`, label: `Navigation: ${item.label}`, level: "low_risk" });
  }
  for (const view of blueprint.views) {
    items.push({ ref: `view:${view.key}`, label: `Saved view: ${view.name}`, level: "low_risk" });
  }
  for (const dashboard of blueprint.dashboards) {
    items.push({ ref: `dashboard:${dashboard.key}`, label: `Dashboard: ${dashboard.name}`, level: "low_risk" });
  }
  ...
  for (const workType of blueprint.workTypes) {
    items.push({ ref: `worktype:${workType.key}`, label: `Work type: ${workType.label}`, level: "structural" });
  }
  ...
  for (const workflow of blueprint.workflows) {
    items.push({ ref: `workflow:${workflow.key}`, label: `Workflow: ${workflow.name}`, level: "structural" });
    for (const step of workflow.steps) {
  ...
  for (const trigger of blueprint.triggers) {
    items.push({ ref: `trigger:${trigger.key}`, label: `Trigger: ${trigger.event}`, level: "structural" });
  }
  for (const coworker of blueprint.coworkers) {
    items.push({ ref: `coworker:${coworker.key}`, label: `Coworker: ${coworker.name}`, level: "structural" });
  }
  for (const skill of blueprint.skills) {
    items.push({ ref: `skill:${skill.key}`, label: `Skill draft: ${skill.name}`, level: "structural" });
  }
  for (const rule of blueprint.attentionRules) {
    items.push({ ref: `attention:${rule.key}`, label: `Attention rule: ${rule.key}`, level: "structural" });
  }
  for (const requirement of blueprint.integrationRequirements) {
    items.push({
  ...
  for (const policy of blueprint.permissionPolicies) {
    items.push({ ref: `permission:${policy.key}`, label: `Permission: ${policy.key}`, level: "external_authority" });
  }
  for (const policy of blueprint.autonomyPolicies) {
    items.push({ ref: `autonomy:${policy.key}`, label: `Autonomy: ${policy.key}`, level: "external_authority" });
  }
  for (const app of blueprint.installedAppRecommendations) {
    items.push({ ref: `app:${app.appKey}`, label: `App: ${app.appKey}`, level: "external_authority" });
  }
  return items;
}
...
export function summarizeApprovalGates(blueprint: WorkspaceBlueprint): ApprovalGateSummary {
  const gates: ApprovalGateSummary = { blueprintLevel: [], explicitWorkspaceChange: [], externalAuthority: [] };
...
export function buildReviewModel(
  ...
  const impactByRef = new Map(classifyBlueprintImpact(blueprint).map((item) => [item.ref, item.level]));

  const statusOf = (ref: string, fallback: BlueprintItemStatus = "ready"): { status: BlueprintItemStatus; reason: string | null } => {
  ...
  const coworkers: ReviewSectionItem[] = blueprint.coworkers.map((coworker) => {
    const missing = coworker.requiredCapabilities.filter((key) =>
