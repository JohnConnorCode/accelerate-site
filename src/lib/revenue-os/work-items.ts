...
export class WorkClaimLostError extends Error {
  constructor() { super("Work item claim is no longer owned by this execution"); }
}
...
export async function transitionOwnedWorkItem(
  ...
  if (!item.lease_owner || !item.claimed_at || !item.lease_expires_at) throw new WorkClaimLostError();
  const { data, error } = await supabase.from("work_items")
    .update(changes)
    .eq("id", item.id).eq("tenant_id", item.tenant_id)
    .eq("lease_owner", item.lease_owner).eq("claimed_at", item.claimed_at)
...
export async function settleWorkItem(
  ...
  const changes: Record<string, unknown> = {
    outcome, lease_owner: null, lease_expires_at: null,
    error: null, finished_at: null, next_check_at: null, next_check_reason: null,
  };
  switch (result.status) {
    case "completed": changes.status = "completed"; changes.finished_at = now; break;
    case "skipped": changes.status = "cancelled"; changes.finished_at = now; break;
    case "deferred":
      if (!Number.isFinite(Date.parse(result.nextCheckAt)) || Date.parse(result.nextCheckAt) <= Date.parse(now)) {
        throw new Error("Deferred work requires a future next check");
      }
      Object.assign(changes, {
        status: "waiting", next_check_at: result.nextCheckAt, next_check_reason: outcome,
    ...
    case "failed": {
      ...
      if (item.attempt_count >= item.max_attempts) {
        ...
        Object.assign(changes, {
          status: "pending", next_check_at: new Date(Date.now() + minutes * 60_000).toISOString(),
          next_check_reason: `${result.status}: ${outcome}. Retrying in ${minutes} minutes.`,
        });
      }
      break;
    }
    default: throw new Error("Unknown work result disposition");
  }
  ...
  try {
    await recordAudit(supabase, {
      actorEmail: "system", action: `work_item.${result.status}`,
      entityType: "work_item", entityId: item.id, source: "automation",
      before: { status: item.status, attempt_count: item.attempt_count, outcome: item.outcome, error: item.error, finished_at: item.finished_at, next_check_at: item.next_check_at, next_check_reason: item.next_check_reason },
      after: { attempt_count: item.attempt_count, ...changes, disposition: result.status, artifacts: result.artifacts ?? [] },
    });
  } catch (error) {
  ...
  if (result.status === "completed") {
    try {
      await recordActivity(supabase, {
        activityType: "work_item_completed", title: `Work completed: ${item.objective}`,
        summary: outcome, source: "work_engine", actorEmail: "system",
        externalId: `work_item:${item.id}:completed`,
        contactId: item.entity_type === "contact" ? item.entity_id ?? undefined : undefined,
        opportunityId: item.entity_type === "opportunity" ? item.entity_id ?? undefined : undefined,
      });
    } catch (error) {
...
export async function withWorkItem(
  ...
  const claim = await claimWorkItem(supabase, { kind, leaseOwner, leaseDurationMs: input?.leaseDurationMs });
  const base = { claimed: claim.claimed, persisted: false, workItemId: claim.workItemId,
    existingStatus: claim.existingStatus, recoveredStale: claim.recoveredStale, errors: [] as string[] };
  if (!claim.claimed) return { ...base, value: null };
  const { data, error } = await supabase.from("work_items").select("*").eq("id", claim.workItemId).single();
  if (error || !data) return { ...base, value: null, errors: [error?.message ?? "Claimed item not found"] };
  // Snapshot: mutable test clients and future adapters must not alter our fence.
  const item = { ...data } as WorkItem;
  if (item.lease_owner !== leaseOwner) return { ...base, value: null, errors: ["Work claim superseded before start"] };
  let result: WorkResult;
  try {
    await transitionOwnedWorkItem(supabase, item, { status: "in_progress", started_at: new Date().toISOString() });
    result = await work(item);
  } catch (error) {
    if (error instanceof WorkClaimLostError) return { ...base, value: null, errors: [error.message] };
    result = { status: "failed", outcome: safeErrorMessage(error) };
  }
  try {
    const errors = await settleWorkItem(supabase, item, result);
    return { ...base, value: result, persisted: true, errors };
  } catch (error) {
