...
export async function executePipelineChange(
  ...
  actionType: "transition_opportunity" | "update_opportunity_details" | "create_opportunity" | "update_opportunity_record" | "reorder_opportunities",
  actorEmail: string,
  payload: Record<string, unknown>,
) {
  const systemSource = systemSourceForDatabase(database);
  ...
  if (actionType === "reorder_opportunities") {
    const result = await database.from("opportunities").select("*").in("id", (payload.updates as { id: string }[]).map((item) => item.id)).order("id");
    if (result.error) throw new Error(result.error.message);
    data = result.data;
  } else if (actionType !== "create_opportunity") {
    const result = await database.from("opportunities").select("*").eq("id", payload.opportunityId).maybeSingle();
    if (result.error || !result.data) throw new Error(result.error?.message ?? "Opportunity not found");
    data = result.data;
  }
