...
export type GoogleOperatorError = {
  code: "not_configured" | "not_connected" | "reconnect_required" | "tenant_unavailable" | "connection_failed" | "sync_failed";
  message: string;
};
...
function signature(payload: string): string {
  return createHmac("sha256", stateSigningKey()).update(`google-oauth-state.v1:${payload}`).digest("base64url");
}
...
export function verifyGoogleOAuthStateBinding(
  ...
  try {
    ...
    return decoded.state === expected.state
...
export function googleOperatorError(error: unknown, operation: GoogleOperation): GoogleOperatorError {
  const message = error instanceof Error ? error.message : "";
  if (/OAuth is not configured|ENCRYPTION_KEY is not configured/i.test(message)) {
    return { code: "not_configured", message: "Google OAuth credentials and token encryption must be configured before connecting." };
  }
  ...
  if (/refresh token|invalid_grant|encrypted envelope|encrypted secret format/i.test(message)) {
    return { code: "reconnect_required", message: "Google authorization must be renewed from the Integrations workspace." };
  }
  if (/tenant execution is unavailable|workspace is not active|explicit tenant context/i.test(message)) {
    return { code: "tenant_unavailable", message: "Google is unavailable while this workspace is inactive." };
  }
  return operation === "sync"
    ? { code: "sync_failed", message: "Google sync failed. Review the connection and latest source receipt before retrying." }
    : { code: "connection_failed", message: "Google connection verification failed. Retry from the Integrations workspace." };
}
...
export function googleServerErrorSummary(error: unknown, operation: GoogleOperation) {
  ...
  return { operation, code: projected.code, errorType: error instanceof Error ? error.name : "UnknownError" };
}
