 ...
 */
export async function suppressContactFromCampaignEmail(
  ...
  let contactUpdate = supabase
    .from("contacts")
    .update({ communication_status: communicationStatus, updated_at: new Date().toISOString() })
  ...
  await recordAudit(supabase, {
    ...
    action:
      communicationStatus === "unsubscribed" ? "contact.unsubscribed" : "contact.email_suppressed",
    entityType: "contact",
    ...
    metadata: {
      ...
      source_receipt_id: input.sourceReceiptId ?? null,
    },
  });
 ...
 */
export async function stopCampaignMemberships(
  supabase: SupabaseClient,
  input: {
    contactId: string;
  ...
  const stoppedKey = createHash("sha256")
    .update(
      stopped
    ...
    .digest("hex")
    .slice(0, 20);
  const activityKey = input.sourceReceiptId
    ? `campaign-stop:${input.sourceReceiptId}`
    : `campaign-stop:${input.contactId}:${input.campaignId ?? "all"}:${input.reason}:${stoppedKey}`;
  if (stopped.length) {
    await recordActivity(supabase, {
      ...
      metadata: {
        reason: input.reason,
        stopped_member_ids: stopped.map((member) => member.member_id),
      },
    });
  }
  await recordAudit(supabase, {
    ...
    metadata: {
      ...
      reason: input.reason,
      source_receipt_id: input.sourceReceiptId ?? null,
      affected_members: stopped?.map((member) => member.member_id) ?? [],
    },
  });
  return { stopped: stopped.length, memberIds: stopped.map((member) => member.member_id) };
}

export async function campaignMemberMaySend(
  supabase: SupabaseClient,
  input: { memberId: string; campaignId: string },
) {
  const { data: member, error } = await supabase
    .from("campaign_members")
    .select(
      "id,status,contacts!campaign_members_contact_id_tenant_fkey(communication_status),campaigns!campaign_members_campaign_id_tenant_fkey(status,version,approved_version)",
    )
  ...
  return (
    ...
    campaign.version === campaign.approved_version
  );
}
