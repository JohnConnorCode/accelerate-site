...
export async function proposeAction(
  ...
  if (
    ...
    const table = ["update_conversation_status", "assign_conversation", "link_conversation_record"].includes(input.actionType) ? "conversations" : ["update_task", "delete_task"].includes(input.actionType)
      ? "tasks"
      : "opportunities";
    const id = table === "conversations" ? input.payload.conversationId : table === "opportunities" ? input.payload.opportunityId : input.payload.taskId;
...
export async function claimApprovedAction(
  ...
  identityDecision?: { expectedPayload: Record<string, unknown>; decision: Record<string, unknown>; conversationState: Record<string, unknown> },
) {
  ...
  let query = supabase
    .from("action_queue")
    .update({
      ...(identityDecision ? { payload: { ...identityDecision.expectedPayload, approvedDecision: identityDecision.decision, conversationState: identityDecision.conversationState } } : {}),
      status: "executing",
  ...
  if (identityDecision) query = query.eq("action_type", "identity_review").eq("payload", JSON.stringify(identityDecision.expectedPayload));
