...
export function pipelineMetrics(rows: Array<{ stage: string; estimated_value: unknown; probability: unknown }>, stages: PipelineStageResolver) {
  ...
  return {
    ...
    weightedValue: Math.round(open.reduce((sum, row) => sum + Number(row.estimated_value || 0) * Math.min(100, Math.max(0, Number(row.probability || 0))) / 100, 0)),
  };
}
