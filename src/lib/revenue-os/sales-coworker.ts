import "server-only";
import { deferWork, type WorkResult } from "./work-result";
...
const qualifyLeadHandler: WorkKindHandler = async (supabase, wi) => {
  ...
  if (!contact) {
    return { status: "skipped", outcome: `Contact ${wi.entity_id} not found — skipping qualification` };
  }
...
const draftFollowupHandler: WorkKindHandler = async (supabase, wi) => {
  const { data: opportunity, error } = await supabase.from("opportunities")
    .select("id,stage").eq("tenant_id", wi.tenant_id).eq("id", wi.entity_id).maybeSingle();
  if (error) throw new Error(error.message);
  if (!opportunity || ["won", "lost"].includes(opportunity.stage)) {
    return { status: "skipped", outcome: "Opportunity missing or closed; follow-up is no longer needed" };
  }
  const existing = await findWorkDraft(supabase, wi);
  if (existing) return { status: "completed", outcome: "Follow-up proposal already prepared", artifacts: [existing] };
  ...
  return draft
    ...
    : { status: "partial", outcome: "Draft preparation returned without a valid proposal", artifacts: result.artifacts };
};
...
const reviewStaleProposalHandler: WorkKindHandler = async (supabase, wi) => {
  ...
  if (opportunity.stage !== "proposal") {
    return { status: "skipped", outcome: `Opportunity ${opportunity.company_name} no longer in proposal stage (now: ${opportunity.stage})` };
  }
  ...
  return { status: "completed", outcome, artifacts: [{ type: "work_item", id: child.workItem.id }] };
};
...
const scheduleFollowupCheckHandler: WorkKindHandler = async (supabase, wi) => {
  ...
  if (["won", "lost"].includes(opportunity.stage)) {
    return { status: "skipped", outcome: `Opportunity ${opportunity.company_name} is ${opportunity.stage} — no follow-up needed` };
  }
