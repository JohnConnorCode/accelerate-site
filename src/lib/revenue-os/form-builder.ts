...
import { z } from "zod";
import { callFormBuilderRpc, createPlatformServiceRoleClient, tenantIdForDatabase } from "@/lib/supabase/server";
...
export const formSchemaValidator = z
  ...
  .superRefine((schema, context) => {
    ...
    if (names.size > MAX_ELEMENTS)
      context.addIssue({ code: "custom", message: "Forms support at most 40 elements, including panel fields" });
  });
...
export function validateFormResponse(schema: FormSchema, input: unknown) {
  ...
  for (const field of fields) {
    ...
    switch (field.type) {
      ...
      case "rating": valid = typeof value === "number" && Number.isInteger(value) && value >= 1 && value <= (field.rateMax ?? 5); break;
      case "checkbox": valid = Array.isArray(value) && new Set(value).size === value.length && value.every((choice) => typeof choice === "string" && field.choices?.includes(choice)); break;
      case "dropdown":
      case "radiogroup": valid = typeof value === "string" && !!field.choices?.includes(value); break;
      default:
        valid = field.type === "text" && field.inputType === "number"
          ? typeof value === "number" && Number.isFinite(value)
          : typeof value === "string";
        if (valid && field.inputType === "email") valid = z.email().safeParse(value).success;
        if (valid && field.inputType === "date") valid = typeof value === "string" && /^\d{4}-\d{2}-\d{2}$/.test(value) && !Number.isNaN(Date.parse(value)) && new Date(value).toISOString().slice(0, 10) === value;
    }
...
async function writeFormDefinition(
  ...
  const { data, error } = await callFormBuilderRpc(supabase, "write_form_definition", {
    p_operation: operation, p_id: input.id,
    p_expected_updated_at: input.expectedUpdatedAt ?? null,
    p_patch: patch, p_actor_email: input.actorEmail,
  });
  if (error) throw new Error(error.message);
...
export async function createFormDefinition(
  supabase: SupabaseClient,
  input: { tenantId: string; name: string; description?: string; actorEmail: string; schema?: unknown },
): Promise<FormDefinition> {
  const name = z.string().trim().min(1).max(120).parse(input.name);
  const schema = input.schema === undefined ? { elements: [] } : formSchemaValidator.parse(input.schema);
  assertSchemaSize(schema);
  return writeFormDefinition(supabase, { ...input, id: randomUUID() }, "create", {
    name, description: z.string().trim().max(2000).parse(input.description ?? ""),
    schema, share_token: newShareToken(),
  });
}
...
export async function saveFormDefinition(
  ...
  input: {
    ...
    schema: unknown; actorEmail: string; expectedUpdatedAt: string;
  },
): Promise<FormDefinition> {
  ...
  return writeFormDefinition(supabase, input, "save", {
    schema,
    ...(input.name === undefined ? {} : { name: z.string().trim().min(1).max(120).parse(input.name) }),
    ...(input.description === undefined ? {} : { description: z.string().trim().max(2000).parse(input.description) }),
  });
}
...
export async function setFormStatus(
  supabase: SupabaseClient,
  input: {
    tenantId: string; id: string; status: "draft" | "published" | "archived";
    actorEmail: string; expectedUpdatedAt: string;
  },
): Promise<FormDefinition> {
  z.iso.datetime({ offset: true }).parse(input.expectedUpdatedAt);
  const { data: current, error } = await supabase.from("form_definitions")
    .select("schema,updated_at").eq("tenant_id", input.tenantId).eq("id", input.id).maybeSingle();
  if (error) throw new Error(error.message);
  if (!current) throw new Error("Form not found");
  const schema = input.status === "published" ? formSchemaValidator.parse(current.schema) : current.schema;
...
export async function isFormModuleEnabled(
  ...
  const { data, error } = await supabase.from("tenants").select("status,config").eq("id", tenantId).maybeSingle();
...
export async function recordFormSubmission(
  ...
  const { data, error } = await supabase.rpc("record_form_submission", {
    p_token: z.string().regex(/^[a-f0-9]{64}$/).parse(input.token),
...
export async function submitPublicForm(token: string, response: unknown, requestId: string) {
  ...
  return recordFormSubmission(createPlatformServiceRoleClient("forms:public-submit"), {
    token, schema: form.schema, response, requestId,
  });
}
...
async function reviewFormSubmission(
  ...
  if (tenantIdForDatabase(supabase) !== input.tenantId) throw new Error("Form review does not match its workspace");
  const { data, error } = await callFormBuilderRpc(supabase, "review_form_submission", {
    p_id: input.id, p_decision: decision, p_request_id: input.requestId, p_actor_email: input.actorEmail,
  });
  if (error) throw new Error(error.message);
  return z.object({
    submissionId: z.uuid(), decision: z.enum(["accepted", "rejected"]),
    duplicate: z.boolean(), actionId: z.uuid().nullable(),
  }).strict().parse(data);
}
...
export async function acceptFormSubmission(
  ...
  const { data: submission, error } = await supabase.from("form_submissions")
    .select("contact_email").eq("tenant_id", input.tenantId).eq("id", input.id).maybeSingle();
  if (error) throw new Error(error.message);
  if (!submission) throw new Error("Submission not found");
  if (!submission.contact_email) throw new Error("This response has no email address, so it cannot become a lead");
  ...
  try {
    const current = await supabase.from("action_queue").select("status")
      .eq("tenant_id", input.tenantId).eq("id", receipt.actionId!).maybeSingle();
...
export async function executeFormIntake(supabase: SupabaseClient, raw: unknown, actionId: string) {
  ...
  if (tenantIdForDatabase(supabase) !== input.tenantId) throw new Error("Form intake does not match its workspace");
  const action = await supabase.from("action_queue").select("id").eq("tenant_id", input.tenantId)
    .eq("id", actionId).eq("action_type", "accept_form_submission").eq("status", "executing").maybeSingle();
  if (action.error || !action.data) throw new Error("A claimed form intake approval is required");
  const { data: submission, error } = await supabase.from("form_submissions")
    .select("id,form_id,contact_email,contact_name,status").eq("tenant_id", input.tenantId).eq("id", input.submissionId).maybeSingle();
  ...
  const form = await supabase.from("form_definitions").select("name").eq("tenant_id", input.tenantId).eq("id", submission.form_id).maybeSingle();
  ...
  const result = await ingestInboundLead(supabase, {
    name: ((submission.contact_name as string | null) ?? email.split("@")[0] ?? "Website inquiry").slice(0,120),
    email, source: "contact_form", sourceRecordId: `form-submission:${submission.id}`,
...
export async function executeFormPublish(
  ...
  return setFormStatus(supabase, { tenantId, id: input.formId, status: "published", actorEmail, expectedUpdatedAt: input.expectedUpdatedAt });
}
