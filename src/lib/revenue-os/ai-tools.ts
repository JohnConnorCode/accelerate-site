...
import {
  ...
  siteEditorExecuteSchema,
} from "@/lib/site-studio/editor-contract";
import {
  readSiteEditor,
  previewSiteChange,
  stageSiteChange,
  suggestSitePage,
} from "@/lib/site-studio/editor-service";
import {
  executeDelegatedSiteChange,
  isVerifiedSiteDelegation,
  type SiteEditorDelegation,
 ...
 * both directions.
 */
export function assertImpactHonoured(
  tool: AiToolRegistration,
  output: unknown,
  ...
  if (tool.executionPolicy === "site-studio-delegation") {
    if (
      ...
      !isVerifiedSiteDelegation(context?.siteEditorDelegation) ||
      (output as { authorization?: { mode?: string; scope?: string } })?.authorization?.mode !==
        "delegated" ||
...
const registry: AiToolRegistration[] = [
  {
    ...
    description:
      "Read the installation owner's website: paginated pages, content, assets, collections, entries, configuration, history, receipts, models, or portable export chunks. Supply revisionId to inspect a historical version. Use view=schema and schema=command/page/document/ai to discover exact supported fields before preparing changes. Never exposes other workspace data.",
    inputSchema: z.toJSONSchema(siteEditorReadSchema, { io: "input" }),
    parseInput: (input) => siteEditorReadSchema.parse(input),
    outputSchema: { type: "object" },
    serviceTarget: "site-studio.editor",
  ...
  },
  {
    ...
    description:
      "Preview exact website changes without saving. command requires requestKey UUID and expectedVersion from read_site_editor. Operations: edit with typed changes (create_page, put_page, remove_page, put_asset, remove_asset, put_collection, remove_collection, put_entry, remove_entry, configure); save with a complete portable document; publish/rollback with revisionId; unpublish. Returns a digest and summary. put_page covers sections, layout, copy and metadata. create_page requires a stable id, title, path and starter (service, landing, article), optionally cloneId.",
    inputSchema: {
      type: "object",
      required: ["command"],
  ...
  },
  {
    ...
    description:
      "Stage the exact command and digest returned by prepare_site_change. Creates a reviewable proposal; does not save or publish website content. Returns the proposal id, digest and exact summary for confirmation.",
    inputSchema: {
      type: "object",
  ...
  },
  {
    ...
    description:
      "Execute only an exact staged Site Studio proposal under the installation owner's revocable OAuth delegation. Supply actionId, digest and the exact human-readable summary. Saving stays private; publish, unpublish and rollback affect the public website. Ask for the client's write confirmation. No generic action approval or other workspace mutation is supported.",
  ...
  },
  {
    ...
    description:
      "Generate or revise a candidate website page using the same owner-only AI service as the editor. Requires instruction, page, mode (generate/edit), business, and optional model and priceCeiling. Uses the configured model budget but never saves or publishes; review and stage the returned page separately.",
    inputSchema: {
      type: "object",
      required: ["instruction", "page", "mode", "business"],
      properties: {
  ...
  },
  {
    ...
    description:
      "Read the business brief, source-backed attention, coworker progress and enabled App follow-up. Each source reports its own freshness and availability.",
    inputSchema: z.toJSONSchema(z.object({}).strict()),
    impact: "read",
    confirmationRequired: false,
    connectionRequirement: "none",
    serviceTarget: "revenue-os.today",
    outputSchema: { type: "object" },
  ...
  },
  {
    ...
    description:
      "Read this member's private Today views and shared defaults with revision numbers. Read before preparing any layout or preference change.",
    inputSchema: z.toJSONSchema(z.object({}).strict()),
    impact: "read",
    confirmationRequired: false,
    connectionRequirement: "none",
    serviceTarget: "revenue-os.today-views",
    outputSchema: { type: "object" },
  ...
  },
  {
    ...
    description:
      "Preview a complete personal or shared Today view document. Preserve unrelated views/preferences. Use its current revision and a new UUID requestId. Supports adding, editing, deleting, duplicating, setting defaults, pins and muted findings. Does not save.",
  ...
  },
  {
    ...
    description:
      "Prepare the exact previewed Today view change for human approval. Supply unchanged change and digest; no save occurs before approval.",
    inputSchema: z.toJSONSchema(
      z.object({ change: todaySaveSchema, digest: z.string().length(64) }).strict(),
    ),
    impact: "internal_write",
    confirmationRequired: true,
    connectionRequirement: "none",
