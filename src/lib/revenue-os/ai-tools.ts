...
const SNAPSHOT_OUTPUT_SCHEMA = {
  type: "object",
  required: ["unreadable", "queue", "openOpportunityCount", "openPipelineValue", "topOpportunities", "unreadConversationCount", "activeCampaigns", "openProposals", "truncated"],
  properties: {
 ...
 */
export function validateToolOutput(toolName: string, schema: Record<string, unknown>, output: unknown): void {
  const type = typeof schema.type === "string" ? schema.type : undefined;
  if (type === "array") {
    if (!Array.isArray(output)) throw new Error(`${toolName} returned an invalid output: expected an array.`);
    return;
  }
  ...
  for (const [key, spec] of Object.entries(properties)) {
    ...
    if (spec.type === "array" && !Array.isArray(result)) throw new Error(`${toolName} returned an invalid output: "${key}" must be an array.`);
    if (spec.type === "number" && (typeof result !== "number" || !Number.isFinite(result))) throw new Error(`${toolName} returned an invalid output: "${key}" must be a finite number.`);
    if (spec.type !== "array" && spec.type !== "number" && typeof result !== spec.type) throw new Error(`${toolName} returned an invalid output: "${key}" must be a ${spec.type}.`);
  }
}

function availabilityFor(tool: AiToolRegistration, context?: Pick<AiToolContext, "toolPack">): { available: boolean; reason: string } {
  if (context?.toolPack && !PACK_TOOL_NAMES[context.toolPack].includes(tool.name)) {
    return { available: false, reason: `${tool.name} is not available in the ${context.toolPack} tool pack.` };
  }
  ...
  return { available: true, reason: "Available through the bounded Revenue OS service; no provider connection is called directly." };
}
...
const registry: AiToolRegistration[] = [
  { name: "get_today_snapshot", description: "Read the founder's prioritized operator queue and a summary of current revenue state. Returns counts and the top items, not the full database.", inputSchema: { type: "object", properties: {}, additionalProperties: false }, outputSchema: SNAPSHOT_OUTPUT_SCHEMA, serviceTarget: "revenue-os.operator-queue", connectionRequirement: "none", impact: "read", confirmationRequired: false, execute: async ({ supabase }) => {
  ...
  { name: "search_pipeline", description: "Search live opportunities by company or email. Never invent a record or metric.", inputSchema: { type: "object", properties: { query: { type: "string" }, stage: { type: "string", enum: [...REVENUE_STAGES] } }, additionalProperties: false }, outputSchema: ARRAY_OUTPUT_SCHEMA, serviceTarget: "revenue-os.pipeline-search", connectionRequirement: "none", impact: "read", confirmationRequired: false, execute: async ({ supabase }, input) => {
  ...
  { name: "get_record_timeline", description: "Read the bounded canonical activity timeline for one contact, company, or opportunity. Every item includes its source receipt and occurrence time.", inputSchema: { type: "object", properties: { contactId: { type: "string" }, companyId: { type: "string" }, opportunityId: { type: "string" } }, additionalProperties: false }, outputSchema: TIMELINE_OUTPUT_SCHEMA, serviceTarget: "revenue-os.activity-ledger", connectionRequirement: "none", impact: "read", confirmationRequired: false, execute: async ({ supabase }, input) => {
  ...
  { name: "propose_send_email", description: "Stage an outbound email for founder approval. This never sends directly.", inputSchema: { type: "object", properties: { to: { type: "string" }, subject: { type: "string" }, body: { type: "string" }, opportunityId: { type: "string" }, contactId: { type: "string" }, reasoning: { type: "string" } }, required: ["to", "subject", "body", "reasoning"], additionalProperties: false }, outputSchema: ACTION_OUTPUT_SCHEMA, serviceTarget: "revenue-os.action-queue", connectionRequirement: "none", impact: "external_action", confirmationRequired: true, execute: async ({ supabase, actorEmail }, input) => { requireEmail(value(input, "to")); return proposeAction(supabase, { actionType: "send_email", title: `Send email: ${value(input, "subject") || "Untitled"}`, description: previewOf(String(input.body || "")), urgency: "normal", payload: input, reasoning: value(input, "reasoning") || "", sourceContext: "admin_ai", entityType: "opportunity", entityId: value(input, "opportunityId"), dedupeKey: `ai-email:${value(input, "to")}:${value(input, "subject")}`.slice(0, 220), proposedBy: actorEmail, expiresAt: new Date(Date.now() + 86400000).toISOString() }); } },
  { name: "propose_stage_change", description: "Stage a pipeline movement for founder approval. Evidence must be included.", inputSchema: { type: "object", properties: { opportunityId: { type: "string" }, stage: { type: "string", enum: [...REVENUE_STAGES] }, reason: { type: "string" }, lossReason: { type: "string" } }, required: ["opportunityId", "stage", "reason"], additionalProperties: false }, outputSchema: ACTION_OUTPUT_SCHEMA, serviceTarget: "revenue-os.action-queue", connectionRequirement: "none", impact: "internal_write", confirmationRequired: true, execute: async ({ supabase, actorEmail }, input) => proposeAction(supabase, { actionType: "transition_opportunity", title: `Move opportunity to ${value(input, "stage")}`, description: value(input, "reason") || "", urgency: "normal", payload: input, reasoning: value(input, "reason") || "", sourceContext: "admin_ai", entityType: "opportunity", entityId: value(input, "opportunityId"), dedupeKey: `ai-stage:${value(input, "opportunityId")}:${value(input, "stage")}`, proposedBy: actorEmail, expiresAt: new Date(Date.now() + 86400000).toISOString() }) },
  { name: "propose_task", description: "Stage a concrete operator task for approval.", inputSchema: { type: "object", properties: { title: { type: "string" }, description: { type: "string" }, dueDate: { type: "string" }, priority: { type: "string", enum: ["high", "medium", "low"] }, opportunityId: { type: "string" } }, required: ["title", "priority"], additionalProperties: false }, outputSchema: ACTION_OUTPUT_SCHEMA, serviceTarget: "revenue-os.action-queue", connectionRequirement: "none", impact: "internal_write", confirmationRequired: true, execute: async ({ supabase, actorEmail }, input) => {
...
export function listRevenueAiCapabilities(context?: Pick<AiToolContext, "toolPack">): RevenueAiCapabilityDescriptor[] {
...
export async function executeRegisteredRevenueTool(context: AiToolContext, name: string, input: Record<string, unknown>) {
  ...
  const availability = availabilityFor(tool, context);
  if (!availability.available) throw new Error(`${tool.name} is unavailable: ${availability.reason}`);
