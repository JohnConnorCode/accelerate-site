...
export async function compensateAction(
  ...
  const conversation = ["assign_conversation", "update_conversation_status"].includes(
    String(action.action_type),
  );
  const { data, error: undoError } = await supabase.rpc(
    conversation ? "apply_conversation_action" : "apply_local_action",
    {
