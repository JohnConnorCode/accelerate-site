...
export function normalizeCampaignPolicy(value: unknown): Required<CampaignPolicy> {
  ...
  return {
    daily_limit: Math.min(
      process.env.CAMPAIGN_AUTOMATION_ENABLED === "true" ? 200 : 10,
      Math.max(1, Number(input.daily_limit) || DEFAULT_POLICY.daily_limit),
    ),
...
export function globalDailySendCap(): number {
  ...
  return Number.isFinite(override) && override > 0
    ? Math.min(override, automationCeiling * 10)
    : automationCeiling;
}
 ...
 * duplicated.
 */
export async function recoverStaleCampaignSendClaims(
  ...
  for (const row of data ?? []) {
    await recordStaleClaimRecovery(supabase, {
      ...
      detail:
        "A campaign send claimed this member and never reported a terminal state, so the claim was released for a later executor to retry under the original send key.",
    });
  }
...
async function claimCampaignMemberSend(
  supabase: SupabaseClient,
  memberId: string,
  claimKey: string,
) {
  const { data, error } = await supabase
    .rpc("claim_campaign_member_send", { p_member_id: memberId, p_claim_key: claimKey })
    .maybeSingle();
...
export async function activateCampaign(supabase: SupabaseClient, id: string, actorEmail: string) {
  const { data: campaign, error } = await supabase
    .from("campaigns")
    .select("id,status,version,approved_version,name")
    .eq("id", id)
    .maybeSingle();
  if (error) throw new Error(error.message);
  ...
  if (count > 1 && process.env.CAMPAIGN_AUTOMATION_ENABLED !== "true")
    throw new Error(
      "The money-first pilot is limited to one email step until automated reply and bounce stops are enabled",
    );
...
export async function executeDueCampaignMembers(
  supabase: SupabaseClient,
  now = new Date(),
  campaignId?: string,
) {
  ...
  let campaignQuery = supabase
    .from("campaigns")
    .select("id,name,status,version,approved_version,policy,sender_name,sender_email")
    .eq("status", "active");
  ...
  if (campaigns?.length) {
    const { data: recoveryPlaybooks, error: recoveryError } = await supabase
      .from("recovery_playbooks")
      .select("campaign_id,offer_label,booking_url")
      .in(
        "campaign_id",
    ...
    if (recoveryError && !isMissingRevenueSchema(recoveryError))
      throw new Error(recoveryError.message);
    for (const playbook of recoveryPlaybooks ?? [])
      recoveryByCampaign.set(playbook.campaign_id, playbook);
  }
  let sent = 0;
  let failed = 0;
  let stopped = 0;
  ...
  const { count: sentTodayAllCampaigns } = await supabase
    .from("messages")
    .select("id", { count: "exact", head: true })
  ...
  for (const campaign of campaigns ?? []) {
    ...
    const { data: members, error: memberError } = await supabase
      .from("campaign_members")
      .select(
        "id,email,current_step,contact_id,opportunity_id,send_attempts,contacts!campaign_members_contact_id_tenant_fkey(full_name),opportunities!campaign_members_opportunity_id_tenant_fkey(name,stage)",
      )
    ...
    for (const member of members ?? []) {
      const opportunity = Array.isArray(member.opportunities)
        ? member.opportunities[0]
        : member.opportunities;
      if (
        opportunity &&
        ["meeting", "proposal", "negotiation", "won", "lost", "booked", "showed"].includes(
          opportunity.stage,
        )
      ) {
        await stopCampaignMemberships(supabase, {
          ...
          reason: opportunity.stage === "won" ? "opportunity_converted" : "opportunity_progressed",
          source: "automation",
        });
        stopped++;
        continue;
      ...
      try {
        if (
          !(await campaignMemberMaySend(supabase, { memberId: member.id, campaignId: campaign.id }))
        ) {
          await supabase
            .from("campaign_members")
            .update({ status: "stopped", stop_reason: "pre_send_policy_check", next_send_at: null })
            .eq("id", member.id)
            .eq("status", "sending");
          stopped++;
          continue;
        ...
        await supabase
          ...
          .update({
            ...
            next_send_at: exhausted
              ? null
              : new Date(now.getTime() + attempts * 3600000).toISOString(),
          })
          .eq("id", member.id)
          .eq("status", "sending");
        failed++;
