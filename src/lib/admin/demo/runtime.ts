...
const keyFor = (id: DemoScenarioId) => `accelerate:admin-demo:${id}:v1`;
...
function queue(pack: DemoScenarioPack, state: DemoState) {
  const actions = pack.actions.filter((item) => !state.completedActions.includes(item.id)).map((item, index) => ({ id: `action:${item.id}`, kind: "approval", title: item.title, summary: item.description, urgency: index < 2 ? "high" : "normal", dueAt: dateOffset(0), sourceTimestamp: new Date(Date.now() - (index + 1) * 3_600_000).toISOString(), priorityReason: item.description, recommendedNextAction: "Review the exact simulated change", href: `/admin/today?focus=approvals&action=${item.id}` }));
  const tasks = pack.tasks.filter((item) => item.status === "pending" && !state.completedTasks.includes(item.id)).slice(0, 6).map((item) => ({ id: `task:${item.id}`, kind: "task", title: item.title, summary: "Linked to the latest conversation and opportunity context.", urgency: item.priority === "high" ? "high" : "normal", dueAt: dateOffset(item.dueOffset), sourceTimestamp: new Date(Date.now() - 7_200_000).toISOString(), priorityReason: item.dueOffset < 0 ? "The commitment is overdue." : "The next action is due soon.", recommendedNextAction: "Complete or snooze this task", href: "/admin/today" }));
...
function campaigns(pack: DemoScenarioPack) {
  return ["New inquiry follow-up", "Seasonal re-engagement", "Referral thank-you"].map((name, index) => ({
...
function contentItems(pack: DemoScenarioPack) {
  const titles = ["A practical planning guide", "What customers should expect", "The questions we hear most", "How our process protects your time", "A seasonal readiness checklist", "A customer story with clear outcomes", "The cost of waiting too long", "How to choose the right next step", "Behind the scenes at our team", "A simple comparison guide"];
  return titles.map((title, index) => ({ id: `content-${index}`, title: `${title}: ${pack.category}`, slug: `${title.toLowerCase().replace(/[^a-z0-9]+/g, "-")}-${index}`, status: ["idea", "outline", "draft", "review", "published"][index % 5]!, category: ["lead-generation", "automation", "industry", "foundational"][index % 4]!, target_keywords: [pack.category.toLowerCase(), "planning", "next steps"], pillar: ["Lead Gen", "Automation", "Industry", "Foundational"][index % 4]!, funnel_stage: ["awareness", "consideration", "decision"][index % 3]!, target_publish_date: dateOffset(index * 5 - 8), actual_publish_date: index % 5 === 4 ? dateOffset(-index) : undefined, author: pack.tenant.founder.fullName, notes: "Fictional editorial brief grounded in current customer questions.", seo_title: `${title} | ${pack.name}`, seo_description: `A useful, specific resource from ${pack.name}.`, word_count_target: 900 + index * 100, created_at: ago(240 + index * 24), updated_at: ago(index * 5 + 1) }));
}

function featureBoard(pack: DemoScenarioPack) {
  const titles = ["Automated inquiry routing", "Conversation response-time report", "Customer onboarding checklist", "Payment outcome connection", "Calendar capacity alerts", "Referral source attribution", "Mobile approval shortcuts", "Knowledge-source citations", "Campaign exclusion preview", "Weekly operating brief", "Duplicate contact resolution", "Delivery milestone tracking", "Customer health signals", "Team workload view", "Provider reliability receipts"];
