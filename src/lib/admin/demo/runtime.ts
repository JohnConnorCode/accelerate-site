...
import { TODAY_TOOL_NAMES, defaultTodayViews, todaySaveSchema, type TodayViews } from "@/lib/admin/today-workspace";
...
function aiCapabilities(tenantConfig: { modules: Partial<Record<string, boolean>> }) {
  ...
  rows.push(
    ...TODAY_TOOL_NAMES.map((name) => [name, name === "get_today_workspace" ? "Read source-backed Today context and App follow-up." : "Read, preview or propose this member’s saved Today arrangement. Changes require exact approval.", name.startsWith("propose_") ? "internal_write" : "read", name.startsWith("propose_"), ["core", "pipeline", "outreach"], "revenue-os.today-views"] as (typeof rows)[number]),
...
export function installAdminDemoRuntime(scenarioId: DemoScenarioId) {
  ...
  const demoFetch: typeof window.fetch = async (input, init) => {
    ...
    if (path === "/api/admin/revenue-os/today/views") {
      ...
      if (method === "GET") return jsonResponse({ ...views, userId: "demo-member", tenantId: scenarioId });
      ...
      const change = parsed.data, fingerprint = JSON.stringify(change);
      const prior = state.todayViewReceipts?.[change.requestId];
      if (prior) return prior.fingerprint === fingerprint ? jsonResponse(prior.result) : jsonResponse({ error: "Request key content changed" }, 409);
      if (views[change.scope].revision !== change.revision) return jsonResponse({ error: "Today views changed in another session. Reload before saving." }, 409);
      const result = { revision: change.revision + 1, document: change.document };
      state.todayViews = { ...views, [change.scope]: result };
      state.todayViewReceipts = { ...state.todayViewReceipts, [change.requestId]: { fingerprint, result } };
      saveState(scenarioId, state);
      return jsonResponse({ ...result, simulated: true });
    }
    if (path === "/api/admin/revenue-os/today") {
      const at = new Date().toISOString();
      const region = <T,>(data: T): TodayRegion<T> => ({ data, state: data === null || (Array.isArray(data) && !data.length) ? "empty" : "ready", observedAt: at });
      ...
      const activity = history.slice(0, 12).map((row) => ({
        id: row.id, title: row.action.replaceAll(".", " ").replaceAll("_", " "),
        summary: null, at: row.createdAt, href: "/admin/activity",
      }));
      const opportunities = opportunityRows(pack, state).filter((o) => !["won", "lost"].includes(o.canonical_stage));
      const apps = getActiveModules({ modules: { ...DEMO_BUSINESS_MODULES, ...state.moduleOverrides } }).filter((m) => m.today).map((m) => {
        const items = m.today!.source === "collection_case"
          ? seedDemoCollections(pack, business).filter((c) => c.status === "open").slice(0, 8).map((c) => ({ id: c.id, title: c.name + " · " + c.currency.toUpperCase(), detail: c.nextAction, sourceType: "collection_case", sourceId: c.id, href: "/admin/collections?case=" + c.id }))
          : (business.radar ??= seedRadar(pack)).opportunities.filter((o) => o.state !== "archived").slice(0, 8).map((o) => ({ id: o.id, title: o.title, detail: o.summary, sourceType: "radar_opportunity", sourceId: o.id, href: "/admin/radar/opportunities/" + o.id }));
        return { id: m.id, name: m.name, href: m.today!.href, items, state: items.length ? "ready" as const : "empty" as const };
      });
      const snapshot: TodaySnapshot = {
        generatedAt: at, attention: region(attention), activity: region(activity),
        facts: region(todayFacts(attention, activity)), apps: region(apps), brief: region(null),
        handling: region(aiRuns(pack, state).runs.slice(0, 3).map((run) => ({
          id: run.id, title: run.promptPreview, status: run.status, owner: "Workspace AI",
          outcome: run.resultPreview, nextCheckAt: null, nextCheckReason: null, href: "/admin/ai",
        }))),
        metrics: region({ openOpportunities: opportunities.length, pipelineValue: opportunities.reduce((sum, o) => sum + o.estimated_value, 0), weightedValue: Math.round(opportunities.reduce((sum, o) => sum + o.estimated_value * o.probability / 100, 0)) }),
      };
