...
export const siteEditorChangeSchema = z.discriminatedUnion("kind", [
  z.object({ kind: z.literal("create_page"), id, title: z.string().min(1).max(160), path: websitePageSchema.shape.path, starter: z.enum(["service", "landing", "article"]), cloneId: id.optional() }).strict(),
  ...
  z.object({ kind: z.literal("put_entry"), collectionId: id, entry: shape.collections.element.shape.entries.element }).strict(),
  z.object({ kind: z.literal("remove_entry"), collectionId: id, id }).strict(),
  z.object({
    kind: z.literal("configure"),
    identity: shape.identity.optional(), theme: shape.theme.optional(),
    navigation: shape.navigation.optional(), header: shape.header.removeDefault().optional(),
    footer: shape.footer.optional(), dock: shape.dock.removeDefault().optional(),
  }).strict(),
]);
export const siteEditorCommandSchema = z.union([
  websiteCommandSchema,
  z.object({
    operation: z.literal("edit"), requestKey: z.uuid(),
...
export const siteEditorStageSchema = siteEditorPrepareSchema.extend({
  digest: z.string().regex(/^[a-f0-9]{64}$/),
}).strict();
export const siteEditorExecuteSchema = z.object({
  actionId: z.uuid(), digest: z.string().regex(/^[a-f0-9]{64}$/),
  summary: z.string().min(1).max(500),
}).strict();
export const siteEditorReadSchema = z.object({
  view: z.enum(["pages", "page", "assets", "collections", "collection", "entry", "configuration", "history", "receipts", "models", "export"]),
  state: z.enum(["draft", "published"]).default("draft"),
  id: id.optional(), collectionId: id.optional(),
  offset: z.number().int().min(0).max(8_000_000).default(0),
  limit: z.number().int().min(1).max(50).default(20),
}).strict();
export const SITE_EDITOR_TOOL_NAMES = [
  "read_site_editor", "prepare_site_change", "stage_site_change",
  "execute_site_change", "suggest_site_page",
] as const;
