...
export function siteEditorOAuthConfig() {
  ...
  const clientIds = (process.env.SITE_STUDIO_OAUTH_CLIENT_IDS ?? "")
    .split(",")
    .map((id) => id.trim())
    .filter(Boolean);
...
export async function authenticateSiteEditor(token: string): Promise<SiteEditorDelegation> {
  ...
  const [tenant, membership, delegation] = await Promise.all([
    database
      ...
      .eq("id", ACCELERATE_TENANT_ID)
      .maybeSingle(),
    database
      .from("tenant_memberships")
      .select("role,status")
      .eq("tenant_id", ACCELERATE_TENANT_ID)
      .eq("user_id", user.id)
      .maybeSingle(),
    database
      .from("site_editor_delegations")
      .select("*")
      .eq("tenant_id", ACCELERATE_TENANT_ID)
      .eq("user_id", user.id)
  ...
  const auth: AdminAuthorization = {
    kind: "actor",
    tenant: tenant.data as TenantSummary,
  ...
  const result = {
    auth,
    grant: siteDelegationSchema.parse(delegation.data),
...
export async function manageSiteDelegation(
  auth: AdminAuthorization,
  input: { operation: "grant"; clientId: string } | { operation: "revoke"; grantId: string },
) {
  assertWebsiteOwner(auth);
  const config = siteEditorOAuthConfig();
  if (input.operation === "grant" && !config.clientIds.includes(input.clientId))
...
export async function executeDelegatedSiteChange(delegation: unknown, raw: unknown) {
  if (!isVerifiedSiteDelegation(delegation))
    throw new Error("Connect with owner-approved Site Studio OAuth to execute changes");
  const input = siteEditorExecuteSchema.parse(raw);
  const { auth, grant } = delegation;
  assertWebsiteOwner(auth);
  const { data: action, error: readError } = await auth.database
    .from("action_queue")
    .select("payload")
    .eq("tenant_id", auth.tenant.id)
    .eq("id", input.actionId)
    .eq("action_type", "site_website_change")
    .maybeSingle();
  if (readError || !action) throw new Error("Website proposal unavailable");
  const command = parseWebsiteCommand(action.payload.command);
  if (
    websiteCommandDigest(command) !== input.digest ||
    action.payload.digest !== input.digest ||
  ...
  const { data, error } = await host.rpc("execute_delegated_site_change", {
    ...
    p_session_id: delegation.sessionId,
  ...
  if (error)
    throw new Error(
      "The delegated website change was refused. Read the current state and delegation before retrying.",
    );
  return z
    .object({
      id: z.uuid(),
      action_type: z.literal("site_website_change"),
      receipt: websiteReceiptSchema,
      authorization: z
        .object({
          mode: z.literal("delegated"),
          scope: z.literal("site-studio"),
          grantId: z.uuid(),
          clientId: z.string(),
          userId: z.uuid(),
        })
