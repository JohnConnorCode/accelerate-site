...
function parseRegeneratedChildren(value: unknown): SiteLeafNode[] {
  ...
  return children.map((child, index) => {
    ...
    if (!parsed.success)
      throw new Error(`Regenerated node ${index + 1} is invalid: ${parsed.error.issues[0]?.message ?? "schema refusal"}`);
    return parsed.data;
  });
}
...
export async function regenerateSection(
  ...
  let raw: unknown;
  try {
    raw = await complete(buildSectionSystemPrompt(), buildSectionUserPrompt(section, input.direction));
  } catch (error) {
    throw new Error(
