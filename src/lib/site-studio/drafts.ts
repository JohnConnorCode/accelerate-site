...
export async function createSiteDraft(
  ...
  if ((await repo.list()).some((draft) => draft.slug === slug)) throw new SlugInUseError(slug);
...
export async function discardSiteDraft(
  ...
  if (expectedChecksum && expectedChecksum !== current.checksum)
    throw new Error("This draft changed; reload before discarding");
  if (!(await repo.remove(id, current.checksum))) throw new DraftNotFoundError(id);
