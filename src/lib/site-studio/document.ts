...
const ctaSchema = z.object({ label: z.string().min(1).max(60), href }).strict();
...
const featureGridSchema = z
  .object({
    ...
    items: z
      .array(
        z.object({ title: z.string().min(1).max(80), body: z.string().min(1).max(300) }).strict(),
      )
      .min(1)
      .max(6),
  })
...
export const siteDraftSchema = z
  .object({
    ...
    brief: z.string().min(1).max(2400).optional(),
