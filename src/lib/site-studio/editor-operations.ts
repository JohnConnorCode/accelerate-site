...
function put<T extends { id: string }>(items: T[], value: T): T[] {
  return items.some(item => item.id === value.id)
    ? items.map(item => item.id === value.id ? value : item) : [...items, value];
}
...
export function applySiteEditorCommand(website: WebsiteDocument, raw: SiteEditorCommand): WebsiteCommand {
  ...
  for (const change of input.changes) {
    switch (change.kind) {
      case "create_page":
        if (next.pages.some(page => page.id === change.id)) throw new Error("Page identity already exists");
        next.pages.push(createWebsitePage(next, change));
        break;
      case "put_page": next.pages = put(next.pages, change.page); break;
      case "remove_page": next.pages = remove(next.pages, change.id); break;
      case "put_asset": next.assets = put(next.assets, change.asset); break;
      case "remove_asset": next.assets = remove(next.assets, change.id); break;
      case "put_collection": next.collections = put(next.collections, change.collection); break;
      case "remove_collection": next.collections = remove(next.collections, change.id); break;
      case "put_entry":
      case "remove_entry": {
        ...
        collection.entries = change.kind === "put_entry" ? put(collection.entries, change.entry) : remove(collection.entries, change.id);
        break;
      }
      case "configure": {
        const { kind: _kind, ...configuration } = change;
  ...
  return parseWebsiteCommand({
    operation: "save", requestKey: input.requestKey, expectedVersion: input.expectedVersion,
