...
export async function resolveTenantProviderSecrets(tenantSlug: string, provider: "resend" | "calendly") {
  ...
  const { data: connection } = await platform.from("integration_connections")
  ...
  const read = (key: string) => {
    ...
    if (!isEncryptedSecret(value)) throw new Error(`${provider} ${key} is not in the encrypted envelope`);
    return decryptSecret(value);
  };
  const allowEnvironment = tenant.id === ACCELERATE_TENANT_ID && (!connection || connection.environment_fallback_allowed);
  const context: TenantSystemContext = { kind: "system", tenantId: tenant.id, tenantSlug: tenant.slug, source: `${provider}-webhook` };
  return {
    context,
