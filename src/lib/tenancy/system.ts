...
import { ACCELERATE_TENANT_ID, ACCELERATE_TENANT_SLUG, getTenantRequestContext, type TenantSystemContext } from "@/lib/tenancy/context";
...
export async function listTenantSystemContexts(input: {
  ...
  if (input.provider) {
    const { data, error } = await platform.from("integration_connections")
      .select("tenant_id")
      .eq("provider", input.provider)
      .eq("status", "connected");
    if (error) throw new Error(error.message);
    tenantIds = [...new Set((data || []).map((connection) => connection.tenant_id))];
    if (input.includeBootstrapFallback && !tenantIds.includes(ACCELERATE_TENANT_ID)) tenantIds.push(ACCELERATE_TENANT_ID);
  }
  ...
  if (error) throw new Error(error.message);
  return (tenants || []).map((tenant) => ({ kind: "system", tenantId: tenant.id, tenantSlug: tenant.slug, source: input.source }));
}

export function bootstrapFallbackContext(source: string): TenantSystemContext {
  return { kind: "system", tenantId: ACCELERATE_TENANT_ID, tenantSlug: ACCELERATE_TENANT_SLUG, source };
}

export async function resolveActiveTenantSystemContext(tenantSlug: string, source: string): Promise<TenantSystemContext | null> {
  if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(tenantSlug)) return null;
  const platform = createPlatformServiceRoleClient(`tenant-system-resolver:${source}`);
  const { data, error } = await platform.from("tenants").select("id,slug,status").eq("slug", tenantSlug).maybeSingle();
  if (error) throw new Error(error.message);
