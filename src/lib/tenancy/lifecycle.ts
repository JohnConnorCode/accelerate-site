...
export interface PlatformTenantActor {
  userId: string;
  email: string;
}
...
function rpcData<T>(data: unknown, error: { message: string } | null): T {
  if (error) {
    if (/duplicate|unique/i.test(error.message)) {
      throw new TenantLifecycleError(409, "That workspace slug or membership already exists", {
        cause: error,
      });
    }
    if (/not found/i.test(error.message)) {
      throw new TenantLifecycleError(404, "The requested tenant resource was not found", {
        cause: error,
      });
    }
    if (
      /^(Invitations are disabled while tenant is (?:suspended|archived)|The bootstrap tenant cannot be suspended or archived|Archived tenants cannot be reactivated|The platform owner membership cannot be revoked)$/i.test(
        error.message,
      )
    ) {
    ...
    if (/required|invalid/i.test(error.message)) {
      throw new TenantLifecycleError(400, "The tenant lifecycle request is invalid", {
        cause: error,
      });
    }
    throw new TenantLifecycleError(500, "The tenant lifecycle operation could not be completed", {
      cause: error,
    });
  }
  ...
  for (let page = 1; page <= 100; page += 1) {
    ...
    if (error)
      throw new TenantLifecycleError(
        502,
        "The administrator directory is temporarily unavailable",
        { cause: error },
      );
...
async function resolveInvitedUser(
  ...
  if (error || !userId || !tokenHash) {
    throw new TenantLifecycleError(502, "The administrator invitation link could not be prepared", {
...
async function recordInvitationDelivery(input: {
  ...
  if (error)
    throw new TenantLifecycleError(500, "The invitation delivery receipt could not be recorded", {
      cause: error,
    });
}
...
export async function inviteTenantAdmin(input: {
  ...
  const { data: tenant, error: tenantError } = await lookupDatabase
    .from("tenants")
    .select("id,slug,name,status")
    .eq("id", input.tenantId)
    .single();
  if (tenantError || !tenant) {
    throw new TenantLifecycleError(404, "The requested workspace was not found", {
      cause: tenantError || undefined,
    });
  }
  if (!["provisioning", "active"].includes(tenant.status)) {
    throw new TenantLifecycleError(
      409,
      `Invitations are disabled while tenant is ${tenant.status}`,
    );
  }
  ...
  try {
    delivery = await getResend().emails.send(
      {
        ...
        html: tenantAdminInvitationEmail({
    ...
    if (delivery.error || !delivery.data?.id) {
      ...
      return {
        ...
        warning:
          "Access is recorded, but the invitation email was not delivered. Use Resend invitation to try again safely.",
      };
    }
  } catch {
    return {
      ...
      operatorMessage: `Invitation pending for ${normalized}`,
      warning:
        "Access is recorded, but the email provider outcome is unknown. Check Resend before sending another invitation.",
    };
  }
  ...
  try {
    ...
    return {
      ...
      warning:
        "The provider accepted the invitation, but its local audit receipt could not be recorded. Do not resend until the receipt is reconciled.",
    };
  }
  return {
    membership,
...
export async function activateInvitedTenantMembership(input: {
  tenantId: string;
  tenantSlug: string;
  userId: string;
  email: string;
  ...
  const { data: tenant, error: tenantError } = await database
    ...
    .maybeSingle();
  if (tenantError)
    throw new TenantLifecycleError(500, "The invitation workspace could not be verified", {
      cause: tenantError,
    });
  ...
  const { data: membership, error: membershipError } = await database
    ...
    .maybeSingle();
  if (membershipError)
    throw new TenantLifecycleError(500, "The invitation could not be activated", {
      cause: membershipError,
    });
  if (!membership) {
    const { data: active } = await database
      .from("tenant_memberships")
      .select("id,tenant_id,user_id,invited_email,status")
      .eq("tenant_id", input.tenantId)
      .eq("user_id", input.userId)
...
export async function createTenantWorkspace(input: {
  ...
  if (
    !input.adminEmail ||
    input.adminEmail.trim().toLowerCase() === input.actor.email.toLowerCase()
  ) {
    return { tenant, membership: null, warning: null };
  }
  try {
    const invitation = await inviteTenantAdmin({
      tenantId: tenant.id,
    ...
    return {
      ...
      operatorMessage: invitation.operatorMessage,
    ...
    return {
      ...
      membership: null,
      warning:
        "Workspace created safely, but the first administrator invitation could not be completed. Retry the invitation from the workspace card.",
    };
  }
}

export async function setTenantLifecycleStatus(input: {
  tenantId: string;
...
export async function revokeTenantAdmin(input: {
  membershipId: string;
  actor: PlatformTenantActor;
}) {
