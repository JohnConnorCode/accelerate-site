...
function parsePrice(value: string | undefined): number {
  ...
  if (!Number.isFinite(amount) || amount < 0)
    throw new Error("Approved service pricing contains an invalid amount");
  return amount;
}
...
export function assertApprovedPrice(input: {
  item: string;
  oneTime: number;
  monthly: number;
}): void {
  ...
  if (!approved || approved.oneTime !== input.oneTime || approved.monthly !== input.monthly) {
    throw new Error(
      "Pricing for " +
        input.item +
        " is not an approved catalog price. Leave pricing for founder scope confirmation instead.",
    );
  }
}

export function assertApprovedPricingRows(
  rows: Array<{ item: string; oneTime: number; monthly: number }>,
): void {
  for (const row of rows) assertApprovedPrice(row);
}

export function assertApprovedPriceComponent(input: {
  item: string;
  amount: number;
  ...
  if (!approved || approved[input.kind] !== input.amount) {
    throw new Error(
      "Pricing for " + input.item + " is not an approved catalog " + input.kind + " amount.",
    );
  }
}
