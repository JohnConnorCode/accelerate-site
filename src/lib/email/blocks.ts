...
export const EMAIL_LAYOUTS: Array<{
  id: string;
  name: string;
  subject: string;
  previewText: string;
...
const interpolate = (value: string, variables: Record<string, string>) =>
  value.replace(/\{\{([A-Za-z0-9_]+)\}\}/g, (_, key: string) => variables[key] ?? `{{${key}}}`);
...
export function createEmailBlock(
  ...
  switch (type) {
    ...
    case "paragraph":
      return { id, type, text: "Write the message your reader needs." };
    case "button":
      return { id, type, text: "Book a call", url: `${siteUrl()}/contact` };
    case "divider":
      return { id, type };
    case "spacer":
...
export function validateEmailBlocks(value: unknown): EmailBlock[] | null {
  ...
  const valid = value.every((block) => {
    ...
    if (candidate.type === "heading" || candidate.type === "paragraph")
      return typeof candidate.text === "string" && candidate.text.length <= 4_000;
...
export function parseStoredEmailBlocks(value: string | null | undefined): EmailBlock[] | null {
  ...
  try {
    return validateEmailBlocks(JSON.parse(value.slice(STORAGE_PREFIX.length)));
  } catch {
    return null;
  }
}
  ...
  return blocks
    .flatMap((block) => {
      if (block.type === "heading" || block.type === "paragraph")
        return [interpolate(block.text, variables)];
      if (block.type === "button")
        return [`${interpolate(block.text, variables)}: ${interpolate(block.url, variables)}`];
      return [];
    })
    .filter(Boolean)
    .join("\n\n");
...
const paragraph = (text: string): MailyNode => ({
  type: "paragraph",
...
function emailDocument(blocks: EmailBlock[], variables: Record<string, string>) {
  const content: MailyNode[] = [
    {
      ...
      content: [{ type: "text", text: tenant.brand.name }],
    },
    { type: "horizontalRule" },
  ];
  for (const block of blocks) {
    if (block.type === "heading")
      content.push({
        ...
        content: [{ type: "text", text: interpolate(block.text, variables) }],
      });
    if (block.type === "paragraph")
      interpolate(block.text, variables)
        .split(/\n{2,}/)
        .forEach((text) => content.push(paragraph(text.replace(/\n/g, " "))));
    if (block.type === "button")
      content.push({
        type: "button",
        attrs: {
          text: interpolate(block.text, variables),
          url: interpolate(block.url, variables),
          alignment: "left",
          buttonColor: "#1b211e",
          textColor: "#c8ef58",
          borderRadius: "smooth",
  ...
  content.push(
    { type: "horizontalRule" },
    paragraph(`${tenant.brand.name} · ${tenant.brand.domain}`),
  );
...
export async function renderEmailBlocks(
  ...
  try {
    const html = await render(
      emailDocument(blocks, variables) as never,
      { preview: previewText || undefined } as never,
    );
    return { html, text };
  } catch {
    const body = blocks
      .map((block) => {
        if (block.type === "heading")
          return `<h1 style="margin:0 0 16px;font-size:28px;line-height:1.2;color:#151611">${escapeHtml(interpolate(block.text, variables))}</h1>`;
        if (block.type === "paragraph")
          return `<p style="margin:0 0 16px;font-size:15px;line-height:1.65;color:#3f4744">${escapeHtml(interpolate(block.text, variables)).replace(/\n/g, "<br>")}</p>`;
        if (block.type === "button")
          return `<p style="margin:24px 0"><a href="${escapeHtml(interpolate(block.url, variables))}" style="display:inline-block;border-radius:8px;background:#1b211e;padding:14px 18px;color:#c8ef58;font-weight:700;text-decoration:none">${escapeHtml(interpolate(block.text, variables))} →</a></p>`;
        if (block.type === "divider")
          return `<hr style="border:0;border-top:1px solid #dfe5df;margin:24px 0">`;
        return `<div style="height:${block.height}px"></div>`;
      })
      .join("");
    return {
      text,
      html: `<!doctype html><html><body style="margin:0;background:#eef1ee;font-family:Arial,sans-serif"><main style="max-width:600px;margin:32px auto;padding:32px;background:#fff;border-radius:16px">${body}</main></body></html>`,
    };
  }
}
