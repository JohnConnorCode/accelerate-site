...
const transactional: EmailTemplateDefinition[] = [
  {
    ...
    variables: ["name", "planSummary", "planLink"],
    sampleData: sample,
    renderDefaultHtml: (v) =>
      planConfirmationEmail(
        v.name || "there",
        v.planSummary || "Your recommendations are ready.",
        v.planLink || `${siteUrl()}/contact`,
      ),
  },
  {
  ...
  },
  {
    ...
    renderDefaultHtml: (v) =>
      roiReportEmail({
        name: v.name,
        roiPercentage: Number(v.roiPercentage || 0),
        additionalMonthlyRevenue: v.additionalMonthlyRevenue || "$0",
        annualRevenueImpact: v.annualRevenueImpact || "$0",
        timeSavedPerWeek: v.timeSavedPerWeek || "0",
        paybackPeriodMonths: v.paybackPeriodMonths || "0",
      }),
  },
  {
    key: "admin-lead",
    ...
    bodyTemplate:
      "New plan inquiry needs review.\n\nName: {{name}}\nEmail: {{email}}\nPhone: {{phone}}\nBusiness: {{business}}\nIndustry: {{industry}}\n\nOpen the record, confirm the fit, and choose the next action.",
    variables: ["name", "email", "phone", "business", "industry"],
    sampleData: sample,
    renderDefaultHtml: (v) =>
      adminLeadNotificationEmail({
        name: v.name || "Unknown",
...
const sequenceDefinitions: EmailTemplateDefinition[] = Object.entries(emailSequences).flatMap(
  ([type, steps]) =>
    steps.map((step) => ({
      key: `${type.replaceAll("_", "-")}-${step.stepNumber}`,
      name: `${type.replaceAll("_", " ").replace(/\b\w/g, (value) => value.toUpperCase())} · Email ${step.stepNumber}`,
      description: step.delayDays
        ? `Automated follow-up after ${step.delayDays} days.`
        : "Immediate sequence message.",
      category: "Automated sequences",
      subjectTemplate: step.subject,
      bodyTemplate: step.bodyTemplate,
      variables: [
        ...new Set(
          `${step.subject} ${step.bodyTemplate}`
            .match(/\{\{([A-Za-z0-9_]+)\}\}/g)
...
export function renderDefinition(
  ...
  variables = definition.sampleData,
) {
