...
export async function getTenantResend(supabase: SupabaseClient): Promise<Resend> {
  ...
  const tenantId = tenantIdForDatabase(supabase) || (context?.kind === "actor" ? context.tenant.id : context?.tenantId);
  ...
  const credentials = connection?.encrypted_credentials && typeof connection.encrypted_credentials === "object"
  ...
  if (encryptedApiKey && connection?.status === "connected") {
    if (!isEncryptedSecret(encryptedApiKey)) throw new Error("Resend credential is not in the encrypted envelope");
    return new Resend(decryptSecret(encryptedApiKey));
  }
  if (encryptedApiKey) throw new Error("Resend is not active for this tenant");
  const mayUseEnvironment = tenantId === ACCELERATE_TENANT_ID && (!connection || connection.environment_fallback_allowed);
...
export async function getTenantFromEmail(supabase: SupabaseClient): Promise<string> {
  ...
  const { data: connection, error } = await supabase.from("integration_connections")
    ...
    .maybeSingle();
  if (error) throw new Error(error.message);
  const configured = typeof connection?.account_email === "string" ? connection.account_email.trim().toLowerCase() : "";
...
export async function getTenantReplyToEmail(supabase: SupabaseClient): Promise<string> {
  ...
  const { data: connection, error } = await supabase.from("integration_connections")
    ...
    .maybeSingle();
  if (error) throw new Error(error.message);
  const settings = connection?.settings && typeof connection.settings === "object"
    ? connection.settings as Record<string, unknown>
    : {};
  const configured = typeof settings.reply_to_email === "string" ? settings.reply_to_email.trim().toLowerCase() : "";
  if (connection?.status === "connected") {
    ...
    const legacySender = typeof connection.account_email === "string" ? connection.account_email.trim().toLowerCase() : "";
