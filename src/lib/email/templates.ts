...
export function planConfirmationEmail(
  ...
  return emailWrapper(`
    ...
    <div style="background-color:#0A0A0A;border-radius:8px;padding:16px;margin:16px 0;border-left:3px solid #D4AF37;">
      <p style="margin:0;font-size:14px;color:rgba(255,255,255,0.65);line-height:1.6;">${summary}</p>
    </div>
...
export function adminLeadNotificationEmail(leadData: {
  ...
  return emailWrapper(`
    ...
    <table style="width:100%;font-size:14px;color:rgba(255,255,255,0.65);" cellpadding="4">
      <tr><td style="color:rgba(255,255,255,0.38);width:100px;">Name</td><td>${leadData.name}</td></tr>
      <tr><td style="color:rgba(255,255,255,0.38);">Email</td><td><a href="mailto:${leadData.email}" style="color:#F5D060;">${leadData.email}</a></td></tr>
      ${leadData.phone ? `<tr><td style="color:rgba(255,255,255,0.38);">Phone</td><td>${leadData.phone}</td></tr>` : ""}
      ${leadData.business ? `<tr><td style="color:rgba(255,255,255,0.38);">Business</td><td>${leadData.business}</td></tr>` : ""}
      <tr><td style="color:rgba(255,255,255,0.38);">Industry</td><td style="text-transform:capitalize;">${leadData.industry.replace(/_/g, " ")}</td></tr>
    </table>
...
export function adminContactNotificationEmail(formData: {
  ...
  return emailWrapper(`
    ...
    <table style="width:100%;font-size:14px;color:rgba(255,255,255,0.65);" cellpadding="4">
      <tr><td style="color:rgba(255,255,255,0.38);width:100px;">Name</td><td>${formData.name}</td></tr>
      <tr><td style="color:rgba(255,255,255,0.38);">Email</td><td><a href="mailto:${formData.email}" style="color:#F5D060;">${formData.email}</a></td></tr>
      ${formData.phone ? `<tr><td style="color:rgba(255,255,255,0.38);">Phone</td><td>${formData.phone}</td></tr>` : ""}
      ${formData.businessType ? `<tr><td style="color:rgba(255,255,255,0.38);">Business</td><td>${formData.businessType}</td></tr>` : ""}
    </table>
    <div style="background-color:#0A0A0A;border-radius:8px;padding:16px;margin:16px 0;border-left:3px solid #D4AF37;">
      <p style="margin:0;font-size:14px;color:rgba(255,255,255,0.65);line-height:1.6;">${formData.message}</p>
    </div>
    ${goldButton("Reply to Lead", `mailto:${formData.email}`)}
  `);
}
