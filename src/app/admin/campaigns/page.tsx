...
export default function CampaignsPage() {
  ...
  const loadPreview = useCallback(async (id: string) => {
    ...
    try {
      setPreview(
        await fetchJson(`/api/admin/revenue-os/campaigns/preview?id=${encodeURIComponent(id)}`),
      );
    } catch (previewError) {
      setActionError(
        previewError instanceof Error ? previewError.message : "Could not build campaign preview.",
      );
    } finally {
      setSaving(false);
    }
  ...
  const campaignAction = async (id: string, action: "activate" | "pause" | "run") => {
    setSaving(true);
    setActionError("");
    try {
      ...
      setActionError(
        updateError instanceof Error ? updateError.message : "Could not update campaign.",
      );
    } finally {
      setSaving(false);
    }
  ...
  const create = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setActionError(
        createError instanceof Error ? createError.message : "Could not create campaign.",
      );
    } finally {
      setSaving(false);
    }
        ...
        {data && !data.schemaReady ? (
          ...
          data && (
            <div className="grid gap-4 xl:grid-cols-[0.9fr_1.1fr]">
                    ...
                    <AdminSurface key={campaign.id} padding="lg">
                      <div className="flex items-start justify-between gap-4">
                        <div className="min-w-0">
                          ...
                          <p className="admin-copy mt-1 text-xs">
                            Version <span className="tabular-nums">{campaign.version}</span>
                      ...
                      <div className="mt-5 grid grid-cols-3 gap-2">
                        {[
                          ...
                          {
                            label: "Active",
                            value: memberCounts.active || memberCounts.queued || 0,
                          },
                          ...
                          >
                            ...
                            <p className="admin-copy mt-0.5 text-[10px]">{metric.label}</p>
                          </div>
                        ))}
                      </div>
              ...
              <div>
                {preview ? (
                  <AdminSurface padding="none" className="sticky top-6 overflow-hidden">
                    <div className="flex items-start justify-between gap-4 p-5 sm:p-6">
                      <div>
                        ...
                        <p className="admin-copy mt-1 text-pretty text-sm">
                          Nothing sends from this screen until the exact version is activated.
                        </p>
                      </div>
                      <button
                        type="button"
                      ...
                      <div>
                        ...
                        <div className="mt-3 flex flex-wrap gap-2">
                          {Object.entries(preview.policy)
                            .filter(([key, value]) => key.startsWith("stop_") && value)
                            .map(([key]) => (
                              <span
                                key={key}
                              ...
                              >
                                ...
                                {key.replace("stop_on_", "Stop on ").replaceAll("_", " ")}
                              </span>
                            ))}
                        </div>
                      </div>
                      <div>
                        <h3 className="text-sm font-semibold text-[var(--admin-ink)]">Sequence</h3>
                  ...
                  <AdminSurface className="grid min-h-80 place-items-center text-center">
                    <div>
                      ...
                      <p className="admin-copy mt-1 max-w-sm text-pretty text-sm">
                        Review eligible recipients, exclusions, stop rules, sequence timing, and
                        personalized copy before activation.
                      </p>
                    </div>
                  </AdminSurface>
                )}
          ...
          <div className="rounded-[20px] p-5 sm:p-6">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="admin-copy mt-1 text-pretty text-sm">
                  Create the version, recipients, limits, and sequence. You will see a dry run
                  before activation.
                </p>
              </div>
              <button
