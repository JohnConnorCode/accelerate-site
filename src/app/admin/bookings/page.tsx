...
export default function AdminBookingsPage() {
  const bookingsQuery = useAdminQuery<{ opportunities: Opportunity[]; metrics: Metrics }>(
    ["admin", "bookings"],
    "/api/admin/bookings?days=90",
  );
          ...
          {[
            {
              ...
              note: `${metrics?.qualifiedToBooked || 0}% booked`,
              icon: Target,
            },
            {
              label: "Booked",
              value: metrics?.booked || 0,
              note: `${metrics?.bookedToShowed || 0}% showed`,
              icon: CalendarCheck2,
            },
            {
              label: "Calls held",
              value: metrics?.showed || 0,
              note: `${metrics?.noShow || 0} no-shows`,
              icon: UserCheck,
            },
            {
              label: "Won revenue",
              value: `$${(metrics?.wonRevenue || 0).toLocaleString()}`,
              note: `$${(metrics?.pipelineValue || 0).toLocaleString()} pipeline`,
              icon: DollarSign,
            },
            ...
            <table className="w-full min-w-[980px] text-left text-sm">
              <thead className="border-b border-[var(--admin-border)] font-mono text-[9px] uppercase tracking-[0.12em] text-[var(--admin-muted)]">
                <tr>
                  ...
                  <th className="px-4 py-4">Fit</th>
                  <th className="px-4 py-4">Leak</th>
                  <th className="px-4 py-4">Source</th>
                  <th className="px-4 py-4">Call</th>
                ...
                {items.map((item) => (
                  <tr key={item.id} className="text-[var(--admin-ink)]">
                    <td className="px-5 py-4">
                      ...
                      >
                        {new URL(item.company_website).hostname}
                      </a>
                      <p className="admin-copy mt-1 text-xs">{item.email}</p>
                    </td>
                    <td className="px-4 py-4">
                      ...
                      >
                        {item.revenue_band.replace(/_/g, " ")}
                      </span>
                      <p className="admin-copy mt-1 text-xs">{item.role.replace(/_/g, " ")}</p>
                    </td>
                    <td className="px-4 py-4">{item.primary_leak.replace(/_/g, " ")}</td>
                    <td className="px-4 py-4">
                      {item.utm_source || "Direct"}
                      <p className="admin-copy mt-1 text-xs">{item.utm_campaign || "—"}</p>
                    </td>
