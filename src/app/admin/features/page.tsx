...
function assigneeOptions(owners: string[], current: string | null) {
  const unique = [...new Set([tenant.founder.name, tenant.founder.fullName, ...owners, current].filter(Boolean))] as string[];
  return unique;
}
...
function FeatureCard({
  ...
  const claimedBy = feature.lease_owner || (feature.status === "in_progress" ? feature.owner : null);
        ...
        <div className="grid gap-5 px-5 py-5 pb-32 sm:grid-cols-2 sm:px-6 sm:py-6 sm:pb-32">
          ...
          <div className="sm:col-span-2">
            <div className="flex items-center justify-between gap-2">
              ...
              {feature && !form.subtasks.length && parseAcceptanceLines(form.acceptance_criteria).length > 0 && (
                <button
                  type="button"
            ...
            <div className="mt-2">
              <KanbanChecklist
                ...
                onMove={(id, direction) => commitSubtasks(moveSubtask(form.subtasks, id, direction))}
                onAdd={(title) =>
                  commitSubtasks([
                    ...form.subtasks,
...
export default function FeaturesPage() {
  ...
  const { columns: liveColumns, createColumn, renameColumn, deleteColumn } = useKanbanColumns("features");
  const columns = useMemo(
    () =>
      liveColumns.map((column) => {
        if (column.column_key !== "in_progress" || parseWipLimit(column.metadata) != null) return column;
  ...
  const owners = useMemo(
    () =>
      [...new Set(features.map((feature) => feature.owner).filter((value): value is string => Boolean(value)))].sort(),
    [features],
  );
  useEffect(() => {
    try {
  ...
  const filtered = useMemo(
    () =>
      features.filter((feature) => {
        ...
        if (ownerFilter === "unassigned") {
          ...
          const mine = [tenant.founder.name, tenant.founder.fullName, tenant.founder.email].map((value) =>
            value.toLowerCase(),
          );
          const owner = (feature.owner || "").toLowerCase();
          const lease = (feature.lease_owner || "").toLowerCase();
          if (!mine.some((value) => owner === value || owner.includes(value) || lease.includes(value)))
            return false;
    ...
    [capability, category, features, milestone, ownerFilter, priority, search],
  );
  ...
  const persistSubtasks = async (feature: FeatureRequest, subtasks: FeatureSubtask[]) => {
    ...
    try {
      const updated = await fetchJson<FeatureRequest>("/api/admin/features", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: feature.id, subtasks }),
      });
  ...
  const persistOwner = async (feature: FeatureRequest, owner: string | null) => {
    ...
    try {
      const updated = await fetchJson<FeatureRequest>("/api/admin/features", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: feature.id, owner }),
      });
      applyFeatureUpdate(updated, false);
    } catch (error) {
      setData((current) =>
  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      <FeatureDialog
        ...
        onPersistSubtasks={openFeature ? persistSubtasks : undefined}
      />
    </div>
  );
}
