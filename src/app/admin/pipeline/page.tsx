...
export default function PipelinePage() {
  ...
  const load = useCallback(async () => {
    setError("");
    try {
      setData(await fetchJson("/api/admin/revenue-os/pipeline"));
    } catch (loadError) {
      setError(
  ...
  const metrics = useMemo(() => {
    ...
    return {
      ...
      proposals: items.filter((item) =>
        ["proposal", "negotiation"].includes(
          item.canonical_stage ?? item.stage,
        ),
      ).length,
  ...
  const updateStage = async (item: Opportunity, next: RevenueStage) => {
    const lossReason =
      next === "lost"
        ? window.prompt("Why was this opportunity lost?")?.trim()
        : undefined;
  ...
  return (
    <div className="space-y-6 pb-10">
      <PageHeader
        title="Pipeline"
        subtitle="One revenue pipeline for inbound and outbound, from first signal through won client."
        actions={
          <>
            <button
              type="button"
            ...
            <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              {[
                ...
                },
                {
                  ...
                  note: "Proposal or negotiation",
                  icon: CalendarClock,
                },
                {
                  label: "Won revenue",
                  value: money(metrics.won),
                  note: "Recorded outcomes",
                  icon: Building2,
                },
            ...
            <AdminSurface padding="none" className="overflow-hidden">
              ...
              <div className="overflow-x-auto border-t border-[var(--admin-border)]">
                <table className="w-full min-w-[1040px] text-left text-sm">
                  <thead className="bg-black/[0.018] font-mono text-[9px] uppercase tracking-[0.12em] text-[var(--admin-muted)] dark:bg-white/[0.018]">
                    <tr>
                  ...
                  <tbody className="divide-y divide-[var(--admin-border)]">
                    {filtered.map((item) => {
                      const canonical = item.canonical_stage ?? "new";
                      return (
                        <tr
                          key={item.id}
                          className="transition-[background-color] duration-150 hover:bg-black/[0.018] dark:hover:bg-white/[0.018]"
                        >
                          ...
                          <td className="px-4 py-4">
                            ...
                            {(item.contact?.primary_email || item.email) && (
                              <p className="admin-copy mt-0.5 flex items-center gap-1 text-xs">
                                <Mail className="size-3" />
                                {item.contact?.primary_email || item.email}
                              </p>
                            )}
                          </td>
                          <td className="px-4 py-4 text-xs capitalize text-[var(--admin-muted)]">
                            {item.source || "Unknown"}
                          </td>
                          <td className="px-4 py-4 text-right font-mono text-sm tabular-nums text-[var(--admin-ink)]">
                            {money(item.estimated_value)}
                          </td>
                          <td className="max-w-[240px] px-4 py-4">
                            <p className="truncate text-xs text-[var(--admin-ink)]">
                              {item.next_action || "Set next action"}
                            </p>
                            <p className="admin-copy mt-0.5 text-[10px] tabular-nums">
                              {item.next_action_at
                                ? new Date(
                                    item.next_action_at,
                                  ).toLocaleDateString()
                          ...
                          <td className="px-5 py-4">
                            <select
                              ...
                              onChange={(event) =>
                                void updateStage(
                                  item,
                                  event.target.value as RevenueStage,
                                )
                              }
                              className={cn(
                                "min-h-10 rounded-xl border-0 px-3 text-xs font-semibold outline-none ring-1 ring-inset ring-black/5 transition-[opacity,box-shadow] duration-150 focus:ring-2 focus:ring-[var(--admin-ink)]/35 dark:ring-white/10",
                                stageTone[canonical],
                              )}
                            >
                              {REVENUE_STAGES.map((value) => (
                      ...
                      <tr>
                        <td colSpan={6} className="px-6 py-16 text-center">
                          ...
                          <p className="admin-copy mt-1 text-xs">
                            Adjust the filters or create the first opportunity.
                          </p>
                        </td>
                      </tr>
                    )}
            ...
            <div className="rounded-[20px] p-5 sm:p-6">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="admin-copy mt-1 text-pretty text-sm">
                    Creates or resolves the contact and company before opening
                    the opportunity.
                  </p>
                </div>
                <button
              ...
              <div className="mt-6 grid gap-4 sm:grid-cols-2">
                {[
                  ...
                  <label
                    key={field.name}
                    className="text-xs font-semibold text-[var(--admin-ink)]"
                  >
                    {field.label}
                    <input
                      name={field.name}
                      ...
                      className="mt-1.5 min-h-11 w-full rounded-xl border border-[var(--admin-border)] bg-[var(--admin-surface-subtle)] px-3.5 text-sm font-normal text-[var(--admin-ink)] outline-none transition-[border-color,box-shadow] duration-150 focus:border-[var(--admin-ink)] focus:ring-2 focus:ring-[var(--admin-ink)]/10"
                    />
                  </label>
                ))}
              </div>
