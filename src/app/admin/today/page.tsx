...
import { AlarmClock, ArrowRight, Check, CheckCircle2, Loader2, RefreshCw, TriangleAlert, X } from "lucide-react";
...
export default function TodayPage() {
  ...
  const overviewQuery = useAdminQuery<Overview>(["today", "overview"], "/api/admin/revenue-os/overview");
  const actionsQuery = useAdminQuery<{ actions: ActionRow[] }>(["today", "actions"], "/api/admin/revenue-os/actions");
  ...
  const openReview = (action: ActionRow, href = `/admin/today?focus=approval&action=${encodeURIComponent(action.id)}`, trigger?: HTMLElement) => {
    dismissedActionRef.current = null;
  ...
  const decide = async (id: string, decision: "approve" | "reject") => {
    ...
    try {
      ...
      closeReview();
      queryClient.setQueryData<{ actions: ActionRow[] }>(["today", "actions"], (current) => current ? { actions: current.actions.filter((action) => action.id !== id) } : current);
      queryClient.setQueryData<Overview>(["today", "overview"], (current) => current ? { ...current, queue: current.queue.filter((item) => item.id !== `action:${id}`) } : current);
      await refresh();
      window.dispatchEvent(new Event("admin:priority-refresh"));
    } catch (decisionError) {
      setMutationError(decisionError instanceof Error ? decisionError.message : "Could not handle the action.");
    } finally {
      setActing(null);
    }
  };
  ...
  const updateTask = async (id: string, action: "complete" | "snooze") => {
    ...
    try {
      ...
      queryClient.setQueryData<Overview>(["today", "overview"], (current) => current ? { ...current, queue: current.queue.filter((item) => item.id !== `task:${id}`) } : current);
      await refresh();
      window.dispatchEvent(new Event("admin:priority-refresh"));
    } catch (taskError) {
  ...
  if (!overview && error) return <div className="space-y-7 pb-10">
    ...
    <AdminSurface tone="attention" className="mx-auto flex max-w-2xl flex-col items-start gap-4 p-6 sm:flex-row sm:items-center">
      ...
      <div className="min-w-0 flex-1"><h2 className="text-sm font-semibold text-[var(--admin-ink)]">Today is temporarily unavailable</h2><p className="admin-copy mt-1 text-sm">{error} No work was hidden or changed. Retry the live read, or open Setup Center to inspect system health.</p><div className="mt-4 flex flex-wrap gap-2"><button type="button" onClick={() => void refresh()} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-[var(--admin-ink)] px-4 text-xs font-semibold text-[var(--admin-surface)]"><RefreshCw className="size-3.5" /> Retry</button><Link href="/admin/setup" className="inline-flex min-h-11 items-center gap-2 rounded-xl px-4 text-xs font-semibold text-[var(--admin-ink)] shadow-[var(--admin-shadow-border)]">Open Setup <ArrowRight className="size-3.5" /></Link></div></div>
    </AdminSurface>
  </div>;
      ...
      {overview && !overview.schemaReady ? <RevenueSetupGate /> : overview && (
        <div className="space-y-5 sm:space-y-7" data-today-content-stack>
          <AdminSurface padding="none" className="overflow-hidden" aria-label="Operating summary">
            ...
            {[
              { label: "Priority work", value: urgentCount, note: `${overview.queue.length} total` },
              { label: "Open pipeline", value: formatMoney(overview.metrics.pipelineValue), note: `${formatMoney(overview.metrics.weightedValue)} weighted` },
              { label: "Unread replies", value: overview.metrics.unreadConversations, note: "Synced conversations" },
              { label: "Active campaigns", value: overview.metrics.activeCampaigns, note: `${overview.metrics.pendingProposals} proposals awaiting` },
            ].map(({ label, value, note }) => (
              <div key={label} className="min-w-0 px-4 py-3.5 sm:min-h-[96px] sm:px-5 sm:py-4">
                <dt className="truncate text-[9px] font-semibold uppercase tracking-[0.06em] text-[var(--admin-muted)] sm:text-[10px] sm:normal-case sm:tracking-normal">{label}</dt><dd className="mt-1 truncate text-[clamp(1.25rem,6vw,1.75rem)] font-semibold tabular-nums tracking-[-0.045em] text-[var(--admin-ink)] sm:text-2xl">{value}</dd><p className="admin-copy mt-0.5 truncate text-[9px] sm:text-[10px]">{note}</p>
              </div>
            ))}
            </dl>
          </AdminSurface>

          {!demo && overview.health.status === "attention" && <AdminSurface
            tone="default"
          ...
          <section className="grid items-start gap-4 xl:grid-cols-[minmax(0,1fr)_22rem] 2xl:grid-cols-[minmax(0,1fr)_24rem]" data-today-workspace>
            ...
            <AdminSurface padding="none" className="hidden self-start overflow-hidden xl:sticky xl:top-24 xl:block" data-today-approval-rail>
              ...
              <div className="divide-y divide-[var(--admin-border)] border-t border-[var(--admin-border)]">
                {actions.slice(0, showAllApprovals ? 8 : 3).map((action) => (
                  <div key={action.id} className="px-5 py-4"><div className="min-w-0"><h3 className="text-pretty text-sm font-semibold leading-5 text-[var(--admin-ink)]">{action.title}</h3><p className="admin-copy mt-1 line-clamp-2 text-pretty text-xs leading-5">{action.description || action.reasoning || "Review before execution."}</p><div className="mt-3 flex items-center gap-2"><button type="button" onClick={(event) => openReview(action, undefined, event.currentTarget)} disabled={acting === action.id} aria-haspopup="dialog" data-approval-review={action.id} className="inline-flex min-h-10 flex-1 items-center justify-center gap-1.5 rounded-lg bg-[var(--admin-ink)] px-3 text-xs font-semibold text-[var(--admin-surface)] transition-[opacity,transform] duration-150 hover:opacity-85 active:scale-[0.96] disabled:opacity-50">{acting === action.id ? <Loader2 className="size-3.5 animate-spin" /> : <Check className="size-3.5" />} Review</button><button type="button" onClick={() => void decide(action.id, "reject")} disabled={acting === action.id} aria-label={`Reject ${action.title}`} title="Reject proposal" className="grid size-10 shrink-0 place-items-center rounded-lg text-[var(--admin-muted)] shadow-[var(--admin-shadow-border)] transition-[color,box-shadow,transform] duration-150 hover:text-rose-700 hover:shadow-[var(--admin-shadow-border-hover)] active:scale-[0.96] disabled:opacity-50 dark:hover:text-rose-300"><X className="size-3.5" /></button></div></div></div>
                ))}
