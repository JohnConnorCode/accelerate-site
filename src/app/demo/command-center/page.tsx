    ...
    <main className="demo-launcher relative min-h-screen overflow-hidden bg-[var(--demo-launcher-canvas)] px-4 pb-5 pt-[calc(var(--site-header-h)+env(safe-area-inset-top)+1.25rem)] text-[var(--demo-launcher-ink)] antialiased transition-colors duration-200 sm:px-6 sm:pb-7 sm:pt-[calc(var(--site-header-h)+env(safe-area-inset-top)+1.75rem)]">
      ...
      <div className="relative mx-auto max-w-[1320px]">
        <header className="grid gap-8 pb-12 pt-[clamp(3.5rem,9vw,8rem)] lg:grid-cols-[minmax(0,1.3fr)_minmax(18rem,.48fr)] lg:items-end lg:gap-16 lg:pb-20">
          <div>
            <p className="admin-demo-enter admin-demo-d1 font-mono text-[10px] font-semibold uppercase tracking-[0.19em] text-[var(--demo-launcher-faint)]">Full admin demo</p>
            <h1 className="admin-demo-enter admin-demo-d2 mt-4 max-w-[14ch] text-balance font-display text-[clamp(3rem,6.8vw,6.9rem)] font-semibold leading-[0.86] tracking-[-0.06em]">Explore the full admin through five real operating models.</h1>
          </div>
          <div className="admin-demo-enter admin-demo-d3 border-l-2 border-[var(--demo-launcher-accent)] pl-5 sm:pl-6">
            ...
            <p className="mt-5 font-mono text-[10px] uppercase leading-5 tracking-[0.12em] text-[var(--demo-launcher-accent)]">Browser-only fictional workspaces. Nothing can send, schedule, or change a live record.</p>
          </div>
        </header>

        <section aria-labelledby="demo-scenarios">
          <div className="admin-demo-enter admin-demo-d4 mb-5 flex items-end justify-between gap-4 border-t border-[var(--demo-launcher-rule)] pt-5">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-6">
            {DEMO_SCENARIO_SUMMARIES.map((scenario, index) => (
              <div key={scenario.id} className={cn("admin-demo-enter lg:col-span-2", index === 4 && "sm:col-span-2", index >= 3 && "lg:col-span-3")} style={{ animationDelay: `${420 + index * 100}ms` }}>
                <Link
        ...
        <p className="admin-demo-enter py-8 text-center font-mono text-[9px] uppercase leading-5 tracking-[0.12em] text-[var(--demo-launcher-faint)]" style={{ animationDelay: "920ms" }}>Each business keeps separate data and appearance state in this browser session.</p>
      </div>
    </main>
  );
}
