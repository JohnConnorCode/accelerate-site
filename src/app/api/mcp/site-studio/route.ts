...
export async function POST(request: Request) {
  ...
  try {
    ...
    console.warn("[site-studio] Scoped MCP OAuth configuration unavailable");
    return NextResponse.json(
      { error: "Site Studio OAuth is not configured" },
      { status: 503, headers },
    );
  }
  let delegation;
  try {
    ...
    console.warn("[site-studio] Scoped MCP authorization refused");
    return NextResponse.json(
      { error: "Connect as the installation owner with an active Site Studio delegation." },
      {
        status: 401,
        headers: {
          ...headers,
  ...
  const result = await runWithTenantRequestContext(delegation.auth, () =>
    handleMcpRequest(body, {
      supabase: delegation.auth.database,
      actorEmail: delegation.auth.user.email!,
      tenantSlug: delegation.auth.tenant.slug,
      tenantConfig: {
        ...tenant,
        modules: delegation.auth.tenant.config.modules as Record<string, boolean> | undefined,
      },
