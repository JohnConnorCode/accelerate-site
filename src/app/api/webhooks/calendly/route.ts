...
function signatureMatches(raw: string, supplied: string, secret: string): boolean {
  ...
  return values.some(
    (candidate) => candidate.length === digest.length && timingSafeEqual(candidate, digest),
  );
}
...
export async function handleCalendlyWebhook(
  request: NextRequest,
  input?: { webhookSecret?: string; tenantContext?: TenantSystemContext },
) {
  ...
  const authorized =
    headerSecret === secret ||
    querySecret === secret ||
    Boolean(signature && signatureMatches(raw, signature, secret));
  ...
  const isBootstrapTenant =
    !input?.tenantContext || input.tenantContext.tenantId === ACCELERATE_TENANT_ID;
  ...
  const [{ data: opportunity }, { data: contact }] = await Promise.all([
    supabase
      ...
      .eq("email", email)
      .maybeSingle(),
    supabase.from("contacts").select("id").eq("primary_email", email).maybeSingle(),
  ]);
  ...
  if (body.event === "invitee.created" && contactId) {
    ...
    await recordActivity(supabase, {
      ...
      summary: body.payload.scheduled_event?.start_time
        ? `Scheduled for ${body.payload.scheduled_event.start_time}.`
      ...
      metadata: {
        ...
        scheduled_event_uri: body.payload.scheduled_event?.uri ?? null,
      },
    });
  }
  ...
  if (!opportunity) {
    ...
    if (receiptError) {
      console.error(
        "[calendly] receipt insert failed for unmatched booking:",
    ...
    const { error: notificationError } = await supabase.from("admin_notifications").insert({
      type: "new_contact",
      title: contact
        ? "Calendly booking from a recovery contact"
        : "Calendly booking without a qualifier",
    ...
    if (notificationError) {
      ...
      return NextResponse.json(
        { error: "Booking recorded but the operator could not be notified" },
        { status: 500 },
      );
    }
  ...
  if (body.event === "invitee.created") {
      ...
      return NextResponse.json(
        {
          error:
            error instanceof Error
              ? error.message
              : "Calendar transition blocked by pipeline state",
        },
        { status: 409 },
      );
    }
    ...
    try {
      await updateOpportunityRecord(supabase, { id: opportunity.id, actorEmail: "calendly", effectKey: `calendly:${receiptId}:details`, patch: {
    ...
    if (notification.error) {
      console.error(
        "[calendly] booking notification FAILED (booking stored):",
    ...
    const isCurrentInvitee =
      !opportunity.calendly_invitee_uri || opportunity.calendly_invitee_uri === body.payload.uri;
    if (isCurrentInvitee) {
      try {
        await transitionOpportunity(supabase, {
          id: opportunity.id,
      ...
      try {
        await updateOpportunityRecord(supabase, { id: opportunity.id, actorEmail: "calendly", effectKey: `calendly:${receiptId}:details`, patch: { canceled_at: body.created_at || "now", scheduled_at: null } });
      } catch {
