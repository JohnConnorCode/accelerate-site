  ...
  properties: {
    ...
    recommendations: {
      ...
      items: {
        ...
        properties: {
          ...
          pricingOneTime: { type: ["number", "null"], minimum: 0 },
          pricingMonthly: { type: ["number", "null"], minimum: 0 },
    ...
    implementationRoadmap: {
      ...
      items: {
        type: "object",
        additionalProperties: false,
        required: ["phase", "name", "description", "duration", "solutions"],
        properties: {
          phase: { type: "integer" },
          name: { type: "string" },
          description: { type: "string" },
    ...
    roiProjection: {
      type: "object",
      additionalProperties: false,
      required: ["ninetyDay", "twelveMonth", "disclaimer"],
      properties: {
        ninetyDay: {
          type: "object",
          additionalProperties: false,
          required: ["estimatedLeadIncrease", "estimatedTimeSaved", "estimatedRevenueImpact"],
          properties: {
            estimatedLeadIncrease: { type: "string" },
    ...
    investmentSummary: {
      type: "object",
      additionalProperties: false,
      required: ["oneTimeCosts", "monthlyCosts", "totalOneTime", "totalMonthly", "budgetNotes"],
      properties: {
        oneTimeCosts: {
          type: "array",
          items: {
        ...
        totalOneTime: { type: "number", minimum: 0 },
        totalMonthly: { type: "number", minimum: 0 },
        budgetNotes: { type: ["string", "null"] },
      },
    },
...
function validatePlan(value: unknown): DigitalGrowthPlan {
  if (!value || typeof value !== "object")
    throw new Error("OpenRouter returned an invalid growth plan");
  const plan = value as DigitalGrowthPlan & {
    investmentSummary?: { budgetNotes?: string | null };
    recommendations?: Array<Record<string, unknown>>;
  };
  if (
    typeof plan.executiveSummary !== "string" ||
    !Array.isArray(plan.recommendations) ||
    !Array.isArray(plan.implementationRoadmap) ||
    !plan.roiProjection ||
    !plan.investmentSummary ||
    !Array.isArray(plan.nextSteps)
  )
    throw new Error("OpenRouter returned an incomplete growth plan");
  ...
  for (const row of oneTimeCosts)
    assertApprovedPriceComponent({ item: row.item, amount: row.amount, kind: "oneTime" });
  for (const row of monthlyCosts)
    assertApprovedPriceComponent({ item: row.item, amount: row.amount, kind: "monthly" });
  ...
  if (
    plan.investmentSummary.totalOneTime !== expectedOneTime ||
    plan.investmentSummary.totalMonthly !== expectedMonthly
  ) {
    throw new Error(
      "OpenRouter returned investment totals that do not match the approved line items",
    );
  }
...
function validateIntakeData(data: Partial<IntakeFormData>): data is IntakeFormData {
  return !!(data.industry && data.contactName && data.contactEmail && data.consentGiven);
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      {
        error:
          "Failed to generate your growth plan. Please try again or book a strategy session with us.",
      },
      { status: 500 },
    );
  }
...
async function savePlan(
  ...
  const { error } = await supabase
    ...
    .update({
      ...
      estimated_value:
        plan.investmentSummary.totalOneTime + plan.investmentSummary.totalMonthly * 12,
...
function generateFallbackPlan(data: IntakeFormData): DigitalGrowthPlan {
  ...
  const websiteRec = {
    ...
    whyItMatters:
      "Your website is the foundation of your digital presence. Without a site that ranks and converts, every other investment works harder than it should.",
    features: [
      ...
      isHomeServices
        ...
        : "Professional service pages showcasing your expertise",
    ],
  ...
  const automationRec = {
    ...
    features: [
      ...
      isHomeServices
        ? "Automated appointment scheduling for estimates"
        : "Smart qualification to prioritize high-value prospects",
      "Performance dashboard with conversion metrics",
    ],
    estimatedImpact:
      "Response time drops from hours to minutes. Expect 20-30% improvement in inquiry-to-customer conversion.",
    timeline: "1-2 weeks",
    pricingOneTime: 1500,
    pricingMonthly: 400,
  ...
  const agentRec = {
    ...
    whyItMatters:
      "You cannot be available 24/7, but your customers expect instant responses. This agent handles the first touchpoint so you never miss an opportunity.",
    features: [
  ...
  return {
    ...
    implementationRoadmap: [
      {
        ...
        name: "Foundation",
        description:
          "Launch your new website and set up core inquiry capture. This gives you an immediate uplift in online visibility and conversion.",
        duration: "2-3 weeks",
      ...
      },
      {
        ...
        name: "Automation",
        description:
          "Deploy automated follow-up sequences and CRM integration. Every inquiry gets a response within minutes.",
        duration: "1-2 weeks",
      ...
      },
      {
        ...
        name: "Intelligence",
        description:
          "Activate 24/7 AI agent for phone and chat. Full coverage, no missed opportunities.",
        duration: "1-2 weeks",
    ...
    roiProjection: {
      ...
      disclaimer:
        "Projections are based on industry averages from similar implementations. Individual results vary based on market conditions, execution, and existing business fundamentals.",
    },
    investmentSummary: {
