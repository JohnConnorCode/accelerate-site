...
export async function GET(request: NextRequest) {
  ...
  try {
    const contexts = await listTenantSystemContexts({ source: "revenue-campaign-cron", provider: "resend", includeBootstrapFallback: true });
    ...
    for (const context of contexts) {
      ...
      try {
        const { data: playbooks, error } = await supabase.from("recovery_playbooks").select("campaign_id").limit(100);
        if (error) throw new Error(error.message);
        const reconciled = await Promise.all((playbooks ?? []).map((playbook) => reconcileRecoveryOutcomes(supabase, playbook.campaign_id, "system:revenue-campaigns")));
        recovery = { playbooks: reconciled.length, reconciled: reconciled.reduce((total, item) => total + item.reconciled, 0), outcomes: reconciled.reduce((total, item) => total + item.outcomes, 0) };
      } catch (error) {
        if (!isMissingRevenueSchema(error)) throw error;
      }
      const value = { ...summary, recovery };
      return { value, summary: value, status: summary.failed ? "partial" as const : "success" as const };
          });
        });
        tenants.push({ tenant: context.tenantSlug, status: result.claimed ? "completed" : "skipped", runId: result.runId, summary: result.value });
      } catch (error) {
        tenants.push({ tenant: context.tenantSlug, status: "failed", error: error instanceof Error ? error.message : "Campaign job failed" });
      }
    }
