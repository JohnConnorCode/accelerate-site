...
import { listSiteDelegations, manageSiteDelegation, siteEditorOAuthConfig } from "@/lib/site-studio/delegation";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (id) {
      const { data, error } = await auth.database.auth.oauth.getAuthorizationDetails(authorizationId.parse(id));
      if (error || !data) throw new Error("OAuth authorization unavailable");
      if ("client" in data && (!config.clientIds.includes(data.client.id) || data.user.id !== auth.user.id))
        throw new Error("This OAuth client is not allowed for Site Studio");
      return NextResponse.json({ authorization: data }, { headers });
    }
    return NextResponse.json({ delegations: await listSiteDelegations(auth), resource: config.resource }, { headers });
  } catch {
    return NextResponse.json({ error: "Site Studio OAuth is unavailable. Check the configured client, native OAuth server, and migrations." }, { status: 403, headers });
  }
}
export async function POST(request: Request) {
  ...
  try {
    ...
    if (input.operation === "revoke") {
      const grant = await manageSiteDelegation(auth, { operation: "revoke", grantId: input.grantId });
      // The local revocation is already durable. A provider outage cannot undo it.
      await auth.database.auth.oauth.revokeGrant({ clientId: grant.client_id }).catch(() => undefined);
    ...
    if (input.operation === "renew") {
      ...
      return NextResponse.json({ grant: await manageSiteDelegation(auth, { operation: "grant", clientId: input.clientId }) }, { headers });
    }
    ...
    if (!siteEditorOAuthConfig().clientIds.includes(details.data.client.id) || details.data.user.id !== auth.user.id)
      throw new Error("OAuth client or identity does not match this owner");
    let grant;
    if (input.decision === "approve") {
      grant = await manageSiteDelegation(auth, { operation: "grant", clientId: details.data.client.id });
    }
    const result = input.decision === "approve"
      ? await auth.database.auth.oauth.approveAuthorization(input.authorizationId, { skipBrowserRedirect: true })
      : await auth.database.auth.oauth.denyAuthorization(input.authorizationId, { skipBrowserRedirect: true });
    ...
    return NextResponse.json({ error: "Connection change failed. Reload to inspect the current authorization before retrying." }, { status: 400, headers });
  }
}
