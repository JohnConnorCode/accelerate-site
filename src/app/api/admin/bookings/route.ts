...
export async function GET(request: NextRequest) {
  ...
  const booked = opportunities.filter((item) =>
    ["booked", "showed", "proposal", "won"].includes(item.stage),
  ).length;
  const showed = opportunities.filter((item) =>
    ["showed", "proposal", "won"].includes(item.stage),
  ).length;
  const pipelineValue = opportunities
    .filter((item) => ["proposal", "won"].includes(item.stage))
...
export async function PATCH(request: NextRequest) {
  ...
  if (Number.isFinite(body.estimatedValue))
    updatePatch.estimated_value = Math.max(0, Number(body.estimatedValue));
  ...
  if (Object.keys(updatePatch).length > 0) {
    try {
      data = await updateOpportunityRecord(supabase, { id: body.id, actorEmail: auth.user.email || "founder", patch: updatePatch });
    } catch {
  ...
  if (body.stage === "no_show" && current.stage !== "no_show") {
    try {
      await sendNoShowRebookEmail({
        email: current.email,
        opportunityId: body.id,
        token: current.qualifier_token,
      });
    } catch (error) {
