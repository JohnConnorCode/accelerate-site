...
const requestSchema = z.discriminatedUnion("action", [
  z.object({
    ...
    surfaces: z
      .array(z.enum(["contact", "analytics"]))
      .min(1)
      .max(2),
...
export async function GET() {
  ...
  const { data, error } = await authorization.database
    ...
    .select(
      "id,label,key_prefix,surfaces,allowed_origins,status,expires_at,last_used_at,created_at,revoked_at,rate_limit_per_minute",
    )
...
export async function POST(request: NextRequest) {
  ...
  if (!parsed.success)
    return NextResponse.json({ error: "Invalid ingest-key action" }, { status: 400 });
  if (parsed.data.action === "revoke") {
    const { error } = await authorization.database
      .from("tenant_ingest_keys")
    ...
    await recordAudit(authorization.database, {
      ...
      action: "tenant_ingest_key.revoked",
      entityType: "tenant_ingest_key",
  ...
  const { data, error } = await authorization.database
    ...
    .select(
      "id,label,key_prefix,surfaces,allowed_origins,status,expires_at,created_at,rate_limit_per_minute",
    )
    .single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  await recordAudit(authorization.database, {
    actorEmail: authorization.user.email,
    action: "tenant_ingest_key.created",
    entityType: "tenant_ingest_key",
    entityId: data.id,
    after: {
      label: data.label,
