...
export async function GET() {
  ...
  try {
    ...
    console.error("Blueprint list failed:", error instanceof Error ? error.message : error);
...
export async function POST(request: NextRequest) {
  ...
  const blueprintId =
    typeof body.blueprintId === "string" && body.blueprintId.trim() ? body.blueprintId.trim() : undefined;
  ...
  try {
    ...
    console.error("Blueprint save failed:", message);
    ...
    if (/Invalid WorkspaceBlueprint|must be a UUID|changeSummary is required|must not contain secrets/i.test(message)) {
