...
const editableFields = ["title", "description", "status", "priority", "labels", "owner", "target_date", "acceptance_criteria", "notes"] as const;
...
function cleanLabels(value: unknown): string[] {
  ...
  return [...new Set(value.filter((label): label is string => typeof label === "string").map((label) => label.trim().toLowerCase().replace(/\s+/g, "-").slice(0, 32)).filter(Boolean))].slice(0, 12);
}
...
export async function POST(request: NextRequest) {
  ...
  await recordAudit(supabase, { actorEmail: auth.user.email, action: "feature.created", entityType: "feature_request", entityId: data.id, after: data });
...
export async function PATCH(request: NextRequest) {
  ...
  if (Array.isArray(body.reorder)) {
    ...
    if (updates.some((item: { id: string; status: unknown; sort_order: number }) => !item.id || !isFeatureStatus(item.status) || !Number.isFinite(item.sort_order))) {
      return NextResponse.json({ error: "Every reordered card needs a valid id, status, and order" }, { status: 400 });
    }
    const { data: affected, error } = await supabase.rpc("reorder_feature_requests", { updates });
    if (error) return NextResponse.json({ error: error.message }, { status: 500 });
    if (affected !== updates.length) return NextResponse.json({ error: "One or more cards could not be reordered. Refresh and try again." }, { status: 409 });
    await recordAudit(supabase, { actorEmail: auth.user.email, action: "feature.reordered", entityType: "feature_request", metadata: { updates } });
    return NextResponse.json({ success: true, affected });
  }

  if (typeof body.id !== "string") return NextResponse.json({ error: "Feature id is required" }, { status: 400 });
  const { data: before } = await supabase.from("feature_requests").select("*").eq("id", body.id).is("archived_at", null).maybeSingle();
  ...
  if ("title" in update && !update.title) return NextResponse.json({ error: "Title cannot be empty" }, { status: 400 });
  if ("status" in update && !isFeatureStatus(update.status)) return NextResponse.json({ error: "Invalid status" }, { status: 400 });
  if ("priority" in update && !isFeaturePriority(update.priority)) return NextResponse.json({ error: "Invalid priority" }, { status: 400 });
  if (!Object.keys(update).length) return NextResponse.json({ error: "No valid changes supplied" }, { status: 400 });
  const { data, error } = await supabase.from("feature_requests").update(update).eq("id", body.id).select().single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  await recordAudit(supabase, { actorEmail: auth.user.email, action: "feature.updated", entityType: "feature_request", entityId: body.id, before, after: data });
...
export async function DELETE(request: NextRequest) {
  ...
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  await recordAudit(supabase, { actorEmail: auth.user.email, action: "feature.archived", entityType: "feature_request", entityId: id, before });
