...
import { googleOperatorError, googleServerErrorSummary, verifyGoogleOAuthStateBinding } from "@/lib/revenue-os/google-oauth";
...
export async function GET(request: NextRequest) {
  ...
  if (!state || !verifyGoogleOAuthStateBinding(encodedState, { state, tenantId: auth.tenant.id, tenantSlug: auth.tenant.slug })) return redirect("google_error=state_mismatch");
  if (providerError) return redirect(`google_error=${providerError === "access_denied" ? "consent_denied" : "connection_failed"}`);
