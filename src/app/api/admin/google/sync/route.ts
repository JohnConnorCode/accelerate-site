...
export async function PATCH(request: NextRequest) {
  ...
  if (
    !Array.isArray(body.driveFolderIds) ||
    body.driveFolderIds.length > 10 ||
    body.driveFolderIds.some((id) => typeof id !== "string" || !id.trim())
  ) {
  ...
  const { error } = await supabase
    ...
    .update({
      settings: {
        ...(data?.settings ?? {}),
        drive_folder_ids: body.driveFolderIds.map((id) => id.trim()),
      },
    })
    .eq("provider", "google")
    .eq("status", "connected");
