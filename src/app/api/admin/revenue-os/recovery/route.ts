...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const defaultBookingUrl =
      ...
      typeof (booking as Record<string, unknown>).url === "string"
        ? (booking as Record<string, string>).url
        : "";
    return NextResponse.json({ ...workspace, defaultBookingUrl });
  } catch (error) {
    if (isMissingRevenueSchema(error))
      return NextResponse.json({ schemaReady: false, batches: [], playbooks: [] });
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Could not load recovery" },
      { status: 500 },
    );
  }
}
  ...
  try {
    ...
    const steps = Array.isArray(body.steps)
      ? body.steps.map((step) => {
          ...
          return {
            delayDays: Number(value.delayDays),
    ...
    const result = await createRecoveryPlaybook(auth.database, {
      ...
      sourceBatchId: String(body.sourceBatchId || ""),
      motion: String(body.motion || "") as RecoveryMotion,
      relationshipBasis: String(body.relationshipBasis || ""),
      offerLabel: String(body.offerLabel || ""),
      bookingUrl: String(body.bookingUrl || ""),
      timezone: String(body.timezone || "America/Detroit"),
      outcomeWindowDays: Number(body.outcomeWindowDays),
      steps,
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Could not create recovery playbook" },
      { status: 400 },
    );
  }
}
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    if (!body.campaignId || !body.action)
      return NextResponse.json({ error: "Choose a recovery playbook action" }, { status: 400 });
    const supabase = auth.database;
    if (body.action === "activate")
      return NextResponse.json({
        campaign: await activateCampaign(supabase, body.campaignId, auth.user.email || "founder"),
      });
    if (body.action === "pause")
      return NextResponse.json({
        campaign: await pauseCampaign(supabase, body.campaignId, auth.user.email || "founder"),
      });
    const result = await executeDueCampaignMembers(supabase, new Date(), body.campaignId);
    const outcomes = await reconcileRecoveryOutcomes(
      supabase,
      body.campaignId,
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Could not update recovery playbook" },
      { status: 400 },
    );
  }
}
