...
import {
  readTodayViewProposal,
  readTodayViews,
  saveTodayViews,
} from "@/lib/revenue-os/today-views";
  ...
  try {
    ...
    return NextResponse.json(
      await runWithTenantRequestContext(auth, () =>
        digest ? readTodayViewProposal(auth.database, digest) : readTodayViews(auth.database),
      ),
    );
  } catch (error) {
    console.error(
    ...
    return NextResponse.json(
      { error: "Saved views are unavailable. Retry to recover your layout." },
      { status: 503 },
    );
  }
}
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json(
      await runWithTenantRequestContext(auth, () => saveTodayViews(auth.database, body)),
    );
  } catch (error) {
