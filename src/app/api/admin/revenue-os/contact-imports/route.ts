...
function errorResponse(error: unknown) {
  ...
  const batchId =
    error && typeof error === "object" && "batchId" in error
      ? String((error as { batchId?: unknown }).batchId || "")
      : null;
  if (contactImportSchemaUnavailable(error)) {
    return NextResponse.json(
      { error: "Contact Import migration is not applied", schemaReady: false, batchId },
      { status: 503 },
    );
  }
  const status = /not found/i.test(message)
    ? 404
    : /changed|stale|already|cannot be|not approved/i.test(message)
      ? 409
      : /OpenRouter is not configured/i.test(message)
        ? 503
        : 400;
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({
      schemaReady: true,
      batches: await listContactImportBatches(supabase),
    });
  } catch (error) {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (action === "approve") {
      if (typeof body.batchId !== "string" || typeof body.expectedDigest !== "string")
        return NextResponse.json(
          { error: "batchId and expectedDigest are required" },
          { status: 400 },
        );
      return NextResponse.json({
        schemaReady: true,
        batch: await approveContactImport(supabase, {
    ...
    if (action === "execute") {
      ...
      return NextResponse.json({
        schemaReady: true,
        batch: await executeContactImport(supabase, { batchId: body.batchId, actorEmail }),
      });
    }
