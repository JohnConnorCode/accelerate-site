...
export async function GET(request: NextRequest) {
  ...
  if (conversations.error) {
    if (isMissingRevenueSchema(conversations.error)) return NextResponse.json({ schemaReady: false, conversations: [], messages: [] });
    return NextResponse.json({ error: "Could not load conversations" }, { status: 500 });
  }
  let messages: unknown[] = [];
  if (id) {
    const result = await supabase.from("messages").select("*").eq("conversation_id", id).order("created_at", { ascending: true });
    if (result.error) return NextResponse.json({ error: "Could not load messages" }, { status: 500 });
    messages = result.data ?? [];
  }
  return NextResponse.json({ schemaReady: true, conversations: conversations.data ?? [], messages });
}

export async function PATCH(request: NextRequest) {
  const auth = await requireAdmin();
  if (auth instanceof NextResponse) return auth;
  const body = await request.json() as { id?: string; status?: string; intent?: string; opportunityId?: string | null; contactId?: string | null };
  if (!body.id) return NextResponse.json({ error: "Conversation id is required" }, { status: 400 });
  const patch: Record<string, unknown> = {};
  if (body.status && ["open", "waiting", "resolved", "archived"].includes(body.status)) patch.status = body.status;
