...
export async function GET() {
  ...
  const payload: AiCapabilitiesPayload = {
    ...
    safety: {
      ...
      registeredExternalActions: capabilities.filter((capability) => capability.impact === "external_action").length,
      registeredDestructiveActions: capabilities.filter((capability) => capability.impact === "destructive").length,
      readsMayExecuteDirectly: capabilities.some((capability) => capability.impact === "read" && !capability.confirmationRequired),
      writesRequireApproval: capabilities.filter((capability) => capability.impact === "internal_write").every((capability) => capability.confirmationRequired),
      externalActionsRequireApproval: capabilities.filter((capability) => capability.impact === "external_action").every((capability) => capability.confirmationRequired),
      destructiveActionsAvailable: capabilities.some((capability) => capability.impact === "destructive"),
    },
  };
