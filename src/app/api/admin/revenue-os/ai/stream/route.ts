...
function parsePageContext(value: unknown): CommandPageContext | null {
  ...
  if (typeof candidate.pathname !== "string" || !candidate.pathname.startsWith("/admin"))
    return null;
  const context: CommandPageContext = { pathname: candidate.pathname.slice(0, 240) };
  if (
    candidate.entity &&
    ["opportunity", "contact", "company"].includes(String(candidate.entity.type)) &&
    typeof candidate.entity.id === "string"
  ) {
    context.entity = {
      type: candidate.entity.type as "opportunity" | "contact" | "company",
...
export async function POST(request: NextRequest) {
  ...
  let turn;
  try {
    turn = await openAiConversationTurn(supabase, {
      actorEmail,
      conversationId,
      content: body.text,
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Could not start AI conversation" },
      { status },
    );
  }
