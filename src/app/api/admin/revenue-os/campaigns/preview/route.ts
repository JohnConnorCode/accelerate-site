...
export async function GET(request: NextRequest) {
  ...
  const [campaignResult, stepsResult, membersResult] = await Promise.all([
    ...
    supabase.from("campaign_members").select("id,email,status,current_step,contacts!campaign_members_contact_id_tenant_fkey(full_name),opportunities!campaign_members_opportunity_id_tenant_fkey(name,stage)").eq("campaign_id", id).order("created_at").limit(500),
  ]);
