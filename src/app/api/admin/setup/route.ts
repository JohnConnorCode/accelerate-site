...
import {
  createBootstrapServiceRoleClient,
...
export async function GET() {
  ...
  const supabaseConfigured = configured(
  ...
  const [
    ...
    ? await Promise.all([
        platform
          ...
          .eq("source", "revenue-os-master-plan")
          .is("archived_at", null)
          .limit(1),
        supabase
          .from("integration_connections")
          .select(
            "account_email,status,scopes,last_success_at,last_error,settings,encrypted_access_token,encrypted_refresh_token,token_expires_at",
          )
          .eq("provider", "google")
          .maybeSingle(),
        supabase
          .from("source_runs")
          .select("source_key,status,summary,error,finished_at")
          .order("started_at", { ascending: false })
          .limit(40),
        supabase
          .from("job_runs")
          .select("job_key,status,summary,error,finished_at,claimed_at")
        ...
        supabase
          ...
          .select("id,status,review_digest,approval_digest,ai_provider", { count: "exact" })
          .limit(1),
        platform
          .from("schema_verification_runs")
          .select("contract_version,status,failure_detail,checked_at")
  ...
  const schemaCenter = computeSchemaCenterStatus({
    ...
    latestVerification: latestSchemaVerification
      ? latestSchemaVerification.contract_version === REVENUE_SCHEMA_CONTRACT_VERSION &&
        latestSchemaVerification.status === "success"
  ...
  const requiredGoogleScopes = GOOGLE_SCOPES.filter(
  ...
  const googleTokenHealth = {
    accessEnvelopeValid:
      ...
      isEncryptedSecret(google.encrypted_access_token),
    refreshEnvelopeValid:
      typeof google?.encrypted_refresh_token === "string" &&
      isEncryptedSecret(google.encrypted_refresh_token),
    expiresAt: google?.token_expires_at ?? null,
  };
  const googleConnected =
    google?.status === "connected" &&
    Boolean(google.account_email) &&
    googleScopesReady &&
    googleTokenHealth.accessEnvelopeValid &&
    googleTokenHealth.refreshEnvelopeValid;
  const latestSource = new Map<string, SourceRunRow>();
  for (const run of sourceRunsResult.data ?? [])
    if (!latestSource.has(run.source_key)) latestSource.set(run.source_key, run);
  const latestJob = new Map<string, JobRunRow>();
  for (const run of jobRunsResult.data ?? [])
    if (!latestJob.has(run.job_key)) latestJob.set(run.job_key, run);
  const scheduler = schedulerResult.data as SchedulerStatus | null;
  const healthSnapshotRun = latestJob.get("system-health-snapshot");
  const healthSnapshotFresh =
    healthSnapshotRun?.status === "success" &&
  ...
  const stalledJobs = [...latestJob.values()]
    .filter(
      (run) =>
        run.status === "running" &&
        (!run.claimed_at || Date.now() - Date.parse(run.claimed_at) > STALLED_JOB_MINUTES * 60_000),
    )
  ...
  const { data: lastOutbound } = supabaseConfigured
    ? await supabase
        .from("messages")
        .select("status,sent_at,created_at,provider_id")
        .eq("direction", "outbound")
        .order("created_at", { ascending: false })
        .limit(1)
        .maybeSingle()
    : { data: null };
  const emailReadiness = resendDeliveryReadiness({
    configured: configured("RESEND_API_KEY", "RESEND_FROM_EMAIL"),
    lastOutbound,
  });
  ...
  const checks: SetupCapability[] = [
    {
      ...
      description: supabaseConfigured
        ? "Runtime database credentials are present."
        : "Add the project URL, anonymous key, and server-only service key.",
      accomplishes:
        "Stores the pipeline, conversations, campaign state, tasks, proposals, and audit history.",
      ...
      action: {
        label: "Open Supabase API settings",
    ...
    },
    {
      ...
      lastSuccessAt:
        latestSchemaVerification?.status === "success" ? latestSchemaVerification.checked_at : null,
      lastFailure: schemaCenter.ready
        ? null
        : (latestSchemaVerification?.failure_detail ??
          runtimeSchema.issues[0]?.message ??
          schemaCenter.reason),
    ...
    },
    {
      ...
      description: configured("ADMIN_EMAIL")
        ? `Admin access is restricted to ${process.env.ADMIN_EMAIL}.`
        : "Set the one email allowed to use the admin.",
      accomplishes:
        "Prevents any other authenticated Supabase account from entering or calling admin APIs.",
    ...
    },
    {
      ...
      description: featureBoardReady
        ? `${featureBoardResult.count} agent-ready Revenue OS work items are loaded and ordered.`
        : !featureBoardResult.error
          ? "No managed work items are loaded. Run npm run seed:features -- --apply."
          : "Apply migrations/20260816-feature-board.sql, then seed the managed backlog.",
      accomplishes:
        "Turns upcoming work into one owned, labeled, prioritized delivery queue instead of scattered notes.",
      status: featureBoardReady ? "ready" : "action",
      required: false,
      keys: ["migrations/20260816-feature-board.sql", "npm run seed:features -- --apply"],
      nextRun: setupNextRun("config"),
      action: {
        label: featureBoardReady
          ? "Open Feature Board"
          : featureBoardResult.error
            ? "Open Supabase SQL editor"
            : "Review backlog instructions",
        href: featureBoardReady
          ? "/admin/features"
          : featureBoardResult.error
            ? supabaseDashboard("/sql/new")
    ...
    },
    {
      ...
      description: configured("NEXT_PUBLIC_SITE_URL")
        ? "Public links and OAuth callbacks use a stable production origin."
        : "Set NEXT_PUBLIC_SITE_URL to the production www URL.",
      accomplishes:
        "Keeps proposal links, OAuth callbacks, and email links on the correct deployment.",
    ...
    },
    {
      ...
      accomplishes:
        "Sends approved campaigns, confirmations, proposals, and transactional messages with receipts.",
    ...
    },
    {
      ...
      description: configured("RESEND_WEBHOOK_SECRET")
        ? "Signed Resend delivery, bounce, complaint, and engagement receipts can update the canonical message and suppression ledger."
        : "Add a signed Resend webhook for /api/webhooks/resend, then save its secret as RESEND_WEBHOOK_SECRET.",
      accomplishes:
        "Turns real delivery failures and spam complaints into immediate campaign suppression instead of assuming every API acceptance reached an inbox.",
      status: configured("RESEND_WEBHOOK_SECRET") ? "ready" : "action",
      required: false,
      keys: ["RESEND_WEBHOOK_SECRET", "/api/webhooks/resend"],
      nextRun: setupNextRun("outbound-send"),
      action: {
        label: "Open Resend webhooks",
    ...
    },
    {
      ...
      description: emailStudioReady
        ? `${emailStudioResult.count ?? 0} editable email revision${emailStudioResult.count === 1 ? " is" : "s are"} stored. Built-in copy remains the fallback.`
        : "Apply the Email Studio migration to edit, test, and publish the copy used by real sends.",
      accomplishes:
        "Provides safe draft/live email editing, rendered previews, founder-only test sends, and an auditable revision history without another provider key.",
      status: emailStudioReady ? "ready" : "action",
      required: false,
      keys: ["migrations/20260816-email-studio.sql"],
      nextRun: setupNextRun("outbound-send"),
      action: {
        label: emailStudioReady ? "Open Email Studio" : "Open Supabase SQL editor",
        href: emailStudioReady ? "/admin/emails" : supabaseDashboard("/sql/new"),
        external: !emailStudioReady,
      },
    },
    {
      id: "google_oauth",
      ...
      description: googleConfigured
        ? "OAuth credentials and server-side token encryption are configured."
        : "Add OAuth credentials and an encryption key before connecting Workspace.",
      accomplishes:
        "Creates one protected connection for Gmail, Calendar, and selected Drive folders.",
    ...
    },
    {
      ...
      description: googleConnected
        ? `${google.account_email} is connected with all required scopes.`
        : google?.last_error
          ? `Connection needs attention: ${google.last_error}`
          : "Connect the founder's Workspace account after OAuth is configured.",
      accomplishes:
        "Synchronizes real conversations and meetings and makes selected Drive documents available to operations.",
      status: !googleConfigured
        ? "disabled"
        : googleConnected
          ? "ready"
          : google?.status === "degraded" || google?.status === "revoked"
            ? "degraded"
            : "action",
      required: false,
    ...
    },
    {
      ...
      description:
        latestSource.get("gmail")?.status === "success"
          ? "Gmail threads are synchronizing into Conversations."
          : "Run the first Workspace sync after connecting Google.",
      accomplishes:
        "Keeps replies, thread history, unread work, and campaign stop conditions current.",
      status: !googleConnected
        ? "disabled"
        : latestSource.get("gmail")?.status === "success"
          ? "ready"
          : latestSource.get("gmail")?.status === "failed"
            ? "degraded"
            : "action",
      required: false,
      lastSuccessAt:
        latestSource.get("gmail")?.status === "success"
          ? latestSource.get("gmail")?.finished_at
          : null,
    ...
    },
    {
      ...
      description:
        latestSource.get("google_calendar")?.status === "success"
          ? "Upcoming and recent calendar events are available to Today and call prep."
          : "Calendar activates with the first Workspace sync.",
      accomplishes:
        "Connects meetings to the pipeline and supports meeting preparation and follow-up.",
      status: !googleConnected
        ? "disabled"
        : latestSource.get("google_calendar")?.status === "success"
          ? "ready"
          : latestSource.get("google_calendar")?.status === "failed"
            ? "degraded"
            : "action",
      required: false,
      lastSuccessAt:
        latestSource.get("google_calendar")?.status === "success"
          ? latestSource.get("google_calendar")?.finished_at
          : null,
    ...
    },
    {
      ...
      description:
        latestSource.get("google_drive")?.status === "success"
          ? "Configured Drive folders are indexed."
          : "Optional: add specific folder IDs; unrelated Drive files remain unavailable.",
      accomplishes:
        "Grounds research, call notes, and proposal drafts in approved source documents.",
      status: !googleConnected
        ? "disabled"
        : latestSource.get("google_drive")?.status === "success"
          ? "ready"
      ...
      lastSuccessAt:
        latestSource.get("google_drive")?.status === "success"
          ? latestSource.get("google_drive")?.finished_at
          : null,
    ...
    },
    {
      ...
      accomplishes:
        "Makes AI-assisted revenue work inspectable and safe without granting arbitrary database or provider access.",
    ...
    },
    {
      ...
      description: !contactImporterReady
        ? "Apply the Contact Import migration."
        : openRouterReady
          ? `${contactImporterResult.count ?? 0} import batch${contactImporterResult.count === 1 ? " is" : "es are"} stored with review and execution receipts.`
          : "The import ledger is ready; this workspace still needs its OpenRouter key for cleanup and mapping.",
      accomplishes:
        "Cleans pasted lists, CSV, TSV, JSON, and notes into reviewed canonical contacts without sending messages or creating opportunities.",
      ...
      nextRun: setupNextRun("config"),
      action: {
        label: contactImporterReady ? "Open Contact Import" : "Open Supabase SQL editor",
        href: contactImporterReady ? "/admin/contact-imports" : supabaseDashboard("/sql/new"),
        external: !contactImporterReady,
      },
    },
    {
      id: "campaigns",
      ...
      description:
        campaignReadiness.status === "ready"
      ...
      accomplishes:
        "Automates approved follow-up while preserving pause, reply, booking, bounce, and unsubscribe stops.",
    ...
    },
    {
      ...
      description:
        !proposalResult.error && configured("NEXT_PUBLIC_SITE_URL")
          ? "Public proposals can be viewed, accepted, or declined without payment."
          : "Apply the proposal schema and configure the public site URL.",
      accomplishes:
        "Turns proposal interest into an auditable pipeline outcome and founder follow-up.",
    ...
    },
    {
      ...
      description: firstPartyAnalyticsReady
        ? "Page views and conversion events are captured by this site and reported beside canonical revenue—no analytics vendor account or API key required."
        : "Apply migrations/20260816-first-party-analytics.sql to enable turn-key website measurement.",
      accomplishes:
        "Measures the path from page visit and conversion signal through qualified opportunity and revenue without a third-party analytics dependency.",
      status: firstPartyAnalyticsReady ? "ready" : "action",
      required: true,
      keys: ["migrations/20260816-first-party-analytics.sql"],
      nextRun: setupNextRun("public-event"),
      action: firstPartyAnalyticsReady
    ...
    },
    {
      ...
      accomplishes:
        "Adds booking and cancellation attribution through the canonical opportunity service without treating API tokens as health.",
    ...
    },
    {
      ...
      accomplishes:
        "Removes the daily-only ceiling for proactive intelligence without moving business rules out of Revenue OS services.",
      status:
        schedulerResult.error || !scheduler?.configured
          ? "action"
      ...
      keys: [
        ...
        "/api/cron/system-health-snapshot",
      ],
      ...
      lastFailure:
        scheduler?.last_run_status === "failed"
          ...
          : healthSnapshotRun?.status === "failed"
            ? healthSnapshotRun.error
            : null,
    ...
    },
    {
      ...
      description: configured("CRON_SECRET")
        ? "Cron endpoints are protected and record terminal job receipts."
        : "Set a long random CRON_SECRET before scheduling jobs.",
      accomplishes:
        "Makes automation health, failures, retries, and last successful work visible instead of silently returning 200.",
      ...
      keys: ["CRON_SECRET"],
      lastSuccessAt:
        [...latestJob.values()].find((run) => run.status === "success")?.finished_at ?? null,
