...
export async function GET() {
  ...
  const platformDatabase = authorization.isPlatformAdmin
    ? createPlatformServiceRoleClient("tenant-directory")
    : null;
  const database = platformDatabase || authorization.database;
  const tenantQuery = database
    .from("tenants")
    .select("id,slug,name,status,config_version,created_at,updated_at")
    .order("name", { ascending: true });
  const membershipQuery = database
    .from("tenant_memberships")
    .select("id,tenant_id,user_id,invited_email,role,status,invited_at,activated_at,revoked_at")
    .order("invited_at", { ascending: true });
  if (!authorization.isPlatformAdmin) membershipQuery.eq("user_id", authorization.user.id);
  const [{ data: tenants, error: tenantError }, { data: memberships, error: membershipError }] =
    await Promise.all([tenantQuery, membershipQuery]);
  if (tenantError || membershipError)
    return NextResponse.json(
      { error: tenantError?.message || membershipError?.message || "Could not load workspaces" },
      { status: 500 },
    );
  return NextResponse.json({
    currentTenantId: authorization.tenant.id,
    isPlatformAdmin: authorization.isPlatformAdmin,
    platformOwnerUserId: authorization.isPlatformAdmin ? authorization.user.id : null,
    tenants: tenants || [],
...
export async function POST(request: NextRequest) {
  ...
  if (!parsed.success)
    return NextResponse.json({ error: "Invalid tenant action" }, { status: 400 });
  const input = parsed.data;
  const actor: PlatformTenantActor = {
    userId: authorization.user.id,
    email: authorization.user.email?.trim().toLowerCase() || "",
  };
  if (!actor.email)
  ...
  try {
    ...
    return NextResponse.json(
      {
        error: "Tenant action failed safely. Retry, or check server logs if the problem continues.",
      },
      { status: 500 },
    );
  }
