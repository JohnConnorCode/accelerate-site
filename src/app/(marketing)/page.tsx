...
export async function generateMetadata() {
  if (distributionProfile() === "neutral") {
    ...
    return { title: { absolute: identity.title }, description: identity.description, alternates: { canonical: identity.siteUrl } };
  }
  ...
  if (distributionProfile() === "neutral") return (
    <main className="mx-auto flex min-h-screen max-w-3xl flex-col justify-center gap-6 px-6 py-16">
      ...
      <nav aria-label="Workspace entry" className="flex flex-wrap gap-6">
        <Link href="/admin" className="underline underline-offset-4">Open your workspace</Link>
        <Link href="/demo/command-center" className="underline underline-offset-4">Explore fictional demo workspaces</Link>
      </nav>
      <p className="text-sm">Connect your own services in Setup. Demo changes stay in this browser.</p>
    </main>
  );
