...
targetKeywords:
  [
    "AI document processing",
    "automate data entry",
    "document automation small business",
...
<ComparisonTable
  ...
  rows={[
    [
      "Extraction model",
      "Reads the document, returns structured fields with confidence",
      "Claude (vision), GPT-4o (vision), Google Document AI, AWS Textract",
      "Pay-per-page or per-token, often pennies per document",
    ],
    [
      "Orchestration",
      "Watches the inbox/folder, calls the model, routes results, writes to your system",
      "Make, n8n, Zapier",
      "Free to ~$50/mo",
    ],
    [
      ...
      "QuickBooks, HubSpot, Airtable, your CRM",
...
<Callout type="warning" title="On contracts specifically">
  Contracts are where blind extraction is most dangerous. Pulling the parties, the effective date,
  and the renewal date from a contract is reasonable to automate. Deciding whether an
  indemnification clause is acceptable is not, and no confidence score should let it through
  unreviewed. For contracts, automate the data fields and the routing (this contract goes to legal,
  that renewal date creates a reminder task), but keep a human on anything that requires
