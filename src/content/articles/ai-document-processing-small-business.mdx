...
<StatHighlight value="2 hrs to under 10 min" label="time to onboard a new client after we automated SparkBlox's document pipeline" />
...
Onboarding dropped from **2 hours to under 10 minutes per client**. The number that actually matters to the business: with that time reclaimed, SparkBlox grew its client base **70% in Q1 without hiring a single additional person.** That is the real return on document automation. It is not "we save a few hours." It is "we can take on dramatically more business with the same team," because the work that used to gate growth no longer requires a human to push paper.
...
<QuoteBlock
  ...
  author="Operations Lead, SparkBlox"
