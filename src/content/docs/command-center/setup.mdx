...
Gmail follow-up drafts use a separate, optional **Gmail draft access** check. Grant it only if you want approved follow-ups saved to Gmail Drafts. This consent uses Google’s `gmail.compose` scope, which can technically send as well; the Command Center follow-up path uses draft creation only and never sends. Existing Gmail sync does not require this optional permission.
...
## Start with a saved task

A fresh clone opens a fictional demo without credentials. Connect your own
workspace using the [installation guide](/docs/self-hosting/installation), then
create a fictional contact and a task, reload, and confirm both remain saved.
Complete the task and check its activity. Email, AI and scheduled work can be
connected afterward; they are not prerequisites for this first manual result.
