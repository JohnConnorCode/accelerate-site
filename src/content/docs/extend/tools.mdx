...
When an assistant needs a new business operation, register a narrow tool backed by the same service the interface uses. The registry in `src/lib/revenue-os/ai-tools.ts` supplies the generated [AI tool reference](/docs/intelligence/tools), so developers can inspect the current input contract and capability requirements.
