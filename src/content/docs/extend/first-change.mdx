...
You need Git, Node.js 22 or later, npm and an editor. For team work, get the reviewed branch and exact commit from the maintainer before cloning; the public default branch may not contain the assigned feature.

```bash
node --version
npm --version
git clone --branch <reviewed-branch> https://github.com/JohnConnorCode/accelerate-site.git
cd accelerate-site
npm ci
npm run dev
```

Replace `<reviewed-branch>` with the supplied branch. After cloning, compare `git rev-parse HEAD` with the supplied commit. Resolve a mismatch before installing or editing.
