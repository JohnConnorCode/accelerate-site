...
A module is the unit a workspace actually turns on and off, and adding one never means touching the application's core array. You drop a JSON manifest in `extensions/` and the pages it names, and that's the whole registration surface.
