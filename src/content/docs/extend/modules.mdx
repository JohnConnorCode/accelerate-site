...
A manifest-registered module's API routes must also be listed in `MODULE_API_DIRECTORIES` (`scripts/verify-module-route-guards.mjs`) or they will not refuse when the module is off.
