...
description: "Give an App its own lifecycle, and link tasks, decisions, and history back to the real record."
updated: "2026-09-09"
---

A creative review has a brief, versions, reviewers, requested changes, and a final accepted version, and compressing all of that into a single generic task status would throw away exactly the meaning that makes it useful. Give the review its own record and lifecycle instead, and link ordinary tasks or approvals to it only when they represent genuinely additional work or a specific decision.
