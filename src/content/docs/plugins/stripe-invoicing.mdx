...
**Demo verification limit:** The documentation review found an invoice-history rendering error when a simulated result lacks its currency. The issue is tracked as `invoice-demo-missing-currency`; this guide does not claim that the current demo invoice journey passed. The workflow below describes the intended controls and should be verified after that repair.
