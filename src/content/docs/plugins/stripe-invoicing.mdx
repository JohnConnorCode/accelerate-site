...
Preparing an invoice means bringing a customer, amount and agreed terms together correctly. Stripe invoicing combines existing customer records, a configured Stripe connection and reviewed operations, with provider results recorded separately from the requests that produced them.

<DocsFigure
  src="/images/docs/plugins/stripe-invoicing.png"
  alt="The Stripe invoicing screen in the fictional Northline Roofing workspace, showing a new customer invoice form and its preview."
  width={1440}
  height={1000}
  caption="Stripe invoicing in the fictional Northline Roofing workspace, clearly labeled as a fictional, simulated session. Fictional demo data."
/>
...
The **All invoices** section reads every invoice in the connected Stripe account, including invoices created outside this workspace. It loads results in bounded pages. The app's reviewed draft and send actions remain in the invoice tools below; Stripe's status is the source of truth for invoice payment state.
