...
1. Run `npm run db:migrate:all` in the deployment environment, then run `npm run db:verify-schema -- --record`. The ordered catalog includes `migrations/20260925-stripe-subscriptions.sql`, which creates the tenant billing tables and webhook receipt ledger.
