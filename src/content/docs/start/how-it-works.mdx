...
When an agent stops partway through a change, its saved checkpoint carries the
source and remaining steps to a replacement. Install the recovery migration and
enable the project's recovery policy to let the normal pickup flow resume expired
work. The previous checkout stays available for inspection, while the board gives
the replacement its own attempt and retains the earlier history.
