...
Then run only the command that matches the row. Each command below exists in `package.json`. `scripts/verify-docs.ts` fails this guide if a documented `npm run` script is removed.
...
An expired claim still occupies a WIP slot. `scripts/agent-dispatch.ts` has no force bypass; it tells the worker to ask an operator to review the claim on Feature Board. Do not delete another agent's worktree to "fix" startup.
