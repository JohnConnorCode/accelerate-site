...
The bundled homepage has editable native sections. Other source routes remain intact until explicitly replaced by published content; wholesale import of their original layouts is not implemented. Assets reference existing paths or HTTPS images. File uploading and scheduled publication remain unavailable.

The owner-only `/api/mcp/site-studio` endpoint exposes five editor tools through revocable native OAuth delegation. It covers the installation document, pages, layouts, text, branding, navigation, assets, collections, import/export, publication and history using the same validation and write service as the UI. Private tenant drafts and general workspace operations remain outside this connection. Legacy API keys do not gain execution rights. Follow [ChatGPT setup](../../docs/self-hosting/SITE-STUDIO-CHATGPT.md), including the token-role hook, before enabling the connection. Run `test:site-editor` and `test:site-editor:postgres` for controlled authorization and transaction checks; a real client connection is a separate deployment verification.
