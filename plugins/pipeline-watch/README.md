...
Find open opportunities needing follow-up. This is a deterministic, read-only report, disabled by default.
...
1. Open **Plugins** (`/admin/plugins`) in an authorized workspace and enable
   **Pipeline Watch**. Configuration uses the shared module controls; this report has
...
Non-won/lost/archived opportunities appear when their next action is overdue or their last update is at least seven days old.

The host reads tenant-scoped opportunities (name, stage, update time and next action time). Each source is bounded
...
The equivalent AI/MCP tool is `run_pipeline_watch`; the host rechecks current
...
A fictional open opportunity last updated ten days ago appears once; a won deal is excluded. Run `npm run test:report-plugins`
...
[manifest](../../extensions/pipeline-watch.module.json), preserving the declared source
