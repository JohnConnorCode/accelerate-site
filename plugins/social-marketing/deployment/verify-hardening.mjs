...
try {
  ...
  const source = readFileSync(
    new URL("libraries/helpers/src/utils/read.or.fetch.ts", root),
    "utf8",
  );
  ...
  vm.runInNewContext(compiled, {
    ...
    require: (name) =>
      name === "axios"
        ? async () => {
            remoteCalls++;
            return { data: Buffer.from("remote fixture") };
          }
        : require(name),
  });
  const read = exports.readOrFetch;
  assert.equal(
    (await read("https://social.example.test/uploads/approved.png")).toString(),
    "approved fixture bytes",
  );
  assert.equal(remoteCalls, 0, "Private local media must not fetch through the public proxy");
  await assert.rejects(
    read("https://social.example.test/uploads/escape.png"),
    /Invalid local media path/,
  );
  await assert.rejects(
    read("https://social.example.test/uploads/..%2fprivate.txt"),
    /Invalid local media path/,
  );
  await assert.rejects(read("https://social.example.test/uploads/missing.png"), /ENOENT/);
  assert.equal(
    (await read("https://external.example.test/image.png")).toString(),
    "remote fixture",
  );
  assert.equal(remoteCalls, 1, "Other provider helper behavior remains unchanged");
  console.log(
    "Patched media reader: local bytes, symlink/traversal refusal, missing file and external-source behavior passed",
  );
} finally {
