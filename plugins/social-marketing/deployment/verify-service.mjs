...
async function request(path, options = {}) {
  ...
  const response = await fetch((privateBootstrap ? privateOrigin : origin) + path, { ...init, redirect: "error", signal: AbortSignal.timeout(15000) });
  return response;
}
...
if (phase === "bootstrap") {
  ...
  for (const name of ["alpha", "beta"]) {
    const registration = await request("/api/auth/register", {
      privateBootstrap: true, method: "POST", headers: { "content-type": "application/json" },
...
const foreignImage = await request("/api/public/v1/posts", {
  method: "POST", headers: { authorization: fixtures[1].key, "content-type": "application/json" },
  body: JSON.stringify({ type: "draft", posts: [{ integration: { id: "unconnected-fixture" }, value: [{ content: "Never publish this fixture", image: [fixtures[0].media] }] }] }),
});
...
writeFileSync(`evidence/${phase}.json`, JSON.stringify({
  ...
  publicUploadURLsDenied: true, foreignMediaRefused: true, publicRegistrationDenied: true,
  productionReady: false,
  remaining: ["Real connected-page and publication isolation require provider fixtures.", "Temporal history and full host restoration remain unverified.", "LinkedIn access and persistent HTTPS host are not configured."],
}, null, 2));
console.log(`${phase}: two organization identities and stored image bytes verified; public media and bootstrap refused; foreign media reference refused`);
