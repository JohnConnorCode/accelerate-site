...
const safePath = (path) => typeof path === "string" && path.length > 0 && !isAbsolute(path) &&
  !path.split(/[\\/]/).some((part) => !part || part === "." || part === "..");
const forbidden = (path) => path.split(/[\\/]/).some((part) =>
  /^(?:\.git|\.vercel|node_modules|\.next.*|\.env(?:\..*)?|.*\.(?:pem|key|p12|pfx))$/i.test(part));
...
export function exportNeutralStarter(root, output) {
  ...
  if (existsSync(output)) throw new Error("Starter output must be a new directory; existing source is never overwritten.");
  ...
  for (const [target, source] of Object.entries(replacements)) {
    if (!safePath(target) || !safePath(source) || !source.startsWith("distribution/neutral-starter/") || forbidden(target) || forbidden(source)) throw new Error("Unsafe starter replacement path.");
  ...
  for (const [target, source] of plan) {
    ...
    if (!lstatSync(full).isFile() || realpathSync(full) !== full) throw new Error(`Starter source must be a regular file: ${source}`);
  }
  if (JSON.stringify(manifest.environment) !== JSON.stringify({ NEXT_PUBLIC_DISTRIBUTION_PROFILE: "neutral" })) {
  ...
  for (const [target, source] of plan.sort(([a], [b]) => a.localeCompare(b))) {
    ...
    receipts.push({ path: target, source, sha256: createHash("sha256").update(readFileSync(destination)).digest("hex") });
  }
  writeFileSync(resolve(output, ".env"), "NEXT_PUBLIC_DISTRIBUTION_PROFILE=neutral\n");
  writeFileSync(resolve(output, "neutral-starter-receipt.json"), JSON.stringify({ schemaVersion: 1, profile: "neutral", files: receipts }, null, 2) + "\n");
  return { output, files: receipts.length, receipt: "neutral-starter-receipt.json" };
}
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  try {
    const args = process.argv.slice(2);
    if (args.length !== 2 || args[0] !== "--output") throw new Error("Usage: node scripts/export-neutral-starter.mjs --output /new/absolute/path");
