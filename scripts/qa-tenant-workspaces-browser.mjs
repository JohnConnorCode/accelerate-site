...
for (const fixture of fixtures) {
  ...
  await page.route("**/api/admin/tenants", (route) => {
    if (route.request().method() === "POST") {
      ...
      if (inviteRequest?.action !== "invite")
        return route.fulfill({
          status: 405,
          contentType: "application/json",
          body: JSON.stringify({ error: "Visual QA only simulates invitation recovery" }),
        });
      return route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify({
          membership: {
            ...
            invited_email: "owner@harbor.example",
            status: "invited",
          },
          deliveryStatus: "sent",
    ...
    if (route.request().method() !== "GET")
      return route.fulfill({
        status: 405,
        contentType: "application/json",
        body: JSON.stringify({ error: "Visual QA never mutates tenant state" }),
      });
    return route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({
        ...
        tenants: [
          ...
          {
            id: "tenant-lake",
            slug: "lake-street-law",
            name: "Lake Street Law",
            status: "suspended",
        ...
        memberships: [
          {
            id: "member-founder-1",
            tenant_id: "tenant-accelerate",
            user_id: "platform-owner",
            invited_email: "founder@example.com",
            status: "active",
          },
          {
            id: "member-founder-2",
            tenant_id: "tenant-northline",
            user_id: "platform-owner",
            invited_email: "founder@example.com",
            status: "active",
          },
          {
            ...
            invited_email: "operator@northline.example",
            status: "active",
          },
          {
            id: "member-client-2",
  ...
  const response = await page.goto(`${base}/t/accelerate/admin/tenants`, {
    waitUntil: "domcontentloaded",
    timeout: 60_000,
  });
  if (!response || response.status() >= 400)
    throw new Error(`${fixture.name} tenant directory returned ${response?.status()}`);
  console.log(`tenant-workspace-qa: ${fixture.name} loaded ${page.url()} (${response.status()})`);
  await page
    .getByRole("heading", { name: "Tenant operations" })
  ...
  if (await page.getByRole("heading", { name: "Accelerate" }).isVisible())
    throw new Error(`${fixture.name} tenant search did not filter the directory`);
  await page.getByLabel("Search tenants").fill("");
  await page.getByLabel("Workspace name").fill("North Star Dental");
  if ((await page.getByLabel("URL slug").inputValue()) !== "north-star-dental")
    throw new Error(`${fixture.name} generated tenant slug is not stable`);
  ...
  if (resendMetrics.width < 40 || resendMetrics.height < 40)
    throw new Error(`${fixture.name} resend target is smaller than 40px`);
  if (
    !resendMetrics.transitionProperty.includes("transform") ||
    resendMetrics.transitionProperty.includes("all")
  )
    throw new Error(`${fixture.name} resend transition is not intentional`);
  await resend
    .locator("xpath=../..")
    .screenshot({ path: `${output}/invitation-${fixture.name}.png` });
  await resend.focus();
  await page.keyboard.press("Enter");
  await page
    .getByText("Invitation sent to owner@harbor.example", { exact: true })
    .waitFor({ state: "visible" });
  if (
    inviteRequest?.tenantId !== "tenant-harbor" ||
    inviteRequest?.adminEmail !== "owner@harbor.example" ||
    !/^[0-9a-f-]{36}$/i.test(inviteRequest?.requestId || "")
  ) {
    throw new Error(
      `${fixture.name} resend did not preserve tenant, recipient, and request identity`,
    );
  }
  ...
  if (dimensions.width > dimensions.viewport + 2)
    throw new Error(
      `${fixture.name} workspace overflow ${dimensions.width} > ${dimensions.viewport}`,
    );
  ...
  await page.route("**/api/admin/tenant/providers", (route) => {
    if (route.request().method() !== "GET")
      return route.fulfill({
        status: 405,
        contentType: "application/json",
        body: JSON.stringify({ error: "Visual QA never mutates provider state" }),
      });
    return route.fulfill({
      status: 200,
      contentType: "application/json",
        ...
        providers: [
          {
            ...
            connected_at: "2026-08-31T12:00:00.000Z",
            credential_source: "tenant",
            key_metadata: {
              label: "Northline production",
              limit: 50,
              limit_remaining: 41.25,
              limit_reset: "monthly",
              usage: 8.75,
              is_free_tier: false,
              expires_at: null,
  ...
  await page.goto(`${base}/t/accelerate/admin/integrations`, {
    waitUntil: "domcontentloaded",
    timeout: 60_000,
  });
  await page
    ...
    .waitFor({ state: "visible", timeout: 30_000 });
  await page
    .getByRole("heading", { name: "Provider credentials" })
    .waitFor({ state: "visible", timeout: 30_000 });
  ...
  if ((await apiKeyField.getAttribute("type")) !== "text")
    throw new Error(`${fixture.name} credential reveal control did not update the field`);
  ...
  await page
    .locator(".admin-main")
    .evaluate((element) => element.scrollTo({ top: 0, behavior: "instant" }));
  const integrationDimensions = await page.evaluate(() => ({
    width: document.documentElement.scrollWidth,
    viewport: innerWidth,
  }));
  if (integrationDimensions.width > integrationDimensions.viewport + 2)
    throw new Error(
      `${fixture.name} integrations overflow ${integrationDimensions.width} > ${integrationDimensions.viewport}`,
    );
    ...
    const diagnostics = await page.evaluate(async () =>
      Promise.all(
        [
          ...
          "/api/admin/revenue-os/priority",
          "/api/admin/revenue-os/ai/conversations?limit=30",
        ].map(async (url) => {
          try {
