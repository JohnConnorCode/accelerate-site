...
async function main() {
  ...
  try {
    ...
    const { error: jobError } = await supabase.from("job_runs")
    ...
    check("a stalled job makes health require attention", withStalled.status === "attention", withStalled.status);
    check("a stalled job raises a concern that names it", withStalled.concerns.some((c) => c.kind === "job" && c.key === JOB_KEY), withStalled.concerns.filter((c) => c.kind === "job"));
    check("the stalled concern explains what will happen next", withStalled.concerns.some((c) => c.key === JOB_KEY && /take the claim over/i.test(c.detail)), withStalled.concerns.find((c) => c.key === JOB_KEY)?.detail);
    check("the stalled job increases the attention count", withStalled.attentionCount > baseline.attentionCount, { baseline: baseline.attentionCount, withStalled: withStalled.attentionCount });

    // 2. A webhook that arrived and could not be processed.
    const { error: hookError } = await supabase.from("webhook_receipts").insert({
      id: RECEIPT_ID, provider: "resend", event_type: "email.bounced",
      status: "failed", error: "Verification: suppression could not be applied",
    });
    ...
    check("a failed webhook receipt is surfaced", withBoth.webhookFailures.some((f) => f.id === RECEIPT_ID), withBoth.webhookFailures.length);
    check("a failed webhook raises its own concern", withBoth.concerns.some((c) => c.kind === "webhook"), withBoth.concerns.filter((c) => c.kind === "webhook"));
    check("both failures are counted, not collapsed", withBoth.attentionCount >= withStalled.attentionCount + 1, { withStalled: withStalled.attentionCount, withBoth: withBoth.attentionCount });
  } finally {
  ...
  check("health returns to its previous reading once the causes are gone", restored.attentionCount === baseline.attentionCount, { baseline: baseline.attentionCount, restored: restored.attentionCount });
...
main().catch((error) => {
  console.error("Health truth verification errored:", error instanceof Error ? error.message : error);
  console.error(`Look for leftovers: job_runs job_key ${JOB_KEY}, webhook_receipts id ${RECEIPT_ID}.`);
  process.exit(1);
});
