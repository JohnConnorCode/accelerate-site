  ...
  for (const name of files) {
    ...
    if (manifest.knowledge) {
      const declared = new Set(
        (manifest.report?.sources ?? manifest.workflow?.sources ?? []).map((source) => source.name),
      );
      for (const name of manifest.knowledge.sourceNames)
        if (!declared.has(name))
          fail(name, "Knowledge must reuse an existing host-reviewed report or workflow source");
    }
