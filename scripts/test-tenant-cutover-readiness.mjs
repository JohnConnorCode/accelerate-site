...
const compatibilitySql = readFileSync("migrations/20260830-tenant-uniqueness-compatibility.sql", "utf8");
...
assert.equal(expectedIndexes.length, 28, "the release gate must inventory every temporary compatibility index");
assert.ok(expectedArtifacts.indexes.length > expectedIndexes.length, "the release gate must also inventory original-name global indexes");
assert.equal(expectedArtifacts.constraints.length, 10, "the release gate must inventory every removed global uniqueness constraint");
...
for (const mutation of [
  ...
  { worktrees: [...cleanRepository.worktrees, { path: "/tmp/investigation", branch: "investigation", head: "b".repeat(40) }] },
  { unmergedBranches: ["investigation"] },
  { requiredTrackedFiles: [{ path: "migration.sql", tracked: false }] },
]) {
  assert.equal(resultFor("repository", evaluateRepository({ ...cleanRepository, ...mutation })).status, "blocked");
}
...
assert.equal(resultFor("post-migration", evaluateDatabase("post-migration", safeDatabase, expectedArtifacts)).status, "ready");
assert.equal(resultFor("post-migration", evaluateDatabase("post-migration", { ...safeDatabase, suspensionGuardApplied: false }, expectedArtifacts)).status, "blocked");
assert.equal(resultFor("post-migration", evaluateDatabase("post-migration", { ...safeDatabase, legacyIndexes: [expectedArtifacts.indexes[0]] }, expectedArtifacts)).status, "blocked");
assert.equal(resultFor("post-migration", evaluateDatabase("post-migration", { ...safeDatabase, emailTemplatesCompositePrimaryKey: false }, expectedArtifacts)).status, "blocked");

const postDeployDatabase = safeDatabase;
assert.equal(resultFor("post-deploy", evaluateDatabase("post-deploy", postDeployDatabase, expectedArtifacts)).status, "ready");
...
for (const unsafe of [
  ...
  assert.equal(resultFor("pre-activation", evaluateDatabase("pre-activation", { ...postDeployDatabase, ...unsafe }, expectedArtifacts, "controlled-client")).status, "blocked");
}

const databaseReaderSource = readDatabaseState.toString();
assert.doesNotMatch(databaseReaderSource, /\b(?:DELETE\s+FROM|UPDATE\s+public\.|INSERT\s+INTO|TRUNCATE|DROP\s+TABLE|ALTER\s+TABLE|CREATE\s+TABLE)\b/i, "cutover readiness must remain read-only");
const productionProofSource = readFileSync("scripts/verify-tenant-production-isolation.mjs", "utf8");
assert.ok(productionProofSource.includes("--confirm-controlled-production-isolation"), "production isolation proof must require an explicit mutation confirmation");
assert.ok(productionProofSource.includes('PROJECT_REF = "skjypuwkceoiunyhhqlm"') && productionProofSource.includes("`${PROJECT_REF}.supabase.co`"), "production isolation proof must remain fixed to the Accelerate project");
assert.match(productionProofSource, /finally \{[\s\S]*Promise\.allSettled[\s\S]*setStatus\(tenant, "suspended"/, "production proof failures must always suspend every controlled tenant");
console.log(JSON.stringify({ result: "passed", repositoryFailureModes: 5, compatibilityIndexes: expectedIndexes.length, activationFailureModes: 4, productionMutationGuard: true }));
