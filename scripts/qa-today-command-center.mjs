...
const overview = {
  ...
  queue: [
    { id: "conversation:1", kind: "reply", title: "Apex Roofing needs a proposal revision", summary: "2 unread messages · buying", urgency: "high", dueAt: "2026-08-16T08:00:00.000Z", sourceTimestamp: "2026-08-16T08:00:00.000Z", priorityReason: "Buying signal needs a reply", recommendedNextAction: "Read the latest message and stage a grounded reply.", href: "/admin/conversations?thread=1" },
    { id: "task:1", kind: "task", title: "Confirm discovery call objectives", summary: "Capture decision makers and desired outcomes before Monday.", urgency: "critical", dueAt: "2026-08-15", sourceTimestamp: "2026-08-15T14:00:00.000Z", priorityReason: "Overdue commitment", recommendedNextAction: "Open the opportunity and complete or reschedule the commitment.", href: "/admin/pipeline?opportunity=1" },
    { id: "task:2", kind: "task", title: "Prepare the weekly pipeline review", summary: "Collect blockers and next actions before the operator review.", urgency: "normal", dueAt: "2026-08-18", sourceTimestamp: "2026-08-16T07:00:00.000Z", priorityReason: "Due 2026-08-18", recommendedNextAction: "Complete the commitment, or snooze it to a specific date.", href: "/admin/today" },
    { id: "action:1", kind: "approval", title: "Approve follow-up to Summit Mechanical", summary: "A concise, founder-reviewed response is ready to send.", urgency: "high", dueAt: "2026-08-16T09:00:00.000Z", sourceTimestamp: "2026-08-16T09:00:00.000Z", priorityReason: "Approval required before execution", recommendedNextAction: "Review the exact action and approve or reject it.", href: "/admin/today?focus=approvals" },
    { id: "action:2", kind: "approval", title: "Review campaign activation", summary: "The campaign is staged but must not start without approval.", urgency: "normal", dueAt: "2026-08-16T09:30:00.000Z", sourceTimestamp: "2026-08-16T09:30:00.000Z", priorityReason: "Approval required before execution", recommendedNextAction: "Review the exact action and approve or reject it.", href: "/admin/today?focus=approval&action=2" },
    { id: "proposal:1", kind: "proposal", title: "Westlake automation proposal", summary: "Viewed and awaiting a response.", urgency: "normal", dueAt: "2026-08-21T12:00:00.000Z", sourceTimestamp: "2026-08-16T10:00:00.000Z", priorityReason: "Proposal expires within seven days", recommendedNextAction: "Review proposal activity and send a deliberate follow-up.", href: "/admin/proposals" },
  ],
  integrations: [],
  health: {
...
let mutableActions = [
  { id: "1", action_type: "send_email", title: "Approve follow-up to Summit Mechanical", description: "A concise, founder-reviewed response is ready to send.", urgency: "high", reasoning: null, status: "pending", created_at: "2026-08-16T09:00:00.000Z", expires_at: null, payload: { to: "owner@summit.example", subject: "Next steps", body: "Thanks for the conversation. Here are the agreed next steps." } },
  { id: "2", action_type: "activate_campaign", title: "Review campaign activation", description: "The campaign is staged but must not start without approval.", urgency: "normal", reasoning: "The operator asked to review the audience before launch.", status: "pending", created_at: "2026-08-16T09:30:00.000Z", expires_at: null, payload: { campaignId: "campaign-1" } },
];
const aiRuns = { schemaReady: true, feedback: { helpful: 3, notHelpful: 1 }, runs: [{ id: "qa-agent-run", surface: "admin_command_stream", provider: "openrouter", model: "openai/gpt-5-mini", tool_pack: "core", conversation_id: "qa-conversation", duration_ms: 4000, status: "completed", tool_names: ["get_today_snapshot", "propose_task"], input_tokens: 720, output_tokens: 190, result_preview: "Focus first on the overdue discovery-call commitment, then reply to Apex Roofing.", error: null, started_at: "2026-08-16T10:00:00.000Z", finished_at: "2026-08-16T10:00:04.000Z", feedback: "helpful" }] };
...
async function pageFor(viewport, reducedMotion = "no-preference") {
  ...
  page.on("response", (response) => {
    ...
    if (failOverview && response.url().includes("/api/admin/revenue-os/overview")) expectedFailureResponses += 1;
  ...
  await page.route("**/api/admin/revenue-os/overview", (route) => failOverview
    ? route.fulfill({ status: 503, contentType: "application/json", body: JSON.stringify({ error: "Priority sources are temporarily unavailable." }) })
    : route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ ...overview, generatedAt: new Date().toISOString(), queue: mutableQueue }) }));
  await page.route("**/api/admin/revenue-os/priority", (route) => route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ generatedAt: overview.generatedAt, summary: { total: mutableQueue.length, urgent: mutableQueue.filter((item) => ["critical", "high"].includes(item.urgency)).length, critical: mutableQueue.filter((item) => item.urgency === "critical").length }, items: mutableQueue }) }));
  await page.route("**/api/admin/notifications", (route) => route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ notifications: [], unreadCount: 0, urgentCount: 0, priority: { status: "ready", summary: { total: mutableQueue.length, urgent: mutableQueue.filter((item) => ["critical", "high"].includes(item.urgency)).length, critical: mutableQueue.filter((item) => item.urgency === "critical").length }, items: mutableQueue.slice(0, 5) } }) }));
  await page.route("**/api/admin/revenue-os/actions", async (route) => {
    if (route.request().method() === "PATCH") {
      ...
      await route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ action: { id: body.id, status: body.decision === "approve" ? "approved" : "rejected" } }) });
      return;
    }
    await route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ actions: mutableActions }) });
  });
  ...
  await page.route("**/api/admin/revenue-os/ai/stream", (route) => {
    const answer = "Focus first on the overdue discovery-call commitment, then reply to Apex Roofing.";
    const events = [
      ...
      { type: "final", conversationId: "qa-conversation", messageId: "qa-assistant-message", runId: "qa-agent-run", text: answer, proposedActions: [] },
    ];
    return route.fulfill({ status: 200, contentType: "text/event-stream; charset=utf-8", body: events.map((event) => `event: ${event.type}\ndata: ${JSON.stringify(event)}\n\n`).join("") });
  });
...
await desktop.page.getByText("Next: Read the latest message and stage a grounded reply.", { exact: true }).waitFor();
...
await desktopPriority.getByText("Next: Open the opportunity and complete or reschedule the commitment.", { exact: true }).waitFor();
...
await desktop.page.getByRole("dialog", { name: "Command Center attention" }).getByRole("button", { name: "Close command center alerts" }).click();
...
await desktop.page.getByText("Showing the last successful snapshot", { exact: true }).waitFor({ state: "detached" });
...
await desktop.page.goto(`${base}/admin/today?focus=approval&action=1`, { waitUntil: "domcontentloaded" });
const approvalDialog = desktop.page.getByRole("dialog", { name: "Approve follow-up to Summit Mechanical" });
...
if (await desktop.page.getByText("Approve follow-up to Summit Mechanical", { exact: true }).count()) throw new Error("Approved action remained in the shared operator queue");
...
if (await desktop.page.getByText("Review campaign activation", { exact: true }).count()) throw new Error("Rejected action remained in the shared operator queue");
...
await desktop.page.getByRole("button", { name: "Snooze Prepare the weekly pipeline review until tomorrow" }).click();
await desktop.page.getByText("Prepare the weekly pipeline review", { exact: true }).waitFor({ state: "detached" });
...
await desktop.page.goto(`${base}/admin/ai-operations`, { waitUntil: "domcontentloaded", timeout: 60_000 });
...
await desktop.page.goto(`${base}/admin/analytics`, { waitUntil: "domcontentloaded", timeout: 60_000 });
...
await mobile.page.getByRole("region", { name: "Priority work" }).getByText("Priority work", { exact: true }).waitFor();
...
if (mobileAlertBounds.top < 0 || mobileAlertBounds.bottom > 846 || mobileAlertBounds.width > 392) throw new Error(`Mobile alerts are not contained: ${JSON.stringify(mobileAlertBounds)}`);
await mobile.page.waitForTimeout(250);
await mobile.page.screenshot({ path: `${outDir}/priority-attention-mobile-reduced.png`, fullPage: true });
await mobileAlerts.getByRole("button", { name: "Close command center alerts" }).click();
await mobileAlerts.waitFor({ state: "detached" });
await mobile.page.goto(`${base}/admin/analytics`, { waitUntil: "domcontentloaded", timeout: 60_000 });
...
if (expectedFailureResponses < 2) throw new Error("Today failure-state QA did not exercise both degraded and fatal reads");
...
console.log("Today command-center desktop/mobile, deep-action, mutation-counter, degraded, fatal, retry, and focus-filter QA passed.");
