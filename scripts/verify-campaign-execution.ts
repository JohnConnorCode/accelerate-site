...
import { activateCampaign, executeDueCampaignMembers, globalDailySendCap } from "../src/lib/revenue-os/campaigns";
...
async function purge(supabase: ReturnType<typeof createServiceRoleClient>) {
  const { data: campaigns } = await supabase.from("campaigns").select("id").like("name", `${PREFIX}%`);
  for (const campaign of campaigns ?? []) {
    const { data: members } = await supabase.from("campaign_members").select("id").eq("campaign_id", campaign.id);
    for (const member of members ?? []) {
      const { data: messages } = await supabase.from("messages").select("conversation_id").eq("metadata->>campaign_id", campaign.id);
  ...
  const { data: strays } = await supabase.from("messages").select("id,conversation_id").like("subject", `${PREFIX}%`);
...
async function main() {
  ...
  if (process.argv.includes("--cleanup")) {
    ...
    const { count } = await supabase.from("campaigns").select("*", { count: "exact", head: true }).like("name", `${PREFIX}%`);
    console.log(JSON.stringify({ mode: "cleanup", leftover: count ?? 0, result: (count ?? 0) === 0 ? "clean" : "incomplete" }, null, 2));
    if (count) process.exit(1);
    return;
  }

  const { data: existing } = await supabase.from("campaigns").select("id").like("name", `${PREFIX}%`);
  if (existing?.length) throw new Error(`${existing.length} verification campaign(s) already present. Run with --cleanup first.`);
  ...
  const { data: contact, error: contactError } = await supabase.from("contacts")
    .insert({ full_name: `Campaign Verify ${runId}`, primary_email: `${PREFIX}+${runId}@example.invalid`, communication_status: "active", source: "verification" })
    .select("id").single();
  ...
  const { error: stepError } = await supabase.from("campaign_steps").insert({
    campaign_id: campaign.id, step_order: 0, delay_days: 0,
    subject_template: `${PREFIX} step one for {{first_name}}`,
    body_template: "Hello {{first_name}}, this is an automated verification message. No action is needed.",
    active: true,
  });
  ...
  const { error: memberError } = await supabase.from("campaign_members").insert({
    campaign_id: campaign.id, contact_id: contact.id, email: SINK, status: "queued",
  });
  if (memberError) throw new Error(`member: ${memberError.message}`);

  await activateCampaign(supabase, campaign.id, "verification@local");

  const { data: activated } = await supabase.from("campaign_members").select("next_send_at,contact_id").eq("campaign_id", campaign.id).single();
  check("activation makes an existing member due", Boolean(activated?.next_send_at), activated?.next_send_at);
  check("the member carries a canonical contact", Boolean(activated?.contact_id), activated?.contact_id);
  ...
  const { data: message } = await supabase.from("messages").select("status,provider_id,recipient_emails")
    .eq("metadata->>campaign_id", campaign.id).maybeSingle();
  ...
  check("the receipt records the recipient", (message?.recipient_emails ?? []).includes(SINK), message?.recipient_emails);

  const { data: advanced } = await supabase.from("campaign_members").select("status,current_step,send_attempts").eq("campaign_id", campaign.id).single();
  check("the member advanced past the only step", advanced?.status === "completed", advanced);
  check("the retry budget is clean after a delivered step", (advanced?.send_attempts ?? 0) === 0, advanced?.send_attempts);
  ...
  const { count: messageCount } = await supabase.from("messages").select("*", { count: "exact", head: true }).eq("metadata->>campaign_id", campaign.id);
  check("only one message exists after replay", messageCount === 1, messageCount);

  await purge(supabase);
  const { count: leftover } = await supabase.from("campaigns").select("*", { count: "exact", head: true }).like("name", `${PREFIX}%`);
  check("cleanup removed the verification campaign", (leftover ?? 0) === 0, leftover);

  console.log(JSON.stringify({
    runId, recipient: SINK, globalDailyCap: globalDailySendCap(),
    firstRun: first, replayRun: second,
...
main().catch((error) => {
  console.error("Campaign execution verification errored:", error instanceof Error ? error.message : error);
