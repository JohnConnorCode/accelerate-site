...
const { data: linkData, error: linkError } = await supabase.auth.admin.generateLink({
  ...
  options: { redirectTo: `${base}/auth/callback?next=/admin/analytics` },
});
if (linkError || !linkData?.properties?.hashed_token)
  throw linkError || new Error("Could not create sign-in token");
const { data: verified, error: verifyError } = await supabase.auth.verifyOtp({
  token_hash: linkData.properties.hashed_token,
  type: "magiclink",
});
if (verifyError || !verified.session)
  throw verifyError || new Error("Could not exchange sign-in token");
...
const responseFor = (url) => {
  ...
  return {
    ...
    funnel: {
      opportunities: filtered ? 4 : 24,
      qualified: filtered ? 3 : 15,
      meetings: filtered ? 2 : 9,
      proposals: filtered ? 1 : 5,
      won: filtered ? 1 : 3,
      wonRevenue: filtered ? 12000 : 42000,
      pipelineValue: filtered ? 36000 : 185000,
    },
    rates: {
      qualified: 62.5,
      meeting: 60,
      proposal: 55.6,
      win: 60,
      inquiryToWin: filtered ? 25 : 12.5,
    },
    attribution: { missing: filtered ? 0 : 2 },
    forecast: {
      weightedPipeline: filtered ? 17400 : 78250,
      unweightedPipeline: filtered ? 36000 : 185000,
      method:
        "Open estimated value multiplied by each opportunity's recorded probability. This is a planning estimate, not booked revenue.",
    },
    communication: {
      status: "ready",
      inboundConversations: filtered ? 3 : 12,
      repliedConversations: filtered ? 3 : 9,
      replyRate: filtered ? 100 : 75,
      medianResponseHours: filtered ? 1.5 : 3.2,
    },
    quality: {
      missingAttribution: filtered ? 0 : 2,
      missingOwner: filtered ? 0 : 3,
      missingNextAction: filtered ? 1 : 5,
      unrecognizedStage: 0,
      impossibleStageSequences: 0,
    },
    filterOptions: {
      sources: ["Referral", "Website"],
      owners: ["founder@acceleratewith.us", "Unassigned"],
      campaigns: ["August outreach", "Unassigned"],
      stages: ["qualified", "proposal", "won"],
    },
    appliedFilters: {
    ...
    sources: filtered
      ? [{ source: "Referral", opportunities: 4, won: 1, revenue: 12000 }]
      : [
          { source: "Referral", opportunities: 4, won: 1, revenue: 12000 },
          { source: "Website", opportunities: 20, won: 2, revenue: 30000 },
        ],
    web: {
      status: "ready",
      pageViews: 320,
      ...
      engagementEvents: 75,
      conversionRate: 7.5,
      topPages: [{ label: "/", count: 110 }],
      sources: [{ label: "google", count: 120 }],
      conversionEvents: [{ label: "contact form submitted", count: 12 }],
      eventCount: 344,
      lastCapturedAt: "2026-08-24T12:00:00Z",
    },
  };
};
...
async function makePage(viewport, reducedMotion = "no-preference") {
  ...
  await page.route("**/api/admin/revenue-os/analytics**", (route) =>
    route.fulfill({
      status: 200,
  ...
  await page.route("**/api/admin/revenue-os/priority", (route) =>
    route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({ summary: { urgent: 0 }, items: [] }),
    }),
  );
  await page.route("**/api/admin/notifications", (route) =>
    route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({ notifications: [], unreadCount: 0, urgentCount: 0 }),
    }),
  );
...
await desktop.page.screenshot({
  path: `${outDir}/analytics-decision-layer-desktop.png`,
  fullPage: true,
});
await desktop.page.reload({ waitUntil: "domcontentloaded" });
if ((await desktop.page.getByLabel("Source").inputValue()) !== "Referral")
  throw new Error("Analytics source filter did not persist");
if ((await desktop.page.getByLabel("Owner").inputValue()) !== "founder@acceleratewith.us")
  throw new Error("Analytics owner filter did not persist");
...
if (overflow.scroll > overflow.client)
  throw new Error(`Analytics escaped mobile viewport: ${JSON.stringify(overflow)}`);
await mobile.page.screenshot({
  path: `${outDir}/analytics-decision-layer-mobile.png`,
  fullPage: true,
});
...
console.log(
  "Analytics filters, persistence, facts-vs-forecast, reply evidence, data quality, reduced motion, and mobile containment passed.",
);
