...
const { data: allContacts } = await sb
  .from("contacts")
  .select("id,primary_email,tenant_id,created_at")
  .ilike("primary_email", "qa-inbound-%")
  .order("created_at", { ascending: false })
  .limit(5);
console.log("ALL contacts:", JSON.stringify(allContacts, null, 2));
const { data: allOpps } = await sb
  .from("opportunities")
  .select("id,email,stage,tenant_id,source,source_detail,created_at")
  .ilike("email", "qa-inbound-%")
  .order("created_at", { ascending: false })
  .limit(5);
