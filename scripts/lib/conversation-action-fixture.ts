...
export function conversationActionFixture(rows: (table: string) => Row[], args: Row) {
  ...
  if (args.p_undo) {
    ...
    compensation.receipt = { undone: action.action_type, detail: { conversation: structuredClone(current) } };
    rows("audit_log").push({ action: "action.compensated", entity_id: action.id });
    return compensation.receipt;
  }
  if (!isDeepStrictEqual(current, payload.expectedState)) throw new Error("Conversation changed since preview");
  let audit: string; let activity: string;
  if (args.p_operation === "update_conversation_status") {
    ...
    audit = `conversation.status_${payload.status}`; activity = "conversation_status_updated";
  } else if (args.p_operation === "assign_conversation") {
    current.metadata = { ...(current.metadata as Row), assigned_to: payload.assigneeEmail };
    audit = payload.assigneeEmail ? "conversation.assigned" : "conversation.unassigned"; activity = "conversation_assigned";
  } else {
    ...
    if (action.action_type === "identity_review") {
      ...
      if (decision.decision === "link" && patch.contact_id !== decision.contactId) throw new Error("Link outside approved decision");
    }
    Object.assign(current, patch);
    for (const [field, value] of Object.entries(patch)) {
      if (!value) continue;
      const claimId = `claim-${rows("claims").length + 1}`;
      rows("claims").push({ id: claimId, entity_type: "conversation", entity_id: current.id, field, proposed_value: value, best_evidence: "human_entered", status: "verified" });
      rows("evidence").push({ claim_id: claimId, source_type: "operator_link", strength: "human_entered" });
    }
    audit = "conversation.linked_record"; activity = "conversation_linked";
  }
  ...
  if (action.action_type === "identity_review") action.result = { conversationEffect: { payload, result } };
  else Object.assign(action, { status: "executed", result,
