...
const PHASE_LABEL = {
  A: "A (Complete Loop One — See + Remember)",
  B: "B (Agent Runtime foundation — Notice + Act primitives)",
  C: "C (Reference coworker — Sales end-to-end loop)",
  D: "D (Plugin SDK + MCP)",
  E: "E (Additional coworkers/plugins + documentation)",
};
...
export function generateBuildPlanContent() {
  ...
  // Prettier aligns markdown table pipes, while `prettier --check .` must stay
  // green on this generated file at the same time `verify:build-plan --check`
  // demands byte-identity with this generator's output. So the generator pads
  // the table itself, exactly the way Prettier would: cells left-aligned to
  // the widest cell in their column, separator dashes matching that width.
  const phaseHeader = ["Phase", "Backlog", "Planned", "In progress", "Blocked", "Shipped", "Total"];
  ...
  for (const phase of ["A", "B", "C", "D", "E"]) {
    ...
    phaseRows.push([
      ...
      String(counts.in_progress),
      String(counts.blocked),
      String(counts.shipped),
      String(cards.length),
    ]);
  }
  const colWidths = phaseHeader.map((_, i) => Math.max(...phaseRows.map((row) => row[i].length)));
  const formatTableRow = (row) =>
    `| ${row.map((cell, i) => cell.padEnd(colWidths[i])).join(" | ")} |`;
  let phaseTable = `${formatTableRow(phaseHeader)}\n`;
  phaseTable += `| ${colWidths.map((width) => "-".repeat(width)).join(" | ")} |\n`;
  for (const row of phaseRows.slice(1)) phaseTable += `${formatTableRow(row)}\n`;
  ...
  const dispatchable = active
    .filter(
      (c) =>
        (c.status === "backlog" || c.status === "planned") &&
        (milestoneOf(c) === "now" || milestoneOf(c) === "next"),
    )
  ...
  let inProgressList = inProgress.length
    ? inProgress
        .map((c) => `- \`${c.seed_key}\` — ${c.title} (${c.owner ?? "no owner recorded"})`)
        .join("\n")
    : "_None._";
