...
`;
  return format(content, { ...(await resolveConfig(outPath)), filepath: outPath });
}

export async function buildPlanIsInSync() {
  const existing = (() => {
    try {
  ...
  return existing === (await generateBuildPlanContent());
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  if (checkOnly) {
    ...
    writeFileSync(outPath, await generateBuildPlanContent());
