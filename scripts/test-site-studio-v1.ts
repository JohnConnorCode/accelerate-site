...
import { FileSiteDraftRepository, draftChecksum } from "../src/lib/site-studio/store";
import {
...
import { StaleDraftError, reviseSiteDraft } from "../src/lib/site-studio/revision";
import {
...
async function main() {
  ...
  assert.throws(
    () =>
      parseSiteDocument({
        ...template,
        root: [{ id: "bad", type: "marquee-3d", props: {}, styles: { background: "surface" } }],
      }),
    /type/,
  );
  assert.throws(
        ...
        root: [
          {
            ...
            type: "section",
            children: [
              { id: "b1", type: "button", props: { label: "Go", href: "javascript:alert(1)" } },
            ],
          },
        ],
      }),
  ...
  assert.ok(
    links.some((link) => link.props.href === "/contact"),
    "CTA href renders",
  );
  const sections = findAll(page, (element) => element.props["data-site-section"] === "hero");
  ...
  await assert.rejects(
    generatePageDocument({ serviceName: "X", audience: "Y", outcome: "Z" }, async () => ({
      ...validBody,
      root: [
        {
          id: "hero",
  ...
  const created = await createSiteDraft(serviceStore, { brief, mode: "template" }, noGenerator);
  ...
  assert.equal(
    (
      revised.document.root[0]?.children.find((node) => node.id === heroId) as {
  ...
  await assert.rejects(
    reviseSiteDraft(serviceStore, revised.id, {
      patches: [
        { op: "setProp", nodeId: heroId, path: "heading", value: "See https://invented.example/x" },
      ],
    }),
    /invents links/,
  ...
  const pruned = await reviseSiteDraft(serviceStore, renamed.id, {
    patches: [
      { op: "remove", nodeId: "extra-note" },
      { op: "move", nodeId: "questions", toIndex: 0 },
    ],
  });
  assert.ok(
  ...
  assert.equal(
    (
      regenerated.document.root.find((section) => section.id === "hero")?.children[0] as {
        props: { heading: string };
      }
    )?.props.heading,
    "Evenings, returned.",
