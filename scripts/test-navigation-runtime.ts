...
assert.doesNotMatch(mobileNav, /framer-motion|AnimatePresence|<motion\./, "The interactive mobile drawer must use interruptible CSS state transitions");
assert.match(mobileNav, /inert=\{!isOpen\}/, "The mounted closed drawer must remain outside the interaction tree");
assert.match(mobileNav, /requestAnimationFrame\(\(\) => closeButtonRef\.current\?\.focus\(\)\)/, "The open drawer must receive focus on the next paint instead of waiting on animation timing");
assert.match(publicHeader, /dataset\.mobileNavigation = "open"/, "The public shell must suspend hidden background rendering while mobile navigation owns the viewport");
...
assert.doesNotMatch(publicRouteKeyframes, /filter:/, "Public route entry must not rasterize a full-page blur layer");
assert.doesNotMatch(styles, /\.route-entry(?:\.is-leaving)?\s*\{[^}]*filter:/, "Public route state must remain opacity-only");
assert.match(styles, /\.route-entry h1\[tabindex="-1"\]:focus[\s\S]*outline: none/, "Programmatic route focus must not draw a broken native outline around the destination heading");
assert.match(styles, /@media \(max-width: 640px\)[\s\S]*\.atmo-a,[\s\S]*animation: none !important/, "Mobile must not keep decorative ambient drift running offscreen");
...
assert.match(hero, /if \(supportsFinePointer && !document\.hidden\) startSpotlight\(\)/, "Mobile must not run the hero spotlight loop continuously without pointer input");
