...
test("explicit expired pickup preserves dirty retained work while automatic pickup skips it", async () => {
  ...
  writeFileSync(join(path, "README.md"), "Tracked retained implementation\n");
  f.card.status = "in_progress";
  f.card.revision = 5;
  ...
  try {
    ...
    const successor = JSON.parse(explicit.stdout).worktree;
    assert.notEqual(successor, path, "expired continuation preserves the predecessor checkout");
    assert.equal(
      readFileSync(join(successor, "README.md"), "utf8"),
      "Tracked retained implementation\n",
    );
