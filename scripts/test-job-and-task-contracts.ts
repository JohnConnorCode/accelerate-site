...
function jobHarness(options: { claimed?: boolean; recoveredStale?: boolean; existingStatus?: string } = {}) {
  const db = new MemorySupabase({ job_runs: [], admin_notifications: [], sent_emails: [], messages: [] });
...
async function rejects(run: () => Promise<unknown>, includes: string, because: string) {
  ...
  assert.ok(message.toLowerCase().includes(includes.toLowerCase()), `${because}\n  expected the message to mention "${includes}"\n  got: ${message}`);
}
...
async function main() {
  ...
  const success = await withJobRun(good.client, "revenue-campaigns", async () => ({ value: { sent: 3 }, summary: { sent: 3, failed: 0 } }));
  assert.deepEqual(success.value, { sent: 3 });
  assert.equal(success.claimed, true);
  const closed = good.rows("job_runs")[0]!;
  assert.equal(closed.status, "success", "a job that returned must not be left `running`; that is what blocks the next claim");
  assert.deepEqual(closed.summary, { sent: 3, failed: 0 }, "the summary is what the health surface reads");
  ...
  await rejects(
    () => withJobRun(bad.client, "revenue-campaigns", async () => { throw new Error("provider refused the batch"); }),
    "provider refused",
  ...
  assert.equal(failed.status, "failed", "a crashed job must write its failed receipt, or the row stays `running` and the partial unique index blocks every future claim");
  assert.match(String(failed.error), /provider refused/, "the failure reason must reach the ledger");
  ...
  let ran = false;
  const skipped = await withJobRun(busy.client, "revenue-campaigns", async () => { ran = true; return { value: null, summary: {} }; });
  assert.equal(ran, false, "losing the claim must skip the work entirely; running it anyway is how a job double-sends");
  assert.equal(skipped.claimed, false);
  assert.equal(skipped.existingStatus, "running", "the caller must be able to say what already owns the job");
  ...
  const late = new MemorySupabase({ job_runs: [{ id: "run-9", status: "failed", error: "died mid-batch", summary: null, finished_at: "2026-08-19T00:00:00.000Z" }] });
  await finishJobRun(late.client, "run-9", { sent: 99 });
  assert.equal(late.rows("job_runs")[0]!.status, "failed", "finishJobRun must not resurrect a run that already failed");
  assert.equal(late.rows("job_runs")[0]!.error, "died mid-batch", "the real failure reason must survive");

  const done = new MemorySupabase({ job_runs: [{ id: "run-9", status: "success", error: null, summary: { sent: 2 }, finished_at: "2026-08-19T00:00:00.000Z" }] });
  await failJobRun(done.client, "run-9", new Error("late failure"));
  assert.equal(done.rows("job_runs")[0]!.status, "success", "failJobRun must not overwrite a run that already succeeded");

  // ---- A claim RPC error surfaces rather than silently not claiming ----

  const brokenRpc = new MemorySupabase({ job_runs: [] });
  brokenRpc.rpc("claim_revenue_job_run", () => ({ data: null, error: { message: "function does not exist" } }));
  await rejects(
  ...
  const first = await createRevenueTask(tasks.client, { title: "Call Northside back", source: "inbound", dedupeKey: "inbound:opp-1", actorEmail: "system" });
  assert.equal(first.deduplicated, false);
  assert.equal(tasks.rows("tasks").length, 1);
  assert.equal(tasks.rows("activities").length, 1, "a created task must leave an activity receipt; it is how Today explains itself");

  const again = await createRevenueTask(tasks.client, { title: "Call Northside back", source: "inbound", dedupeKey: "inbound:opp-1", actorEmail: "system" });
  ...
  assert.equal((again.task as { id: string }).id, (first.task as { id: string }).id, "the existing task must be returned so the caller can link to it");
  ...
  const afterCompletion = await createRevenueTask(tasks.client, { title: "Call Northside back", source: "inbound", dedupeKey: "inbound:opp-1", actorEmail: "system" });
  assert.equal(afterCompletion.deduplicated, false, "dedupe is scoped to open tasks; a completed one must not suppress the next real follow-up forever");
  ...
  await createRevenueTask(undeduped.client, { title: "Ad hoc", source: "manual", actorEmail: "john@acceleratewith.us" });
  await createRevenueTask(undeduped.client, { title: "Ad hoc", source: "manual", actorEmail: "john@acceleratewith.us" });
  assert.equal(undeduped.rows("tasks").length, 2, "without a dedupe key every task must be created");

  await rejects(
    () => createRevenueTask(undeduped.client, { title: "   ", source: "manual", actorEmail: "john@acceleratewith.us" }),
    "title is required",
  ...
  assert.deepEqual(normalizeCampaignPolicy(undefined), { daily_limit: 10, stop_on_reply: true, stop_on_booking: true, stop_on_bounce: true, stop_on_unsubscribe: true }, "the default policy must stop on every signal; opting out has to be deliberate");
  assert.equal(normalizeCampaignPolicy({ daily_limit: 5000 }).daily_limit, 10, "a campaign must not be able to raise its own ceiling past the automation gate");
  assert.equal(normalizeCampaignPolicy({ daily_limit: 0 }).daily_limit, 10, "0 is falsy and falls through to the default, which is the right call: a campaign silently capped at zero would look like a quiet success while sending nothing");
  assert.equal(normalizeCampaignPolicy({ daily_limit: -5 }).daily_limit, 1, "a negative limit must floor at 1 rather than producing a nonsense budget");
  assert.equal(normalizeCampaignPolicy({ daily_limit: "not a number" }).daily_limit, 10, "a malformed limit must fall back to the default rather than becoming NaN");
  assert.equal(normalizeCampaignPolicy({ stop_on_reply: false }).stop_on_reply, false, "an explicit opt-out must be honoured");
  assert.equal(normalizeCampaignPolicy({ stop_on_reply: "no" }).stop_on_reply, true, "only an explicit false disables a stop rule; anything else stays safe");

  assert.equal(globalDailySendCap(), 10, "without the automation flag the account-wide ceiling is the conservative one");
  process.env.CAMPAIGN_AUTOMATION_ENABLED = "true";
  assert.equal(globalDailySendCap(), 200);
  assert.equal(normalizeCampaignPolicy({ daily_limit: 5000 }).daily_limit, 200, "the automation flag raises the ceiling but does not remove it");
  ...
  assert.equal(globalDailySendCap(), 2000, "an override must still be bounded; a typo must not uncap the sending domain");
  delete process.env.CAMPAIGN_AUTOMATION_ENABLED;
  delete process.env.CAMPAIGN_GLOBAL_DAILY_LIMIT;

  // ---- Template rendering leaves no placeholders ----------------------

  assert.equal(renderCampaignTemplate("Hi {{first_name}}, about {{topic}}.", { first_name: "Dana", topic: "intake" }), "Hi Dana, about intake.");
  assert.equal(renderCampaignTemplate("Hi {{first_name}},", { first_name: null }), "Hi ,", "a missing value must render empty rather than leaking `{{first_name}}` into a prospect's inbox");
  assert.equal(renderCampaignTemplate("Hi {{ first_name }},", { first_name: "Dana" }), "Hi Dana,", "whitespace inside the braces must still match");
