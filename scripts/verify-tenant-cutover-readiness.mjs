...
function command(args, { allowFailure = false } = {}) {
  ...
  return { status: result.status ?? 1, stdout: result.stdout.trimEnd(), stderr: result.stderr.trim() };
}
...
export function readRepositoryState() {
  ...
  const upstreamResult = command(["rev-parse", "--abbrev-ref", "--symbolic-full-name", "@{upstream}"], { allowFailure: true });
  ...
  const dirtyPaths = command(["status", "--porcelain=v1", "--untracked-files=all"]).stdout
    .split("\n")
    .filter(Boolean)
  ...
  const unmergedBranches = lines(command(["branch", "--no-merged", "HEAD", "--format=%(refname:short)"]).stdout).filter((entry) => entry !== branch);
  const requiredPaths = [
  ...
  return { branch, head, upstream, upstreamHead, dirtyPaths, worktrees, unmergedBranches, requiredTrackedFiles };
}

export function expectedCompatibilityIndexes(sql) {
  return [...sql.matchAll(/CREATE UNIQUE INDEX IF NOT EXISTS (compat_[a-z0-9_]+)/g)].map((match) => match[1]).sort();
}
...
export function expectedLegacyArtifacts(cutoverSql, compatibilitySql) {
  ...
  const droppedIndexes = [...cutoverSql.matchAll(/DROP INDEX IF EXISTS public\.([a-z0-9_]+)/g)].map((match) => match[1]);
  const recreatedCompositePrimaryKeys = new Set(["email_templates.email_templates_pkey", "admin_settings.admin_settings_pkey"]);
  const droppedConstraints = [...cutoverSql.matchAll(/ALTER TABLE public\.([a-z0-9_]+) DROP CONSTRAINT IF EXISTS ([a-z0-9_]+)/g)]
...
export function evaluateDatabase(stage, state, expectedArtifacts, tenantSlug) {
  const checks = [];
  checks.push(check("database.suspension_guard", state.suspensionGuardApplied === true, state.suspensionGuardApplied ? "Operational RPC authorization rechecks active tenant status." : "The tenant suspension guard migration is not applied."));
  const forbidden = [...state.legacyIndexes, ...state.legacyConstraints].sort();
  checks.push(check("database.uniqueness_cutover", forbidden.length === 0, forbidden.length === 0 ? `All ${expectedArtifacts.indexes.length} legacy/compatibility indexes and ${expectedArtifacts.constraints.length} global constraints are absent.` : `${forbidden.length} obsolete global uniqueness artifact(s) remain: ${forbidden.slice(0, 8).join(", ")}${forbidden.length > 8 ? ", …" : ""}`));
  checks.push(check("database.composite_primary_keys", state.emailTemplatesCompositePrimaryKey === true && state.adminSettingsCompositePrimaryKey === true, state.emailTemplatesCompositePrimaryKey && state.adminSettingsCompositePrimaryKey ? "Email templates and admin settings use tenant-composite primary keys." : "Tenant-composite primary-key cutover is incomplete."));
  checks.push(check("database.no_active_client", state.activeNonBootstrapTenants === 0, state.activeNonBootstrapTenants === 0 ? "No non-bootstrap tenant is active before controlled activation." : `${state.activeNonBootstrapTenants} non-bootstrap tenant(s) are already active.`));
  checks.push(check("database.no_client_provider_effects", state.connectedNonBootstrapProviders === 0, state.connectedNonBootstrapProviders === 0 ? "No non-bootstrap provider connection is enabled." : `${state.connectedNonBootstrapProviders} non-bootstrap provider connection(s) are enabled.`));
  if (stage === "pre-activation") {
    checks.push(check("activation.target_declared", Boolean(tenantSlug), tenantSlug ? `Activation target is ${tenantSlug}.` : "Pre-activation verification requires --tenant=<slug>."));
    checks.push(check("activation.target_exists", state.targetTenant?.exists === true, state.targetTenant?.exists ? `Target tenant exists with status ${state.targetTenant.status}.` : `Target tenant ${tenantSlug || "(missing)"} does not exist.`));
    const safeStatus = state.targetTenant?.status === "provisioning" || state.targetTenant?.status === "suspended";
    checks.push(check("activation.target_inert", safeStatus, safeStatus ? "Target tenant is inert until the founder performs the activation transition." : `Target tenant status ${state.targetTenant?.status || "missing"} is not a safe pre-activation state.`));
    checks.push(check("activation.target_providers_disabled", (state.targetTenant?.connectedProviders ?? 0) === 0, (state.targetTenant?.connectedProviders ?? 0) === 0 ? "Target tenant has no connected provider effects." : `Target tenant has ${state.targetTenant.connectedProviders} connected provider effect(s).`));
  }
  return checks;
}
...
export function readDatabaseState(tenantSlug, expectedArtifacts) {
  ...
  const constraintList = expectedArtifacts.constraints.map((entry) => sqlLiteral(entry.split(".")[1])).join(", ") || "NULL";
  const sql = `
    WITH target AS (
  ...
  if (result.status !== 0) throw new Error((result.stderr || result.stdout || "database readiness query failed").trim());
  return JSON.parse(result.stdout.trim());
}

export function evaluateReceipt(receipt, head, tenantSlug) {
  if (!receipt) return [check("release.receipt", false, "A release receipt is required after deployment.")];
  const checks = [
    check("release.receipt_version", receipt.version === CUTOVER_RECEIPT_VERSION, receipt.version === CUTOVER_RECEIPT_VERSION ? `Receipt uses ${CUTOVER_RECEIPT_VERSION}.` : "Release receipt version is missing or unsupported."),
...
function parseArguments(argv) {
  const value = (name) => argv.find((entry) => entry.startsWith(`--${name}=`))?.slice(name.length + 3) || null;
  const stage = value("stage") || "repository";
  if (!CUTOVER_STAGES.includes(stage)) throw new Error(`--stage must be one of: ${CUTOVER_STAGES.join(", ")}`);
  const excludedWorktrees = argv.filter((entry) => entry.startsWith("--exclude-worktree=")).map((entry) => entry.slice("--exclude-worktree=".length)).filter(Boolean);
...
export function resultFor(stage, checks) {
  ...
  return { contract: CUTOVER_RECEIPT_VERSION, stage, status: blockers.length === 0 ? "ready" : "blocked", passed: checks.length - blockers.length, blocked: blockers.length, checks };
}

async function main() {
  const { stage, tenantSlug, receiptPath, excludedWorktrees } = parseArguments(process.argv.slice(2));
  const repositoryState = readRepositoryState();
  const checks = evaluateRepository(repositoryState, excludedWorktrees);
  if (stage !== "repository") {
    const compatibilitySql = readFileSync(resolve(repoRoot, "migrations/20260830-tenant-uniqueness-compatibility.sql"), "utf8");
    const cutoverSql = readFileSync(resolve(repoRoot, "migrations/20260830-tenant-uniqueness-cutover.sql"), "utf8");
...
if (import.meta.url === pathToFileURL(process.argv[1] || "").href) {
  main().catch((error) => {
    console.error(JSON.stringify({ contract: CUTOVER_RECEIPT_VERSION, status: "error", error: error instanceof Error ? error.message : String(error) }, null, 2));
    process.exitCode = 1;
  });
}
