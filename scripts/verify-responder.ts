...
async function cleanup() {
  ...
  await supabase
    .from("agent_runs")
    .delete()
    .eq("surface", "inbound_responder")
    .like("prompt_preview", `${PREFIX}%`);
  return ids.length;
}
...
async function main() {
  if (process.argv.includes("--cleanup")) {
    console.log(
      JSON.stringify({ mode: "cleanup", opportunitiesRemoved: await cleanup() }, null, 2),
    );
    return;
  }
  ...
  if ((leftovers.data?.length ?? 0) > 0) {
    throw new Error(
      `a previous run left rows behind; run \`npm run verify:responder -- --cleanup\` first`,
    );
  }
  ...
  check(
    `${RESPONDER_APPROVED_VERSION_KEY} exists in admin_settings`,
    approvedSeeded !== null,
    approvedSeeded,
  );
  notes.push(
    `live settings on entry: enabled=${JSON.stringify(enabledSeeded)}, approved=${JSON.stringify(approvedSeeded)}`,
  );
  ...
  const { data: company, error: companyError } = await supabase
    .from("companies")
    .insert({ name: `${PREFIX} Dental ${runId}` })
    .select("id,name")
    .single();
  ...
  try {
    ...
    check(
      "the decline reason is the kill switch",
      !offDecision.sent && offDecision.reason === "policy_disabled",
      offDecision,
    );
    ...
    check(
      "an unapproved version declines",
      !unapproved.sent && unapproved.reason === "policy_not_approved",
      unapproved,
    );
    ...
    const { data: declines } = await supabase
      ...
      .eq("action", "responder.declined");
    check(
      "declines are recorded in the audit ledger",
      (declines?.length ?? 0) >= 2,
      declines?.length,
    );
    ...
    if (!decision.sent) {
      ...
      if (decision.reason === "failed_grounding_check") {
        notes.push(
          `the live model produced a draft the grounding check rejected (${decision.detail}); that is the guard working, but it means no send was proven this run`,
        );
      } else if (decision.reason === "outside_send_window") {
        notes.push(
          "outside the approved send window, so no live send was attempted this run; re-run during business hours for send evidence",
        );
        ...
        notes.push(
          "OPENROUTER_API_KEY or RESEND_API_KEY is absent locally, so the generation and send legs were not exercised; the policy gating above ran against the real settings table",
        );
      } else {
      ...
      const { data: message } = await supabase
        .from("messages")
        .select("id,provider_id,status,body_text,idempotency_key")
        .eq("id", decision.messageId!)
        .maybeSingle();
      check("the send wrote a message receipt", Boolean(message), decision.messageId);
      check(
        "the receipt carries a real provider id",
        Boolean(message?.provider_id),
        message?.provider_id,
      );
      check("the receipt is marked sent", message?.status === "sent", message?.status);
      check(
        "the idempotency key pins one reply per opportunity",
        message?.idempotency_key === `responder:${RESPONDER_POLICY_VERSION}:${opportunity.id}`,
        message?.idempotency_key,
      );
      check(
        "the body is non-empty",
        Boolean(message?.body_text?.trim()),
        (message?.body_text ?? "").slice(0, 60),
      );
      check(
        "the body leaks no placeholder",
        !/\{\{[A-Za-z0-9_]+\}\}/.test(String(message?.body_text ?? "")),
      );

      const { data: run } = await supabase
        .from("agent_runs")
        .select("status,surface,model,result_preview")
        .eq("surface", "inbound_responder")
      ...
      const { data: sentAudit } = await supabase
        .from("audit_log")
        .select("metadata")
        .eq("entity_id", opportunity.id)
        .eq("action", "responder.sent")
        .maybeSingle();
      check(
        "the send is audited with its policy version",
        (sentAudit?.metadata as { policy_version?: string })?.policy_version ===
          RESPONDER_POLICY_VERSION,
        sentAudit?.metadata,
      );
      ...
      check(
        "exactly one message exists for this opportunity",
        (allMessages?.length ?? 0) === 1,
        allMessages?.length,
      );
    }
  } finally {
    ...
    notes.push(
      `live settings restored to enabled=${JSON.stringify(enabledSeeded)}, approved=${JSON.stringify(approvedSeeded)}`,
    );
    await cleanup();
  }
