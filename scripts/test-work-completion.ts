...
class ScopedMemory extends AuthorizedMemorySupabase {
  override get client(): never {
    return bindTenantDatabaseForTest(super.client, "tenant-a") as never;
  }
}
...
function seed(wi = item()) {
  const db = new ScopedMemory({
    work_items: [{ ...wi }],
    tenants: [{ id: "tenant-a", name: "Example workspace", status: "active", config: {} }],
    coworkers: [
      {
        id: "sales",
        name: "Sales",
  ...
  try {
    ...
    for (const gate of [
      ...
      await check(`${gate} never completes work`, async () => {
        ...
        if (gate === "policy-error")
          db.rpc("check_autonomy", () => {
            throw new Error("offline");
          });
        if (gate === "budget")
          db.tables.budget_limits = [
            { coworker_id: "sales", budget_kind: "model_spend", period: "daily", limit_value: 0 },
          ];
        ...
        if (gate === "budget")
          assert.ok(Date.parse(String(db.rows("work_items")[0]!.next_check_at)) > Date.now());
      });
    }
    registerSalesWorkHandlers();
    ...
    await check("foreign-tenant work is rejected before model or run creation", async () => {
      const db = seed();
      let called = false;
      await assert.rejects(
        () =>
          runCoworkerAgentTask(db.client, item({ tenant_id: "tenant-b" }), {
            chat: async () => {
              called = true;
      ...
      assert.equal(called, false);
      assert.equal(db.rows("agent_runs").length, 0);
    });
    ...
    await check(
      ...
      async () => {
        ...
        console.log("DEBUG outcome:", JSON.stringify(result.outcome).slice(0, 400), "status:", result.status);
        assert.ok(
