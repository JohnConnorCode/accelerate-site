...
const serviceFiles = sourceFiles("src").filter((file) =>
  readFileSync(file, "utf8").includes("createServiceRoleClient("),
);
const adminServiceFiles = serviceFiles.filter((file) => file.startsWith("src/app/api/admin/"));
assert.deepEqual(
  adminServiceFiles,
  [],
  "interactive admin routes must use the database returned by requireAdmin",
);

const nonAdminServiceFiles = serviceFiles.filter(
  (file) => !file.startsWith("src/app/api/admin/") && file !== "src/lib/supabase/server.ts",
);
for (const file of nonAdminServiceFiles) {
  assert.doesNotMatch(
    readFileSync(file, "utf8"),
    /createServiceRoleClient\(\s*\)/,
    `${file} uses service-role access without TenantSystemContext`,
  );
}
assert.ok(
  readFileSync("src/lib/tenancy/ingest.ts", "utf8").includes("createServiceRoleClient(context)"),
  "tenant ingest must pass its resolved TenantSystemContext explicitly",
);
