...
async function assertNoOverflow(page, label) {
  ...
  if (width.doc > width.view + 2)
    failures.push(`${label}: overflow ${width.doc}px > ${width.view}px`);
}
...
async function openActivity(viewport, label, reducedMotion = "reduce") {
  ...
  await page.goto(`${base}/demo/command-center/${scenario}/activity`, {
  ...
  await page
    .getByRole("heading", { name: "Narrow the ledger without leaving this page" })
    .waitFor();
  return { context, page };
}

{
  ...
  await desktop.page.waitForFunction(
    () => new URL(location.href).searchParams.get("entity") === "proposal",
  );
  await desktop.page
    .getByRole("heading", { name: "proposal · viewed", exact: false })
    .first()
    .waitFor();
  const source = desktop.page.getByLabel("Source");
  await source.selectOption("public");
  await desktop.page.waitForFunction(
    () => new URL(location.href).searchParams.get("source") === "public",
  );
  const rows = desktop.page.locator("article");
  const count = await rows.count();
  if (count < 1) failures.push("desktop: filtered proposal/public history is empty");
  const actions = await rows.evaluateAll((nodes) =>
    nodes.map((node) => node.querySelector("h2")?.textContent || ""),
  );
  if (actions.some((action) => !action.includes("proposal") || !action.includes("viewed")))
    failures.push(`desktop: filter leaked non-proposal rows (${actions.join(" | ")})`);
  await desktop.page.getByRole("button", { name: /Clear \d+ filters/ }).click();
  await desktop.page.waitForFunction(
    () =>
      !new URL(location.href).searchParams.get("entity") &&
...

{
  ...
  await mobile.page.waitForFunction(
    () => new URL(location.href).searchParams.get("action") === "calendar.synced",
  );
  await mobile.page
    .getByRole("heading", { name: "calendar · synced", exact: false })
    .first()
    .waitFor();
  await assertNoOverflow(mobile.page, "mobile");
  const target = await mobile.page
    .getByLabel("Entity")
...

{
  ...
  const animated = await motion.page.evaluate(
    () =>
      [...document.querySelectorAll(".admin-demo-enter, .demo-scenario-mark")].filter(
  ...
  await motion.page.waitForFunction(
    () => new URL(location.href).searchParams.get("source") === "automation",
  );
  await motion.page
    .getByRole("heading", { name: "calendar · synced", exact: false })
    .first()
...
console.log(
  JSON.stringify(
    {
      result: "passed",
      screenshots: [`${output}/activity-desktop.png`, `${output}/activity-mobile.png`],
      checks: [
        "demo-activity-filters",
        "query-backed-back",
        "mobile-targets",
        "reduced-motion",
        "no-overflow",
        "no-console",
        "no-protected-requests",
      ],
    },
    null,
    2,
