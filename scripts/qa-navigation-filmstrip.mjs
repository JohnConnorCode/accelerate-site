...
for (const run of [
  ...
  if (directEntrance.count < 2 || directEntrance.delays.length < 2) {
    failures.push(
      `${run.name}: direct admin load did not expose a visibly staggered committed entrance (${JSON.stringify(directEntrance)})`,
    );
  }
  ...
  if (run.name.startsWith("mobile")) {
    const intentAcknowledgement = await page.evaluate(() => {
      ...
      return receipt?.acknowledgedAt && receipt?.startedAt
        ? receipt.acknowledgedAt - receipt.startedAt
        : null;
    });
    ...
    if (intentAcknowledgement === null || intentAcknowledgement > acknowledgementBudget)
      failures.push(
        `${run.name}: destination intent acknowledgement took ${intentAcknowledgement ?? "no receipt"}ms (budget ${acknowledgementBudget}ms)`,
      );
  }
  ...
  for (const checkpoint of checkpoints) {
    ...
    if (run.delay && checkpoint === 120) {
      const fallback = await page.evaluate(() => {
        ...
        return {
          ...
          retainedContent: Boolean(
            !root && retainedHeading && Number(getComputedStyle(retainedHeading).opacity) > 0.1,
          ),
        };
      });
      ...
      if (!usefulFallback && !fallback.retainedContent) {
        failures.push(
          `${run.name}: slow route exposed neither retained content nor useful destination-shaped feedback by 120ms`,
        );
      }
    }
    if (run.name.startsWith("mobile")) {
    ...
    await page.screenshot({
      path: `${output}/${run.name}-${String(checkpoint).padStart(3, "0")}.png`,
    });
  }
  if (run.name.startsWith("mobile-cached")) {
    ...
    if (
      ...
      at120 === null ||
      at120 === undefined ||
      Math.abs(at120 - dockIndicatorStart.x) < 12
    ) {
      failures.push(
        `mobile-cached: persistent dock indicator did not move during the cached navigation frame (${JSON.stringify(dockPositions)})`,
      );
    }
  }
  ...
  if (committedAnimations.count < 2 || committedAnimations.delays.length < 2)
    failures.push(
      `${run.name}: committed destination had no visible semantic stagger (${JSON.stringify(committedAnimations)})`,
    );
  if (run.name.startsWith("mobile")) {
    const dockIndicatorEnd = await page.locator(".admin-mobile-dock-active").boundingBox();
    const activeDockLabel = await page
      .locator('.admin-mobile-dock-item[aria-current="page"] span')
      .last()
      .textContent();
    if (
      !dockIndicatorStart ||
      !dockIndicatorEnd ||
      Math.abs(dockIndicatorEnd.x - dockIndicatorStart.x) < 24
    )
      failures.push(
...
// slow read reveals destination-shaped geometry in the region it owns.
{
  ...
  await page
    .locator('a[href="/demo/command-center/northline-roofing/today"]:visible')
    .first()
  ...
  if (beforeThreshold.opacity > 0.05) {
    failures.push(
      `slow-local-data: regional fallback flashed before 120ms (${JSON.stringify(beforeThreshold)})`,
    );
  }
  ...
  await page
    .locator('[data-admin-async-state="ready"]')
