...
export function evaluateSource(source) {
  ...
  const exactScopesPresent =
    scopeOrder.every((index) => index >= 0) &&
    scopeOrder.every((index, position) => position === 0 || index > scopeOrder[position - 1]);
  return [
    check(
      "source.minimum_scopes",
      exactScopesPresent && !/auth\/drive(?:["'])/.test(source.google),
      "OAuth requests the declared Gmail read/send, Calendar events, and Drive read-only scope set.",
    ),
    check(
      "source.tenant_composite_upserts",
      ["tenant_id,provider", "tenant_id,provider,external_id"].every((target) =>
        source.google.includes(`onConflict: "${target}"`),
      ),
      "Google connection, Calendar, and Drive writes declare tenant-composite conflict keys.",
    ),
    check(
      "source.signed_state",
      source.authorize.includes("createGoogleOAuthStateBinding") &&
        source.callback.includes("verifyGoogleOAuthStateBinding"),
      "OAuth state is authenticated, expires, and remains bound to tenant ID and slug.",
    ),
    check(
      "source.safe_errors",
      source.callback.includes("googleOperatorError") &&
        !source.callback.includes("error.message") &&
        !source.authorize.includes("error.message"),
      "OAuth redirects expose stable error codes rather than provider or database messages.",
    ),
    check(
...
export function readProductionEnvironment() {
  const result = spawnSync(
    "npx",
    ["vercel", "env", "ls", "production", "--format", "json", "--non-interactive"],
    {
      cwd: repoRoot,
      encoding: "utf8",
    },
  ...
  if (result.status !== 0)
    throw new Error(
      (result.stderr || result.stdout || "Vercel Production environment lookup failed").trim(),
    );
...
export function evaluateProductionDatabase(state) {
  ...
  return [
    check(
      "production.tenant_active",
      state.tenantStatus === "active",
      state.tenantStatus === "active"
        ? "The bootstrap tenant is active."
        : `Bootstrap tenant status is ${state.tenantStatus || "missing"}.`,
    ),
    check(
      "production.connection",
      state.connection?.exists === true &&
        state.connection.status === "connected" &&
        state.connection.accountEmailPresent === true,
      state.connection?.exists
        ? `Google connection status is ${state.connection.status}; account identity ${state.connection.accountEmailPresent ? "is" : "is not"} present.`
        : "No bootstrap Google connection exists.",
    ),
    check(
      "production.encrypted_credentials",
      state.connection?.refreshEnvelopeValid === true &&
        state.connection?.accessEnvelopeValid === true &&
        state.connection?.tokenExpiresAtPresent === true,
      state.connection?.refreshEnvelopeValid && state.connection?.accessEnvelopeValid
        ? "Access and refresh credentials use the supported encrypted envelope."
        : "Encrypted Google access and refresh credentials are not both usable.",
    ),
    check(
      "production.scopes",
      missingScopes.length === 0,
      missingScopes.length === 0
        ? "The connected account granted every declared minimum scope."
        : `Missing granted scope(s): ${missingScopes.join(", ")}.`,
    ),
    check(
      "production.gmail_first_sync",
      state.runs?.gmail?.status === "success" && Boolean(state.runs.gmail.finishedAt),
      state.runs?.gmail
        ? `Latest Gmail source run is ${state.runs.gmail.status}.`
        : "No Gmail source-run receipt exists.",
    ),
    check(
      "production.calendar_first_sync",
      state.runs?.google_calendar?.status === "success" &&
        Boolean(state.runs.google_calendar.finishedAt),
      state.runs?.google_calendar
        ? `Latest Calendar source run is ${state.runs.google_calendar.status}.`
        : "No Google Calendar source-run receipt exists.",
    ),
    check(
      "production.drive_boundary",
      state.connection?.driveFoldersValid === true && folders.length <= 10 && driveReady,
      state.connection?.driveFoldersValid !== true
        ? "Drive folder settings are unavailable or invalid."
        : folders.length === 0
          ? "No Drive folders are selected; Drive is correctly absent or not configured."
          : `${folders.length} selected Drive folder(s) have a successful bounded source receipt.`,
    ),
  ];
}
...
export function readProductionDatabase() {
  ...
  if (result.status !== 0)
    throw new Error(
      (result.stderr || result.stdout || "Google readiness database query failed").trim(),
    );
...
export function readinessResult(stage, checks) {
  ...
  return {
    contract: GOOGLE_READINESS_VERSION,
    stage,
...
function parseStage(argv) {
  const stage =
    argv.find((entry) => entry.startsWith("--stage="))?.slice("--stage=".length) || "source";
  if (!GOOGLE_READINESS_STAGES.includes(stage))
    throw new Error(`--stage must be one of: ${GOOGLE_READINESS_STAGES.join(", ")}`);
  return stage;
}
