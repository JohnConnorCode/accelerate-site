...
const { data: linkData, error: linkError } = await supabase.auth.admin.generateLink({
  ...
  options: { redirectTo: `${base}/auth/callback?next=/admin/ai` },
});
...
const answer =
  "The highest-leverage next step is to review the two overdue follow-ups, then confirm the proposal awaiting a decision. I found both in the canonical operator queue.";
const events = [
  ...
  { type: "tool_started", name: "get_today_snapshot", index: 0 },
  {
    ...
    index: 0,
    summary: "Read 7 prioritized items",
    failed: false,
  },
  ...
  },
  {
    ...
    proposal: {
      ...
      title: "Follow up with Summit Mechanical",
      impact: "internal_write",
      entityType: "opportunity",
      entityId: ids.conversation,
    },
  },
  ...
  { type: "assistant_delta", delta: answer.slice(76) },
  {
    ...
    messageId: ids.answer,
    runId: ids.run,
    text: answer,
    proposedActions: [ids.proposal],
  },
];
...
async function createPage(viewport, label, colorScheme = "light") {
  ...
  await context.addInitScript(
    ({ conversationId }) =>
      localStorage.setItem("accelerate:admin-ai-conversation", conversationId),
    { conversationId: ids.conversation },
  );
  ...
  await page.route("**/api/admin/revenue-os/priority**", (route) =>
    route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({ summary: { urgent: 2 }, items: [] }),
    }),
  );
  ...
  const run = {
    ...
    conversationId: ids.conversation,
    status: "completed",
    toolNames: ["get_today_snapshot", "propose_task"],
    inputTokens: 740,
    outputTokens: 286,
    durationMs: 1480,
    promptPreview: "What needs my attention today?",
  ...
  await page.route(`**/api/admin/revenue-os/ai/runs/${ids.run}`, (route) =>
    route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({
        ...
        events: [
          ...
          },
          {
            ...
            summary: "Bounded response recorded",
            toolName: null,
            status: "recorded",
  ...
  await page.route("**/api/admin/revenue-os/ai/capabilities", (route) =>
    route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({
        registryVersion: "revenue-os-tools.v3",
        scope: "runtime_registry",
        readinessEvaluated: true,
        capabilities: [
          {
            name: "get_today_snapshot",
            label: "Read today snapshot",
            description: "Read the prioritized operator queue and current revenue summary.",
            impact: "read",
          ...
          },
          {
            name: "propose_task",
            label: "Stage task",
            description: "Prepare an operator task for founder review.",
            impact: "internal_write",
        ...
        safety: {
          registeredReads: 1,
          registeredInternalWrites: 1,
          registeredExternalActions: 0,
...
async function assertNoOverflow(page, label) {
  ...
  if (dimensions.documentWidth > dimensions.viewportWidth + 2)
    failures.push(
      `${label}: document overflow ${dimensions.documentWidth}px > ${dimensions.viewportWidth}px`,
    );
}

const desktop = await createPage({ width: 1440, height: 1000 }, "desktop");
const desktopResponse = await desktop.page.goto(`${base}/admin/ai`, {
  ...
  .catch(async (error) => {
    console.error(
      JSON.stringify({
        ...
        title: await desktop.page.title(),
        body: (await desktop.page.locator("body").innerText()).slice(0, 800),
      }),
    );
    throw error;
  });
  ...
  JSON.stringify(
    {
      result: "passed",
      screenshots: [
        `${outDir}/workspace-desktop.png`,
        `${outDir}/run-history-desktop.png`,
        `${outDir}/capabilities-desktop.png`,
        `${outDir}/global-panel-desktop.png`,
        `${outDir}/global-panel-mobile.png`,
        `${outDir}/workspace-mobile.png`,
        `${outDir}/run-history-mobile.png`,
        `${outDir}/capabilities-mobile.png`,
        `${outDir}/workspace-dark.png`,
        `${outDir}/capabilities-dark.png`,
      ],
      checks: [
        "founder auth",
        "shared conversation",
