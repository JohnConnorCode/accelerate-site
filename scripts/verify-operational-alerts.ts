...
async function main() {
  ...
  try {
    ...
    const { data: raised } = await supabase
      ...
      .select("id,priority,dedupe_key,read")
      .eq("dedupe_key", `alert:job:${JOB_KEY}`);
    ...
    const { count: afterRepeat } = await supabase
      .from("admin_notifications")
    ...
    check(
      "an acknowledged alert can fire again if the condition returns",
      afterAck.notified === true,
      afterAck,
    );
    ...
    const { data: messages } = await supabase
      ...
      .like("idempotency_key", `alert:%${RUN}`);
...
main().catch((error) => {
  console.error(
    "Operational alert verification errored:",
