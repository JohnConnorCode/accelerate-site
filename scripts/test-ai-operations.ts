...
import { AI_TOOL_REGISTRY_VERSION, listRevenueAiCapabilities } from "../src/lib/revenue-os/ai-tools";

type Response = { data: unknown; error: unknown };
type Operation = { table: string; method: string; args: unknown[] };

class Query implements PromiseLike<Response> {
  constructor(private readonly table: string, private readonly response: Response, private readonly operations: Operation[]) {}
  private add(method: string, ...args: unknown[]) { this.operations.push({ table: this.table, method, args }); return this; }
  ...
  gte(...args: unknown[]) { return this.add("gte", ...args); }
  eq(...args: unknown[]) { return this.add("eq", ...args); }
  contains(...args: unknown[]) { return this.add("contains", ...args); }
  or(...args: unknown[]) { return this.add("or", ...args); }
  in(...args: unknown[]) { return this.add("in", ...args); }
  maybeSingle() { this.add("maybeSingle"); return Promise.resolve(this.response); }
...
function fakeClient(responses: Record<string, Response[]>): { client: SupabaseClient; operations: Operation[] } {
...
const rawRun = {
  id: RUN_ID, surface: "admin_command", provider: "openrouter", model: "model-a", tool_pack: "core", conversation_id: null,
  status: "future_status", prompt_preview: "authorization: Bearer secret-token", tool_names: ["get_today_snapshot"], input_tokens: 12,
  output_tokens: 8, duration_ms: 320, result_preview: "api_key=sk-supersecret123456", error: null, started_at: startedAt, finished_at: startedAt,
};
...
assert.throws(() => parseAiRunHistoryFilters(new URLSearchParams("limit=0")), AiOperationsValidationError);
assert.throws(() => parseAiRunHistoryFilters(new URLSearchParams("window=forever")), AiOperationsValidationError);
assert.match(redactAiOperationsSummary("Bearer abc.def api_key=sk-abcdefghijklmnop") ?? "", /Bearer \[redacted\].*api_key=\[redacted\]/);

const filters = parseAiRunHistoryFilters(new URLSearchParams(`window=30d&status=completed&surface=admin_command&pack=core&model=model-a&tool=get_today_snapshot&cursor=${cursor}&limit=10`));
...
for (const [method, field] of [["eq", "status"], ["eq", "surface"], ["eq", "tool_pack"], ["eq", "model"], ["contains", "tool_names"]] as const) {
  assert.ok(historyFixture.operations.some((operation) => operation.method === method && operation.args[0] === field), `${field} must be filtered in the database query`);
}
assert.ok(historyFixture.operations.some((operation) => operation.method === "or" && String(operation.args[0]).includes("started_at.lt.")), "the opaque cursor must become a two-column keyset predicate");
...
const detailEvents = Array.from({ length: 101 }, (_, index) => ({
  ...
  output: index === 0 ? { rating: "sideways" } : { result: { entity_type: "opportunity", entity_id: index === 1 ? RECORD_ID : "--------------------", title: "Bearer event-secret" } },
  created_at: new Date(Date.parse(startedAt) + index).toISOString(),
}));
const detailFixture = fakeClient({ agent_runs: [{ data: rawRun, error: null }], agent_run_events: [{ data: detailEvents, error: null }] });
...
assert.deepEqual(detail.affectedRecords, [{ type: "opportunity", id: RECORD_ID, href: `/admin/pipeline/${RECORD_ID}` }]);
await assert.rejects(() => loadAiRunDetail(detailFixture.client, "--------------------"), AiOperationsValidationError);

const missingDetailFixture = fakeClient({ agent_runs: [{ data: rawRun, error: null }], agent_run_events: [{ data: null, error: { code: "42P01", message: "missing" } }] });
...
for (const route of ["src/app/api/admin/revenue-os/ai/runs/route.ts", "src/app/api/admin/revenue-os/ai/runs/[id]/route.ts"]) {
  const source = readFileSync(route, "utf8");
  assert.match(source, /requireAdmin\(\)/);
  assert.match(source, /AiOperationsValidationError/);
  assert.match(source, /Could not load AI run/);
  assert.doesNotMatch(source, /NextResponse\.json\(\{ error: message \}/, "routes must not return arbitrary internal error messages");
}
...
console.log(JSON.stringify({
  result: "passed",
  checks: ["typed validation", "opaque keyset cursor", "database filters", "missing-event degradation", "summary redaction", "unknown evidence", "event truncation", "canonical affected records", "registry policy truth", "safe route errors"],
}, null, 2));
}
