...
async function assertNoSeriousAxe(page, label) {
  ...
  if (violations.length)
    failures.push(
      `${label}: axe ${violations.map((item) => `${item.id} (${item.nodes.map((node) => node.target.join(" ")).join("; ")})`).join(", ")}`,
    );
}
...
async function openToday(page, label) {
  ...
  for (const delay of [0, 72, 180]) {
    ...
    entranceFrames.push(
      await animatedSurface.evaluate(
        (node, at) => ({
          at,
  ...
  const entranceSignatures = new Set(
    entranceFrames.map((frame) => `${frame.opacity.toFixed(3)}:${frame.transform}`),
  );
  check(
    entranceSignatures.size >= 2 && entranceFrames.at(-1).opacity > entranceFrames[0].opacity,
    `${label}: approval dialog did not visibly interpolate on entry (${JSON.stringify(entranceFrames)})`,
  );
  ...
  check(
    ...
    "today: approval click did not preserve a deep link",
  );
  check(
    await dialog.evaluate((node) => node.contains(document.activeElement)),
    "today: approval dialog did not receive focus",
  );
  await page.keyboard.press("Escape");
  await page.waitForTimeout(16);
  check(
    (await animatedSurface.count()) === 1,
    `${label}: approval dialog unmounted before its exit transition could run`,
  );
...
for (const [label, viewport] of [
  ...
  const geometry =
    buttonBox && panelBox
      ? {
          button: { left: buttonBox.x, right: buttonBox.x + buttonBox.width, top: buttonBox.y },
          panel: {
            left: panelBox.x,
  ...
  if (geometry && label === "desktop") {
    check(
      geometry.panel.left >= geometry.button.right &&
        geometry.panel.left - geometry.button.right <= 20,
      `desktop: notifications are not anchored beside the bell (${JSON.stringify(geometry)})`,
    );
    check(
      geometry.panel.width <= 370 && geometry.panel.height <= 560,
      `desktop: notifications are oversized (${JSON.stringify(geometry)})`,
    );
    check(
      geometry.panel.right <= geometry.viewport.width - 8 &&
        geometry.panel.bottom <= geometry.viewport.height - 8,
      `desktop: notifications escape the viewport (${JSON.stringify(geometry)})`,
    );
  }
  if (geometry && label === "mobile") {
    check(
      geometry.panel.left >= 7 && geometry.panel.right <= geometry.viewport.width - 7,
      `mobile: notification sheet is not inset from the viewport (${JSON.stringify(geometry)})`,
    );
    check(
      geometry.panel.bottom <= geometry.viewport.height - 7 &&
        geometry.panel.height <= geometry.viewport.height * 0.7,
      `mobile: notification sheet owns too much of the page (${JSON.stringify(geometry)})`,
    );
  }
  await page.waitForFunction(() =>
    document.querySelector(".admin-notification-panel")?.contains(document.activeElement),
  );
  check(
    await notifications.evaluate((node) => node.contains(document.activeElement)),
    `${label}: notification panel did not receive focus`,
  );
  await page.screenshot({ path: `${output}/notifications-${label}.png`, fullPage: false });
  await page.keyboard.press("Escape");
  await page.waitForFunction(() =>
    document.activeElement?.matches('button[aria-label^="Open command center alerts"]'),
  );
  check(
    await bell.evaluate((node) => node === document.activeElement),
    `${label}: notification close did not restore bell focus`,
  );
  ...
  check(
    (await contactDialog.getByText("Full message", { exact: true }).count()) === 1,
    `${label}: contact details lack the full message`,
  );
  check(
    (await contactDialog.getByRole("link", { name: "Open relationship" }).count()) === 1,
    `${label}: contact dialog lacks a relationship action`,
  );
  check(
    await contactDialog.evaluate((node) => node.contains(document.activeElement)),
    `${label}: contact dialog did not receive focus`,
  );
  await page.screenshot({ path: `${output}/contact-${label}.png`, fullPage: false });
  await page.keyboard.press("Escape");
  await page.waitForFunction(
    (id) => document.activeElement?.getAttribute("data-contact-row-toggle") === id,
    contactId,
  );
  check(
    await page.evaluate(
    ...
    `${label}: contact close did not restore row focus`,
  );
  ...
  if (label === "mobile") {
    ...
    check(
      (await drawer.getByText("Navigation", { exact: true }).count()) === 0,
      "mobile more: redundant Navigation label remains",
    );
    check(
      (await drawer.getByRole("heading", { name: "More", exact: true }).count()) === 0,
      "mobile more: redundant More heading remains",
    );
    const tools = drawer.getByLabel("Workspace tools");
    check(
      (await tools.getByRole("button", { name: "Search", exact: true }).count()) === 1,
...

{
  ...
  for (const path of ["/roofing", "/demo/command-center"]) {
    ...
    check(
      ...
      `${path}: shared site header is missing or duplicated`,
    );
    check(
      (await page.locator("footer").count()) === 1,
      `${path}: shared site footer is missing or duplicated`,
    );
    if (path === "/roofing") {
      const reducedHero = await page.locator("h1").evaluate((node) => ({
        opacity: getComputedStyle(node).opacity,
        transform: getComputedStyle(node).transform,
        filter: getComputedStyle(node).filter,
      }));
      check(
        reducedHero.opacity === "1" &&
          reducedHero.transform === "none" &&
          reducedHero.filter === "none",
        "/roofing: hero remained hidden or animated under reduced motion",
      );
    }
    await assertNoSeriousAxe(page, path);
    await page.waitForTimeout(500);
    await page.screenshot({
      path: `${output}/${path === "/roofing" ? "roofing" : "launcher"}-public.png`,
      fullPage: true,
    });
  }
  await page.goto(`${base}/demo/command-center/northline-roofing/today`, {
  ...
  check(
    ...
    "entered demo: marketing chrome leaked into the application workspace",
  );
  await context.close();
}

{
  ...
  check(
    contrast >= 4.5,
    `setup: success message contrast is ${contrast.toFixed(2)}, expected at least 4.5`,
  );
