...
const scenarios = process.argv.includes("--one")
  ? ["northline-roofing"]
  : [
      "northline-roofing",
      "alder-ridge-law",
  ...
  const handle = await page.waitForFunction(
    () => {
      ...
      return {
        ...
        viewport: innerWidth,
        overlay: document.querySelector("nextjs-portal")?.textContent || "",
        mainText: main.textContent?.replace(/\s+/g, " ").trim().length || 0,
        logo: Boolean(document.querySelector(".demo-scenario-mark")),
...

{
  ...
  if (!(await page.getByRole("heading", { level: 1, name: "Your business. Working together." }).count()))
    failures.push("launcher: current workflow-led heading is missing");
  if (!(await page.getByRole("heading", { name: "Follow the work through to its result." }).count()))
  ...
  const marks = await page.locator(".demo-scenario-mark").evaluateAll((nodes) =>
    nodes.map((node) => ({
      ...
      animatedParts: [...node.querySelectorAll("*")].filter(
  ...
  await page.waitForTimeout(1_300);
  const firstCard = page.locator("article").filter({ has: page.locator('a[href^="/demo/command-center/"][href$="/today"]').first() }).first();
...

{
  ...
  const facts = await page.evaluate(() => ({
    ...
    animated: [
      ...document.querySelectorAll(".ui-entrance, .ui-entrance *, .demo-scenario-mark, .demo-scenario-mark *"),
  ...
  if (facts.animated)
    failures.push(
      `launcher mobile: ${facts.animated} entrances remained animated under reduced motion`,
    );
  ...
  for (const [label, viewport] of [
    ...
    page.on("console", (message) => {
      if (message.type() === "error")
        failures.push(
          `${scenario} ${label} ${activeRoute}: console ${message.text().split("\n")[0]}`,
        );
    });
    page.on("pageerror", (error) =>
      failures.push(`${scenario} ${label} ${activeRoute}: page ${error.message.split("\n")[0]}`),
    );
    page.on("request", (request) => {
      ...
      if (
        ...
        failures.push(`${scenario} ${label}: protected request escaped demo runtime: ${path}`);
    });
    for (const route of routes) {
      ...
      if (
        /Build Error|Unhandled Runtime Error|Runtime TypeError|Compilation failed/i.test(
          state.overlay,
        )
      )
        failures.push(`${scenario} ${label} ${route}: Next error overlay`);
      if (state.mainText < 120)
        failures.push(`${scenario} ${label} ${route}: view is not credibly populated`);
      if (!state.logo)
        failures.push(`${scenario} ${label} ${route}: shared animated logo is missing`);
      if (
        !state.heading ||
        !state.title.startsWith(`${state.heading} | `) ||
        !state.title.endsWith(" Demo")
      )
        failures.push(
      ...
      if (route === "integrations" && label === "desktop") {
        if (await page.getByRole("navigation", { name: "Breadcrumb" }).count())
          failures.push(
            `${scenario} desktop integrations: redundant top-level breadcrumb repeats the page title`,
          );
      }
      ...
      if (await page.locator(`.admin-nav-link[href="${expectedActiveHref}"]`).count()) {
        ...
        if (activeHref !== expectedActiveHref)
          failures.push(
            `${scenario} ${label} ${route}: active navigation is ${activeHref || "missing"}, expected ${expectedActiveHref}`,
          );
        if (!page.url().includes(`/demo/command-center/${scenario}/${route}`))
          failures.push(
            `${scenario} ${label} ${route}: public URL did not retain the active demo route`,
          );
      }
      if (route === "contacts") {
      ...
      if (route === "inbox") {
        ...
        await page.waitForFunction(
          () =>
            document.querySelector("[data-inbox-refresh]")?.getAttribute("aria-busy") === "true",
          undefined,
          { timeout: 2_000 },
        );
        await page.waitForFunction(
        ...
        if (refreshCount !== 1)
          failures.push(
            `${scenario} ${label} inbox: one refresh click issued ${refreshCount} reads`,
          );
      }
      if (route === "settings") {
        const switches = page.getByRole("switch");
        if ((await switches.count()) !== 6)
          failures.push(
            `${scenario} ${label} settings: expected six semantic notification switches`,
          );
        const geometry = await switches.evaluateAll((nodes) =>
          nodes.map((button) => {
            ...
            return {
              ...
              contained: Boolean(
                ...
                thumbRect.right <= trackRect.right &&
        ...
        if (
          geometry.some(
            (item) =>
              item.button.width < 40 ||
              item.button.height < 40 ||
              item.track?.width !== 48 ||
              item.track?.height !== 28 ||
              item.thumb?.width !== 20 ||
              item.thumb?.height !== 20 ||
              !item.contained,
          )
        )
          failures.push(
            `${scenario} ${label} settings: switch geometry is distorted (${JSON.stringify(geometry)})`,
          );
        if (
          label === "desktop" &&
          ...
          failures.push(
            `${scenario} desktop settings: duplicate Settings breadcrumb remains above the Settings heading`,
          );
        ...
        await page.waitForFunction(
          (value) =>
            document.querySelector('[role="switch"]')?.getAttribute("aria-checked") !== value,
          before,
        );
        if (scenario === "northline-roofing")
          await page.screenshot({
            path: `${output}/${scenario}-${label}-settings.png`,
            fullPage: false,
          });
      }
      if (route === "today") {
        ...
        if (await page.locator("[data-admin-demo-link]").count())
          failures.push(
            `${scenario} ${label}: duplicate demo chooser remains in shared navigation`,
          );
        if (label === "desktop") {
          ...
          await page.waitForTimeout(350);
          const panelBounds = await alertPanel.evaluate((node) => {
          ...
          const anchored = triggerBounds
            ? Math.abs(panelBounds.top - Math.max(12, triggerBounds.y - 8)) <= 3
            : false;
          if (
            panelBounds.position !== "fixed" ||
            !anchored ||
            panelBounds.right < 8 ||
            panelBounds.bottom < 8 ||
            panelBounds.width > 370
          )
            failures.push(
              `${scenario} desktop: notification panel is not a contained, trigger-anchored viewport overlay`,
            );
          await page.screenshot({
            path: `${output}/${scenario}-desktop-notifications.png`,
            fullPage: false,
          });
        ...
        if (label === "mobile") {
          ...
          const alertOverlay = await page.evaluate(() => {
            ...
            return {
              ...
              overlaps:
                sheetRect.bottom > dockRect.top &&
                sheetRect.top < dockRect.bottom &&
                Number(dockStyle.opacity) > 0.01,
              sheetBottom: Math.round(innerHeight - sheetRect.bottom),
            };
          });
          if (
            !alertOverlay?.bodyState ||
            alertOverlay.dockOpacity > 0.01 ||
            alertOverlay.dockPointerEvents !== "none" ||
            alertOverlay.overlaps ||
            Math.abs(alertOverlay.sheetBottom - 8) > 2
          )
            failures.push(
              `${scenario} mobile: notification sheet and dock do not share one collision-free inset overlay state`,
            );
          await page.screenshot({
            path: `${output}/${scenario}-mobile-notifications.png`,
            fullPage: false,
          });
          await page.keyboard.press("Escape");
          ...
          if (
            ...
            failures.push(`${scenario} mobile: notification overlay state remained after close`);
            ...
            return {
              ...
              focusInside: Boolean(
                document.activeElement?.closest(
                  'aside[role="dialog"][aria-label="Admin navigation"]',
                ),
              ),
          ...
          if (drawerA11y.duplicateIds.length)
            failures.push(
              `${scenario} mobile: duplicate ids while navigation is open: ${drawerA11y.duplicateIds.join(", ")}`,
            );
          if (!drawerA11y.focusInside || drawerA11y.focusLabel !== "Close navigation")
            failures.push(`${scenario} mobile: navigation did not move focus to its close control`);
          if (
            !drawerA11y.bodyState ||
            drawerA11y.dockOpacity > 0.01 ||
            drawerA11y.dockPointerEvents !== "none"
          )
            failures.push(
              `${scenario} mobile: More drawer did not take exclusive ownership from the dock`,
            );
          if (drawerA11y.backdropBlur === "none")
            failures.push(
              `${scenario} mobile: More drawer does not use the shared blurred overlay backdrop`,
            );
        }
        let controlsScope =
          label === "mobile"
            ? page.locator('aside[role="dialog"][aria-label="Admin navigation"]')
            : page.locator("[data-admin-sidebar]");
        const demoControlPlacement = await controlsScope
          .locator("[data-admin-demo-bar]")
          .evaluate((node) => ({
            insideFooter: Boolean(node.closest(".admin-nav-footer")),
            position: getComputedStyle(node).position,
          }));
        if (!demoControlPlacement.insideFooter || demoControlPlacement.position === "fixed")
          failures.push(
            `${scenario} ${label}: demo controls are not integrated into the sidebar footer`,
          );
        if (
          label === "mobile" &&
        ...
        if (scenario === "northline-roofing") {
          ...
          if (
            (await page
              .locator('.admin-nav-link[aria-current="page"]')
              .first()
              .getAttribute("href")) !== inboxHref
          )
            failures.push(
              `${scenario} ${label}: client navigation left the wrong sidebar item active`,
            );
          if (
            label === "desktop" &&
            (await page.getByRole("navigation", { name: "Breadcrumb" }).count())
            ...
            failures.push(
              `${scenario} ${label}: top-level Inbox navigation rendered a redundant breadcrumb`,
            );
          ...
          if (
            (await page
              .locator('.admin-nav-link[aria-current="page"]')
              .first()
              .getAttribute("href")) !== todayHref
          )
            failures.push(
              `${scenario} ${label}: back navigation left the wrong sidebar item active`,
            );
          if (label === "mobile") {
            await page.getByRole("button", { name: "Open More" }).click();
            await page.locator('aside[role="dialog"][aria-label="Admin navigation"]').waitFor();
        ...
        if (
          (await controlsScope.locator('[data-admin-demo-bar][data-state="collapsed"]').count()) !==
          1
        )
          failures.push(`${scenario} ${label}: demo controls do not start collapsed`);
        ...
        if (label === "mobile") {
          const mobileControlFacts = await page.evaluate(() => ({
            overflow: document.documentElement.scrollWidth > innerWidth + 2,
            undersized: [
              ...document.querySelectorAll(
                'aside[role="dialog"][aria-label="Admin navigation"] button, aside[role="dialog"][aria-label="Admin navigation"] select, aside[role="dialog"][aria-label="Admin navigation"] a',
              ),
            ].filter((node) => {
              const rect = node.getBoundingClientRect();
          ...
          if (mobileControlFacts.overflow)
            failures.push(
              `${scenario} mobile: open integrated controls caused horizontal overflow`,
            );
          if (mobileControlFacts.undersized)
            failures.push(
              `${scenario} mobile: ${mobileControlFacts.undersized} visible navigation controls are below the 40px hit-area minimum`,
            );
          const drawerFocusableCount = await controlsScope
            .locator(
              'a[href], button:not([disabled]), select:not([disabled]), input:not([disabled]), [tabindex]:not([tabindex="-1"])',
            )
            .count();
          for (let index = 0; index < drawerFocusableCount + 2; index += 1)
          ...
          if (
            ...
            failures.push(`${scenario} mobile: keyboard focus escaped the navigation dialog`);
        }
        if (label === "mobile") {
          await page.screenshot({
            path: `${output}/${scenario}-mobile-navigation.png`,
            fullPage: false,
          });
        }
    ...
    if (scenario === "northline-roofing" && label === "desktop") {
      ...
      if (await desktopSidebar.locator('[data-admin-demo-bar][data-state="collapsed"]').count())
        await desktopSidebar.getByRole("button", { name: "Open demo controls" }).click();
      await desktopSidebar
        .getByRole("combobox", { name: "Demo business" })
        .selectOption("alder-ridge-law");
      ...
      if (await desktopSidebar.getByRole("button", { name: "Open demo controls" }).count())
        await desktopSidebar.getByRole("button", { name: "Open demo controls" }).click();
      await desktopSidebar
        .getByRole("combobox", { name: "Demo business" })
        .selectOption("northline-roofing");
      ...
      const mutation = await page.evaluate(async () => {
        const before = await fetch("/api/admin/revenue-os/priority").then((response) =>
          response.json(),
        );
        const actions = await fetch("/api/admin/revenue-os/actions").then((response) =>
          response.json(),
        );
        await fetch("/api/admin/revenue-os/actions", {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ id: actions.actions[0].id, decision: "approve" }),
        });
        const after = await fetch("/api/admin/revenue-os/priority").then((response) =>
          response.json(),
        );
      ...
      const isolated = await page.evaluate(
        async () =>
          (await fetch("/api/admin/revenue-os/priority").then((response) => response.json()))
            .summary.total,
      );
      if (isolated !== mutation.before)
        failures.push("scenario switch: fictional workspace state leaked between businesses");
      await page.goto(`${base}/demo/command-center/northline-roofing/today`, {
        waitUntil: "domcontentloaded",
      });
      await page.waitForFunction(() => window.__accelerateAdminDemoRuntime === "northline-roofing");
      ...
      if (persisted !== mutation.after)
        failures.push("northline-roofing desktop: demo mutation did not survive refresh");
      ...
      const reset = await page.evaluate(() => ({
        data: sessionStorage.getItem("accelerate:admin-demo:northline-roofing:v3"),
...
for (const scenario of scenarios) {
  ...
  page.on("request", (request) => {
    ...
    if (
      ...
      failures.push(`${scenario} operations: protected request escaped demo runtime: ${path}`);
  });
  await page.goto(`${base}/demo/command-center/${scenario}/today`, {
    waitUntil: "domcontentloaded",
    timeout: 60_000,
  });
  await page.waitForFunction(
  ...
  const operation = await page.evaluate(async (activeScenario) => {
    ...
    const [approval, taskCompletion, stageChange, reply, ai] = await Promise.all([
      fetch("/api/admin/revenue-os/actions", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
      ...
      fetch("/api/admin/revenue-os/pipeline", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: opportunity.id, stage: "proposal" }),
      }).then((response) => response.json()),
      fetch("/api/admin/revenue-os/conversations/reply", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          conversationId: conversation.id,
          body: "This is a safe fictional demo reply.",
        }),
      }).then((response) => response.json()),
    ...
    const [actionsAfter, tasksAfter, pipelineAfter, conversationAfter] = await Promise.all([
      ...
      fetch("/api/admin/revenue-os/tasks").then((response) => response.json()),
      fetch("/api/admin/revenue-os/pipeline").then((response) => response.json()),
      fetch(`/api/admin/revenue-os/conversations?id=${encodeURIComponent(conversation.id)}`).then(
        (response) => response.json(),
      ),
    ]);
    return {
      approval:
        approval.simulated === true &&
  ...
  for (const [name, passed] of Object.entries(operation)) {
    if (!passed)
      failures.push(`${scenario} operations: simulated ${name} did not complete coherently`);
  }
  await context.close();
}
...
for (const scenario of scenarios) {
  for (const [label, viewport] of [
    ...
    const page = await context.newPage();
    page.on("pageerror", (error) =>
      failures.push(`${scenario} ${label} deep links: ${error.message.split("\n")[0]}`),
    );
    ...
    const contract = await page.evaluate(async () => {
      ...
      const [
        ...
        fetch("/api/admin/clients?id=missing-client").then(async (response) => ({
          status: response.status,
          body: await response.json(),
        })),
        fetch("/api/admin/revenue-os/records/opportunity/missing-opportunity").then(
          async (response) => ({ status: response.status, body: await response.json() }),
        ),
        fetch("/api/admin/contacts/timeline?email=missing%40example.test").then((response) =>
          response.json(),
        ),
        ...
        fetch("/api/admin/revenue-os/campaigns/preview?id=missing-campaign").then(
          (response) => response.status,
        ),
        fetch("/api/admin/emails/preview?id=missing-email").then((response) => response.status),
        fetch("/api/admin/revenue-os/contact-imports?id=missing-import").then((response) =>
          response.json(),
        ),
        fetch("/api/admin/revenue-os/ai/conversations/missing-conversation").then(
          (response) => response.status,
        ),
      ]);
      const knownContact = firstContact
        ? await fetch(
            `/api/admin/contacts/timeline?email=${encodeURIComponent(firstContact)}`,
          ).then((response) => response.json())
        : null;
      return {
        clients: clientList.clients,
        opportunities: pipelineList.opportunities,
        unknownClient,
        unknownOpportunity,
        unknownContact,
        unknownRun,
    ...
    if (
      contract.unknownClient.body.client !== null ||
      contract.unknownOpportunity.body.record !== null ||
      contract.unknownContact.canonical.contact !== null ||
      contract.unknownRun.run !== null ||
      contract.unknownProposal.proposal !== null ||
      contract.unknownCampaign !== 404 ||
      contract.unknownEmail !== 404 ||
      contract.unknownImport.batch !== null ||
      contract.unknownConversation !== 404
    )
      failures.push(
        `${scenario} ${label} deep links: an unknown ID substituted or exposed another entity`,
      );
    if (
      contract.knownContact?.canonical?.status !== "connected" ||
      contract.knownContact.canonical.contact?.full_name !==
        contract.opportunities[0]?.contact?.full_name
    )
      failures.push(`${scenario} ${label} deep links: known contact identity did not reconcile`);
    for (const client of contract.clients) {
      await page.goto(
        `${base}/demo/command-center/${scenario}/clients/${encodeURIComponent(client.id)}`,
        { waitUntil: "domcontentloaded", timeout: 60_000 },
      );
      await page
        .getByRole("heading", { name: client.business_name, exact: true })
        .waitFor({ timeout: 30_000 });
      if (await page.getByRole("heading", { name: "Client Not Found", exact: true }).count())
        failures.push(`${scenario} ${label}: valid client ${client.id} rendered not found`);
    }
    for (const opportunity of contract.opportunities) {
      await page.goto(
        `${base}/demo/command-center/${scenario}/pipeline/${encodeURIComponent(opportunity.id)}`,
        { waitUntil: "domcontentloaded", timeout: 60_000 },
      );
      await page
        .getByRole("heading", { name: opportunity.name, exact: true })
        .waitFor({ timeout: 30_000 });
      if (await page.getByText("Opportunity not found", { exact: false }).count())
        failures.push(
          `${scenario} ${label}: valid opportunity ${opportunity.id} rendered not found`,
        );
      if (await page.locator('a[href$="/admin/tasks"], a[href*="/admin/tasks?"]').count())
        failures.push(
          `${scenario} ${label}: opportunity ${opportunity.id} exposes the nonexistent Tasks route`,
        );
    }
    await context.close();
  }
...
for (const appearance of ["light", "dark", "signal", "studio", "frost"]) {
  for (const [label, viewport] of [
    ...
    const page = await context.newPage();
    page.on("pageerror", (error) =>
      failures.push(`appearance ${appearance} ${label}: ${error.message.split("\n")[0]}`),
    );
    await page.goto(`${base}/demo/command-center/northline-roofing/today`, {
      waitUntil: "domcontentloaded",
      timeout: 60_000,
    ...
    const tokens = await page.evaluate(() => {
      ...
      const active = getComputedStyle(
        document.querySelector('.admin-nav-link[aria-current="page"]'),
      );
      return {
        ...
        action: styles.getPropertyValue("--admin-action").trim(),
        navInk: styles.getPropertyValue("--admin-nav-ink").trim(),
    ...
    if (
      ...
      failures.push(`appearance ${appearance} ${label}: incoherent foreground/background tokens`);
    if (tokens.overflow) failures.push(`appearance ${appearance} ${label}: horizontal overflow`);
    if (label === "desktop")
      appearanceFingerprints.set(
        appearance,
        [
          tokens.canvas,
          tokens.surface,
    ...
    if (
      ...
      failures.push(
        `appearance frost ${label}: navigation does not have a distinct violet active treatment`,
      );
    await context.close();
  }
}
if (new Set(appearanceFingerprints.values()).size !== appearanceFingerprints.size)
  failures.push("appearances: two or more themes still resolve to the same shell treatment");
await browser.close();
if (failures.length)
  throw new Error(`Full admin demo QA failures:\n${[...new Set(failures)].join("\n")}`);
