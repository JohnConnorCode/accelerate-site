...
async function main() {
  assert.ok(
    EMAIL_TEMPLATE_DEFINITIONS.length > 0,
    "no email templates registered, so nothing below is being checked",
  );
  ...
  for (const definition of EMAIL_TEMPLATE_DEFINITIONS) {
    ...
    const used = new Set([
      ...placeholders(definition.subjectTemplate),
      ...placeholders(definition.bodyTemplate),
    ]);
    ...
    for (const key of used) {
      if (!declared.has(key))
        drift.push(
          `${definition.key}: uses {{${key}}} but does not declare it in variables[], so no caller passes it and it leaks into the sent email`,
        );
    }
    ...
    for (const key of declared) {
      if (!used.has(key))
        drift.push(`${definition.key}: declares "${key}" in variables[] but no template uses it`);
    }

    assert.ok(
      definition.subjectTemplate.trim(),
      `${definition.key} has an empty subject; a blank subject line reads as spam`,
    );
    ...
    for (const [field, value] of [
      ...
      if (remaining.length)
        leaks.push(
          `${definition.key}: ${field} still contains ${remaining.map((key) => `{{${key}}}`).join(", ")} after rendering with every declared variable`,
        );
    }
    ...
    if (previewLeaks.length) {
      drift.push(
        `${definition.key}: sampleData does not cover ${[...new Set(previewLeaks)].map((key) => `{{${key}}}`).join(", ")}, so the admin preview shows raw placeholders`,
      );
    }
  }
  ...
  assert.equal(
    replaceEmailVariables("Hi {{name}}, re {{topic}}.", { name: "Dana", topic: "intake" }),
    "Hi Dana, re intake.",
  );
  assert.equal(
  ...
  assert.equal(
    replaceEmailVariables("Hi {{name}}.", { name: "" }),
    "Hi .",
    "an explicitly empty value is a caller's decision and is honoured",
  );
  assert.equal(
    replaceEmailVariables("Cost is $1,000 {{note}}", { note: "" }),
    "Cost is $1,000 ",
    "text around a placeholder must be preserved exactly",
  );
  ...
  assert.equal(
    getEmailTemplateDefinition("definitely-not-a-template"),
    null,
    "an unknown key must return null so the caller can fail loudly",
  );
  ...
  assert.equal(
    new Set(keys).size,
    keys.length,
    `duplicate template keys: lookup returns the first match, so a duplicate silently shadows another template. Keys: ${keys.join(", ")}`,
  );
  ...
  assert.equal(
    fallback.source,
    "built_in",
    "with no database reachable the built-in template must answer",
  );
  assert.equal(fallback.versionId, null, "a built-in render has no published version to attribute");
  assert.ok(
    fallback.subject.trim(),
    "a transactional email must never go out with a blank subject, database or not",
  );
  assert.ok(
    fallback.text.trim(),
    "a transactional email must never go out with a blank body; that is the entire purpose of this fallback",
  );
  assert.ok(fallback.html.trim(), "the HTML part must be populated too");
  assert.match(
    fallback.subject,
    /Dana/,
    "the fallback must still substitute variables, not just return the raw template",
  );
    ...
    JSON.stringify(
      {
        ...
        checks: [
          ...
          "renders-without-leakage",
          "sample-data-covers-preview",
          "substitution-semantics",
          "unique-keys",
          "unknown-key-throws",
          "built-in-fallback-never-blank",
        ],
        result: "passed",
      },
      null,
