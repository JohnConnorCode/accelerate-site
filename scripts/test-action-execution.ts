...
function pending(overrides: Row = {}): Row {
  return {
    ...
    payload: { opportunityId: "opp-1", nextAction: "Send the revised scope", nextActionAt: iso(86400000) },
...
function seed(action: Row = pending()) {
  return new MemorySupabase({
    action_queue: [action],
    opportunities: [{ id: "opp-1", name: "Northside Roofing", stage: "qualified", next_action: null, next_action_at: null }],
    audit_log: [],
  });
}
...
async function main() {
  ...
  const ok = seed();
  const result = await approveAndExecuteAction(ok.client, "action-1", "john@acceleratewith.us") as Row;
  assert.equal(result.next_action, "Send the revised scope", "the executor must return what it actually wrote");

  const executed = ok.rows("action_queue")[0]!;
  assert.equal(executed.status, "executed", "a successful action must end `executed`, not linger in `executing`");
  assert.equal(executed.approved_by, "john@acceleratewith.us", "the queue must record who approved it");
  ...
  assert.equal(ok.rows("opportunities")[0]!.next_action, "Send the revised scope", "the side effect must actually have happened");
  ...
  assert.equal(ok.rows("action_queue")[0]!.status, "executed", "the failed second claim must not disturb the completed row");
  ...
  assert.equal(stale.rows("action_queue")[0]!.status, "pending", "a refused claim must leave the row untouched");
  assert.equal(stale.rows("opportunities")[0]!.next_action, null, "no side effect may happen when the claim is refused");
  ...
  await assert.doesNotReject(() => approveAndExecuteAction(eternal.client, "action-1", "john@acceleratewith.us"));
  ...
  const malformed = seed(pending({ action_type: "send_email", payload: { subject: "Hello", body: "Hi there" } }));
  await rejects(
  ...
  assert.equal(malformed.rows("action_queue")[0]!.status, "failed", "a validation failure must be recorded on the row, not left `executing` forever");
  assert.match(String(malformed.rows("action_queue")[0]!.error), /to is required/, "the row must say why it failed");

  // ---- An invalid stage is rejected before the pipeline moves -----------

  const badStage = seed(pending({ action_type: "transition_opportunity", payload: { opportunityId: "opp-1", stage: "definitely_not_a_stage", reason: "test" } }));
  await rejects(
  ...
  assert.equal(badStage.rows("opportunities")[0]!.stage, "qualified", "the opportunity must not have moved");
  ...
  assert.equal(broken.rows("action_queue")[0]!.status, "failed", "a failed execution must be recorded as failed");
  assert.match(String(broken.rows("action_queue")[0]!.error), /unavailable/, "the failure reason must be preserved for the operator");
  ...
  assert.deepEqual(row.result, { reason: "Wrong contact" }, "the reason must be kept; it is the signal the agent-learning loop reads");
  ...
  assert.equal(rejected.rows("opportunities")[0]!.next_action, null, "a rejected action must have caused no side effect");
  ...
  assert.equal(terminal.rows("action_queue")[0]!.status, "failed", "finishAction must not resurrect a row that is not currently executing");
  assert.equal(terminal.rows("action_queue")[0]!.error, "Provider rejected the recipient", "the real failure reason must survive a stray completion");
