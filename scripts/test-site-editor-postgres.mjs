...
try {
  ...
  for (const file of [
    "20260909012125-installation-website-revisions.sql",
  ...
  const command = {
    ...
    requestKey: a,
    expectedVersion: 0,
    document: { schemaVersion: 1, pages: [{ title: "Private" }] },
  };
  const digest = "a".repeat(64),
    summary = "Save a private website draft.";
  sql(
    `INSERT INTO action_queue(id,tenant_id,action_type,payload) VALUES('${a}','${owner}','site_website_change',${json({ tenantId: owner, command, digest, summary })})`,
  );
  ...
  for (const changes of [
    ...
    { sessionId: a },
    { digest: "b".repeat(64) },
    { summary: "Publish everything." },
  ])
    assert.throws(() => sql(execute(changes)), /revoked|required|proposal/i);
  sql(`CREATE FUNCTION reject_delegated_audit() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN IF NEW.action='site_editor.delegated_execution' THEN RAISE EXCEPTION 'controlled audit failure'; END IF; RETURN NEW; END $$;
    CREATE TRIGGER reject_delegated_audit BEFORE INSERT ON audit_log FOR EACH ROW EXECUTE FUNCTION reject_delegated_audit();`);
  assert.throws(() => sql(execute()), /controlled audit failure/);
  assert.equal(
    sql("SELECT count(*) FROM site_website_revisions"),
    "0",
    "failed audit rolls back the website revision",
  );
  assert.equal(
    sql("SELECT count(*) FROM site_website_receipts"),
    "0",
    "failed audit leaves no success receipt",
  );
  assert.equal(
    sql("SELECT status FROM action_queue"),
    "pending",
    "same proposal remains safely retryable",
  );
  sql("DROP TRIGGER reject_delegated_audit ON audit_log;");
  ...
  assert.equal(
    sql(
      "SELECT count(*) FROM action_queue WHERE approved_by IS NOT NULL OR approved_at IS NOT NULL",
    ),
    "0",
  );
  assert.equal(
    sql("SELECT count(*) FROM audit_log WHERE action='site_editor.delegated_execution'"),
    "1",
  );
  assert.equal(sql("SELECT source FROM audit_log WHERE action='site_website.save'"), "ai");
  const savedRevision = raced[0].receipt.draftRevisionId;
  for (const [operation, expectedVersion] of [
    ...
    const nextCommand = {
      ...
      ...(operation === "unpublish" ? {} : { revisionId: savedRevision }),
    };
    sql(
      `INSERT INTO action_queue(id,tenant_id,action_type,payload) VALUES('${actionId}','${owner}','site_website_change',${json({ tenantId: owner, command: nextCommand, digest, summary })})`,
    );
    const receipt = JSON.parse(sql(execute({ actionId, command: nextCommand }))).receipt;
    assert.equal(receipt.version, expectedVersion + 1);
    assert.equal(receipt.publishedRevisionId, operation === "unpublish" ? null : savedRevision);
    assert.deepEqual(JSON.parse(sql(execute({ actionId, command: nextCommand }))).receipt, receipt);
  }
  sql(
    `CREATE OR REPLACE FUNCTION check_autonomy(text,text) RETURNS TABLE(hard_floor boolean,level text) LANGUAGE sql AS $$ SELECT false,'prohibited'::text $$;`,
  );
  assert.throws(() => sql(execute()), /prohibited/);
  sql(
    `CREATE OR REPLACE FUNCTION check_autonomy(text,text) RETURNS TABLE(hard_floor boolean,level text) LANGUAGE sql AS $$ SELECT false,'supervised'::text $$;`,
  );
  for (const role of ["anon", "authenticated", "mcp_site_editor"]) {
    assert.throws(
      () => sql(execute().replace("SET ROLE service_role", `SET ROLE ${role}`)),
      /permission denied/,
    );
    assert.throws(
      () => sql(`SET ROLE ${role}; SELECT * FROM site_editor_delegations`),
      /permission denied/,
    );
  ...
  sql(
    `UPDATE tenants SET status='active' WHERE id='${owner}'; UPDATE tenant_memberships SET status='revoked'`,
  );
  ...
  sql(
    `${context} SELECT manage_site_editor_delegation('revoke','${user}',NULL,'${resource}','owner@example.test','${grant.id}')`,
  );
  assert.throws(() => sql(execute()), /expired or revoked/);
  const hook = JSON.parse(
    sql(
      `SET ROLE supabase_auth_admin; SELECT site_editor_access_token_hook(${json({ client_id: client, claims: { sub: user, aud: "authenticated", role: "authenticated" } })})`,
    ),
  );
  ...
  assert.equal(
    hook.claims.role,
    "mcp_site_editor",
    "revoking never upgrades OAuth tokens to ordinary Data API access",
  );
  assert.throws(
    () =>
      sql(
        `SET ROLE supabase_auth_admin; SELECT site_editor_access_token_hook(${json({ client_id: b, claims: { sub: user, aud: "authenticated", role: "authenticated" } })})`,
      ),
    /no scoped/,
  );
  const otherUser = JSON.parse(
    sql(
      `SET ROLE supabase_auth_admin; SELECT site_editor_access_token_hook(${json({ client_id: client, claims: { sub: b, aud: "authenticated", role: "authenticated" } })})`,
    ),
  );
  assert.equal(
    otherUser.claims.role,
    "mcp_site_editor",
