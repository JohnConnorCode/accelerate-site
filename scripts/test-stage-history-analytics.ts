...
import { summarizeRevenueAnalytics, loadRevenueAnalytics } from "../src/lib/revenue-os/analytics";
...
async function verifyBoundedHistoryRead() {
  for (const status of ["truncated", "unavailable"] as const) {
    const orders: string[] = [];
    const db = {
      from(table: string) {
        const q: Record<string, unknown> = {};
        for (const method of ["select", "gte", "eq", "in", "limit"]) q[method] = () => q;
        q.order = (field: string) => {
          if (table === "stage_events") orders.push(field);
          return q;
        };
          ...
          resolve(
            table === "opportunities"
              ...
              : table === "stage_events"
                ? {
                    data: [],
                    count: status === "truncated" ? 5 : 0,
                    error: status === "unavailable" ? { message: "offline" } : null,
                  }
                : table === "kanban_columns"
                  ? {
                      data: stages.stageKeys.map((column_key) => ({
                        column_key,
                        label: column_key,
                        metadata: { role: stages.role(column_key) },
                      })),
                      error: null,
                    }
                  : { data: [], error: null },
    ...
    const result = await loadRevenueAnalytics(db as never, "fictional-tenant", 60);
    assert.equal(result.quality.stageHistory.inputStatus, status);
    assert.equal(result.quality.stageHistory.incompleteHistory, opportunities.length);
    assert.equal(
      result.funnel.proposals,
      0,
      "incomplete reads never masquerade as current-stage history",
    );
    assert.deepEqual(orders, ["created_at", "id"]);
  }
}
void verifyBoundedHistoryRead()
  .then(() => console.log("Actual analytics loader reports capped and unavailable history reads."))
  .catch((error) => {
    console.error(error);
    process.exitCode = 1;
  });
