...
import { captureFounderNote, FOUNDER_NOTE_MAX_LENGTH, loadFounderKnowledgeNotes, loadFounderNoteAdoptionReport } from "../src/lib/revenue-os/notes";
...
function stubSupabase() {
  ...
  function query(table: string) {
    ...
    self.ilike = () => self;
    self.contains = () => self;
    self.limit = (value: number) => { rowLimit = value; return self; };
    self.order = () => { listMode = true; return self; };
    self.lt = (key: string, value: unknown) => { filters.set(`lt:${key}`, value); return self; };
    ...
    const resolve = () => {
      ...
      if (table === "activities") {
        if (listMode) {
          const rows = activities.filter((row) => [...filters.entries()].every(([key, value]) => {
      ...
      if (table === "audit_log") {
        const match = audits.find((row) => row.action === filters.get("action") && row.entity_type === filters.get("entity_type") && row.entity_id === filters.get("entity_id"));
...
async function main() {
  ...
  const input = { requestId: "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa", body: "Client said the reporting deadline is Friday.\nDo not promise Monday delivery.", actorEmail: "founder@example.com", captureDurationMs: 4_200, captureSource: "keyboard_shortcut" as const };
  ...
  assert.equal(repair.audits.length, 1, "retrying an activity whose audit failed must repair the missing redacted audit receipt");

  const attached = await captureFounderNote(db.client as never, { requestId: "bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb", body: "Prefers a concise weekly summary.", actorEmail: "founder@example.com", contactEmail: "CASEY@example.com", captureDurationMs: 8_800, captureSource: "command_palette" });
  ...
  const third = await captureFounderNote(db.client as never, { requestId: "cccccccc-cccc-4ccc-8ccc-cccccccccccc", body: "Standalone operating constraint.", actorEmail: "founder@example.com", captureDurationMs: 9_500, captureSource: "command_palette" });
  assert.equal(third.contactId, null);

  const knowledge = await loadFounderKnowledgeNotes(db.client as never, { limit: 10 });
  assert.equal(knowledge.length, 3, "the knowledge reader must include attached and standalone notes");
  assert.equal(knowledge[0]?.author, "founder@example.com");
  assert.ok(knowledge.every((note) => note.occurredAt && note.sourceReceipt.startsWith("founder-note:")), "retrieval must preserve date and stable source receipt");
  assert.ok(knowledge.some((note) => note.contactId === db.contact.id), "canonical attachments must survive retrieval");

  const brokenKnowledge = stubSupabase();
  brokenKnowledge.activities.push({ id: "broken", activity_type: "founder_note", source: "admin_note", external_id: "founder-note:broken", title: "Broken", summary: "Missing author", actor_email: null, occurred_at: "2026-08-30T12:00:00.000Z", metadata: {} });
  await rejects(() => loadFounderKnowledgeNotes(brokenKnowledge.client as never), "provenance is incomplete");
  ...
  const adoption = await loadFounderNoteAdoptionReport(db.client as never, { now: new Date("2026-08-31T12:00:00.000Z") });
  assert.equal(adoption.speedEvidenceReady, true, "seven-day, three-day, sub-ten-second evidence should satisfy the mechanical speed gate");
  assert.equal(adoption.cardReady, false, "mechanical telemetry must not pretend founder usefulness was confirmed");
  ...
  await rejects(() => captureFounderNote(db.client as never, { ...input, requestId: "duration", captureDurationMs: -1 }), "capture duration");
  await rejects(() => captureFounderNote(db.client as never, { ...input, requestId: "source", captureSource: "mystery" as never }), "capture source");
  ...
  assert.match(shell, /event\.shiftKey[\s\S]{0,160}key\.toLowerCase\(\) === "m"/, "the global shell must retain the direct capture shortcut");

  console.log(JSON.stringify({ result: "passed", checks: ["canonical activity", "audit redaction", "idempotent replay", "audit repair on retry", "capture telemetry", "bounded knowledge retrieval", "author and date provenance", "adoption gate honesty", "exact contact attachment", "company context", "blank rejection", "length bound", "founder auth", "global command"] }, null, 2));
}
