    ...
    const builder = {
      ...
      limit: (value: number) => {
        ...
        return Promise.resolve({
          data: rows
            .filter((row) => matches(row, filters) && (!before || String(row.occurred_at) < before))
            .sort(
              (a, b) =>
      ...
      single: async () => {
        ...
        const duplicate = rows.find(
          (row) => row.source === inserted?.source && row.external_id === inserted?.external_id,
        );
...
async function main() {
  ...
  const base = {
    ...
    title: "Task created: confirm owner",
    source: "admin",
    externalId: "task:1:created",
    opportunityId: "opp-1",
  ...
  const replay = await recordActivity(store.client, {
    ...base,
    title: "A retry must not rewrite history",
  });
  assert.equal(replay.duplicate, true);
  assert.equal(replay.activity.title, base.title);
  assert.equal(store.rows.length, 1);

  await recordActivity(store.client, {
    ...base,
    activityType: "founder_note",
    title: "Older note",
    externalId: "note:older",
  ...
  await recordActivity(store.client, {
    ...
    title: "Newer email",
    externalId: "email:newer",
    occurredAt: "2026-08-24T12:00:00.000Z",
  });
  const timeline = await loadActivityTimeline(store.client, { opportunityId: "opp-1", limit: 2 });
  assert.deepEqual(
    timeline.map((row) => row.title),
    ["Newer email", base.title],
  );
  const older = await loadActivityTimeline(store.client, {
    opportunityId: "opp-1",
    before: "2026-08-23T12:00:00.000Z",
  });
  assert.deepEqual(
  ...
  await assert.rejects(
    recordActivity(store.client, { ...base, title: "", externalId: "bad:title" }),
    /title is required/,
  );
  await assert.rejects(
    recordActivity(store.client, { ...base, externalId: "" }),
    /external ID is required/,
  );
  await assert.rejects(loadActivityTimeline(store.client, {}), /canonical record ID/);
  await assert.rejects(
    loadActivityTimeline(store.client, { opportunityId: "opp-1", before: "not-a-date" }),
    /cursor is invalid/,
  );
  ...
  walk("src");
  const directWriters = sourceFiles.filter(
    (path) =>
      path !== "src/lib/revenue-os/activities.ts" &&
      /from\(["']activities["']\)\.insert\(/.test(readFileSync(path, "utf8")),
  );
  assert.deepEqual(
    directWriters,
    [],
    "all activity receipts must use the authoritative ledger writer",
  );
  assert.match(
    readFileSync("src/app/api/admin/contacts/timeline/route.ts", "utf8"),
    /loadActivityTimeline/,
    "record timelines must use the canonical ordered reader",
  );
  assert.match(
    readFileSync("src/lib/revenue-os/ai-tools.ts", "utf8"),
    /get_record_timeline[\s\S]*loadActivityTimeline/,
    "AI record context must use the same canonical ordered reader",
  );
  const activityRoute = readFileSync("src/app/api/admin/revenue-os/activity/route.ts", "utf8");
  assert.match(
    activityRoute,
    /requireAdmin/,
    "the canonical activity API must remain founder-only",
  );
  assert.match(
    activityRoute,
    /loadActivityTimeline/,
    ...
    JSON.stringify(
      {
        ...
        checks: [
          ...
          "concurrent-safe reread boundary",
          "ordered bounded timeline",
          "cursor validation",
          "database failure truth",
          "single writer ownership",
          "founder-only API",
          "record and AI reader parity",
        ],
      },
      null,
      2,
