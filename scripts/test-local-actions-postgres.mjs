...
try {
  ...
  started = true;
  // Supabase-owned Auth/Storage primitives exist before application migrations.
  // Business tables and their real RLS still come from the complete catalog.
  sql(`CREATE ROLE anon; CREATE ROLE authenticated; CREATE ROLE service_role BYPASSRLS; CREATE SCHEMA auth; CREATE SCHEMA extensions; CREATE SCHEMA storage;
 ...
 INSERT INTO tenant_memberships(tenant_id,user_id,invited_email,role,status) VALUES('${b}','${u}','owner@example.test','admin','active');`);
  ...
  sql(`INSERT INTO autonomy_policies(tenant_id,action_key,label,level,source,approved_by,approved_at,coworker_id) VALUES
   ('${a}','create_task','Generic refusal','prohibited','system',NULL,NULL,NULL),
   ('${a}','create_task','Specific worker','standing_permission','system','owner@example.test',now(),'native-policy-worker'),
   ('${b}','tasks.create','Other tenant refusal','prohibited','system',NULL,NULL,NULL);`);
  assert.equal(
    sql(context() + `SELECT level FROM check_autonomy('create_task','native-policy-worker');`),
    "standing_permission",
    "existing coworker rule precedes non-floor generic action rule",
  );
  assert.equal(
    sql(context(b) + `SELECT level FROM check_autonomy('create_task','native-policy-worker');`),
    "prohibited",
    "capability policies remain tenant scoped",
  );
  sql(
    `UPDATE autonomy_policies SET is_hard_floor=true WHERE tenant_id='${a}' AND action_key='create_task' AND coworker_id IS NULL;`,
  );
  assert.equal(
    sql(context() + `SELECT hard_floor FROM check_autonomy('create_task','native-policy-worker');`),
    "t",
    "generic hard floor cannot be bypassed by a worker",
  );
  sql(
    `DELETE FROM autonomy_policies WHERE tenant_id='${a}' AND action_key='create_task'; DELETE FROM autonomy_policies WHERE tenant_id='${b}' AND action_key='tasks.create';`,
  );
  ...
  const conversationState = () =>
    JSON.parse(sql(`SELECT to_jsonb(c) FROM conversations c WHERE id='${conversationId}';`));
  const conversationCall = (
    ...
    `SELECT apply_conversation_action('${id}',${quote(operation)},${quote(JSON.stringify(payload))}::jsonb,${quote(actor)},${undo});`;
  ...
  const conversationResolved = JSON.parse(
    sql(conversationCall(statusAction, "update_conversation_status", statusPayload)),
  );
  assert.equal(conversationResolved.unread_count, 0);
  assert.equal(conversationResolved.status, "resolved");
  assert.deepEqual(
    JSON.parse(sql(conversationCall(statusAction, "update_conversation_status", statusPayload))),
  ...
  denied(
    ...
    "undo requires the exact approved actor",
  );
  sql(
    `INSERT INTO autonomy_policies(tenant_id,action_key,label,level,source) VALUES('${a}','crm.write','Conversation test','prohibited','system');`,
  );
  denied(
    conversationCall(statusAction, "update_conversation_status", statusPayload, true),
    "revoked capability forbids undo writes",
  );
  assert.equal(conversationState().status, "resolved");
  sql(`DELETE FROM autonomy_policies WHERE tenant_id='${a}' AND action_key='crm.write';`);
  const statusUndo = sql(
    conversationCall(statusAction, "update_conversation_status", statusPayload, true),
  );
  assert.equal(
    sql(conversationCall(statusAction, "update_conversation_status", statusPayload, true)),
    statusUndo,
  );
  ...
  assert.equal(
    sql(
      `SELECT count(*) FROM audit_log WHERE action='action.compensated' AND entity_id='${statusAction}';`,
    ),
    "1",
  );
  const assignPayload = {
  ...
  const assignedConversation = JSON.parse(
    sql(conversationCall(assignAction, "assign_conversation", assignPayload)),
  );
  ...
  const outsiderAssignment = {
    conversationId,
    assigneeEmail: "outsider@example.test",
    expectedState: conversationState(),
  };
  denied(
    conversationCall(
      stage("assign_conversation", outsiderAssignment),
      "assign_conversation",
      outsiderAssignment,
    ),
    "assignment requires live same-tenant membership",
  );
  assert.deepEqual(conversationState().metadata, { retained: "human note" });
  ...
  sql(
    `UPDATE conversations SET metadata=metadata||'{"newHumanNote":"preserve"}' WHERE id='${conversationId}';`,
  );
  denied(
    conversationCall(staleConversationAction, "update_conversation_status", staleConversation),
    "stale conversation preview",
  );
  denied(
    conversationCall(
      staleConversationAction,
      "update_conversation_status",
      staleConversation,
      false,
      b,
    ),
    "foreign tenant conversation",
  );
  const conversationContact = sql(
    `INSERT INTO contacts(tenant_id,full_name,primary_email) VALUES('${a}','Conversation Contact','conversation@example.test') RETURNING id;`,
  );
  const otherTenantContact = sql(
    `INSERT INTO contacts(tenant_id,full_name,primary_email) VALUES('${b}','Other Conversation Contact','conversation@example.test') RETURNING id;`,
  );
  const badLink = {
    conversationId,
    patch: { contact_id: otherTenantContact },
    expectedState: conversationState(),
  };
  denied(
    conversationCall(
      stage("link_conversation_record", badLink),
      "link_conversation_record",
      badLink,
    ),
    "foreign tenant linked target",
  );
  const linkPayload = {
    conversationId,
    patch: { contact_id: conversationContact },
    expectedState: conversationState(),
  };
  ...
  sql(
    `DROP TRIGGER native_conversation_failure ON audit_log; DROP FUNCTION public.fail_conversation_receipt();`,
  );
  const conversationLinked = sql(
    conversationCall(linkAction, "link_conversation_record", linkPayload),
  );
  assert.equal(
    sql(conversationCall(linkAction, "link_conversation_record", linkPayload)),
    conversationLinked,
  );
  assert.equal(
    sql(
      `SELECT count(*) FROM evidence WHERE source_id='conversation-action:${linkAction}:contact_id';`,
    ),
    "1",
  );
  assert.equal(
    sql(
      `SELECT best_evidence FROM claims WHERE tenant_id='${a}' AND entity_id='${conversationId}' AND field='contact_id';`,
    ),
    "human_entered",
  );
  assert.equal(
    sql(`SELECT count(*) FROM activities WHERE external_id='conversation-action:${linkAction}';`),
    "1",
  );
  denied(
    context() +
      `SELECT * FROM record_evidence('conversation','${conversationId}','contact_id','${conversationContact}','operator_link','forged','human_entered');`,
    "public ingestion RPC stays service-only",
  );
  denied(
    context() +
      `SELECT * FROM private.record_evidence_effect('conversation','${conversationId}','contact_id','${conversationContact}','operator_link','forged','human_entered');`,
    "private evidence writer requires executing exact action",
  );
  denied(
    context() +
      `SELECT * FROM private.record_evidence_effect('conversation','${conversationId}','contact_id','${conversationContact}','operator_link','Founder linked conversation to contact_id ${conversationContact}','human_entered','conversation-action:${linkAction}:contact_id',jsonb_build_object('conversation_id','${conversationId}'::uuid,'actionId','${linkAction}'::uuid),NULL,NULL,'${linkAction}',${quote(JSON.stringify(linkPayload))}::jsonb,'owner@example.test');`,
    "executed approval cannot manufacture new evidence",
  );
  sql(
    machineContext +
      `SELECT * FROM record_evidence('conversation','${conversationId}','contact_id','${conversationContact}','gmail_thread','Exact external email match','verified_external','native-thread');`,
  );
  assert.equal(
    sql(
      `SELECT best_evidence FROM claims WHERE tenant_id='${a}' AND entity_id='${conversationId}' AND field='contact_id';`,
  ...
  const parentPayload = {
    ...
    participant_email: "conversation@example.test",
    candidates: [{ id: conversationContact }],
    approvedDecision: { decision: "link", contactId: conversationContact, companyId: null },
    conversationState: conversationState(),
  };
  const parentLink = {
    conversationId,
    patch: { contact_id: conversationContact, company_id: null },
    expectedState: parentPayload.conversationState,
  };
  const identityParent = stage("identity_review", parentPayload);
  const parentApplied = sql(
    conversationCall(identityParent, "link_conversation_record", parentLink),
  );
  assert.equal(
    sql(conversationCall(identityParent, "link_conversation_record", parentLink)),
    parentApplied,
  );
  assert.equal(
    sql(`SELECT status FROM action_queue WHERE id='${identityParent}';`),
    "executing",
    "child link does not finish its parent",
  );
  assert.equal(
    sql(
      `SELECT count(*) FROM evidence WHERE source_id='conversation-action:${identityParent}:contact_id';`,
    ),
    "1",
  );
  ...
  denied(
    conversationCall(stage("identity_review", malformedParent), "link_conversation_record", {
      ...parentLink,
      expectedState: malformedParent.conversationState,
    }),
    "missing decision fails closed",
  );
  const changedChoice = {
    ...parentPayload,
    approvedDecision: { decision: "link", contactId: otherTenantContact },
    conversationState: conversationState(),
  };
  denied(
    conversationCall(stage("identity_review", changedChoice), "link_conversation_record", {
      ...parentLink,
      expectedState: changedChoice.conversationState,
    }),
    "parent cannot substitute unapproved chosen contact",
  );
  assert.equal(
    sql(
      `SELECT bool_and(NOT prosecdef) FROM pg_proc WHERE proname IN ('apply_conversation_action','record_evidence','record_evidence_effect','require_conversation_action');`,
    ),
    "t",
  );

  // Full human identity decisions finish inside the existing parent transaction.
  const reviewConversationId = sql(
    `INSERT INTO conversations(tenant_id,channel,subject,metadata) VALUES('${a}','manual','Full identity review','{"note":"retain"}') RETURNING id;`,
  );
  const reviewState = () =>
    JSON.parse(sql(`SELECT to_jsonb(c) FROM conversations c WHERE id='${reviewConversationId}';`));
  const identityPayload = (decision, overrides = {}) => ({
    conversation_id: reviewConversationId,
    participant_email: "review@identity.native",
    candidates: [],
    conversationState: reviewState(),
    approvedDecision: {
      decision,
      contactId: null,
      companyId: null,
      fullName: "Reviewed Human",
      phone: null,
      companyName: null,
      companyDomain: "identity.native",
    },
    ...overrides,
  });
  const identityCall = (id, payload, tenant = a, actor = "owner@example.test") =>
    context(tenant, u, actor) +
    `SELECT apply_identity_review_action('${id}',${quote(JSON.stringify(payload))}::jsonb,${quote(actor)});`;
  ...
  sql(
    `CREATE FUNCTION public.refuse_identity_receipt() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN IF NEW.action='identity_review.resolved' THEN RAISE EXCEPTION 'injected identity receipt failure'; END IF; RETURN NEW; END $$; CREATE TRIGGER native_identity_failure BEFORE INSERT ON audit_log FOR EACH ROW EXECUTE FUNCTION public.refuse_identity_receipt();`,
  );
  denied(
    identityCall(humanCreationAction, humanCreation),
    "identity, link and evidence roll back on parent receipt failure",
  );
  assert.equal(
    sql(`SELECT count(*) FROM contacts WHERE source_record_id='${humanCreationAction}';`),
    "0",
  );
  assert.equal(
    sql(`SELECT count(*) FROM companies WHERE source_record_id='${humanCreationAction}';`),
    "0",
  );
  assert.equal(sql(`SELECT count(*) FROM claims WHERE entity_id='${reviewConversationId}';`), "0");
  assert.equal(
    sql(`SELECT result IS NULL FROM action_queue WHERE id='${humanCreationAction}';`),
    "t",
  );
  assert.equal(reviewState().contact_id, null);
  sql(
    `DROP TRIGGER native_identity_failure ON audit_log; DROP FUNCTION public.refuse_identity_receipt();`,
  );
  ...
  assert.equal(
    sql(
      `SELECT first_name||':'||last_name||':'||phone FROM contacts WHERE id='${humanCreated.contact_id}';`,
    ),
    "Reviewed:Human:+1 555 0198",
  );
  assert.equal(
    sql(`SELECT domain FROM companies WHERE id='${humanCreated.company_id}';`),
    "identity.native",
  );
  assert.deepEqual(JSON.parse(sql(identityCall(humanCreationAction, humanCreation))), humanCreated);
  assert.equal(
    sql(
      `SELECT count(*) FROM audit_log WHERE entity_id='${humanCreationAction}' AND action='action.executed';`,
    ),
    "1",
  );
  assert.equal(
    sql(
      `SELECT count(*) FROM activities WHERE external_id='identity_create:${humanCreationAction}';`,
    ),
    "1",
  );
  const noMatchPayload = identityPayload("no_match", {
    participant_email: "stranger@identity.native",
  });
  ...
  assert.equal(
    reviewState().contact_id,
    humanCreated.contact_id,
    "no-match preserves existing human links",
  );
  assert.equal(
    sql(
      `SELECT count(*) FROM evidence WHERE source_id='identity-review:${noMatchAction}:no-match';`,
    ),
    "1",
  );
  ...
  assert.equal(
    sql(
      `SELECT status||':'||(approved_by IS NULL)::text FROM action_queue WHERE id='${deferredAction}';`,
    ),
    "pending:true",
  );
  assert.equal(sql(identityCall(deferredAction, deferredPayload)), deferredResult);
  assert.equal(
    sql(`SELECT count(*) FROM activities WHERE external_id='identity_defer:${deferredAction}';`),
    "1",
  );
  ...
  assert.equal(
    JSON.parse(sql(identityCall(linkedReview, linkedPayload))).contact_id,
    humanCreated.contact_id,
  );
  const retainedPayload = identityPayload("link");
  retainedPayload.approvedDecision.contactId = humanCreated.contact_id;
  retainedPayload.candidates = [{ id: humanCreated.contact_id }];
  const retainedReview = stage("identity_review", retainedPayload);
  sql(
    conversationCall(retainedReview, "link_conversation_record", {
      conversationId: reviewConversationId,
      expectedState: retainedPayload.conversationState,
      patch: { contact_id: humanCreated.contact_id, company_id: humanCreated.company_id },
    }),
  );
  assert.equal(sql(`SELECT status FROM action_queue WHERE id='${retainedReview}';`), "executing");
  assert.equal(
    JSON.parse(sql(identityCall(retainedReview, retainedPayload))).contact_id,
    humanCreated.contact_id,
  );
  assert.equal(
    sql(
      `SELECT count(*) FROM activities WHERE external_id='conversation-action:${retainedReview}';`,
    ),
    "1",
    "retained child link is not repeated",
  );
  assert.equal(
    sql(
      `SELECT count(*) FROM audit_log WHERE action='action.executed' AND entity_id='${retainedReview}';`,
    ),
    "1",
  );
  ...
  denied(
    identityCall(stage("identity_review", otherLink), otherLink),
    "identity review rejects foreign canonical links",
  );
  ...
  personal.approvedDecision.companyName = "Not a business identity";
  personal.approvedDecision.companyDomain = "gmail.com";
  denied(
    identityCall(stage("identity_review", personal), personal),
    "personal domains cannot seed a company",
  );
  const alreadyKnown = identityPayload("create");
  denied(
    identityCall(stage("identity_review", alreadyKnown), alreadyKnown),
    "human create refuses existing identity rather than merging",
  );
  sql(
    `INSERT INTO contacts(tenant_id,full_name,primary_email,alternate_emails) VALUES('${a}','Alternate One','alternate-one@identity.native',ARRAY['ambiguous@identity.native']),('${a}','Alternate Two','alternate-two@identity.native',ARRAY['ambiguous@identity.native']);`,
  );
  const ambiguousCreate = identityPayload("create", {
    participant_email: "ambiguous@identity.native",
  });
  denied(
    identityCall(stage("identity_review", ambiguousCreate), ambiguousCreate),
    "alternate-email ambiguity never silently creates",
  );
  const literalCreate = identityPayload("create", {
    participant_email: "review_%@identity.native",
  });
  const literalReview = stage("identity_review", literalCreate);
  assert.ok(
    JSON.parse(sql(identityCall(literalReview, literalCreate))).contact_id,
    "wildcard email is an exact literal",
  );
  const staleReviewPayload = identityPayload("no_match");
  const staleReviewAction = stage("identity_review", staleReviewPayload);
  sql(
    `UPDATE conversations SET metadata=metadata||'{"human":"newer"}' WHERE id='${reviewConversationId}';`,
  );
  denied(
    identityCall(staleReviewAction, staleReviewPayload),
    "newer human conversation state fences identity decision",
  );
  ...
  sql(
    `INSERT INTO autonomy_policies(tenant_id,action_key,label,level,source) VALUES('${a}','crm.write','Identity capability denied','prohibited','system');`,
  );
  denied(
    identityCall(prohibitedReview, prohibitedReviewPayload),
    "identity capability revoked at effect",
  );
  sql(`DELETE FROM autonomy_policies WHERE tenant_id='${a}' AND action_key='crm.write';`);
  denied(
    identityCall(prohibitedReview, { ...prohibitedReviewPayload, approvedDecision: {} }),
    "malformed changed identity decision",
  );
  denied(
    identityCall(prohibitedReview, prohibitedReviewPayload, a, "spoofed@example.test"),
    "identity actor must match exact parent approval",
  );
  ...
  denied(
    context() +
      `SELECT * FROM private.record_evidence_effect('conversation','${reviewConversationId}','identity_review_decision','${wrongEvidenceAction}','operator_review','Founder decided review@identity.native matches no canonical record','human_entered','identity-review:${wrongEvidenceAction}:no-match',jsonb_build_object('conversation_id','${reviewConversationId}'::uuid,'action_id','${wrongEvidenceAction}'::uuid),NULL,NULL,'${wrongEvidenceAction}',${quote(JSON.stringify(wrongEvidencePayload))}::jsonb,'owner@example.test');`,
    "a link approval cannot manufacture no-match evidence",
  );
  assert.equal(
    sql(
      `SELECT bool_and(NOT prosecdef) FROM pg_proc WHERE proname IN ('apply_identity_review_action','require_identity_review_action');`,
    ),
    "t",
  );
  ...
  console.log(
    JSON.stringify({
      ...
      proofs: [
        ...
        "tenant-coworker-and-hard-floor-policy-preservation",
