...
try {
  ...
  run("psql", [
    ...
    "-c",
    `
    CREATE TABLE agent_run_events(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),tenant_id uuid,run_id uuid,event_type text,tool_name text,output jsonb,created_at timestamptz DEFAULT now());
    INSERT INTO agent_run_events(tenant_id,run_id,event_type,tool_name,output) VALUES
      ('aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa',gen_random_uuid(),'tool_error','search_knowledge_base','{"error":"Provider unavailable"}'),
      ('aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa',gen_random_uuid(),'tool_result','search_knowledge_base','{"result":{"missing":["Stale source"]}}'),
      ('bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb',gen_random_uuid(),'tool_error','search_knowledge_base','{"error":"Foreign failure"}');
    UPDATE action_queue SET status='executed' WHERE id='10000000-0000-4000-8000-000000000001';
  `,
  ]);
  for (let i = 0; i < 2; i++)
    run("psql", [...args, "-f", "migrations/20260928-learning-signals.sql"]);
  run("psql", [
    ...args,
    "-c",
    `
    DO $$ BEGIN
      IF collect_learning_signals()<>3 THEN RAISE EXCEPTION 'missing new learning signals'; END IF;
      IF collect_learning_signals()<>0 THEN RAISE EXCEPTION 'replayed old evidence'; END IF;
      IF (SELECT count(*) FROM learning_signals)<>3 THEN RAISE EXCEPTION 'foreign evidence included'; END IF;
      IF has_function_privilege('authenticated','public.collect_learning_signals()','execute') THEN RAISE EXCEPTION 'collector exposed'; END IF;
    END $$;
    GRANT USAGE ON SCHEMA private TO authenticated;
    SET ROLE authenticated;
    DO $$ BEGIN
      IF (SELECT count(*) FROM knowledge_documents)<>2 THEN RAISE EXCEPTION 'document RLS failed'; END IF;
      IF (SELECT count(*) FROM learning_signals)<>3 THEN RAISE EXCEPTION 'signal RLS failed'; END IF;
      BEGIN
        INSERT INTO learning_signals(tenant_id,kind,source_kind,source_id,receipt_key,details)
        VALUES('bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb','rejection','action','foreign','foreign','Forbidden');
  ...
  console.log(
    "Learning signals: replay, tenant isolation, typed receipt collection and authenticated RLS passed.",
  );
  console.log(
