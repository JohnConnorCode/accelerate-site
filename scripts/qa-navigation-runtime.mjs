...
for (const config of [
  ...
  if (
    config.reducedMotion === "no-preference" &&
    !publicForward.animation.includes("route-entry-in")
  )
    failures.push(`${config.label}: public route entrance did not run`);
  if (config.reducedMotion === "reduce" && publicForward.animation !== "none")
    failures.push("reduced: public route entrance remained animated");
  if (!/WORK\+SHELTER/i.test(publicForward.title))
    failures.push(
      `${config.label}: public destination title is not contextual (${publicForward.title})`,
    );
  if (!publicForward.focused)
    failures.push(
      `${config.label}: public forward navigation did not focus the destination heading`,
    );
  await page.goBack();
  await page.waitForTimeout(1_050);
  const publicRestored = await page.evaluate(() => window.scrollY);
  if (Math.abs(publicRestored - publicOrigin) > 2)
    failures.push(
      `${config.label}: public history restored ${publicRestored}px instead of ${publicOrigin}px`,
    );
  ...
  if (config.reducedMotion === "no-preference" && earlyFallback !== null && earlyFallback < 0.1) {
    failures.push(
      `${config.label}: admin fallback left a blank intermediate frame (${earlyFallback})`,
    );
  }
  await page
  ...
  if (config.reducedMotion === "no-preference" && !adminEntrance.contentAnimations)
    failures.push(`${config.label}: real admin destination did not receive its route entrance`);
  await page.waitForTimeout(1_050);
  const adminForward = await page.evaluate(() => ({
    y: document.querySelector(".admin-main").scrollTop,
    duplicateHeaders: document
      .getAnimations()
      .filter(
        (animation) =>
  ...
  if (adminForward.y > 2)
    failures.push(`${config.label}: admin forward navigation landed at ${adminForward.y}px`);
  if (adminForward.duplicateHeaders)
    failures.push(
      `${config.label}: admin header retained ${adminForward.duplicateHeaders} duplicate entrance animations`,
    );
  ...
  if (!adminForward.focused)
    failures.push(
      `${config.label}: admin forward navigation did not focus the destination heading`,
    );
  await page.goBack();
  await page.waitForTimeout(1_050);
  const adminRestored = await page.evaluate(() => document.querySelector(".admin-main").scrollTop);
  if (Math.abs(adminRestored - adminOrigin) > 2)
    failures.push(
      `${config.label}: admin history restored ${adminRestored}px instead of ${adminOrigin}px`,
    );
  ...
  if (config.label === "mobile") {
    ...
    if (scenarioState.count !== documentCount)
      failures.push("mobile: scenario switch caused a full document reload");
    if (!scenarioState.title.includes("Alder Ridge"))
      failures.push("mobile: scenario identity did not update after client navigation");
    if (scenarioState.documentTitle !== "Today | Alder Ridge Injury Law Demo")
      failures.push(`mobile: scenario title did not update (${scenarioState.documentTitle})`);
    if (scenarioState.y > 2)
      failures.push(`mobile: scenario switch landed at ${scenarioState.y}px`);
  }
