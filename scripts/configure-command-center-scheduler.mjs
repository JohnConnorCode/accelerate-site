...
if (!siteUrl || !/^https:\/\//.test(siteUrl)) fail("NEXT_PUBLIC_SITE_URL must be the production HTTPS origin");
...
async function resolveCanonicalDispatcherUrl(initialUrl) {
  ...
  for (let redirects = 0; redirects < 5; redirects += 1) {
    let response;
    try {
      response = await fetch(current, { method: "HEAD", redirect: "manual", signal: AbortSignal.timeout(10_000) });
    } catch (error) {
      fail(`could not verify the dispatcher origin: ${error instanceof Error ? error.message : "request failed"}`);
    }
    ...
    if (redirected.protocol !== "https:" || redirected.pathname !== "/api/cron/system-health-snapshot") {
...
const dispatcherUrl = await resolveCanonicalDispatcherUrl(`${siteUrl}/api/cron/system-health-snapshot`);
const sql = `
BEGIN;
