...
async function main() {
  ...
  assert.equal(
    applyHouseStyle("no catch, no sales pitch—just a clear plan"),
    "no catch, no sales pitch, just a clear plan",
    "the exact sentence observed in production must be fixed",
  );
  assert.equal(
    applyHouseStyle("a — b"),
    "a, b",
    "a spaced em dash must not leave a double space behind",
  );
  assert.equal(
    applyHouseStyle("a – b"),
    "a, b",
    "a spaced en dash used as a clause break gets the same treatment",
  );
  assert.equal(
    applyHouseStyle("Plain text with no dashes."),
    "Plain text with no dashes.",
    "text without dashes must pass through byte for byte",
  );
  ...
  assert.equal(
    applyHouseStyle("https://calendly.com/john-acceleratewith/30min"),
    "https://calendly.com/john-acceleratewith/30min",
    "a URL with hyphens must survive intact",
  );
  assert.equal(applyHouseStyle("2026-08-20"), "2026-08-20", "a date must survive");
  assert.equal(
    applyHouseStyle("Range: 5-10 hours"),
    "Range: 5-10 hours",
    "a numeric range must survive",
  );
  ...
  const cases = [
    [
      "no catch, no sales pitch—just a clear plan you keep",
      "no catch, no sales pitch, just a clear plan you keep",
    ],
    ["We map the business — then we build.", "We map the business, then we build."],
    [
      "Book here: https://acceleratewith.us/contact",
      "Book here: https://acceleratewith.us/contact",
    ],
  ...
  for (const [input, expected] of cases) {
    ...
    for (const size of [1, 2, 3, 5, 17, 4096]) {
      ...
      assert.equal(
        actual,
        expected,
        `chunk size ${size}: ${JSON.stringify(input)}\n  expected ${JSON.stringify(expected)}\n  got      ${JSON.stringify(actual)}`,
      );
    }
  }
  ...
  for (const ending of ["end.", "end", "e", ""]) {
    ...
    assert.equal(
      await throughStream(text, 3),
      text,
      `the last character must not be swallowed: ${JSON.stringify(text)}`,
    );
  }
  ...
  for (const size of [1, 2, 3, 7]) {
    assert.equal(
      await throughStream(unicode, size),
      unicode,
      `chunk size ${size}: multi-byte text must survive the transform intact`,
    );
  }
  ...
  const offenders = [...promptBody.matchAll(/.{0,40}—.{0,40}/g)]
    ...
    .filter(
      (context) =>
        !context.includes("NEVER use an em dash") && !context.includes("sales pitch—just"),
    );
  assert.deepEqual(
    offenders,
    [],
    `the chat system prompt itself must not contain an em dash: ${offenders.join(" | ")}`,
  );
  assert.match(
    promptBody,
    /NEVER use an em dash/,
    "the prompt must still carry the instruction; the stream filter is a backstop, not a replacement",
  );
