...
async function main() {
  ...
  assert.throws(
    () => decryptTenantSecret(envelope, beta, "openrouter", "api_key"),
    /authenticate|Unsupported|unable/i,
    "ciphertext copied to another tenant must fail authentication",
  );
  assert.throws(
    () => decryptTenantSecret(envelope, alpha, "resend", "api_key"),
    /authenticate|Unsupported|unable/i,
    "ciphertext copied to another provider must fail authentication",
  );

  const tenantCredential = resolveOpenRouterCredentialPolicy({
    tenantId: alpha,
    connection: {
      status: "connected",
      encrypted_credentials: { api_key: envelope },
  ...
  assert.equal(
    ...
    null,
    "client tenants must never inherit the platform key",
  );
  assert.equal(
    resolveOpenRouterCredentialPolicy({
    ...
    "platform",
    "bootstrap may use the explicit migration fallback",
  );
  assert.equal(
    resolveOpenRouterCredentialPolicy({
      tenantId: ACCELERATE_TENANT_ID,
    ...
    null,
    "disconnect must override bootstrap fallback",
  );
  assert.throws(
    () =>
      resolveOpenRouterCredentialPolicy({
    ...
    OpenRouterCredentialError,
    "a tenant-bound envelope cannot be replayed under another tenant ID",
  );
  ...
  assert.deepEqual(
    {
      ...
      remaining: metadata.limitRemaining,
      usage: metadata.usage,
    },
    { label: "tenant-prod…123", limit: 50, remaining: 42.5, usage: 7.5 },
  );
  assert.equal(verificationCalls, 1);
  await assert.rejects(
    () => validateOpenRouterApiKey("not-a-key"),
    (error: unknown) => error instanceof OpenRouterCredentialError && error.status === 400,
  );
  ...
  await assert.rejects(
    () => validateOpenRouterApiKey(secret),
    (error: unknown) => error instanceof OpenRouterCredentialError && error.status === 400,
  );
  ...
  assert.equal(
    providerCalled,
    false,
    "production gateway must reject unscoped calls before provider traffic",
  );

  const providerApi = readFileSync("src/app/api/admin/tenant/providers/route.ts", "utf8");
  assert.ok(
    providerApi.search(/const metadata = await validateOpenRouterApiKey/) <
      providerApi.search(/encrypted_credentials:\s*{\s*api_key:\s*encryptTenantSecret/),
    "key verification must precede the OpenRouter connection write",
  );
  ...
  for (const file of productCallers)
    assert.match(
      readFileSync(file, "utf8"),
      /database:/,
      `${file} must pass explicit tenant database context to OpenRouter`,
    );

  console.log(
    JSON.stringify({ result: "passed", checks: 16, productCallers: productCallers.length }),
  );
}
...
main()
  ...
  .finally(() => {
    ...
    if (original.encryptionKey === undefined) delete process.env.GOOGLE_TOKEN_ENCRYPTION_KEY;
    else process.env.GOOGLE_TOKEN_ENCRYPTION_KEY = original.encryptionKey;
    if (original.openRouterKey === undefined) delete process.env.OPENROUTER_API_KEY;
