...
for (const [label, viewport] of [
  ...
  page.on("request", (request) => {
    ...
    if (
      ...
      !pathname.startsWith("/api/admin/emails/preview") &&
      !pathname.startsWith("/api/admin/emails/history")
    )
  ...
  if (!initialEmail.includes("Northline Roofing & Exteriors"))
    failures.push(`${label}: preview does not carry the active scenario brand`);
  if (/Accelerate/i.test(initialEmail))
    failures.push(`${label}: preview leaks the Accelerate brand into fictional customer email`);
  ...
  await page
    .getByText("Draft saved. Live email is unchanged until you publish.", { exact: true })
  ...
  if (
    label === "mobile" &&
    overlayGeometry.toastBottom != null &&
    overlayGeometry.dockTop != null &&
    overlayGeometry.toastBottom > overlayGeometry.dockTop - 8
  )
    failures.push(`${label}: operation receipt overlaps the navigation dock`);
  await page.getByRole("button", { name: "Edit" }).click();
  await page.getByRole("button", { name: "Mobile preview" }).click();
  await page.locator('iframe[title="Exact email preview"]').waitFor({ timeout: 15_000 });
  const state = await page.evaluate(() => ({
    overflow: document.documentElement.scrollWidth > innerWidth + 2,
    surfaces: [...document.querySelectorAll(".admin-surface")].map((node) => ({
  ...
  if (state.surfaces.some((surface) => !surface.radius || surface.radius === "0px"))
    failures.push(`${label}: surface did not inherit the shared radius token`);
  ...
  if (modalFacts.blur === "none")
    failures.push(`${label}: modal does not use the shared blurred backdrop`);
  if (
    label === "mobile" &&
    (modalFacts.top < 8 || modalFacts.bottom > modalFacts.viewportHeight - 8)
  )
    failures.push(`${label}: ordinary modal is still presented as an edge-pinned drawer`);
...
// invalidate the demo's scenario isolation.
{
  ...
  for (const [scenarioId, brandName] of scenarioBrands) {
    await page.goto(`${base}/demo/command-center/${scenarioId}/emails`, {
      waitUntil: "domcontentloaded",
      timeout: 60_000,
    });
    ...
    if (!text.includes(brandName))
      failures.push(`${scenarioId}: preview does not render its own scenario brand`);
    if (/Accelerate/i.test(text))
      failures.push(`${scenarioId}: preview leaks Accelerate into the fictional customer email`);
  }
  await context.close();
}
...
await browser.close();
if (failures.length)
  throw new Error(`Email Studio QA failures:\n${[...new Set(failures)].join("\n")}`);
