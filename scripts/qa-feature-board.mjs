...
const seed = [
  ["Apply and verify the Revenue OS migrations", "planned", "urgent", ["database", "launch"], "Run the production migrations in order and confirm every Setup Center schema check.", "John", "2026-08-18"],
  ["Run production admin QA on desktop and mobile", "planned", "urgent", ["qa", "launch"], "Verify the critical founder workflows at real breakpoints.", "John", "2026-08-19"],
  ["Complete campaign unsubscribe handling", "backlog", "urgent", ["campaigns", "compliance"], "Add durable suppression and operator-visible receipts.", null, null],
  ["Configure Google OAuth and first sync", "backlog", "high", ["google", "integration"], "Connect Gmail, Calendar, and approved Drive folders.", null, null],
  ["Add Resend delivery webhooks", "in_progress", "high", ["email", "webhooks"], "Process delivery signals idempotently and stop on hard failures.", "John", "2026-08-22"],
  ["Calendar confirmation workflow", "blocked", "medium", ["calendar", "ai"], "Require approval of exact attendees and meeting time.", null, null],
  ["Canonical source-to-revenue attribution", "shipped", "high", ["analytics", "revenue"], "Reconcile the reporting model against won revenue.", "John", "2026-08-14"],
].map(([title, status, priority, labels, description, owner, target_date], index) => ({
  id: randomUUID(), seed_key: null, title, description, status, priority, labels,
  sort_order: (index + 1) * 1000, owner, target_date,
  acceptance_criteria: "The result is verified in production and recorded in the audit history.",
  notes: null, source: "qa", archived_at: null,
  created_at: new Date(Date.now() - index * 86_400_000).toISOString(), updated_at: new Date().toISOString(),
}));
...
async function mockBoard(page) {
  await page.route("**/api/admin/features**", async (route) => {
    ...
    if (request.method() === "GET") return route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ schemaReady: true, features: seed }) });
    if (request.method() === "PATCH") return route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ success: true, affected: seed.length }) });
    return route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify(seed[0]) });
  });
}
...
await desktopPage.mouse.move(dragBox.x + dragBox.width / 2 + 12, dragBox.y + dragBox.height / 2 + 2, { steps: 4 });
await desktopPage.waitForTimeout(250);
await desktopPage.mouse.move(dropBox.x + dropBox.width / 2, dropBox.y + dropBox.height - 48, { steps: 30 });
