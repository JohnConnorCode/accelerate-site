...
assert.equal(
  validateEmailBlocks([{ id: "bad", type: "button", text: "Go", url: "javascript:alert(1)" }]),
  null,
);
assert.equal(blocksFromPlainText("One\n\nTwo").length, 2);

void (async () => {
  const rendered = await renderEmailBlocks(
    blocks,
    { name: "Maya", message: "Review the operating context." },
    layout.previewText,
  );
