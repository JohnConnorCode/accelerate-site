...
try {
  ...
  for (const file of ["20260914-form-builder.sql", "20260919203846_form_submission_safety.sql", "20260919204344_form_definition_commands.sql", "20260919211809_form_review_commands.sql"])
    sql(readFileSync(`migrations/${file}`, "utf8"));
  const definition = { elements: [{ name: "email", type: "text", inputType: "email", isRequired: true }] };
  const json = value => "'" + JSON.stringify(value).replaceAll("'", "''") + "'::jsonb";
  const token = "a".repeat(64), otherToken = "b".repeat(64), foreignToken = "c".repeat(64);
  ...
  const command = (key, response = { email: "person@example.test" }, share = token, schema = definition) =>
  ...
  for (const role of ["anon","authenticated"])
    assert.throws(() => sql(command(a).replace("SET ROLE service_role",`SET ROLE ${role}`)), /permission denied/);
  ...
  const created = JSON.parse(sql(definitionCommand("create", null, {name:"Draft",schema:definition,share_token:"d".repeat(64)})));
  const saved = JSON.parse(sql(definitionCommand("save", created.updated_at, {schema:definition,name:"Saved"})));
  assert.throws(() => sql(definitionCommand("save",created.updated_at,{schema:definition})),/Form changed/);
  const definitionsRace = await Promise.allSettled([
    runAsync(definitionCommand("save",saved.updated_at,{schema:definition,name:"Racing edit"})),
    runAsync(definitionCommand("status",saved.updated_at,{schema:definition,status:"published"})),
  ]);
  assert.equal(definitionsRace.filter(result => result.status === "fulfilled").length,1);
  assert.match(definitionsRace.find(result => result.status === "rejected").reason.message,/Form changed/);
  const review = (decision, requestId) => `SET app.tenant='${a}'; SET ROLE service_role; SELECT review_form_submission('${receipts[0].submissionId}','${decision}','${requestId}','owner@example.test');`;
  assert.throws(() => sql(review("accepted",key)), /reused/);
  const acceptKey="44444444-4444-4444-8444-444444444444", rejectKey="55555555-5555-4555-8555-555555555555";
  const reviewRace = await Promise.allSettled([runAsync(review("accepted",acceptKey)),runAsync(review("rejected",rejectKey))]);
  assert.equal(reviewRace.filter(result=>result.status==="fulfilled").length,1);
  assert.match(reviewRace.find(result=>result.status==="rejected").reason.message,/already reviewed/);
  const reviewReceipt=JSON.parse(reviewRace.find(result=>result.status==="fulfilled").value);
  assert.equal(sql("SELECT count(*) FROM action_queue"),reviewReceipt.decision==="accepted" ? "1" : "0");
  assert.equal(JSON.parse(sql(review(reviewReceipt.decision,reviewReceipt.decision==="accepted"?acceptKey:rejectKey))).duplicate,true);
  sql(`UPDATE tenants SET config='{"modules":{"form-builder":false}}' WHERE id='${a}'`);
  assert.throws(() => sql(command(key)), /Form not found/);
  sql(`UPDATE tenants SET config='{"modules":{"form-builder":true}}',status='suspended' WHERE id='${a}'`);
  assert.throws(() => sql(command(key)), /Form not found/);
  console.log("PASS: real PostgreSQL concurrent submission replay, form/payload/tenant key binding, one notification, full rollback, stale definition and disabled/suspended tenant refusal.");
} finally {
