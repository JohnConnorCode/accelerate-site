...
import { listAuditHistory, proposalAuditSummary, recordAudit, redactAuditValue } from "../src/lib/revenue-os/audit";
...
assert.equal((redactAuditValue({ refresh_token: "abc", email: "a@example.com" }) as { refresh_token: string; email: string }).refresh_token, "[redacted]");
assert.equal((redactAuditValue({ refresh_token: "abc", email: "a@example.com" }) as { email: string }).email, "a@example.com");
...
} as { title: string; status: string; client_name: string; total_one_time: number; total_monthly: number; lead_id: string; opportunity_id: string });
assert.equal(summary?.title, "Westlake roof");
assert.equal(summary?.status, "sent");
assert.equal("content" in (summary ?? {}), false, "proposal audit summaries must omit document content");
...
async function main() {
  ...
  await assert.rejects(
    () => listAuditHistory(broken.client, {}),
    /Audit read failed/,
  );

  const listed = new MemorySupabase({
    audit_log: [
      { id: "a1", actor_email: "john@acceleratewith.us", action: "proposal.created", entity_type: "proposal", entity_id: "p1", source: "admin", before_state: null, after_state: { status: "draft" }, metadata: {}, created_at: "2026-08-01T12:00:00.000Z" },
      { id: "a2", actor_email: "system@acceleratewith.us", action: "calendar.synced", entity_type: "integration", entity_id: "google_calendar", source: "automation", before_state: null, after_state: { stored: 3 }, metadata: {}, created_at: "2026-08-15T12:00:00.000Z" },
      { id: "a3", actor_email: null, action: "proposal.viewed", entity_type: "proposal", entity_id: "p1", source: "public", before_state: { status: "sent" }, after_state: { status: "viewed" }, metadata: {}, created_at: "2026-08-20T12:00:00.000Z" },
      { id: "a4", actor_email: "john@acceleratewith.us", action: "settings.updated", entity_type: "admin_settings", entity_id: "BOOKING_MODE", source: "admin", before_state: { configured: false }, after_state: { configured: true }, metadata: {}, created_at: "2026-08-21T12:00:00.000Z" },
    ],
  });
  const byActor = await listAuditHistory(listed.client, { actor: "john@acceleratewith.us" });
  assert.deepEqual(byActor.entries.map((entry) => entry.id), ["a4", "a1"]);
  const byEntity = await listAuditHistory(listed.client, { entityType: "proposal" });
  assert.deepEqual(byEntity.entries.map((entry) => entry.action), ["proposal.viewed", "proposal.created"]);
  ...
  assert.deepEqual(byDate.entries.map((entry) => entry.id), ["a4", "a3"]);
  ...
  assert.match(coverage.activityPage, /actor[\s\S]*entity[\s\S]*action[\s\S]*source/, "Activity page must expose actor, entity, action, and source filters");
  ...
  assert.doesNotMatch(coverage.proposals, /after:\s*data/, "Proposal audit must not persist the full proposal document");
  assert.match(coverage.publicProposal, /proposal\.viewed/);
  assert.match(coverage.publicProposal, /Proposal view audit failed/, "A public view must still return after an audit write failure");
  assert.match(coverage.publicProposal, /proposal\.accepted/);
  assert.match(coverage.publicProposal, /has_reason/, "Decline reasons stay out of the audit payload");
  assert.match(coverage.settings, /settings\.updated/);
  assert.match(coverage.settings, /configured: Boolean/);
  assert.doesNotMatch(coverage.settings, /after:\s*\{[^}]*\bvalue\s*:/, "Settings audit must not persist setting values");
  assert.match(coverage.calendar, /calendar\.synced/);
  const calendarAudit = coverage.calendar.slice(coverage.calendar.indexOf("action: \"calendar.synced\""));
  assert.doesNotMatch(calendarAudit.slice(0, 900), /attendees/, "Calendar audit summaries must not include attendee payloads");
