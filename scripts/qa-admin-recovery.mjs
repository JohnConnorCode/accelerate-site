...
const { data: linkData, error: linkError } = await supabase.auth.admin.generateLink({
  ...
  options: { redirectTo: `${base}/auth/callback?next=/admin/emails` },
});
if (linkError || !linkData?.properties?.hashed_token)
  throw linkError || new Error("Could not generate QA session");
const { data: verified, error: verifyError } = await supabase.auth.verifyOtp({
  token_hash: linkData.properties.hashed_token,
  type: "magiclink",
});
if (verifyError || !verified.session)
  throw verifyError || new Error("Could not exchange QA session");
...
const emailList = [
  {
    ...
    description: "Confirms a revenue-leak audit request.",
    category: "Transactional",
    subject: "Your audit request",
    variables: ["name"],
    hasDraft: false,
    source: "built_in",
    updatedAt: null,
  ...
  {
    id: "manual-audit-followup-1",
    name: "Manual Audit Followup · Email 1",
    description: "Immediate sequence message.",
...
let detail = {
  ...
  category: "Transactional",
  variables: ["name"],
  subjectTemplate: "Your audit request — Accelerate",
  bodyTemplate: "Hi {{name}},\n\nJohn will review your company and reply personally.",
  previewText: "",
  subject: "Your audit request — Accelerate",
  html: "<!doctype html><html><body style='margin:0;background:#0a0a0a;color:#eee;font-family:Arial;padding:40px'><div style='max-width:560px;margin:auto;background:#151515;padding:32px;border-radius:14px'><h1>Your request is with John</h1><p style='line-height:1.7;color:#bbb'>John will review your company and reply personally.</p></div></body></html>",
  source: "built_in",
  hasDraft: false,
  updatedAt: null,
};
const sentHistory = [
  {
    ...
    body: "Hi Sarah,\n\nI reviewed the company and found the first place to focus.",
    status: "sent",
    providerId: "resend_qa_1",
...
async function contextFor(viewport, reducedMotion = "no-preference", colorScheme = "light") {
  ...
  await page.route("**/api/admin/emails/preview**", async (route) => {
    ...
    if (request.method() === "POST")
      return route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify({ success: true, to: process.env.ADMIN_EMAIL }),
      });
    ...
    return route.fulfill({
      ...
      contentType: "application/json",
      body: JSON.stringify(
        url.searchParams.get("id") ? detail : { schemaReady: true, emails: emailList },
      ),
    });
  });
  await page.route("**/api/admin/emails/history**", (route) =>
    route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({ history: sentHistory }),
    }),
  );
  ...
  await page.route("**/api/admin/send-email", (route) =>
    route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({ success: true }),
  ...
  await page.route("**/api/admin/revenue-os/pipeline**", (route) =>
    route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({ schemaReady: true, opportunities: [] }),
    }),
  );
  await page.route("**/api/admin/revenue-os/campaigns**", (route) =>
    route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({ schemaReady: true, campaigns: [] }),
    }),
  );
  await page.route("**/api/admin/leads**", (route) =>
    route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({ leads: [], total: 0, totalPages: 1 }),
    }),
  );
  ...
  await page.route("**/api/admin/contacts/timeline**", (route) =>
    route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({
        ...
        timeline: [
          {
            type: "opportunity",
            title: "Pipeline: Sarah Co",
            description: "Stage: qualified · Next: Send audit",
            timestamp: "2026-08-16T12:00:00.000Z",
            sourceId: "opp-1",
            link: "/admin/pipeline?search=sarah%40example.com",
          },
        ],
        canonical: {
          schemaReady: true,
          ...
          opportunities: [{ id: "opp-1", stage: "qualified", estimated_value: 5000, won_value: 0 }],
        },
      }),
    }),
  );
...
async function assertDialogFits(page, viewport) {
  ...
  if (
    ...
    throw new Error(`Dialog is outside the ${viewport.width}x${viewport.height} viewport`);
}
...
if (!sidebarPersistedAcrossNavigation)
  throw new Error("Admin sidebar remounted during client-side navigation");
...
await desktop.page
  ...
  .fill("Updated audit request — Accelerate");
...
if (
  ...
  dialogBox.y + dialogBox.height > 1000
)
  throw new Error("Publish dialog is outside the desktop viewport");
await desktop.page.screenshot({ path: `${outDir}/admin-email-publish-dialog.png`, fullPage: true });
await desktop.page.keyboard.press("Escape");

await desktop.page
  .getByRole("heading", { name: "Publish this email draft?" })
...
if (
  (await desktop.page.getByPlaceholder("Search company, person, or email").inputValue()) !==
  "sarah@example.com"
)
  throw new Error("Pipeline deep-link did not restore the contact search");
...
await mobile.page
  ...
  .getByRole("link", { name: "Email Studio", exact: true })
  .waitFor();
...
if (mobileOverflow > 1)
  throw new Error(`Contact command view overflows mobile by ${mobileOverflow}px`);
await mobile.page.screenshot({
  path: `${outDir}/admin-contact-command-mobile.png`,
  fullPage: true,
});
...
await browser.close();
if (errors.length)
  throw new Error(`Console/server errors during admin recovery QA:\n${errors.join("\n")}`);
...
console.log(
  "Admin shell, responsive sidebar, Email Studio, canonical contact handoff, history, and modal QA passed.",
);
