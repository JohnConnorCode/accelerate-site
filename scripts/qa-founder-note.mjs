...
async function openAdmin(
  viewport,
  label,
  { failFirstSave = false, colorScheme = "light", reducedMotion = "reduce" } = {},
) {
  ...
  page.on("console", (message) => {
    ...
    if (message.type() === "error" && !expectedFailedSave)
  ...
  await page.route("**/api/admin/revenue-os/notes", async (route) => {
    ...
    if (failFirstSave && saves === 1) {
      return route.fulfill({
        status: 503,
        contentType: "application/json",
        body: JSON.stringify({ error: "Operating memory is temporarily unavailable" }),
      });
    }
    return route.fulfill({
      status: 201,
  ...
  const response = await page.goto(`${base}/admin/today`, {
    waitUntil: "domcontentloaded",
    timeout: 60_000,
  });
  if (!response || response.status() >= 500)
    failures.push(`${label}: HTTP ${response?.status() ?? "none"}`);
...
function assertRequest(request, expectedNote, expectedSource = "command_palette") {
  ...
  if (request.contactEmail !== "sarah@example.com")
    throw new Error("Note did not retain the canonical contact attachment");
  if (!/^[0-9a-f-]{36}$/.test(request.requestId))
    throw new Error("Note request is missing its idempotency key");
  if (request.captureSource !== expectedSource)
    throw new Error(`Expected ${expectedSource} capture source, received ${request.captureSource}`);
  if (!Number.isInteger(request.captureDurationMs) || request.captureDurationMs < 0)
    throw new Error("Note request is missing valid open-to-save timing");
}

const desktop = await openAdmin({ width: 1440, height: 1000 }, "desktop", {
  failFirstSave: true,
...
await desktopDialog
  .getByPlaceholder("What happened, what was decided, or what should not be forgotten?")
  .fill("Sarah approved the custom automation discovery plan.");
...
await desktop.page
  .getByRole("alert")
  .getByText("Operating memory is temporarily unavailable")
  .waitFor();
...
if (desktop.requests.length !== 2)
  throw new Error(`Expected two save attempts, received ${desktop.requests.length}`);
assertRequest(desktop.requests[0], "Sarah approved the custom automation discovery plan.");
if (desktop.requests[0].requestId !== desktop.requests[1].requestId)
  throw new Error("Retry changed the note idempotency key");
...
await shortcutDialog
  .getByPlaceholder("What happened, what was decided, or what should not be forgotten?")
  .fill("Shortcut capture is fast enough to use in the moment.");
...
if (shortcutRequest.contactEmail !== null)
  throw new Error("Standalone shortcut note unexpectedly inherited an attachment");
if (shortcutRequest.captureSource !== "keyboard_shortcut")
  throw new Error("Direct shortcut did not preserve its capture source");
if (shortcutRequest.captureDurationMs > 10_000)
  throw new Error(
    `Direct shortcut capture took ${shortcutRequest.captureDurationMs}ms, above the ten-second acceptance threshold`,
  );
...
await mobileDialog
  .getByPlaceholder("What happened, what was decided, or what should not be forgotten?")
  .fill("Mobile capture preserves operating context.");
...
if (mobileLayout.documentWidth > mobileLayout.viewportWidth + 2)
  throw new Error(
    `Founder note overflows mobile by ${mobileLayout.documentWidth - mobileLayout.viewportWidth}px`,
  );
...
await dark.page
  .getByRole("dialog", { name: "Capture what you know" })
  .getByPlaceholder("What happened, what was decided, or what should not be forgotten?")
  .fill("Dark-mode capture check.");
