...
} from "../src/lib/revenue-os/integrations";
import {
  INTEGRATION_REGISTRY_VERSION,
  integrationRegistry,
} from "../src/lib/revenue-os/integration-registry";
...
assert.equal(
  baseline.providers.length,
  integrationRegistry.length,
  "the API projection must not silently omit registry providers",
);
assert.equal(
  provider(baseline, "supabase").status,
  "action",
  "a missing scheduler receipt keeps the overall foundation from claiming full readiness",
);
assert.equal(
  provider(baseline, "supabase").capabilities.find((item) => item.id === "canonical-data")?.status,
  "ready",
  "runtime behavior, not environment presence, makes canonical data ready",
);
assert.equal(
  provider(baseline, "google").status,
  "available",
...
assert.equal(
  provider(configuredOnly, "google").status,
  "action",
  "OAuth environment variables without a live connection require action",
);

const connected = buildIntegrationCatalog(
  evidence({
    configured: {
      supabase: true,
      google: true,
    ...
    sourceRuns: [
      {
        ...
        finishedAt: "2026-08-23T17:01:00.000Z",
        error: null,
      },
      {
        key: "google_calendar",
...
assert.equal(
  google.status,
  "action",
  "a send capability with no behavioral receipt keeps the provider from claiming full readiness",
);
      ...
      ? [
          {
            ...
            last_sync_at: "2026-08-20T10:00:00.000Z",
...
assert.equal(
  provider(stale, "google").capabilities.find((item) => item.id === "gmail-read")?.status,
  "degraded",
  "stale success is degraded, not green forever",
);

const failed = buildIntegrationCatalog(
  evidence({
    configured: {
      supabase: true,
      google: false,
      resend: true,
      resend_webhooks: true,
      openrouter: false,
    },
    ...
    webhooks: [
      {
        ...
        error: "token=super-secret provider rejection",
      },
    ],
  }),
  now,
);
const resendFeedback = provider(failed, "resend").capabilities.find(
  (item) => item.id === "feedback",
);
assert.equal(resendFeedback?.status, "degraded");
assert.ok(
  !resendFeedback?.statusReason.includes("super-secret"),
  "operator diagnostics must redact token-like values",
);
...
assert.equal(
  provider(missingSchema, "google").status,
  "degraded",
  "missing receipt tables must never produce a healthy integration",
);

for (const definition of integrationRegistry) {
  assert.ok(
    definition.capabilities.length > 0,
    `${definition.id} needs at least one bounded capability`,
  );
  assert.ok(definition.cost.detail.length > 0, `${definition.id} must declare cost posture`);
  assert.ok(
    definition.guardrail.length > 0,
    `${definition.id} must declare its ownership/safety boundary`,
  );
...
assert.match(
  route,
  /requireAdmin\(\)/,
  "the catalog API must fail closed through founder authorization",
);
assert.match(
  route,
  /loadIntegrationCatalog/,
  "the route must remain a thin adapter over the authoritative read model",
);
