...
async function main() {
  ...
  await scenario("provider errors are bounded and redact credentials", async () => {
    stubFetch([
      { status: 400, body: { error: { message: `Bearer exposed-token ${"x".repeat(600)}` } } },
    ]);
    await assert.rejects(
      () => openRouterChat(ask),
      (error: unknown) => {
        ...
        assert.ok(
          ...
          "provider errors must stay within the trace-safe bound",
        );
        return true;
      },
    );
  ...
  await scenario("a caller cancellation is not treated as a provider failure", async () => {
    ...
    await assert.rejects(
      () => openRouterChat({ ...ask, signal: controller.signal }),
      (error: unknown) => {
  ...
  await scenario("a caller signal no longer detaches the request timeout", async () => {
    ...
    assert.notEqual(
      calls[0]?.signal,
      controller.signal,
      "the caller signal replaced the timeout signal",
    );
  });
  ...
  await scenario("text streaming retains the provider receipt and resolved model", async () => {
    ...
    const stream = await openRouterTextStream(ask, (receipt) => {
      metadata = receipt;
    });
  ...
  await scenario("structured output requests a strict JSON schema", async () => {
    stubFetch([
      {
        status: 200,
        body: { ...okBody, choices: [{ message: { role: "assistant", content: '{"ok":true}' } }] },
      },
    ]);
  ...
  await scenario("malformed structured JSON is rejected before a domain write", async () => {
    stubFetch([
      {
        status: 200,
        body: { ...okBody, choices: [{ message: { role: "assistant", content: "not-json" } }] },
      },
    ]);
    await assert.rejects(
      () => openRouterJson(jsonAsk),
      (error: unknown) => {
        assert.ok(error instanceof OpenRouterError);
    ...
    stubFetch([
      {
        ...
        body: {
          ...okBody,
          choices: [{ message: { role: "assistant", content: '{"ok":"nope"}' } }],
        },
      },
    ]);
    await assert.rejects(
  ...
  for (const file of sources) {
    ...
    assert.match(
      source,
      /@\/lib\/ai\/openrouter/,
      `${file} must call the shared OpenRouter adapter`,
    );
    assert.doesNotMatch(
      source,
      /@anthropic-ai\/sdk|from ["']openai["']/,
      `${file} must not call a direct provider SDK`,
    );
  }
  assert.equal(
    existsSync("src/app/api/admin/ai-insights/route.ts"),
    false,
    "pre-canonical insights must stay deleted rather than return a successful empty AI result",
  );
  ...
  assert.doesNotMatch(
    setup,
    /OPENROUTER_API_KEY.{0,80}sk-or/,
    "Setup must not echo the OpenRouter secret",
  );
  assert.match(
    readFileSync("src/app/api/chat/route.ts", "utf8"),
    /isTenantOpenRouterConfigured\(supabase\)/,
  );
  assert.match(
  ...
  for (const file of ["src/lib", "src/app"].flatMap(walk)) {
    ...
    assert.doesNotMatch(
      ...
      `${file} reintroduced a direct Anthropic client`,
    );
    assert.doesNotMatch(
      source,
      /from ["']openai["']/,
      `${file} reintroduced a direct OpenAI client`,
    );
  }
...
main()
  .catch((error) => {
    console.error(
      "OpenRouter resilience test failed:",
