  ...
  globalThis.fetch = (async (url: string | URL, init?: RequestInit) => {
    ...
    if (href.includes("resend.com")) {
      ...
      if (options.sendFails) {
        ...
        return { ok: false, status: 422, headers: new Headers(), json: async () => body, text: async () => JSON.stringify(body) };
      }
...
const GOOD_REPLY = "Thanks for getting in touch. You mentioned inquiries are slipping through when your front desk is busy, and that is exactly the kind of gap worth looking at properly before changing anything.\n\nI would like to understand how work reaches you today and where it stalls. Reply with a couple of times that suit you this week and I will send an invite.";
...
function harness(overrides: { settings?: Row[]; contacts?: Row[]; clients?: Row[]; messages?: Row[]; tenants?: Row[] } = {}) {
  return new MemorySupabase({
    ...
    contacts: overrides.contacts ?? [{ id: "contact-1", communication_status: "active", lifecycle_stage: "lead" }],
...
const INPUT = {
  ...
  inquiry: "We keep missing calls when the front desk is busy and I think we are losing new patients because of it.",
...
async function main() {
  ...
  assert.equal((audit!.metadata as Row).policy_version, RESPONDER_POLICY_VERSION, "the audit must name the policy version that authorised the send");

  const message = ok.rows("messages")[0]!;
  assert.ok(String(message.body_text).includes("Reply with a couple of times") || String(message.body_text).includes("reply with a couple"), "the drafted body must be what was sent");
  assert.equal(message.idempotency_key, `responder:${RESPONDER_POLICY_VERSION}:opp-1`, "one acknowledgement per opportunity must be enforced at the sender, not merely by the caps");

  // ---- Kill switch, read at execution -----------------------------------

  stubNetwork(GOOD_REPLY);
  await expectDecline(
    harness({ settings: [{ key: RESPONDER_ENABLED_KEY, value: "false" }, { key: RESPONDER_APPROVED_VERSION_KEY, value: RESPONDER_POLICY_VERSION }] }),
    {}, "policy_disabled",
  ...
  await expectDecline(harness({ settings: [] }), {}, "policy_disabled", "with no setting at all the responder must default to off, not to on");

  // ---- Version pinning ---------------------------------------------------

  await expectDecline(
    harness({ settings: [{ key: RESPONDER_ENABLED_KEY, value: "true" }, { key: RESPONDER_APPROVED_VERSION_KEY, value: "inbound-responder.v0" }] }),
    {}, "policy_not_approved",
  ...
  await expectDecline(harness(), { existingOpportunity: true }, "not_first_touch", "only a first touch is acknowledged automatically; a live conversation belongs to a human");
  await expectDecline(harness(), { now: OUTSIDE_WINDOW }, "outside_send_window", "an automated reply at 3am reads as a robot and is outside the approved window");
  await expectDecline(harness(), { inquiry: "hi" }, "inquiry_too_thin", "there is nothing to ground a reply in, so replying would mean inventing something");

  await expectDecline(
    harness({ contacts: [{ id: "contact-1", communication_status: "unsubscribed", lifecycle_stage: "lead" }] }),
    {}, "contact_suppressed",
  ...
  await expectDecline(
    harness({ messages: [{ id: "m1", direction: "outbound", recipient_emails: ["dana@northsidedental.com"], created_at: "2026-08-19T12:00:00.000Z" }] }),
    {}, "already_contacted",
  ...
  await expectDecline(busy, {}, "daily_cap_reached", "the account-wide daily ceiling is what protects the sending domain");
  ...
  stubNetwork("Happy to help. We can do this for $2,500 a month and I have you down for Tuesday at 3pm.");
  const ungrounded = harness();
  await expectDecline(ungrounded, {}, "failed_grounding_check", "a draft that invents pricing and books a slot must never reach the prospect");
  const rejectedRun = ungrounded.rows("agent_runs")[0]!;
  assert.equal(rejectedRun.status, "partial", "a rejected draft must be kept on the ledger so the failure is reviewable, not discarded");
  assert.match(String(rejectedRun.error), /Rejected:/, "the ledger must say what the grounding check objected to");

  // ---- Provider and model failures never lose the inquiry ---------------

  stubNetwork(GOOD_REPLY, { sendFails: true });
  const failedSend = harness();
  await expectDecline(failedSend, {}, "send_failed", "a provider failure must be recorded, not thrown into the caller that is capturing the lead");
  ...
  for (const [draft, description] of mustFail) {
    ...
    assert.equal(verdict.ok, false, `the grounding check must reject ${description}: ${draft.slice(0, 60)}`);
  }
