  ...
  private execute(): QueryResult {
    ...
    if (this.operation === "insert") {
      ...
      for (const payload of payloads) {
        if (this.table === "activities" && rows.some((row) => row.source === payload.source && row.external_id === payload.external_id)) {
...
class MemorySupabase {
  ...
  from(table: string) { this.touched.add(table); return new MemoryQuery(this, table); }
}
...
async function run() {
  ...
  const canonical = memorySupabase({
    contacts: [
      { id: "contact-source", full_name: "Source Owner", primary_email: "old@example.com", alternate_emails: [] , source_record_type: "crm", source_record_id: "lead-1" },
      { id: "contact-email", full_name: "Email Owner", primary_email: "owner@example.com", alternate_emails: ["alias@example.com"] },
    ],
    companies: [
      { id: "company-source", name: "Source Company", domain: "source.example", source_record_type: "crm", source_record_id: "lead-1" },
  ...
  assert.equal(primary?.id, "contact-email", "normalized primary email must resolve deterministically");
  const alternate = await findCanonicalContactByEmail(canonical as never, "alias@example.com");
  assert.equal(alternate?.id, "contact-email", "alternate email must resolve to the same canonical contact");
  assert.equal(exactIlike("person_%@example.com"), "person\\_\\%@example.com", "identity lookups must escape SQL LIKE metacharacters");
  ...
  assert.equal(sourceFirst.contact.id, "contact-source", "source record identity must take precedence over email");
  assert.equal(sourceFirst.company.id, "company-source", "source record company identity must take precedence over domain");
  ...
  assert.deepEqual(replay, sourceFirst, "replayed source records must resolve to the same canonical identity");

  const ambiguous = memorySupabase({
    contacts: [
      { id: "contact-primary", full_name: "Primary", primary_email: "shared@example.com", alternate_emails: [] },
      { id: "contact-alternate", full_name: "Alternate", primary_email: "other@example.com", alternate_emails: ["shared@example.com"] },
    ],
  });
  ...
  assert.equal(ambiguousMatch.status, "ambiguous", "import identity inspection must surface ambiguity explicitly");
  assert.equal(ambiguousMatch.contactCandidates.length, 2, "ambiguous match must include both contact candidates");

  const approved = memorySupabase({
    contacts: [
      { id: "identity-target", full_name: "Identity Target", primary_email: "owner@replay.example", alternate_emails: [], metadata: {} },
      { id: "identity-stale", full_name: "Stale Target", primary_email: "other@replay.example", alternate_emails: [] },
    ],
  ...
  assert.equal(secondImport.contactId, firstImport.contactId, "replay must return the original contact ID");
  assert.equal(secondImport.companyId, firstImport.companyId, "replay must return the original company ID");
  assert.equal(
    approved.rows.contacts!.filter(
      (contact) => contact.source_record_type === "contact_import_row" && contact.source_record_id === approvedInput.rowId,
    ).length,
    1,
    "replay must not create a second contact for the same import row",
  );
  assert.equal(approved.rows.activities!.length, 1, "duplicate replay activity receipt must be tolerated");
  assert.equal(approved.rows.audit_log!.length, 2, "first import and replay must both be auditable");
  ...
  assert.equal(manual.contactId, "identity-target", "explicit expected contact should update the deterministic target");

  const stale = memorySupabase({
    contacts: [
      { id: "identity-target", full_name: "Identity Target", primary_email: "other@replay.example", metadata: {} },
      { id: "identity-target-2", full_name: "Changed Target", primary_email: "changed@replay.example", metadata: {} },
    ],
    companies: [],
    activities: [],
    audit_log: [],
  ...
  const companyAmbiguous = memorySupabase({
    contacts: [{ id: "contact-domain", full_name: "Domain Contact", primary_email: "owner@example.net", alternate_emails: [] }],
    companies: [
  ...
  assert.equal(companyManual.companyId, "company-domain-a", "manual ambiguous company selection should use the approved canonical ID");

  for (const forbidden of ["opportunities", "messages", "campaign_members", "action_queue"]) {
    assert.equal(approved.touched.has(forbidden), false, `identity import must not touch ${forbidden}`);
  }
  ...
  assert.equal(companyAmbiguous.touched.has("companies"), true, "company ambiguity test should inspect company rows");
  assert.equal(stale.touched.has("contacts"), true, "stale-selection test should inspect contacts");

  console.log("Identity resolution precedence, manual conflict resolution, ambiguity refusal, stale guards, and replay stability passed.");
}
