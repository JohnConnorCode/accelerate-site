...
import { PROJECT_REF, POOLER_HOST, POOLER_PORT, repoRoot, runPsql } from "./lib/accelerate-database.mjs";
...
if (!requestedFile) {
  fail("pass one SQL file, for example: npm run db:migrate -- migrations/20260816-contact-importer.sql");
}
...
if (
  ...
  (!repoRelative.startsWith(`migrations${sep}`) &&
    !repoRelative.startsWith(`supabase${sep}`))
) {
