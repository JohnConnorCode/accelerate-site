...
function manufacturingBlueprint(): Record<string, unknown> {
  ...
  return {
    ...
    businessSummary:
      "Custom manufacturing: sales, deposit, sampling, client review, production, QA, shipment, final payment.",
    evidenceRefs: [
      {
        kind: "fact",
...
async function main() {
  ...
  assert.ok(
    validation.blocked.some(
      (item) => item.kind === "unavailable_capability" && item.key === "drive",
    ),
  ...
  assert.ok(
    validation.ready.includes(
      "workflow:won_opportunity_onboarding/step:create_project:orders.create",
    ),
  );
    ...
    workflows: [
      {
        ...
        steps: [
          { key: "s1", kind: "action", description: "Do magic", capabilityKey: "teleport.freight" },
        ],
  ...
  assert.ok(
    boardValidation.blocked.some(
      (item) => item.kind === "unknown_entity" && item.key === "teleporter",
    ),
  );
      ...
      board["key"] === "production"
        ? {
            ...
            columns: [
              {
                key: "vendor_sampling",
                label: "Vendor Sampling",
                lifecycleStates: ["vendor_sampling"],
              },
  ...
  await assert.rejects(
    saveBlueprintVersion(client, {
      ...
      document: {
        ...manufacturingBlueprint(),
        businessSummary: "Install <script>alert(1)</script> as a dashboard",
      },
      changeSummary: "x",
    }),
    /executable markup/,
  ...
  const liveResult = await validateBlueprintAgainstLive(
    liveClient,
    tenantA,
    manufacturingBlueprint(),
  );
  ...
  assert.equal(
    gates.blueprintLevel.length +
      gates.explicitWorkspaceChange.length +
      gates.externalAuthority.length,
    impacts.length,
  ...
  assert.ok(
    review.boards.some((board) => board.ref === "board:production" && board.status === "ready"),
  );
  assert.ok(
    review.workflows.some((flow) => flow.status === "approval"),
    "email.send gates the workflow",
  );
  assert.ok(
    review.integrations.some(
      (item) => item.ref === "integration:drive" && item.status === "blocked",
    ),
  );
