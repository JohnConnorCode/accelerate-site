...
const receiptsByDb = new WeakMap<MemorySupabase, Map<string, { command: Record<string, unknown>; result: Row }>>();
...
function runProposalLifecycle(
  ...
  }
  {
    ...
    else if (op === "view" && (p.viewed_at != null || ["accepted", "declined", "expired", "superseded"].includes(status)))
      changed = false;
    ...
    else if (["accept", "decline", "expire", "revise"].includes(op) && !["sent", "viewed"].includes(status))
      throw new Error("Proposal is no longer open for a response or revision");

    if (changed && ["accept", "decline"].includes(op) && expiresAt !== null && expiresAt <= Date.now())
    ...
    if (changed) {
      ...
      Object.assign(p, {
        ...
        ...(op === "decline" ? { decline_reason: String(command.reason).trim().slice(0, 1000) } : {}),
...
(async () => {
  ...
  checks.push("accept succeeds once, records one event/activity/notification/task");
  {
    ...
    assert.equal(db.rows("activities").length, 2, "one decision activity, one task-created activity");
  ...
  checks.push("concurrent decisions on the same open proposal settle on exactly one outcome");
  {
    ...
    const responses = await Promise.all(
      [r1, r2, r3].map(({ request, params }) => run(db, () => handleProposalPost(request, { params }))),
    );
    ...
    assert.ok(["accepted", "declined"].includes(finalStatus as string), "settled to a terminal status");
    for (const body of bodies) {
      assert.equal(body.status, finalStatus, "every racer's response matches the settled outcome");
    }
    assert.equal(
      db.rows("proposal_events").filter((row) => ["accepted", "declined"].includes(row.event_type as string))
        .length,
      1,
  ...
  checks.push("decline without a reason is rejected before any write");
  {
    const db = harness();
    const ip = freshIp();
    const { request, params } = post("tok-1", { decision: "declined" }, ip);
    const response = await run(db, () => handleProposalPost(request, { params }));
    assert.equal(response.status, 400);
    assert.equal(db.rows("proposals")[0]!.status, "sent", "no state change on validation failure");
  }
