...
} from "../src/lib/revenue-os/subscriptions-contract";
import {
...
assert.equal(
  stripeHostedUrl("https://checkout.stripe.com/c/pay/session", "checkout.stripe.com"),
  "https://checkout.stripe.com/c/pay/session",
);
assert.equal(
  stripeHostedUrl("https://checkout.stripe.com.evil.test/c/pay/session", "checkout.stripe.com"),
  null,
);
assert.equal(
  stripeHostedUrl("https://billing.stripe.com:8443/session", "billing.stripe.com"),
  null,
);
assert.throws(() =>
  planInputSchema.parse({ name: "", currency: "usd", interval: "month", amount: 10 }),
);
assert.equal(subscriptionStatusPresentation("active", false).tone, "positive");
assert.equal(subscriptionStatusPresentation("incomplete", false).tone, "warning");
assert.equal(subscriptionStatusPresentation("incomplete", false).label, "Checkout incomplete");
assert.equal(subscriptionStatusPresentation("past_due", false).tone, "danger");
assert.equal(subscriptionStatusPresentation("canceled", false).canCancel, false);
assert.equal(subscriptionStatusPresentation("canceled", true).label, "Ended");
assert.equal(subscriptionStatusPresentation("canceled", true).canResume, false);
assert.equal(subscriptionStatusPresentation("active", true).canResume, true);
assert.equal(subscriptionStatusPresentation("incomplete", true).canResume, false);
assert.equal(subscriptionStatusPresentation("incomplete", true).label, "Checkout incomplete");
...
async function serviceFlow() {
    ...
    integration_connections: [
      {
        ...
        encrypted_credentials: {
          api_key: encryptTenantSecret(apiKey, tenantId, "stripe", "api_key"),
        },
      },
    ],
  });
  ...
  globalThis.fetch = async (url, init) => {
    ...
    if (parsed.pathname === "/v1/products" && init?.method === "POST")
      return json({
        id: body.get("name") === "Plus" ? "prod_fixtureplus" : "prod_fixturecore",
        object: "product",
      });
    if (parsed.pathname === "/v1/prices" && init?.method === "POST")
      return json({
        id: body.get("product") === "prod_fixtureplus" ? "price_fixtureplus" : "price_fixturecore",
        object: "price",
      });
    ...
    if (parsed.pathname === "/v1/subscriptions/sub_fixturetest" && init?.method === "POST") {
      if (body.get("cancel_at_period_end"))
        providerSubscription.cancel_at_period_end = body.get("cancel_at_period_end") === "true";
      const nextPrice = body.get("items[0][price]");
      if (nextPrice) providerSubscription.items.data[0]!.price.id = nextPrice;
      return json(providerSubscription);
    }
    throw new Error(
      `Unexpected Stripe fixture request: ${init?.method ?? "GET"} ${parsed.pathname}`,
    );
  };
  try {
    const coreRequest = crypto.randomUUID();
    const core = await createSubscriptionPlan(
      db,
      { name: "Core", currency: "usd", interval: "month", amount: 2500 },
      coreRequest,
      "owner@example.com",
      actorId,
    );
    ...
    assert.ok(coreRow);
    coreRow.active = true;
    const coreAgain = await createSubscriptionPlan(
      db,
      { name: "Core", currency: "usd", interval: "month", amount: 2500 },
      coreRequest,
      "owner@example.com",
    ...
    const plus = await createSubscriptionPlan(
      db,
      { name: "Plus", currency: "usd", interval: "month", amount: 5000 },
      crypto.randomUUID(),
      "owner@example.com",
      actorId,
    );
    ...
    plusRow.active = true;
    const checkoutInput = {
      planId: core.id as string,
      requestId: crypto.randomUUID(),
      userId,
      email: "Buyer@Example.com",
      name: "Fixture Buyer",
      origin: "https://accelerate.example",
      tenantSlug: "fixture",
    };
    ...
    const event = {
      ...
      data: {
        object: {
          mode: "subscription",
          subscription: "sub_fixturetest",
          customer: "cus_fixturetest",
          metadata: {
            accelerate_tenant_id: tenantId,
            accelerate_user_id: userId,
            accelerate_plan_id: core.id,
          },
        },
      },
    };
          ...
          data: {
            object: {
              ...
              metadata: {
                ...
                accelerate_tenant_id: "99999999-9999-4999-8999-999999999999",
              },
            },
          },
        }),
    ...
    assert.equal(
      mem.rows("billing_webhook_events").find((row) => row.event_id === "evt_wrongtenant")?.status,
      "failed",
    );
    await assert.rejects(
      () =>
        processStripeWebhook(db, {
          ...event,
          id: "evt_missingtenant",
          data: {
            object: {
              ...event.data.object,
              metadata: { ...event.data.object.metadata, accelerate_tenant_id: undefined },
            },
          },
        }),
      /another workspace/,
    );
    const originalPrice = providerSubscription.items.data[0]!.price.id;
    providerSubscription.items.data[0]!.price.id = "price_fixtureplus";
    await assert.rejects(
      () => processStripeWebhook(db, { ...event, id: "evt_wrongprice" }),
      /does not match this workspace/,
    );
    providerSubscription.items.data[0]!.price.id = originalPrice;
    ...
    const cancel = await updateCustomerSubscription(
      db,
      userId,
      subscriptionId,
      "cancel",
    ...
    const resume = await updateCustomerSubscription(
      db,
      userId,
      subscriptionId,
      "resume",
    ...
    const scheduled = await updateCustomerSubscription(
      db,
      userId,
      subscriptionId,
      "change",
    ...
    await processStripeWebhook(db, {
      id: "evt_invoicefixture",
      type: "invoice.paid",
      livemode: false,
      data: { object: { subscription: subscriptionId } },
    });
    const afterChange = mem
      .rows("billing_subscriptions")
      .find((row) => row.stripe_subscription_id === subscriptionId)!;
    ...
    assert.ok(
      mem.rows("audit_log").some((row) => row.action === "billing.subscription_cancel_scheduled"),
    );
  } finally {
