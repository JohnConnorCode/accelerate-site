...
if (linkError || !linkData?.properties?.hashed_token)
  throw linkError || new Error("Could not generate a Pipeline QA sign-in token");
const { data: verified, error: verifyError } = await supabase.auth.verifyOtp({
  token_hash: linkData.properties.hashed_token,
  type: "magiclink",
});
if (verifyError || !verified.session)
  throw verifyError || new Error("Could not exchange the Pipeline QA sign-in token");
...
let opportunities = Array.from({ length: 27 }, (_, index) => {
  ...
  return {
    ...
    next_action:
      index % 3 === 0
        ? "Confirm decision criteria and implementation owner"
        : "Prepare the next grounded follow-up",
    ...
    contact: {
      full_name: `Operator ${index + 1}`,
      primary_email: `operator${index + 1}@example.com`,
      phone: null,
      title: index % 2 ? "Founder" : "Operations lead",
    },
    company: {
      name: company,
      domain: `company${index + 1}.example`,
      industry: index % 2 ? "Professional services" : "Manufacturing",
      website: null,
    },
  };
});
  ...
  await page.route("**/api/admin/revenue-os/priority", (route) =>
    route.fulfill({
      ...
      body: JSON.stringify({
        ...
        summary: { total: 0, urgent: 0, critical: 0 },
        items: [],
      }),
    }),
  );
    ...
    if (route.request().method() === "PATCH") {
      ...
      if (rejectNextTransition) {
        ...
        await route.fulfill({
          ...
          contentType: "application/json",
          body: JSON.stringify({
            error: "Resolve the qualification requirement before moving this opportunity.",
          }),
        });
        return;
      }
      opportunities = opportunities.map((item) =>
        item.id === body.id ? { ...item, stage: body.stage, canonical_stage: body.stage } : item,
      );
      await route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify({ opportunity: opportunities.find((item) => item.id === body.id) }),
      });
      return;
    }
    await route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({ schemaReady: true, signalsReady: { calendar: true }, opportunities }),
    });
  });
  return { context, page };
}
...
await desktop.page.getByText("Acme Industrial automation program", { exact: true }).waitFor();
if ((await desktop.page.locator("[data-opportunity-id]").count()) !== 1)
  throw new Error("Pipeline board search did not narrow to one canonical opportunity");
...
if (
  (await desktop.page
    .locator("section[aria-labelledby='pipeline-stage-proposal'] [data-opportunity-id]")
    .count()) !== 3
)
  throw new Error("Stage filter did not preserve the expected realistic-data count");
...
await desktop.page
  .getByText("Resolve the qualification requirement before moving this opportunity.", {
    exact: true,
  })
  .waitFor();
if ((await firstStage.inputValue()) !== "new")
  throw new Error("A rejected transition changed the canonical stage in the UI");
await firstStage.selectOption("qualified");
await desktop.page
  .locator("section[aria-labelledby='pipeline-stage-qualified'] [data-opportunity-id='opp-1']")
  .waitFor();
...
if ((await desktop.page.locator("[data-opportunity-id]").count()) !== 1)
  throw new Error("Deep-linked opportunity did not resolve to the exact canonical record");
...
const mobileOverflow = await mobileBoard.evaluate((board) => {
  return {
    boardClient: board.clientWidth,
    boardScroll: board.scrollWidth,
    viewport: window.innerWidth,
  };
});
if (
  mobileOverflow.boardClient > mobileOverflow.viewport ||
  mobileOverflow.boardScroll <= mobileOverflow.boardClient
)
  throw new Error(
    `Pipeline board did not contain horizontal navigation: ${JSON.stringify(mobileOverflow)}`,
  );
await mobileBoard.evaluate((board) => {
  board.scrollLeft = 0;
  window.scrollTo({
    top: board.getBoundingClientRect().top + window.scrollY - 150,
    behavior: "instant",
  });
});
...
const documentOverflow = await mobile.page.evaluate(() => ({
  client: document.documentElement.clientWidth,
  scroll: document.documentElement.scrollWidth,
}));
if (documentOverflow.scroll > documentOverflow.client)
  throw new Error(`Mobile list escaped the viewport: ${JSON.stringify(documentOverflow)}`);
...
console.log(
  "Pipeline canonical board, list persistence, record deep-link, transition validation, search/filter, realistic-volume, and mobile overflow QA passed.",
);
