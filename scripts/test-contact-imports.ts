...
const csv = parseContactImportSource('name,email,notes\n"Martinez, Jane",JANE@Example.com,"Met, then replied"', "csv");
assert.deepEqual(csv, [{ name: "Martinez, Jane", email: "JANE@Example.com", notes: "Met, then replied" }]);
const json = parseContactImportSource('{"contacts":[{"name":"Sam","email":"sam@example.com"}]}', "json");
assert.equal(json.length, 1);
assert.equal(json[0]?.email, "sam@example.com");
const bounded = parseContactImportSource(Array.from({ length: CONTACT_IMPORT_MAX_ROWS + 25 }, (_, index) => `Person ${index}`).join("\n"), "text");
assert.equal(bounded.length, CONTACT_IMPORT_MAX_ROWS);

const clean = validateContactImportFields({ fullName: "  Jane Martinez ", email: " JANE@Example.com ", website: "martinezroofing.com", phone: "+1 (512) 555-0142" });
...
const personalCompany = validateContactImportFields({ fullName: "Jane", email: "jane@gmail.com", companyName: "Jane Roofing" });
...
for (const forbidden of ['from("opportunities")', 'from("messages")', 'from("campaign_members")']) assert.equal(service.includes(forbidden), false, `Contact Import service must not write ${forbidden}`);

console.log("Contact Import parser, normalization, bounds, migration claim, approval digest, canonical identity path, and no-outbound invariants passed.");
