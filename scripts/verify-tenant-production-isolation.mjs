...
const tenants = [
  {
    slug: "isolation-proof-alpha",
    name: "Isolation Proof Alpha",
    userEmail: "isolation-proof-alpha@accelerate.invalid",
  },
  {
    slug: "isolation-proof-beta",
    name: "Isolation Proof Beta",
    userEmail: "isolation-proof-beta@accelerate.invalid",
  },
];
...
async function userClient(user, tenantId) {
  ...
  if (linkError || !link.properties?.hashed_token)
    throw linkError || new Error("Could not create controlled proof session");
  ...
  if (verifyError || !verified.session)
    throw verifyError || new Error("Could not verify controlled proof session");
...
try {
  ...
  for (const tenant of proofTenants) {
    ...
    const { data: existingProvider, error: existingProviderError } = await service
      .from("integration_connections")
      .select("id")
      .eq("tenant_id", tenant.id)
      .eq("provider", sharedProvider)
      .maybeSingle();
    ...
    const { data: existingJob, error: existingJobError } = await service
      ...
      .select("id")
      .eq("tenant_id", tenant.id)
      .eq("idempotency_key", sharedIdempotencyKey)
      .maybeSingle();
  ...
  const { count: duplicateContacts, error: contactCountError } = await service
    .from("contacts")
    .select("id", { count: "exact", head: true })
    .eq("primary_email", sharedEmail);
  if (contactCountError) throw contactCountError;
  const { count: duplicateProviders, error: providerCountError } = await service
    .from("integration_connections")
    .select("id", { count: "exact", head: true })
    .eq("provider", sharedProvider)
    .eq("account_email", "no-provider-effect@tenant-isolation.invalid");
  if (providerCountError) throw providerCountError;
  const { count: duplicateJobs, error: jobCountError } = await service
    .from("job_runs")
    .select("id", { count: "exact", head: true })
    .eq("idempotency_key", sharedIdempotencyKey);
  ...
  assert.equal(
    duplicateContacts,
    2,
    "identical contact identity must persist once per controlled tenant",
  );
  assert.equal(
    duplicateProviders,
    2,
    "identical provider identity must persist once per controlled tenant",
  );
  assert.equal(
    duplicateJobs,
    2,
    "identical idempotency key must persist once per controlled tenant",
  );

  const alphaClient = await userClient(proofUsers[0], proofTenants[0].id);
  const { data: alphaRows, error: alphaReadError } = await alphaClient
    .from("contacts")
    .select("id,tenant_id")
    .eq("primary_email", sharedEmail);
  ...
  assert.equal(
    alphaRows?.[0]?.tenant_id,
    proofTenants[0].id,
    "tenant admin read returned another workspace's row",
  );

  const tamperedClient = await userClient(proofUsers[0], proofTenants[1].id);
  const { data: tamperedRows, error: tamperedError } = await tamperedClient
    .from("contacts")
    .select("id,tenant_id")
    .eq("primary_email", sharedEmail);
  assert.ok(
    tamperedError || !tamperedRows?.length,
    "tampered tenant context exposed a cross-tenant row",
  );
  ...
  const { data: suspendedRows, error: suspendedError } = await suspendedClient
    .from("contacts")
    .select("id")
    .eq("primary_email", sharedEmail);
  assert.ok(
    suspendedError || !suspendedRows?.length,
    "suspended tenant retained operational read access",
  );
  ...
  const { count: auditCount, error: auditError } = await service
    ...
    .in(
      "tenant_id",
      proofTenants.map((tenant) => tenant.id),
    )
    .in("action", [
      "tenant.created",
      "tenant.active",
      "tenant.suspended",
      "tenant.membership_granted",
    ]);
  if (auditError) throw auditError;
  assert.ok(
    (auditCount || 0) >= 6,
    "controlled lifecycle actions did not leave the required platform audit receipts",
  );
  ...
  const cleanup = await Promise.allSettled(
    proofTenants.map((tenant) => setStatus(tenant, "suspended", actor)),
  );
  ...
  if (cleanupFailures.length) {
    ...
    proofError = proofError
      ? new AggregateError(
          [proofError, ...cleanupFailures.map((result) => result.reason)],
          cleanupMessage,
        )
      : new AggregateError(
