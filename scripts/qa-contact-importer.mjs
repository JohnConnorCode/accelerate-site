...
for (const key of ["NEXT_PUBLIC_SUPABASE_URL", "SUPABASE_SERVICE_ROLE_KEY", "ADMIN_EMAIL"])
  if (!process.env[key]) throw new Error(`${key} is required for contact importer QA`);
...
const baseRows = [
  {
    ...
    reviewed_data: {
      ...
      email: "jane@martinezroofing.com",
      phone: "512-555-0142",
      companyName: "Martinez Roofing",
      role: "Owner",
      website: "https://martinezroofing.com",
      industry: "Roofing",
      source: "Austin builders meetup",
      notes: "Interested in faster estimate follow-up",
    },
    warnings: [],
    errors: [],
    match_reason: "No deterministic identity match",
  ...
  },
  {
    ...
    reviewed_data: {
      fullName: "Sam Lee",
      email: "sam@northstarhvac.com",
      phone: null,
      companyName: "Northstar HVAC",
      role: "Operations Director",
      website: "https://northstarhvac.com",
      industry: "HVAC",
    ...
    warnings: [],
    errors: [],
    match_reason: "Exact email match: sam@northstarhvac.com",
    matched_contact_id: "33333333-3333-4333-8333-333333333333",
    imported_contact_id: null,
    error: null,
  },
  {
    ...
    reviewed_data: {
      ...
      industry: null,
      source: null,
      notes: "Company was not present in source",
    },
    warnings: ["Company identity is uncertain"],
    errors: [],
    match_reason: "No deterministic identity match",
    matched_contact_id: null,
    imported_contact_id: null,
...
function readyBatch(rows = baseRows) {
  return {
    ...
    source_row_count: 3,
    proposed_row_count: 3,
    selected_row_count: rows.filter((r) => r.included).length,
    review_digest: "digest-reviewed-v1",
    approval_digest: null,
    ai_model: "openai/gpt-4.1-mini",
    summary: { create: 2, update: 1, skip: 0 },
    error: null,
    approved_by: null,
    approved_at: null,
    completed_at: null,
...
async function contextFor(viewport, reducedMotion = "no-preference") {
  ...
  await page.route("**/api/admin/revenue-os/contact-imports**", async (route) => {
    ...
    if (body.action === "save_review") {
      rows = body.rows.map((review, index) => ({
        ...rows[index],
        included: review.included,
        action: review.action,
        reviewed_data: review.data,
      }));
    ...
    if (body.action === "approve")
      batch = {
        ...batch,
        status: "approved",
        approval_digest: batch.review_digest,
      ...
      rows = rows.map((row) =>
        row.included
          ? {
              ...row,
              status: "imported",
              imported_contact_id: row.matched_contact_id || `imported-${row.row_index}`,
            }
          : { ...row, status: "skipped" },
      );
      batch = {
        ...batch,
        status: "completed",
        summary: { imported: rows.filter((row) => row.status === "imported").length, failed: 0 },
        completed_at: new Date().toISOString(),
        rows,
      };
    }
    return route.fulfill({
      status: body.action === "analyze" ? 201 : 200,
      contentType: "application/json",
      body: JSON.stringify({ schemaReady: true, batch }),
    });
  });
...
async function assertFits(page, viewport) {
  ...
  if (overflow > 1)
    throw new Error(`Contact Import overflows ${viewport.width}px viewport by ${overflow}px`);
  ...
  if (await dialog.count()) {
    ...
    if (
      ...
      throw new Error("Approval dialog is outside viewport");
  }
}

const desktop = await contextFor({ width: 1440, height: 1000 });
await desktop.page.goto(`${base}/admin/contact-imports`, {
  waitUntil: "networkidle",
  timeout: 60_000,
});
await desktop.page.getByRole("heading", { name: "Contact intake", exact: true }).waitFor();
await desktop.page
  .getByTestId("contact-import-source")
  .fill("Jane Martinez, jane@martinezroofing.com\nSam Lee, sam@northstarhvac.com");
...
await desktop.page.screenshot({
  path: `${outDir}/contact-import-review-desktop.png`,
  fullPage: true,
});
...
await desktop.page.screenshot({
  path: `${outDir}/contact-import-approval-desktop.png`,
  fullPage: true,
});
await desktop.page.keyboard.press("Escape");
await desktop.page
  .getByRole("heading", { name: "Approve this exact import snapshot?" })
...
await desktop.page.screenshot({
  path: `${outDir}/contact-import-result-desktop.png`,
  fullPage: true,
});

const mobile = await contextFor({ width: 390, height: 844 });
await mobile.page.goto(`${base}/admin/contact-imports`, {
  waitUntil: "networkidle",
  timeout: 60_000,
});
await mobile.page
...
await mobile.page.screenshot({
  path: `${outDir}/contact-import-review-mobile.png`,
  fullPage: true,
});
...
await assertFits(mobile.page, { width: 390, height: 844 });
await mobile.page.screenshot({
  path: `${outDir}/contact-import-approval-mobile.png`,
  fullPage: true,
});

const reduced = await contextFor({ width: 1280, height: 800 }, "reduce");
await reduced.page.goto(`${base}/admin/contact-imports`, {
  waitUntil: "networkidle",
  timeout: 60_000,
});
...
console.log(
  "Contact Import paste, review edit, approval keyboard dismissal, execution receipt, mobile fit, and reduced-motion QA passed.",
);
