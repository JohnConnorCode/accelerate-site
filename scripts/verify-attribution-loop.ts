...
async function main() {
  ...
  const captured = await ingestInboundLead(supabase, {
    name: `Attribution Run ${runId}`,
    email,
    companyName: `Attribution Co ${runId}`,
    website: `https://attr-${runId}.example.invalid`,
  ...
  const { error: valueError } = await supabase
    .from("opportunities")
    .update({ estimated_value: DEAL_VALUE })
    .eq("id", opportunityId);
  ...
  for (const stage of ["qualified", "meeting", "proposal", "won"]) {
    await transitionOpportunity(supabase, {
      id: opportunityId,
      to: stage,
      actorEmail: "verification@local",
      source: "verification",
      reason: `Loop One attribution verification ${runId}`,
    });
  }

  const { data: finalOpportunity } = await supabase
    .from("opportunities")
    .select("stage,won_value,closed_at,source_detail")
    .eq("id", opportunityId)
    .single();
  check(
    "the opportunity reaches the won stage",
    finalOpportunity?.stage === "won",
    finalOpportunity?.stage,
  );
  check(
    "winning carries the estimated value into won revenue",
    Number(finalOpportunity?.won_value) === DEAL_VALUE,
    finalOpportunity?.won_value,
  );
  check(
    "winning records a close timestamp",
    Boolean(finalOpportunity?.closed_at),
    finalOpportunity?.closed_at,
  );
  check(
    "the opportunity keeps its campaign attribution",
    finalOpportunity?.source_detail === campaign,
    finalOpportunity?.source_detail,
  );
  ...
  check("every stage change left an immutable event", stageEvents === 5, {
    expected: "1 creation + 4 transitions",
    got: stageEvents,
  });
  ...
  check(
    "the funnel counts one more opportunity",
  ...
  check(
    "the funnel credits the won revenue",
    delta("wonRevenue") === DEAL_VALUE,
    delta("wonRevenue"),
  );
  check(
    "a won deal is excluded from open pipeline value",
    delta("pipelineValue") === 0,
    delta("pipelineValue"),
  );
  check(
    "no attribution is reported missing",
    after.attribution.missing === baseline.attribution.missing,
    { baseline: baseline.attribution.missing, after: after.attribution.missing },
  );

  const attributed = after.sources.find((source) => source.source === campaign);
  check(
    "revenue is attributed to the originating campaign",
    attributed?.revenue === DEAL_VALUE,
    attributed,
  );
  ...
  check(
    ...
    restored.funnel.opportunities === baseline.funnel.opportunities &&
      restored.funnel.wonRevenue === baseline.funnel.wonRevenue,
    { baseline: baseline.funnel, restored: restored.funnel },
  );
    ...
    JSON.stringify(
      {
        ...
        baselineFunnel: baseline.funnel,
        withDealFunnel: after.funnel,
        restoredFunnel: restored.funnel,
...
main().catch((error) => {
  console.error(
    "Attribution loop verification errored:",
