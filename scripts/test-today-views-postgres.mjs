...
function run(cmd,args,input){const r=spawnSync(cmd,args,{encoding:"utf8",input});if(r.status!==0)throw new Error(r.stderr||r.stdout);return r.stdout.trim();}
const args=["-X","-q","-t","-A","-h","127.0.0.1","-p",String(port),"-U","postgres","-d","postgres","-v","ON_ERROR_STOP=1"];
const sql=(text)=>run("psql",args,text);
const a="11111111-1111-4111-8111-111111111111",b="22222222-2222-4222-8222-222222222222",member="33333333-3333-4333-8333-333333333333";
const doc=JSON.stringify({version:1,enabled:true,views:[],defaultViewId:null,pins:[],muted:[]});
const call=(owner,revision,key="aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa",document=doc)=>`SELECT public.save_today_views('${owner}',${revision},'${document}'::jsonb,'${key}');`;
const as=(tenant,user,text)=>sql(`SET ROLE authenticated; SET app.tenant='${tenant}'; SET app.actor_id='${user}'; ${text}`);
let started=false;
try{
 ...
 const migration=readFileSync("migrations/20260910-today-workspace.sql","utf8");sql(migration);sql(migration);
 ...
 assert.throws(()=>as(a,a,call(a,0,"bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb")),/another session/);
 assert.throws(()=>as(a,a,call(a,0,undefined,doc.replace('"enabled":true','"enabled":false'))),/different content/);
 ...
 assert.equal(JSON.parse(as(a,a,call("workspace",0,"cccccccc-cccc-4ccc-8ccc-cccccccccccc"))).revision,1);
 assert.equal(as(a,member,"SELECT count(*) FROM today_workspace_views"),"1");
 assert.throws(()=>as(a,a,"UPDATE today_workspace_views SET revision=99"),/permission denied/);
 assert.throws(()=>as(a,a,"DELETE FROM today_view_receipts"),/permission denied/);
 assert.throws(()=>as(a,a,call(a,1,"dddddddd-dddd-4ddd-8ddd-dddddddddddd",'{"version":1,"views":[]}')),/Invalid Today document/);
 const digest="a".repeat(64);
 as(a,a,`INSERT INTO today_view_proposals(tenant_id,actor_id,digest,preview) VALUES('${a}','${a}','${digest}','{}')`);
 ...
 assert.throws(()=>as(a,a,"UPDATE today_view_proposals SET preview='{}'"),/permission denied/);
 sql(`UPDATE tenant_memberships SET status='revoked' WHERE user_id='${a}'`);
 assert.throws(()=>as(a,a,call(a,1,"dddddddd-dddd-4ddd-8ddd-dddddddddddd")),/membership required/);
 assert.equal(as(a,a,"SELECT count(*) FROM today_workspace_views"),"0");
 assert.throws(()=>sql("SET ROLE anon; SELECT public.save_today_views('workspace',0,'{}',gen_random_uuid())"),/permission denied/);
 console.log(JSON.stringify({result:"passed",checks:["idempotent migration","private/shared visibility","atomic revision conflict","exact request replay","audit receipt","direct write refusal","foreign member/tenant refusal","revocation","anonymous refusal"]}));
}finally{if(started)run("pg_ctl",["-D",data,"-m","fast","-w","stop"]);rmSync(root,{recursive:true,force:true});}
