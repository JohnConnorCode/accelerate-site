...
await desktop.demo.getByRole("button", { name: "Approve", exact: true }).first().click();
...
await desktop.demo.getByText(groundedAnswer, { exact: true }).first().waitFor();
...
await mobile.demo.getByLabel("Demo view", { exact: true }).selectOption("people");
await mobile.demo.getByRole("button", { name: /Marcus Reyes/ }).click();
await mobile.demo.getByRole("heading", { name: "Marcus Reyes" }).waitFor();
...
await mobile.demo.getByLabel("Demo view", { exact: true }).selectOption("meeting");
await mobile.demo.getByRole("button", { name: /Apply 5 to the records/ }).click();
await mobile.demo.getByText("Filed. Nothing else was touched.", { exact: true }).waitFor();
...
for (const [view, action] of connectedViews) {
  await mobile.demo.getByLabel("Demo view", { exact: true }).selectOption(view);
  ...
  const actionButton = mobile.demo.getByRole("button", { name: action, exact: true });
  await actionButton.waitFor();
  await actionButton.click();
  await mobile.demo.getByRole("button", { name: "Saved in this demo", exact: true }).waitFor();
