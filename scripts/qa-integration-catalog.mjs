...
async function freshSessionCookies() {
  const { data: linkData, error: linkError } = await supabase.auth.admin.generateLink({
    ...
    options: { redirectTo: `${base}/auth/callback?next=/admin/integrations` },
  });
...
async function openCatalog(
  ...
  const response = await page.goto(`${base}/admin/integrations${query}`, {
  ...
  if (layout.documentWidth > layout.viewportWidth + 2)
    failures.push(
      `${label}: document overflow ${layout.documentWidth}px > ${layout.viewportWidth}px`,
    );
  if (layout.openDialogs) failures.push(`${label}: dialog opened without an operator action`);
  return { context, page };
}

const desktop = await openCatalog(
...
await desktop.page
  .getByRole("status")
  .getByText("Google OAuth is not configured", { exact: true })
  .waitFor();
...
await desktop.page
  .getByText("Microsoft 365", { exact: true })
  .locator("xpath=ancestor::div[contains(@class,'admin-surface')]")
  .getByText("Operating contract")
  .click();
await desktop.page
  .getByText(
...
await desktop.page.screenshot({
  path: `${outDir}/integrations-planned-contract.png`,
  fullPage: true,
});
  ...
  JSON.stringify(
    {
      result: "passed",
      screenshots: [
        `${outDir}/integrations-desktop.png`,
        `${outDir}/integrations-planned-contract.png`,
        `${outDir}/integrations-mobile.png`,
        `${outDir}/integrations-mobile-search.png`,
        `${outDir}/integrations-dark.png`,
      ],
      checks: [
        "authenticated load",
        "desktop",
