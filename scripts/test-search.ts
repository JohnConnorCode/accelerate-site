...
async function main() {
  assert.ok(index.length > 20, `index has only ${index.length} entries, so it is probably not being built`);

  // ---- Every industry is findable by its own name ------------------------

  const industries = index.filter((entry) => entry.group === "Industries");
  assert.ok(industries.length >= 10, `expected every vertical in the index, got ${industries.length}`);
  ...
  assert.equal(firstHref("nonprofit"), "/industries/nonprofits", `"nonprofit" must find the nonprofits page, got ${top("nonprofit").join(", ")}`);
  assert.equal(firstHref("nonprofits"), "/industries/nonprofits");
  assert.equal(firstHref("non profit"), "/industries/nonprofits", "the two-word spelling must work too");
  ...
  assert.ok(narrow < broad, `adding a term must narrow the result set: "ai" gave ${broad}, "ai nonprofit" gave ${narrow}`);
  ...
  assert.equal(searchEntries(index, "nonprofit zzzzqqq", 10).length, 0, "an unmatched term must disqualify the entry entirely");

  // ---- Nothing matches, and that is a real answer ------------------------

  assert.deepEqual(searchEntries(index, "zzzzqqqxyz", 10), [], "a query matching nothing must return nothing rather than noise");
  ...
  for (const query of ["ai", "automation", "follow up", "law"]) {
    assert.deepEqual(top(query, 8), top(query, 8), `"${query}" must rank identically on repeat evaluation`);
  }
  ...
  assert.equal(normalize("Café"), "cafe", "accents must be stripped so an unaccented query still matches");
  assert.equal(normalize("  Medical & Dental  "), "medical dental");

  console.log(JSON.stringify({
    entries: index.length,
    byGroup: Object.fromEntries([...new Set(index.map((entry) => entry.group))].map((group) => [group, index.filter((entry) => entry.group === group).length])),
    result: "passed",
  }, null, 2));
}
