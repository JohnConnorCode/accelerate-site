...
1. **Always deploy with `npm run deploy` (Vercel CLI), never `git push`.**
   The Vercel project lives on **Robert Farrell's** team (`robert-farrells-projects`) and has **Commit-Author Verification** turned on. The team has one member: Robert (`farrellroofingco@gmail.com`). Any deploy whose git commit author isn't Robert lands in `readyState: BLOCKED` with no useful CLI error — just `UNKNOWN` status forever. CLI deploys in `--prebuilt` mode upload `.vercel/output` directly: no source upload, no `.git` read, no commit-author check.
...
2. **Be logged into the right Vercel account first.**
   ...
   - `vercel whoami` → `farrellroofingco-4693`
   - `vercel teams ls` → `robert-farrells-projects`
   
   If you're on a different account (John's personal, Theresa's, etc.), `vercel logout && vercel login` and authorize as the right account in the browser.
...
"readyState": "READY"
"target": "production"
"url": "https://accelerate-site-<hash>-robert-farrells-projects.vercel.app"
```
...
TEAM=team_qHBO9P2V9uF31MH4k6s4mz8F
...
```json
{
  "projectId": "prj_JDk6HGWB7lcgeJlusvWZmYxIIrfj",
  "orgId": "team_qHBO9P2V9uF31MH4k6s4mz8F",
...
```
Team:       robert-farrells-projects     (id team_qHBO9P2V9uF31MH4k6s4mz8F)
Project:    accelerate-site               (id prj_JDk6HGWB7lcgeJlusvWZmYxIIrfj)
Domain:     acceleratewith.us → 307 → www.acceleratewith.us
Owner:      farrellroofingco@gmail.com (Robert Farrell — only team member)
