...
## Keep local verification within the machine's budget

`build`, `build:qa`, and `typecheck` now pass through `scripts/resource-run.mjs`.
For full test suites and browser QA use, for example:

```bash
npm run resources:run -- npm run test:core
npm run resources:run -- npm run qa:work-completion
```

The gate uses an atomic per-user temporary-directory lock shared by all updated
worktrees. It refuses a second job immediately. It requires 5 GiB available disk
at startup, stops below 2 GiB while running, and on macOS requires at least 20%
reported memory availability. Every ten seconds it rechecks capacity and the
owned process group's resident memory, stopping above 3 GiB. Node children
inherit a 2 GiB heap limit per process. These are conservative local limits,
not a hard OS memory quota: native allocations, short spikes between samples,
and other applications still consume memory. Other projects and old checkouts
must adopt the gate to participate in its lock.

Interruptions and resource failures terminate the owned process group, release
the lock, and return failure. The gate never kills an unrelated process. An
uncatchable kill can leave a lock: inspect its `owner.json` PID and checkout,
confirm its children have stopped, then remove that specific abandoned lock.
There is no automatic takeover or capacity bypass. A blocked check is not a
passing check; run larger jobs on an adequately provisioned runner.

Do not wrap `verify:review` itself: its build already acquires the lock. Its core suite and build acquire the lock separately and run sequentially.
Keep one reusable dependency installation per compatible lockfile and dispose
of owned build output and QA browsers after verification. Never delete another
worktree's cache while it may be active.
