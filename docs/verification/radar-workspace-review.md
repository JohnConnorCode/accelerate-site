...
| Dialog keyboard flow | Controlled shared dialogs lacked a Radix Trigger to restore focus | Shared dialog captures and restores the actual opener, including stacked reviews, and preserves the default accessible title when no custom label is supplied | `src/components/admin/AdminDialog.tsx`                                                 |
