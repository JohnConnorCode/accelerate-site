...
- The resource gate initially deferred the build while another checkout held the heavy slot. Its work was preserved; verification started after the slot was released.
...
The supported claim-continuation flow preserved the source in an isolated successor checkout after the lease expired during verification. Byte comparison of `src`, `public`, Next configuration and package files confirmed no build-input differences from the successful build. The same generated production build was reused for the final browser run. The predecessor checkout is retained.
