...
- Final source also passed the general CI checks and the exported neutral starter build and browser journeys. The main application production build includes TypeScript validation. Broader first-run browser regression results are recorded below when complete.
...
Chicago desktop/mobile, the catering hero and nonprofit desktop/mobile were re-opened from the final CI artifact. Their hierarchy, line wrapping, calls to action and preserved nonprofit imagery remain coherent after restoring shared hero motion.
