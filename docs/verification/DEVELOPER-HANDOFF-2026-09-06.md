...
| Handoff gap                                                                | Result                                                                                                                                                                            |
| -------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Unpublished backlog branch and newer local application changes             | Integrated candidate combines both committed histories; published as [PR #34](https://github.com/JohnConnorCode/accelerate-site/pull/34) for review and clone-based verification. |
...
The controlled developer suite passes fresh-clone fetch, missing base, wrong origin, unsafe key, occupied/dirty worktree preservation, consistent cross-worktree paths, pure JSON, token secrecy, exact replay, incompatible endpoint and read-only scope/enforcement checks. Strict lint, formatting, migration catalog and agent contract checks pass. [CI run 34007523775](https://github.com/JohnConnorCode/accelerate-site/actions/runs/34007523775) passed for `c555edd85f631434bc2afbb13bd1c42e4f222b9a`, including the integrated build/typecheck, actual business migration upgrade/replay, core contracts, developer tests and browser journeys. Later control-checkout output guidance has its own passing scoped CLI test. The final candidate receipt is recorded on the live implementation card; Vercel activation remains blocked separately.
...
## Published ticket bases

Six exact `handoff/base-*` snapshots were published and nine ready, unclaimed cards were updated with revision checks. Their approved commit SHAs and execution state stayed unchanged. This resolves missing branches and public `main` lag without publishing another agent's uncommitted changes. The control checkout remains the place to run current board commands even when an application ticket intentionally starts from an older base.

The remaining shared-service rollout is tracked as `developer-shared-dispatch-activation`, with explicit release authorization, prerequisite reviews, scoped access, isolated test context and a complete controlled worker drill.
