...
| Contacts      | Canonical `contacts` records, edits, and relationship history                                   | Website requests, list imports, and identity matching are separate contact workflows, reached from Contacts.                                      |
...
The contact directory reads `contacts`. `/api/admin/contacts` remains the website submission compatibility API; new directory reads use `/api/admin/contacts/directory`. Opening a website request does not mark it reviewed. The operator explicitly marks it reviewed.
...
Task feedback and completed AI run feedback are audit events. Rating totals may inform AI quality choices; free text is retained for operator review and excluded from AI instructions. Consequential actions continue through existing approval and execution services.
