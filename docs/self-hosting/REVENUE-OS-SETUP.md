...
`migrations/20260912174637-atomic-pipeline-actions.sql` adds the invoker `apply_pipeline_action` transaction for the existing pipeline owner. Apply it through the ordered runner before deploying the updated pipeline callers. It binds exact queue payloads or a real tenant-bound system context, checks captured opportunity and live column state, and saves the effect and domain/queue receipts atomically. System transitions carry source-specific effect keys; reserved audit receipts bind the original actor and intent, including no-op outcomes, so a replay cannot overwrite a later edit. Restrictive audit policies prevent authenticated clients from forging or altering those system receipts. Old pipeline proposals without captured row and column previews must be prepared again. Opportunity creation, reorder, and non-task/non-pipeline business write coverage remain separate acceptance work; this migration does not claim universal parity or hosted activation.

`migrations/20260912191919-atomic-opportunity-records.sql` extends the same pipeline transaction for creation, same-stage reordering, fixed booking/qualification fields, and explicit intake identity links. The existing identity resolver becomes a shared invoker function so failed creation rolls back newly created identities as well as its opportunity and receipts. Source identity precedence and ambiguity refusal remain unchanged. The old reorder RPC preserves content/features behavior and refuses its pipeline branch. Apply this additive migration before deploying these callers; no hosted activation is implied by committing it. Task and pipeline coverage still leave other business owners in the original universal executor acceptance scope.

`migrations/20260912205240-atomic-conversation-actions.sql` adds the existing conversation owner's invoker transaction for status, assignment and explicit links. Apply it through the ordered runner before deploying the updated callers. The compatible service-only evidence RPC delegates to the same evidence algorithm; authenticated evidence requires an exact executing conversation or identity-review action and its current link target. Identity review binds the selected decision and conversation snapshot during its existing claim. Link/evidence/receipts are atomic; the broader identity-review creation workflow and other business owners remain in the original universal acceptance scope. This source change does not authorize or claim hosted activation.

`migrations/20260912212656-effective-action-policy.sql` resolves action and capability restrictions inside the existing policy RPC, preserving coworker precedence, strict duplicate selection and approved standing constraints. It also makes task restoration recheck current authority before writing. Apply it before deploying the matching action map. Native proofs must show capability revocation denies task, pipeline and restoration effects without changing records. The same additive repair preserves completed proposal-response tasks as permanent source receipts, matching public proposal replay semantics.

`migrations/20260912214429-atomic-identity-review-decisions.sql` completes the existing identity-review owner's link, create, no-match and defer transaction. Apply it before deploying the matching review service. It binds the exact approved decision and conversation snapshot, reuses the identity lock and conversation/evidence writers, and saves the final parent receipt in the same transaction. Direct evidence calls cannot manufacture a different parent decision. Contact imports and other business owners remain in the original universal executor scope; this migration does not establish hosted activation.

`migrations/20260912221951-atomic-contact-import-rows.sql` makes the existing approved import-row writer atomic and keeps batch claim/finish receipts in their owning RPCs. Deploy the migration before the compatible import service. New reviewed data includes the server-normalized company domain in its existing approval digest. Old approvals are not reinterpreted: unimported rows without that field require saving and approving review again, while imported receipts remain reusable. Preserve old executing batches for explicit reconciliation if they cannot be reviewed in the current UI. Import effects retain source identity and fill blank fields; provider communication and other business owners still remain in the original universal scope. No live rollout is implied by this source change.
