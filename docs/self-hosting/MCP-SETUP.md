...
```json
{
  "mcpServers": {
    "revenue-os": {
      ...
      "args": [
        "tsx",
        "/absolute/path/to/Accelerate-agency/accelerate-site/scripts/revenue-os-mcp.ts"
      ],
      "env": {
...
3. Ask Claude: *"What are my top priorities today in Revenue OS?"* — Claude will invoke `get_today_snapshot` and summarize your queue.
...
```bash
claude mcp add revenue-os -- npx tsx /absolute/path/to/Accelerate-agency/accelerate-site/scripts/revenue-os-mcp.ts
```

Or configure environment variables in your active shell / `.env.local`:
```bash
export ADMIN_EMAIL="john@acceleratewith.us"
...
5. **Command**:
   ```bash
   npx tsx /absolute/path/to/Accelerate-agency/accelerate-site/scripts/revenue-os-mcp.ts
   ```
...
```json
{
  "mcpServers": {
    "revenue-os": {
      "command": "npx",
      "args": ["tsx", "scripts/revenue-os-mcp.ts"],
      "cwd": "/Users/johnconnor/Documents/GitHub/Accelerate-agency/accelerate-site",
      "env": {
        "ADMIN_EMAIL": "john@acceleratewith.us"
      }
    }
