...
BEGIN
 ...
 PERFORM id FROM contacts WHERE tenant_id=NEW.tenant_id AND id=o.contact_id FOR SHARE;
 IF NOT FOUND THEN RAISE EXCEPTION 'Canonical contact unavailable'; END IF;
 IF o.company_id IS NOT NULL THEN
  PERFORM id FROM companies WHERE tenant_id=NEW.tenant_id AND id=o.company_id FOR SHARE;
