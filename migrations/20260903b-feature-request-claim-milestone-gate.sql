-- =============================================================================
-- claim_feature_request follow-up: gate auto-pick (no explicit id/seed_key) to
-- milestone:now|next.
--
-- Without this, the auto-pick path (used by `npm run agent:next` with no
-- --card) ordered purely by priority then sort_order, ignoring milestone
-- entirely. Proven live: it picked open-source-release-readiness, an
-- 'urgent' card explicitly labeled milestone:later ("outside the current
-- delivery circuit... do not start before Now and dependency-ready Next
-- work") ahead of dependency-ready Next work. Relabeling the manifest
-- (20260903-feature-request-claims.sql's sibling data fix, promoting
-- dependency-ready cards out of Later) does nothing on its own if the claim
-- path that actually dispatches work doesn't look at the label.
--
-- An explicit --card <seedKeyOrId> claim still bypasses this gate: a human
-- or agent naming a specific card is stating intent directly, the same way
-- claim_work_item's p_work_item_id path already only checks status, not
-- priority ordering.
-- =============================================================================
...
BEGIN
  ...
  ELSE
    ...
    -- current planning horizon), not just status + priority.
    SELECT * INTO v_target FROM public.feature_requests
...
COMMENT ON FUNCTION public.claim_feature_request(TEXT, UUID, TEXT, INTEGER, INTEGER) IS
  'Atomically claims one eligible Feature Board card for a coding agent. Auto-pick (no explicit id/seed_key) is restricted to milestone:now|next. Recovers stale leases to blocked, enforces the in-progress WIP limit, acquires an advisory lock, and transitions the card to in_progress. Returns whether the claim succeeded.';
