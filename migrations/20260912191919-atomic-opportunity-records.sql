...
BEGIN
 IF p_operation IS NULL OR p_operation NOT IN ('transition_opportunity','update_opportunity_details','create_opportunity','update_opportunity_record','reorder_opportunities') OR nullif(btrim(p_actor),'') IS NULL THEN RAISE EXCEPTION 'Invalid pipeline operation or actor'; END IF;
 ...
 ELSIF p_operation='create_opportunity' THEN
  ...
  INSERT INTO public.opportunities(tenant_id,name,email,contact_id,company_id,stage,pipeline,probability,source,source_detail,source_record_type,source_record_id,estimated_value,next_action,next_action_at,owner_email,metadata,utm_source,utm_medium,utm_campaign,utm_content,utm_term,company_website,role,revenue_band,primary_leak,qualified,qualification_reason,qualifier_token,message_variant)
  VALUES(t,updated.name,updated.email,updated.contact_id,updated.company_id,'new','sales',10,coalesce(updated.source,source_name),updated.source_detail,updated.source_record_type,updated.source_record_id,greatest(0,coalesce(updated.estimated_value,0)),updated.next_action,updated.next_action_at,p_actor,coalesce(updated.metadata,'{}'::jsonb),updated.utm_source,updated.utm_medium,updated.utm_campaign,updated.utm_content,updated.utm_term,updated.company_website,updated.role,updated.revenue_band,updated.primary_leak,updated.qualified,updated.qualification_reason,updated.qualifier_token,updated.message_variant) RETURNING * INTO updated;
  changed:=true;
  ...
  INSERT INTO public.activities(tenant_id,activity_type,title,opportunity_id,contact_id,company_id,source,actor_email,external_id,metadata,occurred_at) VALUES(t,'opportunity_created','Opportunity created: '||coalesce(updated.name,''),updated.id,updated.contact_id,updated.company_id,coalesce(updated.source,source_name),p_actor,'opportunity:'||updated.id::text||':created',provenance||jsonb_build_object('stage','new','estimated_value',updated.estimated_value),happened);
 ELSE
 ...
 IF changed THEN
  ...
  VALUES(t,p_actor,CASE p_operation WHEN 'transition_opportunity' THEN 'opportunity.stage_changed' WHEN 'create_opportunity' THEN 'opportunity.created' WHEN 'reorder_opportunities' THEN 'opportunity.reordered' ELSE 'opportunity.updated' END,'opportunity',coalesce(updated.id,old.id)::text,before_json,result_json,provenance||jsonb_build_object('source',source_name,'reason',p_payload->>'reason'));
 END IF;
 IF p_action_id IS NULL THEN
  INSERT INTO public.audit_log(tenant_id,actor_email,action,entity_type,entity_id,before_state,after_state,metadata)
  VALUES(t,p_actor,'pipeline.system_receipt','opportunity',coalesce(updated.id,old.id)::text,before_json,result_json,provenance||jsonb_build_object('operation',p_operation,'request',request_input,'changed',changed));
 END IF;
