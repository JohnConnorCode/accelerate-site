...
BEGIN
 ...
 ELSE
  ...
  IF p_payload ? 'fillMissing' THEN
   IF jsonb_typeof(p_payload->'fillMissing') IS DISTINCT FROM 'object' OR EXISTS(SELECT 1 FROM jsonb_object_keys(p_payload->'fillMissing') k WHERE k NOT IN ('next_action','next_action_at')) THEN RAISE EXCEPTION 'Invalid intake defaults'; END IF;
