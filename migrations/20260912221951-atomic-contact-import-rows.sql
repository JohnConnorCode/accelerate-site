...
DECLARE
 ...
 contact_ids uuid[]; company_ids uuid[]; contact_id uuid; company_id uuid; changes text[]:='{}'; names text[];
...
BEGIN
 ...
 IF FOUND THEN
  contact_id:=c.id; company_id:=c.company_id; replay:=true;
 ELSE
  ...
  contact_id:=coalesce(r.matched_contact_id,contact_ids[1]); company_id:=coalesce(r.matched_company_id,company_ids[1]);
  IF (r.action='create' AND contact_id IS NOT NULL) OR (r.action='update' AND contact_id IS NULL) THEN RAISE EXCEPTION 'Contact identity changed after review'; END IF;
  IF company_id IS NOT NULL THEN
   SELECT * INTO co FROM public.companies WHERE tenant_id=t AND id=company_id FOR UPDATE;
   UPDATE public.companies SET website=coalesce(nullif(co.website,''),data->>'website'),industry=coalesce(nullif(co.industry,''),data->>'industry') WHERE tenant_id=t AND id=company_id;
  ELSIF v_domain IS NOT NULL AND nullif(data->>'companyName','') IS NOT NULL THEN
   ...
   VALUES(t,data->>'companyName',v_domain,data->>'website',data->>'industry',coalesce(nullif(data->>'source',''),'contact_import'),'contact_import_company_row',r.id,jsonb_build_object('import_batch_id',b.id)) RETURNING id INTO company_id;
  END IF;
  IF contact_id IS NOT NULL THEN
   SELECT * INTO c FROM public.contacts WHERE tenant_id=t AND id=contact_id FOR UPDATE;
   ...
   IF c.company_id IS NULL AND company_id IS NOT NULL THEN changes:=array_append(changes,'company'); END IF;
   UPDATE public.contacts SET phone=coalesce(nullif(c.phone,''),data->>'phone'),title=coalesce(nullif(c.title,''),data->>'role'),company_id=coalesce(c.company_id,apply_contact_import_row.company_id),metadata=coalesce(c.metadata,'{}')||jsonb_build_object('last_contact_import_batch_id',b.id)||CASE WHEN nullif(data->>'notes','') IS NULL THEN '{}'::jsonb ELSE jsonb_build_object('import_notes',data->>'notes') END||CASE WHEN nullif(data->>'source','') IS NULL THEN '{}'::jsonb ELSE jsonb_build_object('imported_source',data->>'source') END WHERE tenant_id=t AND id=contact_id;
  ELSE
   ...
   VALUES(t,names[1],CASE WHEN cardinality(names)>1 THEN array_to_string(names[2:cardinality(names)],' ') ELSE NULL END,data->>'fullName',email,data->>'phone',data->>'role',company_id,coalesce(nullif(data->>'source',''),'contact_import'),'contact_import_row',r.id,jsonb_build_object('import_batch_id',b.id)||CASE WHEN nullif(data->>'notes','') IS NULL THEN '{}'::jsonb ELSE jsonb_build_object('import_notes',data->>'notes') END||CASE WHEN company_id IS NULL AND nullif(data->>'companyName','') IS NOT NULL THEN jsonb_build_object('unlinked_company_name',data->>'companyName') ELSE '{}'::jsonb END) RETURNING id INTO contact_id;
 ...
 VALUES(t,'contact_imported',CASE WHEN r.action='create' THEN 'Imported' ELSE 'Enriched' END||' contact: '||(data->>'fullName'),'Approved contact import'||CASE WHEN nullif(data->>'source','') IS NULL THEN '' ELSE ' · '||(data->>'source') END,contact_id,company_id,'contact_import',p_actor,'row:'||r.id::text,jsonb_build_object('batch_id',b.id,'row_id',r.id,'action',r.action,'changed_fields',changes,'replayed',replay)) ON CONFLICT (tenant_id,source,external_id) WHERE external_id IS NOT NULL DO NOTHING;
 INSERT INTO public.audit_log(tenant_id,actor_email,action,entity_type,entity_id,after_state,metadata)
 VALUES(t,p_actor,CASE WHEN replay THEN 'contact.import_reconciled' WHEN r.action='create' THEN 'contact.imported' ELSE 'contact.enriched' END,'contact',contact_id::text,jsonb_build_object('company_id',company_id,'changed_fields',changes),jsonb_build_object('source','admin','import_batch_id',b.id,'import_row_id',r.id));
 UPDATE public.contact_import_rows SET status='imported',imported_contact_id=contact_id,imported_company_id=company_id,result_summary=jsonb_build_object('replayed',replay,'changed_fields',changes),error=NULL,imported_at=clock_timestamp() WHERE tenant_id=t AND id=r.id;
 INSERT INTO public.contact_import_events(tenant_id,batch_id,row_id,event_type,actor_email,summary) VALUES(t,b.id,r.id,'row_imported',p_actor,jsonb_build_object('action',r.action,'contact_id',contact_id,'company_id',company_id,'replayed',replay));
 RETURN jsonb_build_object('contactId',contact_id,'companyId',company_id,'replayed',replay,'changedFields',changes);
