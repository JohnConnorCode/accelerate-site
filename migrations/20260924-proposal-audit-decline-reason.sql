-- Decline reasons are client-authored text. The lifecycle audit stored the
-- whole proposal row, so a decline reason landed in audit_log. Keep it on the
-- proposal only; the audit records whether a reason was given.
...
BEGIN
 ...
 IF op='decline' AND (
  (p_command ? 'reason' AND jsonb_typeof(p_command->'reason') NOT IN ('string','null'))
  OR coalesce(length(p_command->>'reason'),0)>1000
  OR (src<>'public_link' AND nullif(btrim(regexp_replace(p_command->>'reason','[[:cntrl:]]',' ','g')),'') IS NULL))
 THEN RAISE EXCEPTION 'A bounded decline reason is required for admin decisions; public reasons are optional'; END IF;
 ...
 IF changed THEN
  UPDATE proposals SET status=target,updated_at=clock_timestamp(),
   ...
   decline_reason=CASE WHEN op='decline' THEN nullif(btrim(regexp_replace(p_command->>'reason','[[:cntrl:]]',' ','g')),'') ELSE decline_reason END,
  ...
  VALUES(t,p_actor_email,'proposal.'||coalesce(event,'updated'),'proposal',p.id::text,CASE WHEN src='public_link' THEN 'public' ELSE 'admin' END,
   before_row-ARRAY['share_token','content','decline_reason'],to_jsonb(p)-ARRAY['share_token','content','decline_reason'],jsonb_build_object('operation_key',p_key,'successor_id',successor.id,'has_reason',p.decline_reason IS NOT NULL));
