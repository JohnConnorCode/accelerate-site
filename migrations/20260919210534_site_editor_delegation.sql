...
-- Revoked grants retain their client classification so refreshed tokens cannot
-- regain the ordinary authenticated Data API role.
...
BEGIN
 IF client IS NOT NULL THEN
  SELECT resource INTO audience FROM public.site_editor_delegations
   WHERE client_id=client AND user_id=(claims->>'sub')::uuid ORDER BY created_at DESC LIMIT 1;
  IF FOUND THEN
...
DECLARE t uuid; g public.site_editor_delegations; a public.action_queue; output jsonb; policy record;
BEGIN
 t:=private.authorized_request_tenant_id();
