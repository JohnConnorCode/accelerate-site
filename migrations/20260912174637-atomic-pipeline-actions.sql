-- Reserve system pipeline receipts in the existing audit owner. Authenticated
-- users may read their tenant history but cannot forge or rewrite these receipts.
CREATE UNIQUE INDEX IF NOT EXISTS pipeline_system_receipt_key ON public.audit_log(tenant_id,(metadata->>'systemSource'),(metadata->>'effectKey')) WHERE action='pipeline.system_receipt';
DROP POLICY IF EXISTS pipeline_system_receipt_insert ON public.audit_log;
CREATE POLICY pipeline_system_receipt_insert ON public.audit_log AS RESTRICTIVE FOR INSERT TO authenticated,anon WITH CHECK(action IS DISTINCT FROM 'pipeline.system_receipt');
DROP POLICY IF EXISTS pipeline_system_receipt_update ON public.audit_log;
CREATE POLICY pipeline_system_receipt_update ON public.audit_log AS RESTRICTIVE FOR UPDATE TO authenticated,anon USING(action IS DISTINCT FROM 'pipeline.system_receipt') WITH CHECK(action IS DISTINCT FROM 'pipeline.system_receipt');
DROP POLICY IF EXISTS pipeline_system_receipt_delete ON public.audit_log;
CREATE POLICY pipeline_system_receipt_delete ON public.audit_log AS RESTRICTIVE FOR DELETE TO authenticated,anon USING(action IS DISTINCT FROM 'pipeline.system_receipt');
...
BEGIN
 IF p_operation IS NULL OR p_operation NOT IN ('transition_opportunity','update_opportunity_details') OR nullif(btrim(p_actor),'') IS NULL THEN RAISE EXCEPTION 'Invalid pipeline operation or actor'; END IF;
 IF p_action_id IS NULL THEN
  ...
  IF FOUND THEN
   ...
   RETURN jsonb_build_object('opportunity',prior_receipt.after_state,'changed',false);
  END IF;
 ...
 IF p_action_id IS NULL THEN
  INSERT INTO public.audit_log(tenant_id,actor_email,action,entity_type,entity_id,before_state,after_state,metadata)
  VALUES(t,p_actor,'pipeline.system_receipt','opportunity',old.id::text,to_jsonb(old),to_jsonb(updated),provenance||jsonb_build_object('operation',p_operation,'request',request_input,'changed',changed));
 END IF;
